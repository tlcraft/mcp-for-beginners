...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T07:12:11+00:00",
  "source_file": "05-AdvancedTopics/mcp-multi-modality/README.md",
  "language_code": "nl"
}
-->
# Multi-Modal Integratie

Multi-modale applicaties worden steeds belangrijker in AI, omdat ze rijkere interacties en complexere taken mogelijk maken. Het Model Context Protocol (MCP) biedt een raamwerk voor het bouwen van multi-modale applicaties die verschillende soorten data kunnen verwerken, zoals tekst, afbeeldingen en audio.
...
In deze les leer je hoe je een multi-modale applicatie bouwt.

## Leerdoelen

Aan het einde van deze les kun je:

- Multi-modale keuzes begrijpen
- Een multi-modale app implementeren.

## Architectuur voor Multi-Modale Ondersteuning

Multi-modale MCP-implementaties omvatten doorgaans:

- **Modal-Specifieke Parsers**: Componenten die verschillende mediatypes omzetten in formaten die het model kan verwerken.
- **Modal-Specifieke Tools**: Speciale tools ontworpen om specifieke modaliteiten te behandelen (beeldanalyse, audiobewerking)
- **Geünificeerde Contextbeheer**: Systeem om context te behouden over verschillende modaliteiten heen
- **Responsgeneratie**: Mogelijkheid om reacties te genereren die meerdere modaliteiten kunnen bevatten.

## Multi-Modaal Voorbeeld: Beeldanalyse

In het onderstaande voorbeeld analyseren we een afbeelding en halen we informatie eruit.
...
In het voorgaande voorbeeld hebben we:

- Een `ImageAnalysisTool` gemaakt die afbeeldingen kan analyseren met een hypothetische `IImageAnalysisService`.
- De MCP-server geconfigureerd om grotere verzoeken te verwerken en ondersteuning te bieden voor afbeeldingscontenttypes.
- De beeldanalyse-tool geregistreerd bij de server.
- Een methode geïmplementeerd om afbeeldingen van een URL te downloaden en te analyseren op basis van het gevraagde type (objecten, tekst, gezichten, enz.).
- Gestructureerde resultaten geretourneerd in een formaat dat voldoet aan de MCP-specificatie.

## Multi-Modaal Voorbeeld: Audiobewerking

Audiobewerking is een andere veelvoorkomende modaliteit in multi-modale applicaties. Hieronder staat een voorbeeld van hoe je een audiotranscriptietool implementeert die audiobestanden kan verwerken en transcripties kan teruggeven.
...
- Een `AudioTranscriptionTool` gemaakt die audiobestanden kan transcriberen.
- Het schema van de tool gedefinieerd om ofwel een URL of base64-gecodeerde audiogegevens te accepteren.
- De `execute`-methode geïmplementeerd om audiobewerking en transcriptie af te handelen.
- De MCP-server geconfigureerd om multi-modale verzoeken te verwerken, inclusief audio- en beeldverwerking.
- De audiotranscriptietool geregistreerd bij de server.
- Een methode geïmplementeerd om audiobestanden van een URL te downloaden of base64-audio te decoderen.
- Een `AudioProcessor`-service gebruikt om de daadwerkelijke transcriptielogica uit te voeren.
- De MCP-server gestart om verzoeken te ontvangen.

### Multi-Modaal Voorbeeld: Multi-Modale Responsgeneratie
