...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T19:32:47+00:00",
  "source_file": "11-MCPServerHandsOnLabs/05-MCP-Server/README.md",
  "language_code": "nl"
}
-->
# MCP Server Implementatie

## 🎯 Wat Deze Lab Behandelt

Deze praktische lab begeleidt je bij het implementeren van een productieklare MCP-server met behulp van het FastMCP-framework. Je bouwt de kernstructuur van de server, implementeert database-integratie, creëert tools voor data-toegang en legt de basis voor AI-gestuurde retailanalyse.

## Overzicht

De MCP-server is het hart van onze retailanalyse-oplossing. Het fungeert als een brug tussen AI-assistenten en de PostgreSQL-database, en biedt veilige, intelligente toegang tot bedrijfsdata via een gestandaardiseerd protocol.

Deze lab leert je een robuuste, schaalbare MCP-server te bouwen volgens enterprise-patronen en best practices.

## Leerdoelen

Aan het einde van deze lab kun je:

- **Een FastMCP-server bouwen** met de juiste architectuur en organisatie
- **Database-integratie implementeren** met connection pooling en foutafhandeling
- **MCP-tools creëren** voor database-schema introspectie en query-uitvoering
- **Row Level Security contextbeheer configureren**
- **Gezondheidsmonitoring en observatiefuncties toevoegen**
- **Je MCP-server lokaal testen** en met VS Code

## 📁 Projectstructuur

Laten we de organisatie van de MCP-server bekijken:
...
### Omgevingsconfiguratie (`config.py`)

Laten we eerst een robuust configuratiesysteem creëren:
...
### Belangrijke Configuratiefuncties

- **Omgevingsvariabelen laden**: Automatische ondersteuning voor .env-bestanden
- **Typeveiligheid**: Validatie met dataclasses en type hints
- **Flexibele standaardwaarden**: Logische standaardinstellingen voor ontwikkeling
- **Validatie**: Configuratievalidatie met behulpzame foutmeldingen
- **Beveiliging**: Gevoelige waarden alleen uit omgevingsvariabelen

## 🗄️ Database-integratielaag

### PostgreSQL Provider (`sales_analysis_postgres.py`)

Laten we de database-integratielaag implementeren:
...
### Belangrijke Functies van de Database-laag

- **Connection Pooling**: Efficiënt resourcebeheer met asyncpg
- **RLS-integratie**: Automatische Row Level Security contextinstelling
- **Schema-introspectie**: Dynamische ontdekking van tabelschema's
- **Foutafhandeling**: Uitgebreid foutbeheer en logging
- **Query-formattering**: Resultaatformattering geschikt voor AI
- **Gezondheidsmonitoring**: Controle van databaseconnectiviteit en poolstatus

## 🔧 Hoofdimplementatie van de MCP-server

### FastMCP Server (`sales_analysis.py`)

Laten we nu de hoofd-MCP-server implementeren:
...
### Belangrijke Functies van de MCP-server

- **Toolregistratie**: Declaratieve tooldefinities met typeveiligheid
- **RLS-contextbeheer**: Automatische extractie van gebruikersidentiteit en contextinstelling
- **Foutafhandeling**: Uitgebreid foutbeheer met gebruiksvriendelijke berichten
- **Levenscyclusbeheer**: Correct opstarten/afsluiten met resource-opruiming
- **Gezondheidsmonitoring**: Ingebouwde gezondheidscontrole-eindpunten
- **Ondersteuning voor ontwikkeling**: Hot reload en debugging-mogelijkheden

## 🏥 Gezondheidsmonitoring

### Implementatie van Gezondheidscontrole (`health_check.py`)
...
### VS Code Integratietesten

1. **Configureer VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Test in AI Chat**:
   ...
   - Typ `#zava` en selecteer je server
   - Vraag: "Welke tabellen zijn beschikbaar?"
   - Vraag: "Laat me de top 5 winkels zien op basis van aantal bestellingen"

### Unit Testing

Maak uitgebreide unit tests:
...
Na het voltooien van deze lab zou je het volgende moeten hebben:

✅ **Werkende MCP-server**: FastMCP-server met database-integratie  
✅ **Configuratiebeheer**: Robuuste configuratie op basis van omgeving  
✅ **Database-laag**: PostgreSQL-integratie met connection pooling  
✅ **MCP-tools**: Tools voor schema-introspectie en query-uitvoering  
✅ **RLS-integratie**: Row Level Security contextbeheer  
✅ **Gezondheidsmonitoring**: Uitgebreide gezondheidscontrole-eindpunten  
✅ **Teststrategie**: Lokaal testen en VS Code-integratie  

## 🚀 Wat Nu?

Ga verder met **[Lab 06: Toolontwikkeling](../06-Tools/README.md)** om:

- Je MCP-toolcollectie uit te breiden
- Geavanceerde querypatronen te implementeren
- Data-validatie en transformatie toe te voegen
- Gespecialiseerde analysetools te creëren

## 📚 Aanvullende Bronnen

### FastMCP Framework
- [FastMCP Documentatie](https://github.com/modelcontextprotocol/python-sdk) - Officiële FastMCP-gids
- [MCP Specificatie](https://modelcontextprotocol.io/docs/) - Protocolspecificatie
- [Toolontwikkelingsgids](https://modelcontextprotocol.io/docs/tools/) - MCP-tools maken

### Database-integratie
- [asyncpg Documentatie](https://magicstack.github.io/asyncpg/current/) - PostgreSQL async driver
- [Best Practices voor Connection Pooling](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL tuning
- [Row Level Security Gids](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS-implementatie

### FastAPI Patronen
- [FastAPI Documentatie](https://fastapi.tiangolo.com/) - Webframework referentie
- [Dependency Injection](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI patronen
- [Achtergrondtaken](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Async taakbeheer

---

**Volgende**: Klaar om je tools uit te breiden? Ga verder met [Lab 06: Toolontwikkeling](../06-Tools/README.md)
