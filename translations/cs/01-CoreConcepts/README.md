...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T19:45:11+00:00",
...
- **Bezpečnost při spouštění nástrojů**: Každé spuštění nástroje vyžaduje výslovný souhlas uživatele s jasným pochopením funkce nástroje, parametrů a možných dopadů. Robustní bezpečnostní opatření musí zabránit neúmyslnému, nebezpečnému nebo škodlivému spuštění nástroje.
...
- **Správa oprávnění**: Implementujte detailní systémy oprávnění, které uživatelům umožní kontrolovat, ke kterým serverům, nástrojům a zdrojům mají přístup.
...
- **Validace vstupů**: Validujte všechny parametry a vstupy dat podle definovaných schémat, abyste zabránili útokům typu injection.
- **Auditní logování**: Udržujte komplexní záznamy všech operací pro bezpečnostní monitoring a dodržování předpisů.

[Model Context Protocol (MCP)](https://modelcontextprotocol.io/specification/2025-06-18/) je výkonný, standardizovaný rámec, který optimalizuje komunikaci mezi velkými jazykovými modely (LLM) a externími nástroji, aplikacemi a datovými zdroji. Tento průvodce vás provede základními koncepty MCP, abyste pochopili jeho architekturu klient-server, klíčové komponenty, komunikační mechanismy a osvědčené postupy implementace.
...
- **Lokální datové zdroje**: Soubory, databáze a služby vašeho počítače, ke kterým mohou servery MCP bezpečně přistupovat.
- **Vzdálené služby**: Externí systémy dostupné přes internet, ke kterým se servery MCP mohou připojit prostřednictvím API.

Protokol MCP je vyvíjející se standard používající verzování na základě data (formát YYYY-MM-DD). Aktuální verze protokolu je **2025-06-18**. Nejnovější aktualizace specifikace protokolu naleznete [zde](https://modelcontextprotocol.io/specification/2025-06-18/).

### 1. Hostitelé

V Model Context Protocol (MCP) jsou **Hostitelé** AI aplikace, které slouží jako primární rozhraní, přes které uživatelé interagují s protokolem. Hostitelé koordinují a spravují připojení k více serverům MCP tím, že vytvářejí dedikované klienty MCP pro každé připojení k serveru. Příklady Hostitelů zahrnují:
...
- **Správa připojení klientů**: Vytváření a udržování jednoho klienta MCP na každé připojení k serveru MCP.
- **Ovládání uživatelského rozhraní**: Řízení toku konverzace, interakcí s uživatelem a prezentace odpovědí.  
...
**Klienti** jsou klíčové komponenty, které udržují dedikovaná jednosměrná připojení mezi Hostiteli a servery MCP. Každý klient MCP je vytvořen Hostitelem pro připojení ke konkrétnímu serveru MCP, což zajišťuje organizované a bezpečné komunikační kanály. Více klientů umožňuje Hostitelům připojit se k více serverům současně.

**Klienti** jsou konektory v rámci hostitelské aplikace. Provádějí:

- **Komunikace protokolu**: Odesílání požadavků JSON-RPC 2.0 na servery s výzvami a instrukcemi.
...
**Servery** jsou programy, které poskytují kontext, nástroje a schopnosti klientům MCP. Mohou být spuštěny lokálně (na stejném zařízení jako Hostitel) nebo vzdáleně (na externích platformách) a jsou odpovědné za zpracování požadavků klientů a poskytování strukturovaných odpovědí. Servery zpřístupňují specifické funkce prostřednictvím standardizovaného Model Context Protocol.
...
Servery mohou být vyvíjeny kýmkoli, aby rozšířily schopnosti modelu o specializované funkce, a podporují jak lokální, tak vzdálené scénáře nasazení.
...
Servery MCP mohou zpřístupnit libovolnou kombinaci následujících tří základních primitiv:
...
Zdroje jsou identifikovány pomocí URI a podporují vyhledávání prostřednictvím metod `resources/list` a načítání prostřednictvím `resources/read`:
...
- **Pracovní šablony**: Standardizované sekvence pro běžné úkoly a interakce.
...
Výzvy podporují substituci proměnných a mohou být vyhledány pomocí `prompts/list` a načteny pomocí `prompts/get`:
...
Nástroje jsou definovány pomocí JSON Schema pro validaci parametrů, vyhledávány prostřednictvím `tools/list` a spouštěny pomocí `tools/call`:
...
V Model Context Protocol (MCP) mohou **klienti** zpřístupnit primitivy, které umožňují serverům požadovat další schopnosti od hostitelské aplikace. Tyto primitivy na straně klienta umožňují bohatší a interaktivnější implementace serverů, které mohou přistupovat ke schopnostem AI modelů a interakcím s uživateli.

### Vzorkování

**Vzorkování** umožňuje serverům požadovat dokončení jazykového modelu od AI aplikace klienta. Tento primitiv umožňuje serverům přistupovat ke schopnostem LLM bez nutnosti vlastních závislostí na modelech:

- **Nezávislý přístup k modelu**: Servery mohou požadovat dokončení bez zahrnutí SDK LLM nebo správy přístupu k modelu.
- **AI iniciované serverem**: Umožňuje serverům autonomně generovat obsah pomocí AI modelu klienta.
...
Vzorkování je iniciováno prostřednictvím metody `sampling/complete`, kde servery odesílají požadavky na dokončení klientům.

### Elicitace  

**Elicitace** umožňuje serverům požadovat další informace nebo potvrzení od uživatelů prostřednictvím rozhraní klienta:

- **Požadavky na vstup uživatele**: Servery mohou požádat o další informace potřebné pro spuštění nástroje.
- **Potvrzovací dialogy**: Požadavek na schválení uživatele pro citlivé nebo významné operace.
...
Požadavky na elicitaci jsou prováděny pomocí metody `elicitation/request` pro sběr vstupů uživatele prostřednictvím rozhraní klienta.

### Logování

**Logování** umožňuje serverům odesílat strukturované logovací zprávy klientům pro ladění, monitorování a provozní přehled:

- **Podpora ladění**: Umožňuje serverům poskytovat podrobné logy provádění pro odstraňování problémů.
- **Provozní monitorování**: Odesílání stavových aktualizací a metrik výkonu klientům.
...
Logovací zprávy jsou odesílány klientům, aby poskytly transparentnost operací serveru a usnadnily ladění.

## Tok informací v MCP

Model Context Protocol (MCP) definuje strukturovaný tok informací mezi hostiteli, klienty, servery a modely. Pochopení tohoto toku pomáhá objasnit, jak jsou zpracovávány požadavky uživatelů a jak jsou externí nástroje a data integrovány do odpovědí modelu.

- **Hostitel zahajuje připojení**  
  Hostitelská aplikace (např. IDE nebo chatovací rozhraní) naváže připojení k serveru MCP, obvykle prostřednictvím STDIO, WebSocket nebo jiného podporovaného transportu.

- **Vyjednávání schopností**  
  Klient (vložený v hostiteli) a server si vyměňují informace o svých podporovaných funkcích, nástrojích,
...
- **Notifikace v reálném čase**: Podporuje asynchronní oznámení pro dynamické aktualizace bez nutnosti dotazování  
...
2. **Streamovatelné HTTP**:  
   ...
   - Podporuje standardní autentizaci HTTP (bearer tokeny, API klíče, vlastní hlavičky)  
...
- **Souhlas a kontrola uživatele**: Uživatelé musí poskytnout explicitní souhlas před přístupem k datům nebo provedením operací. Měli by mít jasnou kontrolu nad tím, jaká data jsou sdílena a jaké akce jsou autorizovány, podpořené intuitivním uživatelským rozhraním pro přezkoumání a schválení aktivit.
...
- **Bezpečnost nástrojů**: Před použitím jakéhokoli nástroje je vyžadován explicitní souhlas uživatele. Uživatelé by měli jasně rozumět funkcionalitě každého nástroje a musí být zajištěny robustní bezpečnostní hranice, aby se zabránilo nechtěnému nebo nebezpečnému spuštění nástroje.
...
Tento příklad v JavaScriptu demonstruje, jak vytvořit MCP klienta, který se připojí k serveru, odešle výzvu a zpracuje odpověď včetně jakýchkoli volání nástrojů, která byla provedena.
...
1. **Kontrola oprávnění nástrojů**:  
   Klienti mohou specifikovat, které nástroje může model během relace používat. To zajišťuje, že jsou přístupné pouze explicitně autorizované nástroje, což snižuje riziko nechtěných nebo nebezpečných operací. Oprávnění lze dynamicky konfigurovat na základě preferencí uživatele, organizačních politik nebo kontextu interakce.
...
3. **Validace**:  
   Validace parametrů je vynucována pro všechna volání nástrojů. Každý nástroj definuje očekávané typy, formáty a omezení pro své parametry a server validuje příchozí požadavky odpovídajícím způsobem. To zabraňuje špatně formátovanému nebo škodlivému vstupu dosáhnout implementace nástrojů a pomáhá udržovat integritu operací.
...
- **`initialize` Požadavek**: Zřizuje připojení a vyjednává verzi protokolu a schopnosti  
...
- **`notifications/prompts/list_changed`**: Server upozorňuje klienta na změny šablon výzev  
...
- **Architektura**: MCP používá architekturu klient-server, kde hostitelé spravují více připojení klientů k serverům  
...
- **Základ protokolu**: Postaveno na JSON-RPC 2.0 s verzováním založeným na datu (aktuální: 2025-06-18)  
