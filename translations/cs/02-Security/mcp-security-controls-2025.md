...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T19:58:13+00:00",
...
> **Aktuální standard**: Tento dokument odráží bezpečnostní požadavky [MCP Specifikace 2025-06-18](https://spec.modelcontextprotocol.io/specification/2025-06-18/) a oficiální [MCP Bezpečnostní osvědčené postupy](https://modelcontextprotocol.io/specification/2025-06-18/basic/security_best_practices).

Model Context Protocol (MCP) se významně vyvinul s vylepšenými bezpečnostními kontrolami, které řeší jak tradiční softwarovou bezpečnost, tak hrozby specifické pro umělou inteligenci. Tento dokument poskytuje komplexní bezpečnostní kontroly pro bezpečné implementace MCP k srpnu 2025.
...
**Aktuální standard MCP (2025-06-18)** umožňuje MCP serverům delegovat autentizaci na externí poskytovatele identity, což představuje významné zlepšení bezpečnosti:

**Bezpečnostní Výhody:**
1. **Eliminace Rizik Vlastní Autentizace**: Snižuje povrch zranitelnosti tím, že se vyhýbá vlastním implementacím autentizace  
2. **Podniková Bezpečnostní Úroveň**: Využívá zavedené poskytovatele identity, jako je Microsoft Entra ID, s pokročilými bezpečnostními funkcemi  
...
- **Auditní Logování**: Kompletní logování všech rozhodnutí autorizace pro bezpečnostní monitorování  
...
- **Exfiltrace Prostřednictvím Proxy**: Umožňuje škodlivým aktérům používat servery jako proxy pro neoprávněný přístup k datům  
...
**Osvědčené Postupy:**
- **Krátkodobé Tokeny**: Minimalizujte okno expozice častou rotací tokenů  
- **Vydávání Tokenů na Požádání**: Vydávejte tokeny pouze tehdy, když jsou potřeba pro konkrétní operace  
- **Bezpečné Ukládání**: Používejte hardwarové bezpečnostní moduly (HSM) nebo bezpečné úložiště klíčů  
- **Vázání Tokenů**: Vážte tokeny na konkrétní klienty, relace nebo operace, pokud je to možné  
...
- Správa relací založená na JWT nebo podobných tokenech  
...
- **Schvalovací Procesy**: Výslovný souhlas uživatele pro úpravy nástrojů  
- **Možnosti Vrácení Změn**: Schopnost vrátit se k předchozím verzím nástrojů  
- **Auditování Změn**: Kompletní historie úprav definic nástrojů  
...
- **Validace Přesměrovacích URI**: Přísná validace na základě whitelistu cílových destinací  
...
- **Vynucení Zdroje**: Pevné limity na CPU, paměť a I/O operace  

### **Implementace Minimálních Privilegií**
...
- **Behaviorální Analýza**: Detekce neobvyklého chování v externích komponentech  
...
### **Automatizované Schopnosti Odezvy**
...
- **Sběr Důkazů**: Automatizovaný sběr relevantních bezpečnostních artefaktů  
...
- **Princip Minimálních Privilegií**: Minimální přístupová práva pro všechny komponenty  
