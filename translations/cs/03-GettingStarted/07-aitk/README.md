...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T15:10:53+00:00",
...
Při vytváření AI agenta nejde jen o generování chytrých odpovědí, ale také o to, aby váš agent mohl provádět akce. A právě zde přichází na řadu Model Context Protocol (MCP). MCP umožňuje agentům snadno a konzistentně přistupovat k externím nástrojům a službám. Představte si to jako připojení vašeho agenta k nástrojové sadě, kterou může *skutečně* používat.

Představte si, že připojíte agenta k MCP serveru kalkulačky. Najednou váš agent dokáže provádět matematické operace jen na základě zadání, jako je „Kolik je 47 krát 89?“—bez nutnosti hardcodovat logiku nebo vytvářet vlastní API.
...
> [!WARNING]
> Poznámka pro uživatele macOS. Aktuálně zkoumáme problém, který ovlivňuje instalaci závislostí na macOS. Z tohoto důvodu uživatelé macOS nemohou toto cvičení momentálně dokončit. Jakmile bude k dispozici oprava, aktualizujeme pokyny. Děkujeme za vaši trpělivost a pochopení!
...
**Agent (Prompt) Builder** umožňuje vytvářet a přizpůsobovat vlastní AI agenty. V této části vytvoříte nového agenta a přiřadíte mu model, který bude pohánět konverzaci.
...
Nyní, když jste definovali systémový prompt agenta—který řídí jeho chování a odpovědi—je čas vybavit agenta praktickými schopnostmi. V této části vytvoříte MCP server kalkulačky s nástroji pro provádění výpočtů, jako je sčítání, odčítání, násobení a dělení. Tento server umožní vašemu agentovi provádět matematické operace v reálném čase na základě přirozeného jazyka.

AI Toolkit je vybaven šablonami, které usnadňují vytváření vlastních MCP serverů. Použijeme šablonu pro Python k vytvoření MCP serveru kalkulačky.
...
1. V **Explorer** zobrazení v **Activity Bar** rozbalte adresář **src** a vyberte **server.py**, abyste otevřeli soubor v editoru.
...
1. Zde je rozbor toho, co by mělo proběhnout:
    - Agent vybere nástroje **multiply** a **subtract**, aby pomohl s výpočtem.
    - Příslušné hodnoty `a` a `b` jsou přiřazeny pro nástroj **multiply**.
    - Příslušné hodnoty `a` a `b` jsou přiřazeny pro nástroj **subtract**.
    - Odpověď z každého nástroje je poskytnuta v sekci **Tool Response**.
    - Konečný výstup modelu je uveden v sekci **Model Response**.
1. Zadejte další prompty, abyste agenta dále otestovali. Můžete upravit stávající prompt v poli **User prompt** kliknutím do pole a nahrazením stávajícího promptu.
1. Jakmile dokončíte testování agenta, můžete server zastavit v **terminálu** zadáním **CTRL/CMD+C** pro ukončení.

## Zadání

Zkuste přidat další nástroj do svého souboru **server.py** (např. výpočet druhé odmocniny čísla). Zadejte další prompty, které by vyžadovaly, aby agent využil váš nový nástroj (nebo stávající nástroje). Nezapomeňte restartovat server, aby se načetly nově přidané nástroje.
