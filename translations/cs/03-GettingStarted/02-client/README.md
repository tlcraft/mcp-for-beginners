...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T15:13:36+00:00",
...
- **Vytvořit instanci klienta**. To zahrnuje vytvoření instance klienta a její připojení k vybrané transportní metodě.
- **Rozhodnout, jaké zdroje vypsat**. Váš MCP server obsahuje zdroje, nástroje a výzvy, musíte se rozhodnout, které z nich vypsat.
...
V následující cvičební sekci si dáme čas na rozebrání každého úryvku kódu a vysvětlíme, co se děje.
...
- Importovali potřebné knihovny.
- Vytvořili stdio transport a klienta `mcpClient`. Ten použijeme k vypsání a vyvolání funkcí na MCP Serveru.
...
- Vytvořili třídu klienta, která bere transport jako parametr konstruktoru.
- V metodě `run` vytvořili synchronního MCP klienta pomocí transportu a inicializovali připojení.
- Použili SSE (Server-Sent Events) transport, který je vhodný pro komunikaci založenou na HTTP s MCP servery Java Spring Boot.

#### Rust

Tento Rust klient předpokládá, že server je sourozenecký projekt nazvaný "calculator-server" ve stejném adresáři. Níže uvedený kód spustí server a připojí se k němu.
...
Nyní máme klienta, který se může připojit, pokud bude program spuštěn. Nicméně, zatím nevypisuje své funkce, takže to udělejme nyní:
...
Zde vypisujeme dostupné zdroje pomocí `list_resources()` a nástroje pomocí `list_tools` a tiskneme je.
...
Výše je příklad, jak můžeme vypsat nástroje na serveru. Pro každý nástroj pak tiskneme jeho jméno.
...
1. Pro vytištění výsledku zde je kód pro jeho zpracování:
...
Alternativně můžete spustit kompletní projekt klienta poskytovaný ve složce řešení `03-GettingStarted\02-client\solution\java`:
...
Podívejte se na tento projekt, jak můžete [přidat výzvy a zdroje](https://github.com/modelcontextprotocol/csharp-sdk/blob/main/samples/EverythingServer/Program.cs).
...
- **Funkční strukturu projektu** s řádnými závislostmi a konfigurací.
...
- ✅ **Vyhledávání serveru** (nástroje, zdroje, výzvy, kde je to relevantní)
...
- Mohou být použity jak k vyhledávání, tak k využívání funkcí na serveru.
