...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T21:12:18+00:00",
...
本課程聚焦於在生產環境中開發、測試及部署 MCP 伺服器和功能的進階最佳實踐。隨著 MCP 生態系統日益複雜且重要，遵循既定模式能確保系統的可靠性、可維護性及互操作性。本課程整合了來自實際 MCP 實作的寶貴經驗，指導你打造穩健、高效的伺服器，並有效運用資源、提示及工具。

## 學習目標

完成本課程後，你將能夠：
- 在 MCP 伺服器及功能設計中應用業界最佳實踐
- 制定完整的 MCP 伺服器測試策略
- 為複雜 MCP 應用設計高效且可重用的工作流程模式
- 在 MCP 伺服器中實作適當的錯誤處理、日誌記錄及可觀察性
...
1. **標準化通訊**：MCP 以 JSON-RPC 2.0 為基礎，提供一致的請求、回應及錯誤處理格式，適用於所有實作。

2. **以用戶為中心的設計**：始終優先考慮用戶同意、控制權及透明度。

3. **安全優先**：實施強健的安全措施，包括身份驗證、授權、驗證及速率限制。

4. **模組化架構**：以模組化方式設計 MCP 伺服器，每個工具和資源皆有明確且專注的功能。

5. **有狀態連線**：利用 MCP 能夠跨多次請求維持狀態的特性，實現更連貫且具上下文感知的互動。
...
1. **用戶同意與控制**：在存取資料或執行操作前，務必取得用戶明確同意。清楚提供用戶對資料分享及授權行為的控制權。

2. **資料隱私**：僅在取得明確同意下公開用戶資料，並以適當的存取控制保護資料。防範未授權的資料傳輸。
...
1. **能力協商**：連線建立時交換支援的功能、協議版本、可用工具及資源資訊。

2. **工具設計**：打造專注且單一功能的工具，避免單一工具涵蓋多重責任。

3. **錯誤處理**：實作標準化錯誤訊息及代碼，協助診斷問題、優雅處理失敗並提供可行的回饋。

4. **日誌記錄**：設定結構化日誌以利稽核、除錯及監控協議互動。
...
每個 MCP 工具應有明確且專注的目的。避免打造試圖涵蓋多重責任的龐大工具，應開發專精於特定任務的專用工具。
...
public class ContentDispatcherTool : IMcpTool
{
    ...
    private string DetermineTargetTool(string contentType, string operation)
    {
        return (contentType, operation) switch
        {
            ...
            ("csv", _) =>
IMPORTANT RULES:  
1. 請勿在翻譯內容周圍加上 '''markdown 或其他標籤  
2. 確保翻譯不會過於直譯  
3. 註解也請一併翻譯  
4. 此檔案為 Markdown 格式 - 請勿當作 XML 或 HTML 處理  
5. 請勿翻譯：  
   - [!NOTE], [!WARNING], [!TIP], [!IMPORTANT], [!CAUTION]  
   - 變數名稱、函式名稱、類別名稱  
   - 像 @@INLINE_CODE_x@@ 或 @@CODE_BLOCK_x@@ 這類的佔位符  
   - URL 或路徑  
6. 保持所有原始的 markdown 格式不變  
7. 僅回傳翻譯後的內容，請勿加上任何額外標籤或標記  
請從左至右書寫輸出內容。
("code", _) => "codeAnalyzer",
    ...
    async def execute_with_fallback(self, primary_tool, fallback_tool, parameters):
        try:
            ...
            try:
                ...
                # 兩個工具都失敗
                logging.error(f"主要及備用工具皆失敗。備用錯誤: {str(fallback_error)}")
...
- **忽略效能測試**：在影響生產前找出瓶頸
- **只做孤立測試**：結合單元測試、整合測試與端對端測試
- **API 覆蓋不完整**：確保所有端點與功能都有測試
- **測試環境不一致**：使用容器確保測試環境一致性

## 結論

完整的測試策略對於開發可靠且高品質的 MCP 伺服器至關重要。透過實施本指南中提出的最佳實踐與秘訣，您可以確保 MCP 實作達到最高的品質、可靠性與效能標準。

## 主要重點

1. **工具設計**：遵循單一職責原則，使用依賴注入，並設計可組合性
2. **Schema 設計**：建立清晰且有良好文件的 schema，並加上適當的驗證限制
3. **錯誤處理**：實作優雅的錯誤處理、結構化錯誤回應與重試機制
4. **效能**：使用快取、非同步處理與資源節流
5. **安全性**：徹底的輸入驗證、授權檢查與敏感資料處理
6. **測試**：建立完整的單元測試、整合測試與端對端測試
7. **工作流程模式**：應用既有模式如鏈式、調度器與平行處理

## 練習

設計一個用於文件處理系統的 MCP 工具與工作流程，要求：
...
實作工具的 schema、錯誤處理，以及最適合此場景的工作流程模式。並思考如何測試此實作。
