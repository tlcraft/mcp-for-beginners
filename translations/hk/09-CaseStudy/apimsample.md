...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-14T05:27:06+00:00",
...
Azure API Management 是一項服務，提供您 API 端點之上的閘道功能。它的運作方式是作為您 API 前端的代理，能夠決定如何處理進來的請求。
...
- **AI 功能，如語意快取**、令牌限制與監控等。這些功能能提升回應速度，並幫助您掌握令牌使用狀況。[詳細閱讀](https://learn.microsoft.com/en-us/azure/api-management/genai-gateway-capabilities)。

## 為什麼選擇 MCP + Azure API Management？

Model Context Protocol 正快速成為代理式 AI 應用的標準，並且提供一種一致的方式來公開工具和資料。當您需要「管理」API 時，Azure API Management 是自然的選擇。MCP 伺服器通常會整合其他 API 來處理工具的請求，因此結合 Azure API Management 與 MCP 非常合理。

## 概覽

在這個特定案例中，我們將學習如何將 API 端點公開為 MCP 伺服器。透過這樣做，我們可以輕鬆地將這些端點納入代理式應用，同時利用 Azure API Management 的功能。

## 主要功能

- 您可以選擇想要公開為工具的端點方法。
- 額外功能取決於您在 API 的政策區段中設定的內容，但這裡會示範如何新增速率限制。
...
1. 在 API 中，選擇一個 REST API 來公開為 MCP 伺服器。

1. 選擇一個或多個 API 操作公開為工具。您可以選擇全部操作或特定操作。
...
這裡會使用 Visual Studio Code 和 GitHub Copilot 的 Agent 模式。我們會將 MCP 伺服器加入 *mcp.json*。如此一來，Visual Studio Code 就會成為具代理能力的客戶端，最終使用者可以輸入提示並與該伺服器互動。
...
1. 輸入 API Management 中 MCP 伺服器的 URL。例如：**https://<apim-service-name>.azure-api.net/<api-name>-mcp/sse**（SSE 端點）或 **https://<apim-service-name>.azure-api.net/<api-name>-mcp/mcp**（MCP 端點），注意兩者的傳輸路徑分別是 `/sse` 或 `/mcp`。

1. 輸入您選擇的伺服器 ID。這不是重要值，但有助於您記住此伺服器實例。

1. 選擇將設定儲存到工作區設定或使用者設定。

  - **工作區設定** - 伺服器設定會儲存到當前工作區的 .vscode/mcp.json 檔案中。
  ...
  - **使用者設定** - 伺服器設定會加入全域 *settings.json* 檔案，並在所有工作區可用。設定格式如下：
...
1. 您還需要新增一個標頭設定，確保能正確驗證 Azure API Management。它使用名為 **Ocp-Apim-Subscription-Key** 的標頭。
...
現在無論是在設定檔還是 *.vscode/mcp.json* 中都已完成設定，讓我們試試看。
...
1. 在聊天視窗輸入提示以呼叫工具。例如，如果您選擇了一個查詢訂單資訊的工具，可以向代理詢問訂單狀況。以下是範例提示：
    ...
    **以上畫面會依您設定的工具而異，但概念是您會收到類似的文字回應。**
...
- [使用 Azure API Management VS Code 擴充套件匯入與管理 API](https://learn.microsoft.com/en-us/azure/api-management/visual-studio-code-tutorial)
