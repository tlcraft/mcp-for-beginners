...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T21:16:22+00:00",
...
多模態應用在人工智能中變得越來越重要，能夠實現更豐富的互動和更複雜的任務。Model Context Protocol (MCP) 提供了一個框架，用於構建能處理各種數據類型（如文字、圖片和音頻）的多模態應用。

MCP 不僅支持基於文字的互動，還具備多模態功能，允許模型處理圖片、音頻及其他數據類型。

## 介紹

在本課程中，你將學習如何構建一個多模態應用。
...
- **特定模態工具**：專門處理特定模態（如圖片分析、音頻處理）的工具。
- **統一上下文管理**：維持跨模態上下文的系統。
- **回應生成**：能產生包含多種模態的回應能力。
...
- 建立了一個 `ImageAnalysisTool`，使用假設的 `IImageAnalysisService` 來分析圖片。
- 配置 MCP 伺服器以處理較大請求並支援圖片內容類型。
- 將圖片分析工具註冊到伺服器。
- 實作從 URL 下載圖片並根據請求類型（物件、文字、臉部等）進行分析的方法。
- 以符合 MCP 規範的格式回傳結構化結果。

## 多模態範例：音頻處理

音頻處理是多模態應用中另一個常見的模態。以下示範如何實作一個音頻轉錄工具，能處理音頻檔案並回傳轉錄結果。
...
- 建立了一個 `AudioTranscriptionTool`，能轉錄音頻檔案。
- 定義工具的結構，接受 URL 或 base64 編碼的音頻資料。
- 實作 `execute` 方法來處理音頻處理與轉錄。
- 配置 MCP 伺服器以處理多模態請求，包括音頻和圖片處理。
- 將音頻轉錄工具註冊到伺服器。
- 實作從 URL 下載音頻檔案或解碼 base64 音頻資料的方法。
- 使用 `AudioProcessor` 服務來執行實際的轉錄邏輯。
- 啟動 MCP 伺服器以監聽請求。
