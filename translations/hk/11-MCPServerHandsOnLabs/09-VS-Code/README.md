...
本實驗提供完整指引，教您如何將 MCP 伺服器整合到 VS Code 中，以透過 AI 聊天進行自然語言查詢。您將學習如何配置 VS Code 以最佳化 MCP 使用、調試伺服器連接，以及充分利用 AI 協助的數據庫互動功能。

## 概述

VS Code 的 MCP 整合改變了開發者與數據庫和 API 互動的方式，透過自然語言進行操作。將您的零售 MCP 伺服器連接到 VS Code 聊天功能後，您可以使用對話式 AI 智能查詢銷售數據、產品目錄和商業分析。

此整合讓開發者能夠提出像「顯示本月銷售最好的產品」或「查找 90 天內未購買的客戶」這樣的問題，並獲得結構化的數據回應，而無需撰寫 SQL 查詢。
...
✅ **VS Code MCP 配置**：完成最佳 MCP 整合設置  
✅ **AI 聊天整合**：在 VS Code 中進行自然語言查詢的能力  
✅ **調試工具**：全面的故障排除和連接診斷能力  
✅ **多伺服器設置**：配置多個 MCP 伺服器實例的能力  
✅ **自訂擴展**：針對零售特定功能增強 VS Code 體驗  
