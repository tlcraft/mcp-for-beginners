...
這個實驗課程將指導你使用 FastMCP 框架實現一個可用於生產環境的 MCP 伺服器。你將構建核心伺服器架構，實現數據庫集成，創建數據訪問工具，並為 AI 驅動的零售分析奠定基礎。

## 概述

MCP 伺服器是我們零售分析解決方案的核心。它充當 AI 助手與 PostgreSQL 數據庫之間的橋樑，通過標準化協議提供安全、智能的業務數據訪問。

本實驗課程教你如何按照企業模式和最佳實踐構建一個穩健、可擴展的 MCP 伺服器。
...
- **實現** 數據庫集成，包括連接池和錯誤處理
- **創建** 用於數據庫架構檢視和查詢執行的 MCP 工具
- **配置** 行級安全（Row Level Security）上下文管理
- **添加** 健康監控和可觀察性功能
- **測試** MCP 伺服器的本地實現並使用 VS Code 進行測試
...
- **工具註冊**：使用類型安全的聲明式工具定義
- **RLS 上下文管理**：自動提取用戶身份並設置上下文
- **錯誤處理**：全面的錯誤管理，提供用戶友好的信息
- **生命周期管理**：正確的啟動/關閉和資源清理
- **健康監控**：內置健康檢查端點
- **開發支持**：熱重載和調試功能

## 🏥 健康監控

### 健康檢查實現 (`health_check.py`)
...
✅ **可運行的 MCP 伺服器**：具有數據庫集成的 FastMCP 伺服器  
✅ **配置管理**：基於環境的穩健配置  
✅ **數據庫層**：具有連接池的 PostgreSQL 集成  
✅ **MCP 工具**：架構檢視和查詢執行工具  
✅ **RLS 集成**：行級安全上下文管理  
✅ **健康監控**：全面的健康檢查端點  
✅ **測試策略**：本地測試和 VS Code 集成  
...
### FastMCP 框架
- [FastMCP 文檔](https://github.com/modelcontextprotocol/python-sdk) - 官方 FastMCP 指南
- [MCP 規範](https://modelcontextprotocol.io/docs/) - 協議規範
- [工具開發指南](https://modelcontextprotocol.io/docs/tools/) - 創建 MCP 工具

### 數據庫集成
- [asyncpg 文檔](https://magicstack.github.io/asyncpg/current/) - PostgreSQL 異步驅動
- [連接池最佳實踐](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL 調優
- [行級安全指南](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS 實現

### FastAPI 模式
- [FastAPI 文檔](https://fastapi.tiangolo.com/) - Web 框架參考
- [依賴注入](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI 模式
- [背景任務](https://fastapi.tiangolo.com/tutorial/background-tasks/) - 異步任務管理
