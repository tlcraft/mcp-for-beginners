...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T13:00:37+00:00",
...
本實驗深入探討 Zava 零售系統的 PostgreSQL 資料庫設計。您將學習如何實現一個全面的零售架構，包括向量搜索功能、多租戶資料建模，以及用於資料隔離的行級安全性 (Row Level Security, RLS)。

## 概述

資料庫是我們 MCP 伺服器的基礎，負責存儲多家商店的零售數據，同時保持嚴格的資料隔離。我們使用 PostgreSQL 並結合 pgvector 擴展，啟用語義搜索功能，讓客戶能夠使用自然語言查詢來尋找產品。

我們的架構遵循現代多租戶模式，並通過行級安全性確保使用者只能訪問其授權商店的數據。此方法提供企業級安全性，同時保持最佳性能。
...
- **設計** 可擴展的多租戶零售資料庫架構  
- **實現** PostgreSQL 與 pgvector 的向量搜索功能  
- **配置** 行級安全性以實現資料隔離  
- **生成** 用於測試的逼真樣本數據  
- **優化** 零售工作負載的資料庫性能  
- **實施** 備份與恢復策略  
...
我們的資料庫結合了 PostgreSQL 的企業功能與 pgvector 擴展，實現 AI 驅動的搜索功能：
...
資料庫採用 **共享資料庫，共享架構** 的多租戶模型，並結合行級安全性：
...
✅ **多租戶資料庫設計**：實現行級安全性以確保資料隔離  
✅ **向量搜索功能**：配置 pgvector 以進行語義產品搜索  
✅ **全面架構**：創建適合生產環境的零售資料庫架構  
✅ **樣本數據生成**：生成逼真的測試數據以供開發和測試  
✅ **性能優化**：配置索引並進行查詢優化  
✅ **備份與恢復**：建立可靠的資料保護策略  

## 🚀 下一步

繼續進行 **[實驗 05：MCP 伺服器實現](../05-MCP-Server/README.md)**：

- 構建連接到此資料庫的 FastMCP 伺服器  
- 實現 MCP 協議的資料庫查詢工具  
- 添加使用嵌入的語義搜索功能  
- 配置連接池與錯誤處理  
...
### PostgreSQL 與 pgvector
- [PostgreSQL 文件](https://www.postgresql.org/docs/) - 完整的 PostgreSQL 參考資料  
- [pgvector 擴展](https://github.com/pgvector/pgvector) - PostgreSQL 的向量相似性搜索  
- [PostgreSQL 性能調優](https://wiki.postgresql.org/wiki/Performance_Optimization) - 優化最佳實踐  

### 多租戶架構
- [行級安全性](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - PostgreSQL RLS 文件  
- [多租戶資料架構](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Azure 架構模式  
- [資料庫安全性最佳實踐](https://www.postgresql.org/docs/current/security.html) - PostgreSQL 安全指南  

### 向量資料庫
- [向量搜索基礎](https://www.pinecone.io/learn/vector-database/) - 理解向量資料庫  
- [嵌入模型](https://platform.openai.com/docs/guides/embeddings) - OpenAI 嵌入文件  
- [HNSW 演算法](https://arxiv.org/abs/1603.09320) - 分層可導航小世界圖  

---

**上一章**: [實驗 03：環境設置](../03-Setup/README.md)  
**下一章**: [實驗 05：MCP 伺服器實現](../05-MCP-Server/README.md)  

---

**免責聲明**：  
本文件已使用 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。儘管我們致力於提供準確的翻譯，請注意自動翻譯可能包含錯誤或不準確之處。原始文件的母語版本應被視為權威來源。對於重要信息，建議使用專業人工翻譯。我們對因使用此翻譯而引起的任何誤解或錯誤解釋概不負責。