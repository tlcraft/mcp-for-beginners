...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T09:44:21+00:00",
...
客戶端是自訂的應用程式或腳本，直接與 MCP Server 通訊以請求資源、工具和提示。與使用檢查器工具（提供圖形介面與伺服器互動）不同，撰寫自己的客戶端可以進行程式化和自動化的互動。這讓開發者能將 MCP 功能整合到自己的工作流程中，自動化任務，並打造符合特定需求的客製化解決方案。

## 概覽

本課程介紹 Model Context Protocol (MCP) 生態系統中的客戶端概念。你將學習如何撰寫自己的客戶端並連接到 MCP Server。
...
- **決定要列出哪些資源**。你的 MCP 伺服器包含資源、工具和提示，你需要決定列出哪些。
...
Java 部分，你會建立一個連接到前面練習中 MCP 伺服器的客戶端。使用 [Getting Started with MCP Server](../../../../03-GettingStarted/01-first-server/solution/java) 中相同的 Java Spring Boot 專案結構，在 `src/main/java/com/microsoft/mcp/sample/client/` 資料夾建立一個名為 `SDKClient` 的新 Java 類別，並加入以下匯入：
...
- 定義 `run` 方法，呼叫 `stdio_client` 開啟客戶端會話
...
- 建立 main 方法，設定 SSE 傳輸指向 `http://localhost:8080`，即 MCP 伺服器運行位置
- 建立客戶端類別，建構子接受傳輸物件
- 在 `run` 方法中，使用傳輸建立同步 MCP 客戶端並初始化連線
- 使用 SSE（Server-Sent Events）傳輸，適合與 Java Spring Boot MCP 伺服器的 HTTP 通訊
...
很好，現在我們已取得所有功能。那麼什麼時候使用它們呢？這個客戶端很簡單，必須明確呼叫功能才能使用。下一章我們會建立更進階的客戶端，內建大型語言模型（LLM）。現在先看看如何呼叫伺服器功能：

### -4- 呼叫功能

呼叫功能時，要確保指定正確的參數，有時還要指定要呼叫的名稱。
...
先確保 MCP 伺服器運行於 `http://localhost:8080`，然後執行客戶端：
...
這次作業是利用你學到的知識建立自己的客戶端。

這裡有一個伺服器，你需要透過客戶端程式碼呼叫它，試著為伺服器新增更多功能，讓它更有趣。
...
**解決方案資料夾**包含完整且可直接執行的客戶端實作，示範本教學涵蓋的所有概念。每個解決方案包含獨立的客戶端和伺服器程式碼，組織成自成一格的專案。
...
完整文件與逐步指引請參考：**[📖 解決方案文件](./solution/README.md)**

## 🎯 完整範例

我們提供了涵蓋本教學所有程式語言的完整且可運作的客戶端實作。這些範例展示上述所有功能，可作為參考實作或你專案的起點。

### 可用的完整範例

| 語言 | 檔案 | 說明 |
|------|------|------|
| **Java** | [`client_example_java.java`](../../../../03-GettingStarted/02-client/client_example_java.java) | 使用 SSE 傳輸的完整 Java 客戶端，含完善錯誤處理 |
...
1. **從上表選擇你偏好的語言**
2. **閱讀完整範例檔案，了解完整實作**
3. **依照 [`complete_examples.md`](./complete_examples.md) 中指示執行範例**
4. **根據需求修改並擴充範例**
...
| 近似生產環境設定 | 教學參考用途 |
| 語言專屬工具鏈 | 跨語言比較 |
兩種方法都很有價值——完整專案請使用 **solution folder**，學習和參考請使用 **complete examples**。
