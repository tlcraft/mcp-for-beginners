...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T15:49:22+00:00",
...
[Model Context Protocol (MCP)](https://github.com/modelcontextprotocol) 是一個強大且標準化的框架，優化大型語言模型（LLMs）與外部工具、應用程式及資料來源之間的溝通。這份經過 SEO 優化的指南將帶你了解 MCP 的核心概念，確保你掌握其客戶端-伺服器架構、重要組件、通訊機制及實作最佳實踐。
...
- **遠端服務**：透過網際網路可用的外部系統，MCP 伺服器可透過 API 連接

MCP 協定是一個持續演進的標準，你可以在[協定規範](https://modelcontextprotocol.io/specification/2025-06-18/)查看最新更新。

### 1. Hosts

在 Model Context Protocol (MCP) 中，Hosts 扮演關鍵角色，是使用者與協定互動的主要介面。Hosts 是啟動與 MCP 伺服器連線以存取資料、工具和提示的應用程式或環境。Hosts 範例包括整合開發環境（IDEs）如 Visual Studio Code、AI 工具如 Claude Desktop，或為特定任務設計的自訂代理程式。
...
Clients 是促進 Hosts 與 MCP 伺服器互動的重要組件。Clients 作為中介，讓 Hosts 能存取並利用 MCP 伺服器提供的功能。它們在確保 MCP 架構內通訊順暢及資料交換效率上扮演關鍵角色。
...
Servers 負責處理 MCP clients 的請求並提供適當回應。它們管理各種操作，如資料擷取、工具執行及提示產生。Servers 確保客戶端與 Hosts 之間的通訊高效且可靠，維護互動過程的完整性。
...
Model Context Protocol (MCP) 中的工具是 AI 模型可執行以完成特定任務的函式。這些工具設計用來提升 AI 模型的能力，提供結構化且可靠的操作。主要特點包括：
...
## Client 功能
在 Model Context Protocol (MCP) 中，客戶端向伺服器提供多項關鍵功能，提升協定的整體功能與互動。其中一項重要功能是 Sampling。
...
- **功能協商**  
  客戶端（嵌入於 Host）與伺服器交換彼此支援的功能、工具、資源及協定版本資訊，確保雙方了解本次會話可用的能力。
...
MCP (Model Context Protocol) 建構於 [JSON-RPC 2.0](https://www.jsonrpc.org/) 之上，提供標準化、語言無關的訊息格式，用於 Hosts、Clients 與 Servers 之間的通訊。此基礎確保跨多平台與程式語言的可靠、結構化且可擴充互動。

### 主要協定特性

MCP 擴充 JSON-RPC 2.0，加入工具調用、資源存取及提示管理的額外慣例。支援多種傳輸層（STDIO、WebSocket、SSE），並實現安全、可擴充且語言無關的元件通訊。

#### 🧢 基本協定

- **JSON-RPC 訊息格式**：所有請求與回應皆遵循 JSON-RPC 2.0 規範，確保方法呼叫、參數、結果及錯誤處理結構一致。
- **有狀態連線**：MCP 會話在多次請求間維持狀態，支援持續對話、上下文累積及資源管理。
- **功能協商**：連線建立時，客戶端與伺服器交換支援功能、協定版本、可用工具及資源資訊，確保雙方了解彼此能力並能相應調整。

#### ➕ 額外工具

以下是 MCP 提供的部分額外工具與協定擴充，提升開發者體驗並支援進階場景：
...
- **日誌記錄**：客戶端與伺服器皆可產生結構化日誌，用於稽核、除錯及監控協定互動。

透過這些協定特性，MCP 確保語言模型與外部工具或資料來源間的通訊穩健、安全且靈活。
...
- **工具安全**：調用任何工具前需取得明確使用者同意。使用者應充分了解每個工具的功能，並強制執行嚴格的安全邊界，防止非預期或不安全的工具執行。

遵循這些原則，MCP 確保在所有協定互動中維護使用者信任、隱私與安全。
...
這是一個實用的 .NET 程式碼範例，展示如何實作簡單的 MCP 伺服器及自訂工具。範例示範如何定義與註冊工具、處理請求，以及使用 Model Context Protocol 連接伺服器。
...
3. **驗證**：  
  所有工具調用都會強制執行參數驗證。每個工具定義其參數的預期類型、格式和限制，伺服器會依此驗證傳入請求。這防止格式錯誤或惡意輸入進入工具實作，有助維護操作的完整性。

4. **速率限制**：  
  為防止濫用並確保伺服器資源的公平使用，MCP 伺服器可對工具調用和資源存取實施速率限制。速率限制可依用戶、會話或全局設定，有助防範拒絕服務攻擊或過度資源消耗。

透過結合這些機制，MCP 提供一個安全的基礎，讓語言模型能與外部工具和資料來源整合，同時讓使用者和開發者能細緻控制存取和使用權限。
...
這些結構化訊息確保 MCP 工作流程中的每個步驟都明確、可追蹤且可擴充，支援多輪對話、工具串接及健全的錯誤處理等進階場景。
