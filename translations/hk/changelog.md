...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-15T21:57:06+00:00",
...
此文件記錄了針對「模型上下文協議 (MCP) 初學者課程」所作的所有重要更改。更改以時間倒序記錄（最新更改在最前）。

## 2025年9月15日

### 高階主題擴展 - 自訂傳輸與上下文工程

#### MCP 自訂傳輸 (05-AdvancedTopics/mcp-transport/) - 新增高階實作指南
- **README.md**：完整的自訂 MCP 傳輸機制實作指南
  - **Azure Event Grid 傳輸**：全面的無伺服器事件驅動傳輸實作
    - 提供 C#、TypeScript 和 Python 範例，並整合 Azure Functions
    - 適用於可擴展 MCP 解決方案的事件驅動架構模式
    - Webhook 接收器與推送式訊息處理
  - **Azure Event Hubs 傳輸**：高吞吐量的串流傳輸實作
    - 適用於低延遲場景的即時串流功能
    - 分區策略與檢查點管理
    - 訊息批次處理與效能優化
  - **企業整合模式**：生產級架構範例
    - 分散式 MCP 處理，跨多個 Azure Functions
    - 結合多種傳輸類型的混合傳輸架構
    - 訊息持久性、可靠性與錯誤處理策略
  - **安全性與監控**：整合 Azure Key Vault 與可觀察性模式
    - 管理式身份驗證與最低權限存取
...
#### 上下文工程 (05-AdvancedTopics/mcp-contextengineering/) - 新興的 AI 學科
...
#### 課程導航更新 (README.md)
- **增強的模組結構**：更新課程表以包含新的高階主題
  - 新增上下文工程 (5.14) 和自訂傳輸 (5.15) 條目
  - 所有模組的格式與導航連結保持一致
  - 更新描述以反映當前內容範圍

### 目錄結構改進
- **命名標準化**：將「mcp transport」重命名為「mcp-transport」，以與其他高階主題文件夾保持一致
...
- **MCP-SECURITY-BEST-PRACTICES-2025.md**：根據 MCP 規範 2025-06-18 完整重寫
...
- **MCP-SECURITY-CONTROLS-2025.md**：全面改寫，提供企業級安全框架
  - **9 個全面安全領域**：從基本控制擴展為詳細的企業框架
    ...
    - 使用 SIEM 整合的監控與檢測控制
    - 提供自動化能力的事件響應與恢復
  - **實作範例**：新增詳細的 YAML 配置塊與程式碼範例
  - **Microsoft 解決方案整合**：全面涵蓋 Azure 安全服務、GitHub Advanced Security 與企業身份管理

#### 高階主題安全 (05-AdvancedTopics/mcp-security/) - 生產級實作
...
- **實用實作**：新增 .NET、Java 和 Python 的詳細程式碼範例，並提供企業模式
...
- **協議架構**：引入雙層架構模型
  ...
  - 傳輸層：STDIO（本地）與可串流 HTTP，包含 SSE（遠端）傳輸機制
- **安全框架**：全面的安全原則，包括明確的用戶同意、數據隱私保護、工具執行安全與傳輸層安全
- **通信模式**：更新協議訊息以展示初始化、發現、執行與通知流程
- **程式碼範例**：刷新多語言範例（.NET、Java、Python、JavaScript），以反映當前 MCP SDK 模式
...
### 文件品質改進
- **結構化學習目標**：增強學習目標，提供具體且可行的成果
- **交叉引用**：新增相關安全性與核心概念主題之間的連結
- **最新資訊**：更新所有日期參考與規範連結至最新標準
- **實作指導**：在兩個部分中新增具體且可行的實作指導

## 2025年7月16日

### README 與導航改進
- 完全重新設計 README.md 中的課程導航
- 用更易於訪問的表格格式取代 `<details>` 標籤
