...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T22:45:41+00:00",
...
- کارکردگی اور اعتبار کے لیے MCP نفاذ کو بہتر بنائیں
...
Azure API Management ایک بہترین حل ہے کہ ہم MCP سرورز کو کیسے محفوظ بنا سکتے ہیں۔ خیال یہ ہے کہ آپ اپنے MCP سرور کے سامنے Azure API Management انسٹانس رکھیں اور اسے وہ خصوصیات سنبھالنے دیں جو آپ چاہتے ہیں، جیسے:
...
1. `Microsoft.App` resource provider کو رجسٹر کریں۔  
    * اگر آپ Azure CLI استعمال کر رہے ہیں، تو `az provider register --namespace Microsoft.App --wait` چلائیں۔  
    * اگر آپ Azure PowerShell استعمال کر رہے ہیں، تو `Register-AzResourceProvider -ProviderNamespace Microsoft.App` چلائیں۔ پھر کچھ دیر بعد `(Get-AzResourceProvider -ProviderNamespace Microsoft.App).RegistrationState` چلا کر رجسٹریشن مکمل ہونے کی تصدیق کریں۔  

2. یہ [azd](https://aka.ms/azd) کمانڈ چلائیں تاکہ API Management سروس، فنکشن ایپ (کوڈ کے ساتھ)، اور دیگر تمام ضروری Azure وسائل فراہم کیے جا سکیں:

    ```shell
    azd up
    ```

    یہ کمانڈ Azure پر تمام کلاؤڈ وسائل تعینات کر دے گا۔
...
1. CTRL کلک کریں تاکہ MCP Inspector ویب ایپ کو اس URL سے لوڈ کیا جا سکے جو ایپ دکھا رہی ہے (مثلاً http://127.0.0.1:6274/#resources)  
1. ٹرانسپورٹ کی قسم `SSE` پر سیٹ کریں  
1. URL کو اپنے چلتے ہوئے API Management SSE اینڈپوائنٹ پر سیٹ کریں جو `azd up` کے بعد دکھایا گیا ہو اور **Connect** کریں:
...
اگر تمام مراحل کامیاب ہو گئے ہیں، تو آپ اب MCP سرور سے جُڑ چکے ہیں اور ٹول کال کرنے میں کامیاب ہو گئے ہیں۔
...
- لوکل مشین پر MCP سرور بنائیں اور چلائیں: لوکل مشین پر MCP سرور تیار کریں اور ڈیبگ کریں  
...
- MCP SDKs مخصوص زبانوں کے لیے مضبوط MCP حل نافذ کرنے کے ٹولز فراہم کرتے ہیں  
...
- MCP حل نافذ کرتے وقت سیکیورٹی، کارکردگی، اور ایرر ہینڈلنگ کو مدنظر رکھنا ضروری ہے  
