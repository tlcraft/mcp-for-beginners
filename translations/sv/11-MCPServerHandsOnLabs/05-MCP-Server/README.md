...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T18:02:46+00:00",
...
Denna praktiska labb guidar dig genom att implementera en produktionsklar MCP-server med hjälp av FastMCP-ramverket. Du kommer att bygga serverns kärnstruktur, implementera databasintegration, skapa verktyg för dataåtkomst och lägga grunden för AI-driven detaljhandelsanalys.

## Översikt

MCP-servern är hjärtat i vår lösning för detaljhandelsanalys. Den fungerar som en bro mellan AI-assistenter och PostgreSQL-databasen, och erbjuder säker, intelligent åtkomst till affärsdata via ett standardiserat protokoll.

Denna labb lär dig att bygga en robust och skalbar MCP-server enligt företagsmönster och bästa praxis.

## Lärandemål

Efter denna labb kommer du att kunna:

- **Bygga** en FastMCP-server med korrekt arkitektur och organisation
- **Implementera** databasintegration med anslutningspoolning och felhantering
- **Skapa** MCP-verktyg för databasens schemaanalys och frågeexekvering
- **Konfigurera** hantering av Row Level Security (RLS)-kontext
- **Lägga till** funktioner för hälsokontroll och observabilitet
- **Testa** din MCP-serverimplementering lokalt och med VS Code

## 📁 Projektstruktur

Låt oss undersöka MCP-serverns organisation:
...
### Miljökonfiguration (`config.py`)

Först skapar vi ett robust konfigurationssystem:
...
### Viktiga Funktioner i Konfigurationen

- **Laddning av Miljövariabler**: Automatisk stöd för .env-filer
- **Typ Säkerhet**: Validering med dataklasser och typanvisningar
- **Flexibla Standardvärden**: Förnuftiga standardvärden för utveckling
- **Validering**: Validering av konfiguration med hjälpsamma felmeddelanden
- **Säkerhet**: Känsliga värden endast från miljövariabler

## 🗄️ Databasintegrationslager

### PostgreSQL-leverantör (`sales_analysis_postgres.py`)

Låt oss implementera databasintegrationslagret:
...
### Viktiga Funktioner i Databaslagret

- **Anslutningspoolning**: Effektiv resursförvaltning med asyncpg
- **RLS Integration**: Automatisk inställning av Row Level Security-kontext
- **Schemaanalys**: Dynamisk upptäckt av tabellens schema
- **Felhantering**: Omfattande felhantering och loggning
- **Frågeformatering**: AI-vänlig resultatformatering
- **Hälsokontroll**: Kontroll av databasanslutning och poolstatus

## 🔧 Huvudsaklig MCP Server-implementering

### FastMCP Server (`sales_analysis.py`)

Nu implementerar vi den huvudsakliga MCP-servern:
...
### Viktiga Funktioner i MCP-servern

- **Verktygsregistrering**: Deklarativa verktygsdefinitioner med typanvisningar
- **RLS Kontexthantering**: Automatisk extraktion av användaridentitet och kontextinställning
- **Felhantering**: Omfattande felhantering med användarvänliga meddelanden
- **Livscykelhantering**: Korrekt start/stopp med resursrensning
- **Hälsokontroll**: Inbyggda hälsokontrolländpunkter
- **Utvecklingsstöd**: Stöd för hot reload och felsökning

## 🏥 Hälsokontroll

### Implementering av Hälsokontroll (`health_check.py`)
...
2. **Testa Hälsokontrolländpunkter**:
   ```bash
   # Basic health check
...
### VS Code Integrationstestning

1. **Konfigurera VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Testa i AI Chat**:
   ...
   - Skriv `#zava` och välj din server
   - Fråga: "Vilka tabeller är tillgängliga?"
   - Fråga: "Visa de 5 bästa butikerna baserat på antal beställningar"

### Enhetstestning

Skapa omfattande enhetstester:
...
Efter att ha genomfört denna labb bör du ha:

✅ **Fungerande MCP-server**: FastMCP-server med databasintegration  
✅ **Konfigurationshantering**: Robust miljöbaserad konfiguration  
✅ **Databaslager**: PostgreSQL-integration med anslutningspoolning  
✅ **MCP-verktyg**: Verktyg för schemaanalys och frågeexekvering  
✅ **RLS Integration**: Hantering av Row Level Security-kontext  
✅ **Hälsokontroll**: Omfattande hälsokontrolländpunkter  
✅ **Teststrategi**: Lokal testning och VS Code-integration  

## 🚀 Vad Är Nästa Steg

Fortsätt med **[Lab 06: Verktygsutveckling](../06-Tools/README.md)** för att:

- Utöka din samling av MCP-verktyg
- Implementera avancerade frågemönster
- Lägga till datavalidering och transformation
- Skapa specialiserade analysverktyg

## 📚 Ytterligare Resurser

### FastMCP Ramverk
- [FastMCP Dokumentation](https://github.com/modelcontextprotocol/python-sdk) - Officiell FastMCP-guide
- [MCP Specifikation](https://modelcontextprotocol.io/docs/) - Protokollspecifikation
- [Guide för Verktygsutveckling](https://modelcontextprotocol.io/docs/tools/) - Skapa MCP-verktyg

### Databasintegration
- [asyncpg Dokumentation](https://magicstack.github.io/asyncpg/current/) - PostgreSQL async-drivrutin
- [Bästa Praxis för Anslutningspoolning](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL-optimering
- [Guide för Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS-implementering

### FastAPI Mönster
- [FastAPI Dokumentation](https://fastapi.tiangolo.com/) - Referens för webbramverk
- [Dependency Injection](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI-mönster
- [Bakgrundsuppgifter](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Hantering av asynkrona uppgifter

---

**Nästa**: Redo att utöka dina verktyg? Fortsätt med [Lab 06: Verktygsutveckling](../06-Tools/README.md)
