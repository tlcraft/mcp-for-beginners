...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T17:37:16+00:00",
...
در این بخش، مکانیزم‌های مختلف انتقال موجود در MCP و نقش آن‌ها در فعال‌سازی قابلیت‌های پخش برای ارتباط بلادرنگ بین کلاینت‌ها و سرورها بررسی می‌شود.

### مکانیزم انتقال چیست؟

مکانیزم انتقال نحوه تبادل داده‌ها بین کلاینت و سرور را تعریف می‌کند. MCP از چندین نوع انتقال پشتیبانی می‌کند تا با محیط‌ها و نیازهای مختلف سازگار باشد:
...
- **Streamable HTTP**: انتقال پخش مبتنی بر HTTP مدرن، که از اعلان‌ها و مقیاس‌پذیری بهتر پشتیبانی می‌کند. برای اکثر سناریوهای تولید و ابری توصیه می‌شود.

### جدول مقایسه

برای درک تفاوت‌های این مکانیزم‌های انتقال، جدول زیر را مشاهده کنید:
...
**پخش** تکنیکی در برنامه‌نویسی شبکه است که اجازه می‌دهد داده‌ها به صورت بخش‌های کوچک و قابل مدیریت یا به صورت دنباله‌ای از رویدادها ارسال و دریافت شوند، به جای اینکه منتظر آماده شدن کل پاسخ بمانیم. این روش به ویژه برای موارد زیر مفید است:
...
در سطح کلی، نکات زیر را درباره پخش باید بدانید:
...
- `ServerSentEvent` برای پخش ساختاریافته رویدادها با انواع رویداد
- `WebClient` با `bodyToFlux()` برای مصرف پخش واکنشی
- `delayElements()` برای شبیه‌سازی زمان پردازش بین رویدادها
- رویدادها می‌توانند انواع مختلفی داشته باشند (`info`، `result`) برای مدیریت بهتر در کلاینت
...
|-----------------------|-----------------------------|---------------------------------|
| پاسخ اصلی             | بخش‌بندی شده                | تک‌پاسخ، در پایان               |
...
- **الگوی ارتباط:**
   - پخش کلاسیک HTTP: از کدگذاری انتقال بخش‌بندی شده ساده برای ارسال داده‌ها استفاده می‌کند
   - پخش MCP: از سیستم اعلان ساختاریافته با پروتکل JSON-RPC استفاده می‌کند
...
نتیجه اصلی همچنان به صورت یک پاسخ واحد ارسال می‌شود. با این حال، اعلان‌ها می‌توانند به صورت پیام‌های جداگانه در طول پردازش ارسال شوند و به این ترتیب کلاینت را به صورت بلادرنگ به‌روزرسانی کنند. کلاینت باید قادر به دریافت و نمایش این اعلان‌ها باشد.
...
با سمت سرور شروع کنیم. در MCP، ابزارهایی تعریف می‌کنید که می‌توانند در حین پردازش درخواست‌ها اعلان ارسال کنند. سرور از شیء context (معمولاً `ctx`) برای ارسال پیام به کلاینت استفاده می‌کند.
...
علاوه بر این، برای فعال کردن اعلان‌ها، مطمئن شوید سرور از انتقال پخش (مانند `streamable-http`) استفاده می‌کند و کلاینت هندلر پیام برای پردازش اعلان‌ها پیاده‌سازی کرده است. در اینجا نحوه تنظیم سرور برای استفاده از انتقال `streamable-http` آمده است:
...
در این مثال .NET، ابزار `ProcessFiles` با صفت `Tool` تزئین شده و در حین پردازش هر فایل سه اعلان به کلاینت ارسال می‌کند. متد `ctx.Info()` برای ارسال پیام‌های اطلاعاتی استفاده شده است.
...
کلاینت باید هندلر پیام پیاده‌سازی کند تا اعلان‌ها را هنگام رسیدن پردازش و نمایش دهد.
...
در کد بالا، تابع `message_handler` بررسی می‌کند که آیا پیام ورودی اعلان است یا خیر. اگر اعلان باشد، آن را چاپ می‌کند؛ در غیر این صورت، به عنوان پیام عادی سرور پردازش می‌شود. همچنین توجه کنید که `ClientSession` با `message_handler` برای مدیریت اعلان‌های ورودی مقداردهی اولیه شده است.
...
در این مثال .NET، تابع `MessageHandler` بررسی می‌کند که آیا پیام ورودی اعلان است یا خیر. اگر اعلان باشد، آن را چاپ می‌کند؛ در غیر این صورت، به عنوان پیام عادی سرور پردازش می‌شود. `ClientSession` با هندلر پیام از طریق `ClientSessionOptions` مقداردهی اولیه شده است.

برای فعال کردن اعلان‌ها، مطمئن شوید سرور از انتقال پخش (مانند `streamable-http`) استفاده می‌کند و کلاینت هندلر پیام برای پردازش اعلان‌ها پیاده‌سازی کرده است.
...
- **اشکال‌زدایی و نظارت آسان‌تر:** توسعه‌دهندگان و کاربران می‌توانند ببینند کجا فرآیند ممکن است کند یا متوقف شده باشد.
...
هنگام پیاده‌سازی سرورهای MCP با انتقال‌های مبتنی بر HTTP، امنیت به یک نگرانی حیاتی تبدیل می‌شود که نیازمند توجه دقیق به چندین بردار حمله و مکانیزم‌های محافظتی است.

### مرور کلی

امنیت هنگام در معرض قرار دادن سرورهای MCP از طریق HTTP بسیار مهم است. Streamable HTTP سطوح جدیدی از حملات را معرفی می‌کند و نیازمند پیکربندی دقیق است.

### نکات کلیدی
- **اعتبارسنجی هدر Origin**: همیشه هدر `Origin` را اعتبارسنجی کنید تا از حملات DNS rebinding جلوگیری شود.
- **بایند به localhost**: برای توسعه محلی، سرورها را به `localhost` محدود کنید تا از دسترسی عمومی جلوگیری شود.
- **احراز هویت**: برای استقرار در تولید، احراز هویت (مثلاً کلید API، OAuth) را پیاده‌سازی کنید.
- **CORS**: سیاست‌های Cross-Origin Resource Sharing را برای محدود کردن دسترسی تنظیم کنید.
...
- به‌روزرسانی‌های منظم وابستگی‌ها برای رفع آسیب‌پذیری‌های امنیتی انجام دهید.
...
برای برنامه‌هایی که در حال حاضر از Server-Sent Events (SSE) استفاده می‌کنند، مهاجرت به Streamable HTTP قابلیت‌های بهبود یافته و پایداری بلندمدت بهتری برای پیاده‌سازی‌های MCP شما فراهم می‌کند.
### چرا ارتقا دهیم؟
...
توصیه می‌شود در طول فرایند مهاجرت، سازگاری با کلاینت‌های SSE موجود حفظ شود. در اینجا چند راهکار آورده شده است:

- می‌توانید هر دو روش SSE و Streamable HTTP را با اجرای هر دو حمل‌ونقل روی نقاط انتهایی مختلف پشتیبانی کنید.
- به تدریج کلاینت‌ها را به حمل‌ونقل جدید منتقل کنید.

### چالش‌ها

اطمینان حاصل کنید که در طول مهاجرت به چالش‌های زیر رسیدگی می‌شود:

- به‌روزرسانی همه کلاینت‌ها
- مدیریت تفاوت‌ها در تحویل اعلان‌ها

## ملاحظات امنیتی

امنیت باید در هنگام پیاده‌سازی هر سروری، به‌ویژه هنگام استفاده از حمل‌ونقل‌های مبتنی بر HTTP مانند Streamable HTTP در MCP، در اولویت باشد.

هنگام پیاده‌سازی سرورهای MCP با حمل‌ونقل‌های مبتنی بر HTTP، امنیت به یک نگرانی اساسی تبدیل می‌شود که نیازمند توجه دقیق به چندین بردار حمله و مکانیزم‌های محافظتی است.

### مرور کلی

امنیت هنگام در معرض قرار دادن سرورهای MCP از طریق HTTP حیاتی است. Streamable HTTP سطوح جدیدی از حمله را معرفی می‌کند و نیازمند پیکربندی دقیق است.

در اینجا چند نکته کلیدی امنیتی آورده شده است:

- **اعتبارسنجی هدر Origin**: همیشه هدر `Origin` را برای جلوگیری از حملات DNS rebinding اعتبارسنجی کنید.
- **بایند به localhost**: برای توسعه محلی، سرورها را به `localhost` محدود کنید تا از در معرض قرار گرفتن در اینترنت عمومی جلوگیری شود.
...
علاوه بر این، در هنگام پیاده‌سازی امنیت در سرور استریمینگ MCP خود، این بهترین شیوه‌ها را دنبال کنید:

- هرگز به درخواست‌های ورودی بدون اعتبارسنجی اعتماد نکنید.
- تمام دسترسی‌ها و خطاها را ثبت و نظارت کنید.
- به‌طور منظم وابستگی‌ها را به‌روزرسانی کنید تا آسیب‌پذیری‌های امنیتی رفع شوند.

### چالش‌ها

در پیاده‌سازی امنیت در سرورهای استریمینگ MCP با چالش‌های زیر مواجه خواهید شد:
...
**سناریو:**
یک سرور و کلاینت MCP بسازید که سرور یک فهرست از آیتم‌ها (مثلاً فایل‌ها یا اسناد) را پردازش کند و برای هر آیتم پردازش شده یک اعلان ارسال کند. کلاینت باید هر اعلان را به محض دریافت نمایش دهد.

**مراحل:**

1. یک ابزار سرور پیاده‌سازی کنید که فهرستی را پردازش کرده و برای هر آیتم اعلان ارسال کند.
...
برای ادامه مسیر خود در استریمینگ MCP و گسترش دانش، این بخش منابع اضافی و گام‌های پیشنهادی بعدی برای ساخت برنامه‌های پیشرفته‌تر را ارائه می‌دهد.

### مطالعه بیشتر

- [Microsoft: مقدمه‌ای بر HTTP Streaming](https://learn.microsoft.com/aspnet/core/fundamentals/http-requests?view=aspnetcore-8.0&WT.mc_id=%3Fwt.mc_id%3DMVP_452430#streaming)
