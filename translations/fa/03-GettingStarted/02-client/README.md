...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-11T11:25:49+00:00",
...
این درس مفهوم کلاینت‌ها در اکوسیستم پروتکل مدل کانتکست (MCP) را معرفی می‌کند. شما یاد خواهید گرفت که چگونه کلاینت خود را بنویسید و آن را به یک سرور MCP متصل کنید.
...
- **تصمیم بگیرید که چه منابعی را لیست کنید**. سرور MCP شما دارای منابع، ابزارها و درخواست‌ها است، شما باید تصمیم بگیرید که کدام یک را لیست کنید.
- **کلاینت را به یک برنامه میزبان ادغام کنید**. هنگامی که توانایی‌های سرور را شناختید، باید آن را به برنامه میزبان خود ادغام کنید تا اگر کاربر یک درخواست یا دستور دیگر وارد کرد، ویژگی مربوطه سرور فراخوانی شود.
...
بیایید در بخش تمرین بعدی وقت بگذاریم و هر قطعه کد را تجزیه کنیم و توضیح دهیم که چه اتفاقی می‌افتد.

## تمرین: نوشتن یک کلاینت

همانطور که گفته شد، بیایید وقت بگذاریم و کد را توضیح دهیم، و اگر می‌خواهید، همراه با کدنویسی کنید.
...
بیایید به مرحله ایجاد کلاینت و انتقال برویم.
...
- یک نمونه انتقال stdio ایجاد کرده‌ایم. توجه کنید که چگونه دستور و آرگومان‌ها را برای پیدا کردن و راه‌اندازی سرور مشخص می‌کند، زیرا این چیزی است که هنگام ایجاد کلاینت به آن نیاز خواهیم داشت.
...
- یک متد `run` تعریف کرده‌ایم که به نوبه خود `stdio_client` را فراخوانی می‌کند و یک جلسه کلاینت را شروع می‌کند.
...
توجه داشته باشید، در "Arguments"، می‌توانید به *.csproj* یا به فایل اجرایی اشاره کنید.
...
- در متد `run`، یک کلاینت MCP همزمان با استفاده از انتقال ایجاد کرده‌ایم و اتصال را راه‌اندازی کرده‌ایم.
...
در اینجا منابع موجود را با `list_resources()` و ابزارها را با `list_tools` لیست کرده و چاپ می‌کنیم.
...
در بالا مثالی از نحوه لیست کردن ابزارها در سرور آورده شده است. برای هر ابزار، نام آن را چاپ می‌کنیم.
...
- `listTools()` را فراخوانی کرده‌ایم تا همه ابزارهای موجود را از سرور MCP دریافت کنیم.
- از `ping()` برای تأیید اینکه اتصال به سرور کار می‌کند استفاده کرده‌ایم.
- `ListToolsResult` شامل اطلاعاتی درباره همه ابزارها از جمله نام‌ها، توضیحات و طرح‌های ورودی است.
...
- درخواست را دریافت کرده‌ایم، برای دریافت یک درخواست، `getPrompt()` را با `name` و `arguments` فراخوانی می‌کنیم. کد سرور به این شکل است:
...
- ابزارهای سرور انتظار پارامترهای خاصی دارند (مانند "a"، "b" برای عملیات ریاضی).
...
ابتدا مطمئن شوید که سرور MCP شما در `http://localhost:8080` اجرا می‌شود. سپس کلاینت را اجرا کنید:
...
در اینجا یک سرور وجود دارد که باید از طریق کد کلاینت خود فراخوانی کنید، ببینید آیا می‌توانید ویژگی‌های بیشتری به سرور اضافه کنید تا جالب‌تر شود.
...
پوشه **راه‌حل** شامل پیاده‌سازی‌های کامل و آماده اجرا کلاینت است که تمام مفاهیم پوشش داده شده در این آموزش را نشان می‌دهد. هر راه‌حل شامل کد کلاینت و سرور است که در پروژه‌های جداگانه و مستقل سازماندهی شده‌اند.
...
| **Rust** | [`client_example_rust.rs`](../../../../03-GettingStarted/02-client/client_example_rust.rs) | کلاینت کامل Rust با استفاده از Tokio برای عملیات async |
هر مثال کامل شامل موارد زیر است:

- ✅ **برقراری اتصال** و مدیریت خطاها  
- ✅ **کشف سرور** (ابزارها، منابع، دستورات در صورت نیاز)  
...
4. **مثال را برای نیازهای خاص خود تغییر دهید و گسترش دهید**  

برای مستندات دقیق‌تر درباره اجرای این مثال‌ها و سفارشی‌سازی آن‌ها، به این بخش مراجعه کنید: **[📖 مستندات مثال‌های کامل](./complete_examples.md)**  

### 💡 تفاوت راه‌حل و مثال‌های کامل

| **پوشه راه‌حل**       | **مثال‌های کامل**         |
|--------------------|---------------------|
| ساختار کامل پروژه با فایل‌های بیلد | پیاده‌سازی‌های تک‌فایلی |
| آماده اجرا با وابستگی‌ها         | مثال‌های کد متمرکز     |
| تنظیمات شبیه به محیط تولید       | مرجع آموزشی            |
...
- می‌توان از آن‌ها برای کشف و استفاده از قابلیت‌های سرور استفاده کرد.  
...
- در کنار ابزارهای جایگزین مانند Inspector که در فصل قبل توضیح داده شد، راهی عالی برای آزمایش قابلیت‌های سرور هستند.  
...
- بعدی: [ساخت کلاینت با LLM](../03-llm-client/README.md)  
