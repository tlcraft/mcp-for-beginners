...
ایجاد یک توکن GitHub فرآیندی ساده است. اینجا نحوه انجام آن آمده است:

- به تنظیمات GitHub بروید – روی عکس پروفایل خود در گوشه بالا سمت راست کلیک کنید و Settings را انتخاب کنید.
- به Developer Settings بروید – پایین صفحه اسکرول کنید و روی Developer Settings کلیک کنید.
...
ابتدا باید وابستگی‌های LangChain4j را به فایل `pom.xml` خود اضافه کنید. این وابستگی‌ها برای فعال‌سازی ادغام MCP و پشتیبانی از مدل‌های GitHub لازم هستند:
...
- **یک `ChatLanguageModel` ایجاد کردیم**: که با توکن GitHub شما برای استفاده از مدل‌های GitHub پیکربندی شده است
...
- متد `run` را ایجاد کردیم که مسئول مدیریت جریان برنامه است. تا اینجا فقط ابزارها را فهرست می‌کند اما به زودی موارد بیشتری به آن اضافه خواهیم کرد.
...
- برای هر ابزار، نام، توضیح و اسکیمای آن را فهرست کردیم. این مورد آخر چیزی است که بعداً برای فراخوانی ابزارها استفاده خواهیم کرد.
...
1. کد زیر را برای تبدیل پاسخ از سرور MCP به فرمت ابزاری که LLM می‌تواند استفاده کند، اضافه کنید:
    ...
    کد بالا پاسخ سرور MCP را گرفته و به فرمت تعریف ابزار تبدیل می‌کند که LLM می‌تواند آن را بفهمد.
...
1. حالا ببینیم چگونه می‌توانیم کد موجود را به‌روزرسانی کنیم تا از این تابع استفاده کنیم:
    ...
    - تابع را به‌روزرسانی کردیم تا پاسخ ابزار MCP را به ابزار LLM تبدیل کند. بخش کدی که اضافه کردیم را برجسته می‌کنیم:
...
عالی است، حالا آماده‌ایم درخواست‌های کاربر را مدیریت کنیم، پس به سراغ آن می‌رویم.
...
1. یک مرحله آخر، ببینیم آیا LLM فکر می‌کند باید تابعی فراخوانی شود:
    ...
    - روی لیست فراخوانی توابع پیمایش کردیم.
...
کد تمرین را بردارید و سرور را با ابزارهای بیشتری توسعه دهید. سپس یک کلاینت با LLM مانند تمرین ایجاد کنید و با پرامپت‌های مختلف آن را تست کنید تا مطمئن شوید همه ابزارهای سرور شما به صورت پویا فراخوانی می‌شوند. این روش ساخت کلاینت باعث می‌شود کاربر نهایی تجربه بسیار خوبی داشته باشد، چون می‌تواند از پرامپت‌ها استفاده کند، نه دستورات دقیق کلاینت، و از فراخوانی هر سرور MCP بی‌خبر باشد.
