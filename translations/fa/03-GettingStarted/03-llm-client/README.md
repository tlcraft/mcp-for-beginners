...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-11T11:22:44+00:00",
...
تا اینجا، شما یاد گرفتید که چگونه یک سرور و یک کلاینت ایجاد کنید. کلاینت توانسته است به صورت صریح سرور را فراخوانی کند تا ابزارها، منابع و پرامپت‌های آن را لیست کند. اما این روش خیلی عملی نیست. کاربر شما در عصر عامل‌ها زندگی می‌کند و انتظار دارد از پرامپت‌ها استفاده کند و با یک LLM ارتباط برقرار کند. برای کاربر شما، مهم نیست که آیا شما از MCP برای ذخیره قابلیت‌ها استفاده می‌کنید یا نه، بلکه انتظار دارد از زبان طبیعی برای تعامل استفاده کند. پس چگونه این مشکل را حل کنیم؟ راه‌حل اضافه کردن یک LLM به کلاینت است.

## مرور کلی

در این درس، ما بر اضافه کردن یک LLM به کلاینت تمرکز می‌کنیم و نشان می‌دهیم که چگونه این کار تجربه بهتری برای کاربر فراهم می‌کند.
...
1. اتصال به سرور را برقرار کنید.
...
1. یک پرامپت کاربر را با ارسال آن به LLM همراه با ابزارهای لیست‌شده توسط کلاینت مدیریت کنید.
...
- یک کلاس با دو عضو، `client` و `openai` ایجاد کردیم که به ما کمک می‌کند یک کلاینت را مدیریت کنیم و با یک LLM تعامل کنیم.
...
- **کتابخانه‌های LangChain4j را وارد کردیم**: برای یکپارچگی MCP و عملکرد مدل چت OpenAI.
...
این کد یک برنامه ساده Rust را تنظیم می‌کند که به یک سرور MCP و مدل‌های GitHub برای تعاملات LLM متصل می‌شود.
...
1. ببینیم چگونه می‌توانیم کد موجود را به‌روزرسانی کنیم تا از این تابع استفاده کنیم:
    ...
    - تابع را به‌روزرسانی کردیم تا پاسخ ابزار MCP را به ابزار LLM تبدیل کند. بیایید کدی که اضافه کردیم را برجسته کنیم:
        ...
        طرح ورودی بخشی از پاسخ ابزار است اما در ویژگی "properties" قرار دارد، بنابراین باید آن را استخراج کنیم. علاوه بر این، اکنون `ConvertFrom` را با جزئیات ابزار فراخوانی می‌کنیم. حالا که کارهای سنگین را انجام دادیم، بیایید ببینیم چگونه همه چیز با هم جمع می‌شود وقتی که پرامپت کاربر را مدیریت می‌کنیم.
...
1. یک متد اضافه کنید که برای فراخوانی LLM استفاده می‌شود:
    ...
    - متد بررسی می‌کند که آیا LLM ابزاری را فراخوانی کرده است یا خیر:
...
1. سپس تابعی که LLM را فراخوانی می‌کند اضافه کنید:
    ...
    - نتیجه را بررسی کردیم تا ببینیم چه توابعی باید فراخوانی شوند، اگر وجود داشته باشند.
    - در نهایت، آرایه‌ای از توابع برای فراخوانی ارسال کردیم.
...
1. یک مرحله دیگر، ببینیم آیا LLM فکر می‌کند باید تابعی فراخوانی شود:
    ...
    - برای هر ابزار، نام و آرگومان‌ها را استخراج کردیم و ابزار را در سرور MCP با استفاده از کلاینت MCP فراخوانی کردیم. در نهایت نتایج را چاپ کردیم.
...
اینجا جایی است که بیشتر کار انجام می‌شود. ما LLM را با پرامپت اولیه کاربر فراخوانی می‌کنیم، سپس پاسخ را پردازش می‌کنیم تا ببینیم آیا ابزارهایی باید فراخوانی شوند یا خیر. اگر چنین باشد، آن ابزارها را فراخوانی می‌کنیم و مکالمه را با LLM ادامه می‌دهیم تا زمانی که دیگر نیازی به فراخوانی ابزارها نباشد و پاسخ نهایی را داشته باشیم.
ما چندین بار به مدل زبان (LLM) درخواست ارسال خواهیم کرد، بنابراین بیایید یک تابع تعریف کنیم که این درخواست‌ها را مدیریت کند. کد زیر را به فایل `main.rs` خود اضافه کنید:
...
پاسخ LLM شامل یک آرایه از `choices` خواهد بود. ما باید نتیجه را پردازش کنیم تا ببینیم آیا `tool_calls` وجود دارد یا خیر. این به ما اطلاع می‌دهد که آیا LLM درخواست استفاده از یک ابزار خاص با آرگومان‌ها را دارد. کد زیر را به انتهای فایل `main.rs` خود اضافه کنید تا تابعی برای مدیریت پاسخ LLM تعریف شود:
...
برای استخراج اطلاعات فراخوانی ابزار که LLM برای درخواست‌های MCP بازمی‌گرداند، یک تابع کمکی دیگر اضافه خواهیم کرد تا همه چیز مورد نیاز برای انجام فراخوانی را استخراج کند. کد زیر را به انتهای فایل `main.rs` خود اضافه کنید:
...
با داشتن همه قطعات، اکنون می‌توانیم درخواست اولیه کاربر را مدیریت کرده و به LLM فراخوانی کنیم. تابع `main` خود را به‌روزرسانی کنید تا کد زیر را شامل شود:
...
این کد LLM را با درخواست اولیه کاربر برای جمع دو عدد پرس‌وجو می‌کند و پاسخ را پردازش می‌کند تا فراخوانی ابزارها را به‌صورت پویا مدیریت کند.

عالی، شما موفق شدید!

## تکلیف

کد تمرین را بگیرید و سرور را با ابزارهای بیشتری توسعه دهید. سپس یک کلاینت با LLM، مشابه تمرین، ایجاد کنید و آن را با درخواست‌های مختلف آزمایش کنید تا مطمئن شوید همه ابزارهای سرور شما به‌صورت پویا فراخوانی می‌شوند. این روش ساخت کلاینت به این معناست که کاربر نهایی تجربه کاربری عالی خواهد داشت، زیرا می‌تواند از درخواست‌های ساده استفاده کند، به جای دستورات دقیق کلاینت، و از وجود سرور MCP بی‌اطلاع باشد.
