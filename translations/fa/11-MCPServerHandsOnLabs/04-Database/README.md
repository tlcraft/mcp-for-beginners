...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T14:20:04+00:00",
  "source_file": "11-MCPServerHandsOnLabs/04-Database/README.md",
  "language_code": "fa"
}
-->
# طراحی پایگاه داده و اسکیمای آن

## 🎯 موضوعات این آزمایشگاه

این آزمایشگاه به طراحی پایگاه داده PostgreSQL برای سیستم خرده‌فروشی Zava می‌پردازد. شما یاد خواهید گرفت که یک اسکیمای جامع خرده‌فروشی با قابلیت جستجوی برداری، مدل‌سازی داده‌های چند مستاجری و امنیت سطح ردیف (RLS) برای جداسازی داده‌ها پیاده‌سازی کنید.

## مرور کلی

پایگاه داده اساس سرور MCP ما است که داده‌های خرده‌فروشی را در چندین فروشگاه ذخیره می‌کند و در عین حال جداسازی دقیق داده‌ها را حفظ می‌کند. ما از PostgreSQL با افزونه pgvector استفاده می‌کنیم تا قابلیت‌های جستجوی معنایی را فعال کنیم، به مشتریان اجازه می‌دهیم محصولات را با استفاده از پرسش‌های زبان طبیعی پیدا کنند.

اسکیمای ما از الگوهای مدرن چند مستاجری پیروی می‌کند و امنیت سطح ردیف تضمین می‌کند که کاربران فقط به داده‌های فروشگاه‌های مجاز خود دسترسی داشته باشند. این رویکرد امنیت در سطح سازمانی را فراهم می‌کند و در عین حال عملکرد بهینه را حفظ می‌کند.

## اهداف یادگیری

در پایان این آزمایشگاه، شما قادر خواهید بود:

- **طراحی** اسکیمای پایگاه داده خرده‌فروشی چند مستاجری مقیاس‌پذیر  
- **پیاده‌سازی** PostgreSQL با pgvector برای جستجوی برداری  
- **پیکربندی** امنیت سطح ردیف برای جداسازی داده‌ها  
- **تولید** داده‌های نمونه واقعی برای آزمایش  
- **بهینه‌سازی** عملکرد پایگاه داده برای بارهای کاری خرده‌فروشی  
- **پیاده‌سازی** استراتژی‌های پشتیبان‌گیری و بازیابی  

## 🗃️ معماری پایگاه داده

### PostgreSQL با pgvector

پایگاه داده ما از ویژگی‌های سازمانی PostgreSQL همراه با افزونه pgvector برای جستجوی مبتنی بر هوش مصنوعی استفاده می‌کند:
...
پایگاه داده از مدل چند مستاجری **پایگاه داده مشترک، اسکیمای مشترک** با امنیت سطح ردیف استفاده می‌کند:
...
## 🔍 پیاده‌سازی جستجوی برداری

### جدول تعبیه‌های محصولات
...
## 🔐 تنظیم امنیت سطح ردیف

### نقش‌ها و مجوزهای پایگاه داده
...
### اسکریپت داده‌های آزمایشی واقعی
...
✅ **طراحی پایگاه داده چند مستاجری**: امنیت سطح ردیف را برای جداسازی امن داده‌ها پیاده‌سازی کرده باشید  
✅ **قابلیت‌های جستجوی برداری**: pgvector را برای جستجوی معنایی محصولات پیکربندی کرده باشید  
✅ **اسکیمای جامع**: اسکیمای پایگاه داده خرده‌فروشی آماده تولید ایجاد کرده باشید  
✅ **تولید داده‌های نمونه**: داده‌های آزمایشی واقعی برای توسعه و آزمایش ساخته باشید  
✅ **بهینه‌سازی عملکرد**: شاخص‌ها و بهینه‌سازی پرس‌وجو را پیکربندی کرده باشید  
✅ **پشتیبان‌گیری و بازیابی**: استراتژی‌های حفاظت از داده‌های قوی ایجاد کرده باشید  

## 🚀 گام بعدی

ادامه دهید با **[آزمایشگاه ۰۵: پیاده‌سازی سرور MCP](../05-MCP-Server/README.md)** برای:

- ساخت سرور FastMCP که به این پایگاه داده متصل می‌شود  
- پیاده‌سازی ابزارهای پرس‌وجوی پایگاه داده برای پروتکل MCP  
- افزودن قابلیت‌های جستجوی معنایی با استفاده از تعبیه‌ها  
- پیکربندی اتصال استخر و مدیریت خطا  

## 📚 منابع اضافی

### PostgreSQL و pgvector
- [مستندات PostgreSQL](https://www.postgresql.org/docs/) - مرجع کامل PostgreSQL  
- [افزونه pgvector](https://github.com/pgvector/pgvector) - جستجوی شباهت برداری برای PostgreSQL  
- [تنظیم عملکرد PostgreSQL](https://wiki.postgresql.org/wiki/Performance_Optimization) - بهترین روش‌های بهینه‌سازی  

### معماری چند مستاجری
- [امنیت سطح ردیف](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - مستندات RLS PostgreSQL  
- [معماری داده‌های چند مستاجری](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - الگوهای معماری Azure  
- [بهترین روش‌های امنیت پایگاه داده](https://www.postgresql.org/docs/current/security.html) - راهنمای امنیت PostgreSQL  

### پایگاه داده‌های برداری
- [مبانی جستجوی برداری](https://www.pinecone.io/learn/vector-database/) - درک پایگاه داده‌های برداری  
- [مدل‌های تعبیه](https://platform.openai.com/docs/guides/embeddings) - مستندات تعبیه‌های OpenAI  
- [الگوریتم HNSW](https://arxiv.org/abs/1603.09320) - گراف‌های کوچک قابل پیمایش سلسله‌مراتبی  

---

**قبلی**: [آزمایشگاه ۰۳: تنظیم محیط](../03-Setup/README.md)  
**بعدی**: [آزمایشگاه ۰۵: پیاده‌سازی سرور MCP](../05-MCP-Server/README.md)  
