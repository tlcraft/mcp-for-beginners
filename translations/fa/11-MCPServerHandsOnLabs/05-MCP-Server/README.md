...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T13:45:07+00:00",
...
این آزمایشگاه عملی شما را در پیاده‌سازی یک سرور MCP آماده تولید با استفاده از فریم‌ورک FastMCP راهنمایی می‌کند. شما ساختار اصلی سرور را ایجاد می‌کنید، یکپارچگی پایگاه داده را پیاده‌سازی می‌کنید، ابزارهایی برای دسترسی به داده‌ها ایجاد می‌کنید و پایه‌ای برای تحلیل‌های خرده‌فروشی مبتنی بر هوش مصنوعی فراهم می‌کنید.

## مرور کلی

سرور MCP قلب راه‌حل تحلیل‌های خرده‌فروشی ما است. این سرور به عنوان پلی بین دستیارهای هوش مصنوعی و پایگاه داده PostgreSQL عمل می‌کند و دسترسی امن و هوشمند به داده‌های کسب‌وکار را از طریق یک پروتکل استاندارد فراهم می‌کند.

این آزمایشگاه به شما آموزش می‌دهد که چگونه یک سرور MCP قوی و مقیاس‌پذیر را با الگوها و بهترین شیوه‌های سازمانی بسازید.

## اهداف آموزشی

در پایان این آزمایشگاه، شما قادر خواهید بود:

- **ساختن** یک سرور FastMCP با معماری و سازمان‌دهی مناسب  
- **پیاده‌سازی** یکپارچگی پایگاه داده با مدیریت اتصال و رسیدگی به خطا  
- **ایجاد** ابزارهای MCP برای بررسی ساختار پایگاه داده و اجرای کوئری‌ها  
- **پیکربندی** مدیریت زمینه امنیت سطح ردیف (Row Level Security)  
- **افزودن** ویژگی‌های نظارت بر سلامت و مشاهده‌پذیری  
- **آزمایش** پیاده‌سازی سرور MCP خود به صورت محلی و با VS Code  

## 📁 ساختار پروژه

بیایید سازمان‌دهی سرور MCP را بررسی کنیم:
...
### پیکربندی محیط (`config.py`)

ابتدا یک سیستم پیکربندی قوی ایجاد کنیم:
...
### ویژگی‌های کلیدی پیکربندی

- **بارگذاری متغیرهای محیطی**: پشتیبانی خودکار از فایل .env  
- **ایمنی نوع**: اعتبارسنجی کلاس داده و اشاره‌های نوع  
- **پیش‌فرض‌های انعطاف‌پذیر**: پیش‌فرض‌های منطقی برای توسعه  
- **اعتبارسنجی**: اعتبارسنجی پیکربندی با پیام‌های خطای مفید  
- **امنیت**: مقادیر حساس فقط از متغیرهای محیطی  

## 🗄️ لایه یکپارچگی پایگاه داده

### ارائه‌دهنده PostgreSQL (`sales_analysis_postgres.py`)

بیایید لایه یکپارچگی پایگاه داده را پیاده‌سازی کنیم:
...
### ویژگی‌های کلیدی لایه پایگاه داده

- **مدیریت اتصال**: مدیریت منابع کارآمد با asyncpg  
- **یکپارچگی RLS**: تنظیم خودکار زمینه امنیت سطح ردیف  
- **بررسی ساختار**: کشف پویا ساختار جدول  
- **مدیریت خطا**: مدیریت جامع خطا و ثبت گزارش  
- **فرمت‌بندی کوئری**: فرمت‌بندی نتایج مناسب برای هوش مصنوعی  
- **نظارت بر سلامت**: بررسی اتصال پایگاه داده و وضعیت مدیریت اتصال  

## 🔧 پیاده‌سازی اصلی سرور MCP

### سرور FastMCP (`sales_analysis.py`)

اکنون بیایید سرور MCP اصلی را پیاده‌سازی کنیم:
...
- **مدیریت زمینه RLS**: استخراج خودکار هویت کاربر و تنظیم زمینه  
- **مدیریت خطا**: مدیریت جامع خطا با پیام‌های کاربرپسند  
- **مدیریت چرخه عمر**: راه‌اندازی/خاموشی مناسب با پاکسازی منابع  
- **نظارت بر سلامت**: نقاط پایانی بررسی سلامت داخلی  
- **پشتیبانی توسعه**: قابلیت بارگذاری مجدد و اشکال‌زدایی  

## 🏥 نظارت بر سلامت

### پیاده‌سازی بررسی سلامت (`health_check.py`)
...
### آزمایش یکپارچگی با VS Code

1. **پیکربندی VS Code MCP**:  
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **آزمایش در چت هوش مصنوعی**:  
   - VS Code AI Chat را باز کنید  
   - تایپ کنید `#zava` و سرور خود را انتخاب کنید  
   - بپرسید: "چه جدول‌هایی موجود هستند؟"  
   - بپرسید: "5 فروشگاه برتر بر اساس تعداد سفارش‌ها را نشان بده"  

### آزمایش واحد

آزمایش‌های واحد جامع ایجاد کنید:
...
پس از تکمیل این آزمایشگاه، شما باید داشته باشید:

✅ **سرور MCP عملیاتی**: سرور FastMCP با یکپارچگی پایگاه داده  
✅ **مدیریت پیکربندی**: پیکربندی قوی مبتنی بر محیط  
✅ **لایه پایگاه داده**: یکپارچگی PostgreSQL با مدیریت اتصال  
✅ **ابزارهای MCP**: ابزارهای بررسی ساختار و اجرای کوئری  
✅ **یکپارچگی RLS**: مدیریت زمینه امنیت سطح ردیف  
✅ **نظارت بر سلامت**: نقاط پایانی جامع بررسی سلامت  
✅ **استراتژی آزمایش**: آزمایش محلی و یکپارچگی با VS Code  

## 🚀 مرحله بعدی

ادامه دهید با **[آزمایشگاه 06: توسعه ابزار](../06-Tools/README.md)** برای:

- گسترش مجموعه ابزارهای MCP  
- پیاده‌سازی الگوهای پیشرفته کوئری  
- افزودن اعتبارسنجی و تبدیل داده‌ها  
- ایجاد ابزارهای تحلیل تخصصی  

## 📚 منابع اضافی

### فریم‌ورک FastMCP
- [مستندات FastMCP](https://github.com/modelcontextprotocol/python-sdk) - راهنمای رسمی FastMCP  
- [مشخصات MCP](https://modelcontextprotocol.io/docs/) - مشخصات پروتکل  
- [راهنمای توسعه ابزار](https://modelcontextprotocol.io/docs/tools/) - ایجاد ابزارهای MCP  

### یکپارچگی پایگاه داده
- [مستندات asyncpg](https://magicstack.github.io/asyncpg/current/) - درایور async برای PostgreSQL  
- [بهترین شیوه‌های مدیریت اتصال](https://www.postgresql.org/docs/current/runtime-config-connection.html) - تنظیم PostgreSQL  
- [راهنمای امنیت سطح ردیف](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - پیاده‌سازی RLS  

### الگوهای FastAPI
- [مستندات FastAPI](https://fastapi.tiangolo.com/) - مرجع فریم‌ورک وب  
- [تزریق وابستگی](https://fastapi.tiangolo.com/tutorial/dependencies/) - الگوهای FastAPI  
- [وظایف پس‌زمینه](https://fastapi.tiangolo.com/tutorial/background-tasks/) - مدیریت وظایف async  

---

**مرحله بعدی**: آماده گسترش ابزارهای خود هستید؟ ادامه دهید با [آزمایشگاه 06: توسعه ابزار](../06-Tools/README.md)
