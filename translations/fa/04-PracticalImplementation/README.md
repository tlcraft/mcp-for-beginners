...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T22:45:11+00:00",
...
پیاده‌سازی عملی جایی است که قدرت Model Context Protocol (MCP) به صورت ملموس نمایان می‌شود. در حالی که درک نظریه و معماری پشت MCP اهمیت دارد، ارزش واقعی زمانی آشکار می‌شود که این مفاهیم را برای ساخت، آزمایش و استقرار راه‌حل‌هایی که مشکلات دنیای واقعی را حل می‌کنند، به کار ببرید. این فصل فاصله بین دانش مفهومی و توسعه عملی را پر می‌کند و شما را در فرآیند زنده کردن برنامه‌های مبتنی بر MCP راهنمایی می‌کند.

چه در حال توسعه دستیارهای هوشمند باشید، چه در حال ادغام هوش مصنوعی در جریان‌های کاری کسب‌وکار، یا ساخت ابزارهای سفارشی برای پردازش داده‌ها، MCP پایه‌ای انعطاف‌پذیر فراهم می‌کند. طراحی مستقل از زبان آن و SDKهای رسمی برای زبان‌های برنامه‌نویسی محبوب، دسترسی به آن را برای طیف گسترده‌ای از توسعه‌دهندگان ممکن می‌سازد. با بهره‌گیری از این SDKها، می‌توانید به سرعت نمونه اولیه بسازید، تکرار کنید و راه‌حل‌های خود را در پلتفرم‌ها و محیط‌های مختلف مقیاس دهید.

در بخش‌های بعدی، مثال‌های عملی، کد نمونه و استراتژی‌های استقرار را خواهید یافت که نشان می‌دهند چگونه MCP را در C#، Java، TypeScript، JavaScript و Python پیاده‌سازی کنید. همچنین یاد می‌گیرید چگونه سرورهای MCP خود را اشکال‌زدایی و آزمایش کنید، APIها را مدیریت کنید و راه‌حل‌ها را با استفاده از Azure به ابر منتقل کنید. این منابع عملی برای تسریع یادگیری شما و کمک به ساخت برنامه‌های MCP مقاوم و آماده تولید طراحی شده‌اند.

## مرور کلی

این درس بر جنبه‌های عملی پیاده‌سازی MCP در چند زبان برنامه‌نویسی تمرکز دارد. ما بررسی می‌کنیم چگونه از SDKهای MCP در C#، Java، TypeScript، JavaScript و Python برای ساخت برنامه‌های مقاوم، اشکال‌زدایی و آزمایش سرورهای MCP و ایجاد منابع، پرامپت‌ها و ابزارهای قابل استفاده مجدد استفاده کنیم.
...
مخزن رسمی SDK C# شامل چند نمونه پیاده‌سازی است که جنبه‌های مختلف MCP را نشان می‌دهد:
...
SDK C# MCP در حالت پیش‌نمایش است و APIها ممکن است تغییر کنند. ما این بلاگ را به‌روزرسانی خواهیم کرد تا با تکامل SDK هماهنگ باشد.
...
برای نمونه کامل پیاده‌سازی جاوا، به [نمونه جاوا](samples/java/containerapp/README.md) در دایرکتوری نمونه‌ها مراجعه کنید.
...
SDK پایتون رویکردی پایتونیک برای پیاده‌سازی MCP با ادغام عالی با فریم‌ورک‌های یادگیری ماشین ارائه می‌دهد.
...
Azure API Management پاسخ مناسبی برای چگونگی ایمن‌سازی سرورهای MCP است. ایده این است که یک نمونه Azure API Management را جلوی سرور MCP خود قرار دهید و اجازه دهید ویژگی‌هایی که احتمالاً می‌خواهید مانند موارد زیر را مدیریت کند:
...
اطلاعات بیشتر درباره [مشخصات مجوزدهی MCP](https://modelcontextprotocol.io/specification/2025-03-26/basic/authorization#2-10-third-party-authorization-flow) را بیاموزید.
...
1. ارائه‌دهنده منابع `Microsoft.App` را ثبت کنید.
    ...
    * اگر از Azure PowerShell استفاده می‌کنید، دستور `Register-AzResourceProvider -ProviderNamespace Microsoft.App` را اجرا کنید. سپس پس از مدتی با دستور `(Get-AzResourceProvider -ProviderNamespace Microsoft.App).RegistrationState` بررسی کنید که ثبت‌نام کامل شده است یا خیر.
...
1. با نگه داشتن کلید CTRL روی URL نمایش داده شده توسط برنامه کلیک کنید تا برنامه وب MCP Inspector بارگذاری شود (مثلاً http://127.0.0.1:6274/#resources)
1. نوع انتقال را روی `SSE` تنظیم کنید
1. URL را به نقطه انتهایی SSE مدیریت API خود که پس از اجرای `azd up` نمایش داده شده است تنظیم کنید و **اتصال** دهید:
...
[Remote-mcp-functions](https://github.com/Azure-Samples/remote-mcp-functions-dotnet): این مجموعه مخازن قالب شروع سریع برای ساخت و استقرار سرورهای سفارشی MCP از راه دور با استفاده از Azure Functions با Python، C# .NET یا Node/TypeScript است.
