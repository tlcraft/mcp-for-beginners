...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T16:35:00+00:00",
...
- ریسک‌های امنیتی منحصر به فردی که پروتکل مدل کانتکست (MCP) ایجاد می‌کند را شناسایی و توضیح دهید، از جمله تزریق پرامپت، مسمومیت ابزار، دسترسی‌های بیش از حد و آسیب‌پذیری‌های زنجیره تأمین.
- کنترل‌های مؤثر کاهش‌دهنده ریسک‌های امنیتی MCP را شرح داده و به کار ببرید، مانند احراز هویت قوی، حداقل امتیاز دسترسی، مدیریت امن توکن و تأیید زنجیره تأمین.
...
- اهمیت اعتبارسنجی متادیتای ابزار، نظارت بر تغییرات دینامیک و دفاع در برابر حملات تزریق پرامپت غیرمستقیم را بشناسید.
...
بیایید نگاهی به برخی از روش‌هایی بیندازیم که می‌توانید هنگام پذیرش MCP برای مقابله با ریسک‌های امنیتی شروع کنید.
...
سرور MCP قادر به شناسایی یا تمایز بین کلاینت‌های MCP نخواهد بود وقتی کلاینت‌ها با توکن دسترسی صادر شده از بالا دست تماس می‌گیرند که ممکن است برای سرور MCP مبهم باشد.  
...
اگر سرور MCP توکن‌ها را بدون اعتبارسنجی ادعاهای آن‌ها (مانند نقش‌ها، امتیازات یا مخاطب) یا متادیتای دیگر ارسال کند، بازیگر مخرب در اختیار داشتن توکن سرقت شده می‌تواند از سرور به عنوان پروکسی برای استخراج داده‌ها استفاده کند.
...
- **اجرای شیوه‌های امن توکن:** از [بهترین شیوه‌های مایکروسافت برای اعتبارسنجی توکن و طول عمر آن](https://learn.microsoft.com/en-us/entra/identity-platform/access-tokens) پیروی کنید تا از سوءاستفاده از توکن‌های دسترسی جلوگیری کرده و ریسک تکرار یا سرقت توکن را کاهش دهید.
...
- **حملات تزریق پرامپت:** مهاجمان دستورالعمل‌های مخرب را در پرامپت‌ها یا محتوای خارجی جاسازی می‌کنند که باعث می‌شود سیستم هوش مصنوعی اقدامات ناخواسته انجام دهد یا داده‌های حساس را افشا کند. برای اطلاعات بیشتر: [Prompt Injection](https://simonwillison.net/2025/Apr/9/mcp-prompt-injection/)
...
- **تغییر دینامیک ابزار (Rug Pulls):** تعاریف ابزار می‌توانند پس از تأیید کاربر تغییر کنند و رفتارهای مخرب جدیدی بدون اطلاع کاربر معرفی کنند.
...
## ریسک‌ها
اقدامات ناخواسته هوش مصنوعی انواع مختلفی از ریسک‌های امنیتی را شامل می‌شود که از جمله آن‌ها استخراج داده و نقض حریم خصوصی است.
...
- **به تهدیدات سریع پاسخ دهید:** فرآیندی برای وصله‌کردن یا جایگزینی اجزای آسیب‌دیده و چرخش اسرار یا اعتبارنامه‌ها در صورت شناسایی نفوذ داشته باشید.
...
- بهترین روش‌های برنامه‌نویسی امن در برنامه هوش مصنوعی شما - محافظت در برابر [OWASP Top 10](https://owasp.org/www-project-top-ten/)، [OWASP Top 10 برای LLMها](https://genai.owasp.org/download/43299/?tmstv=1731900559)، استفاده از خزانه‌های امن برای اسرار و توکن‌ها، پیاده‌سازی ارتباطات امن انتها به انتها بین تمام اجزای برنامه و غیره.
...
- به‌روزرسانی دستگاه‌ها، زیرساخت‌ها و برنامه‌ها با وصله‌های جدید
...
- اصول امنیتی همچنان حیاتی هستند: برنامه‌نویسی امن، کمترین امتیاز، تأیید زنجیره تأمین و نظارت مداوم برای بارهای کاری MCP و هوش مصنوعی ضروری‌اند.
- MCP ریسک‌های جدیدی مانند تزریق پرامپت، مسمومیت ابزار و مجوزهای بیش از حد را معرفی می‌کند که نیازمند کنترل‌های سنتی و خاص هوش مصنوعی هستند.
...
- همه اجزا در زنجیره تأمین هوش مصنوعی خود — از جمله مدل‌ها، تعبیه‌ها و ارائه‌دهندگان زمینه — را با همان دقت وابستگی‌های کد در نظر بگیرید.
