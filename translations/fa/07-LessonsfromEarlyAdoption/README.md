...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-20T15:51:36+00:00",
...
- مدیریت پرسش‌های استاندارد را در بخش‌های مختلف حفظ کنند  
- کنترل‌های امنیتی و انطباق قوی را اجرا کنند  
- به‌راحتی بین مدل‌های هوش مصنوعی مختلف بر اساس نیازهای خاص جابجا شوند  
...
**نتایج:** کاهش ۳۰ درصدی هزینه‌های مدل، بهبود ۴۵ درصدی در ثبات پاسخ‌ها و افزایش انطباق در عملیات جهانی.

### مطالعه موردی ۲: دستیار تشخیص پزشکی

یک ارائه‌دهنده خدمات بهداشتی زیرساخت MCP را برای ادغام چندین مدل تخصصی هوش مصنوعی پزشکی توسعه داد و در عین حال از محافظت داده‌های حساس بیماران اطمینان حاصل کرد:

- جابجایی بدون درز بین مدل‌های عمومی و تخصصی پزشکی  
- کنترل‌های سختگیرانه حریم خصوصی و ردگیری حسابرسی  
- ادغام با سیستم‌های پرونده الکترونیکی سلامت (EHR) موجود  
- مهندسی پرسش‌های سازگار برای اصطلاحات پزشکی  
...
**نتایج:** بهبود پیشنهادهای تشخیصی برای پزشکان همراه با حفظ کامل انطباق HIPAA و کاهش قابل توجه تغییر زمینه بین سیستم‌ها.
...
**نتایج:** افزایش انطباق با مقررات، چرخه‌های استقرار مدل ۴۰ درصد سریع‌تر و بهبود ثبات ارزیابی ریسک در بخش‌ها.

### مطالعه موردی ۴: سرور MCP مایکروسافت Playwright برای اتوماسیون مرورگر

مایکروسافت سرور [Playwright MCP](https://github.com/microsoft/playwright-mcp) را برای فعال‌سازی اتوماسیون مرورگر امن و استاندارد از طریق پروتکل مدل کانتکست توسعه داده است. این راه‌حل به عوامل هوش مصنوعی و LLMها اجازه می‌دهد با مرورگرهای وب به روشی کنترل‌شده، قابل حسابرسی و قابل توسعه تعامل کنند—که موارد استفاده‌ای مانند تست خودکار وب، استخراج داده و جریان‌های کاری انتها به انتها را ممکن می‌سازد.

- ارائه قابلیت‌های اتوماسیون مرورگر (ناوبری، پر کردن فرم، گرفتن اسکرین‌شات و غیره) به‌عنوان ابزارهای MCP  
- اجرای کنترل‌های دسترسی سختگیرانه و جداسازی برای جلوگیری از اقدامات غیرمجاز  
- ارائه گزارش‌های دقیق حسابرسی برای تمام تعاملات مرورگر  
...
**نتایج:**  
- فعال‌سازی اتوماسیون مرورگر برنامه‌نویسی امن برای عوامل هوش مصنوعی و LLMها  
- کاهش تلاش‌های تست دستی و بهبود پوشش تست برنامه‌های وب  
- فراهم‌کردن چارچوب قابل استفاده مجدد و توسعه‌پذیر برای ادغام ابزارهای مبتنی بر مرورگر در محیط‌های سازمانی  
...
Azure MCP ([https://aka.ms/azmcp](https://aka.ms/azmcp)) پیاده‌سازی سازمانی و مدیریت‌شده مایکروسافت از پروتکل مدل کانتکست است که برای ارائه قابلیت‌های سرور MCP مقیاس‌پذیر، امن و منطبق به‌عنوان سرویس ابری طراحی شده است. Azure MCP به سازمان‌ها امکان می‌دهد به سرعت سرورهای MCP را مستقر، مدیریت و با خدمات هوش مصنوعی، داده و امنیت Azure ادغام کنند، هزینه‌های عملیاتی را کاهش داده و پذیرش هوش مصنوعی را تسریع نمایند.

- میزبانی کاملاً مدیریت‌شده سرور MCP با مقیاس‌بندی، نظارت و امنیت داخلی  
- ادغام بومی با Azure OpenAI، Azure AI Search و سایر خدمات Azure  
- احراز هویت و مجوز سازمانی از طریق Microsoft Entra ID  
- پشتیبانی از ابزارهای سفارشی، قالب‌های پرسش و اتصال‌دهنده‌های منابع  
...
MCP (پروتکل مدل کانتکست) پروتکلی نوظهور برای تعامل چت‌بات‌ها و دستیاران هوش مصنوعی با ابزارها است. هر نمونه NLWeb نیز یک سرور MCP است که از یک متد اصلی، ask، پشتیبانی می‌کند که برای پرسیدن سؤال از یک وب‌سایت به زبان طبیعی استفاده می‌شود. پاسخ برگشتی از schema.org بهره می‌برد، یک واژگان پرکاربرد برای توصیف داده‌های وب. به طور کلی، MCP همان‌گونه است که NLWeb به Http نسبت به HTML است. NLWeb پروتکل‌ها، قالب‌های Schema.org و کد نمونه را ترکیب می‌کند تا به سایت‌ها کمک کند این نقاط پایانی را به سرعت ایجاد کنند، به نفع انسان‌ها از طریق رابط‌های مکالمه‌ای و ماشین‌ها از طریق تعامل طبیعی عامل به عامل.

دو جزء متمایز در NLWeb وجود دارد:  
- یک پروتکل، بسیار ساده برای شروع، برای ارتباط با سایت به زبان طبیعی و قالبی که از json و schema.org برای پاسخ برگشتی استفاده می‌کند. برای جزئیات بیشتر به مستندات REST API مراجعه کنید.  
- پیاده‌سازی ساده‌ای از (1) که از نشانه‌گذاری موجود بهره می‌برد، برای سایت‌هایی که می‌توانند به عنوان فهرستی از آیتم‌ها (محصولات، دستور غذا، جاذبه‌ها، نقد و بررسی و غیره) انتزاع شوند. همراه با مجموعه‌ای از ویجت‌های رابط کاربری، سایت‌ها می‌توانند به‌راحتی رابط‌های مکالمه‌ای به محتوای خود ارائه دهند. برای جزئیات بیشتر در مورد عملکرد این بخش به مستندات Life of a chat query مراجعه کنید.
...
سرورهای MCP Azure AI Foundry نشان می‌دهند چگونه می‌توان از MCP برای هماهنگی و مدیریت عوامل هوش مصنوعی و جریان‌های کاری در محیط‌های سازمانی استفاده کرد. با ادغام MCP با Azure AI Foundry، سازمان‌ها می‌توانند تعاملات عوامل را استاندارد کنند، از مدیریت جریان کاری Foundry بهره‌مند شوند و استقرارهای امن و مقیاس‌پذیر را تضمین کنند. این رویکرد امکان نمونه‌سازی سریع، نظارت قوی و ادغام بی‌وقفه با خدمات Azure AI را فراهم می‌آورد و از سناریوهای پیشرفته‌ای مانند مدیریت دانش و ارزیابی عوامل پشتیبانی می‌کند. توسعه‌دهندگان از یک رابط یکپارچه برای ساخت، استقرار و نظارت بر خطوط لوله عوامل بهره‌مند می‌شوند، در حالی که تیم‌های فناوری اطلاعات امنیت، انطباق و کارایی عملیاتی بهبود یافته را تجربه می‌کنند. این راه‌حل برای سازمان‌هایی که به دنبال تسریع پذیرش هوش مصنوعی و حفظ کنترل بر فرآیندهای پیچیده مبتنی بر عامل هستند، ایده‌آل است.
...
زمین بازی MCP Foundry محیطی آماده برای آزمایش سرورهای MCP و ادغام‌های Azure AI Foundry فراهم می‌کند. توسعه‌دهندگان می‌توانند به سرعت مدل‌های هوش مصنوعی و جریان‌های کاری عوامل را نمونه‌سازی، تست و ارزیابی کنند با استفاده از منابع موجود در کاتالوگ و آزمایشگاه‌های Azure AI Foundry. این زمین بازی راه‌اندازی را ساده می‌کند، پروژه‌های نمونه ارائه می‌دهد و از توسعه مشترک پشتیبانی می‌کند، و کاوش بهترین روش‌ها و سناریوهای جدید را با کمترین هزینه ممکن می‌سازد. این محیط به ویژه برای تیم‌هایی که به دنبال اعتبارسنجی ایده‌ها، به اشتراک‌گذاری آزمایش‌ها و تسریع یادگیری بدون نیاز به زیرساخت پیچیده هستند مفید است. با کاهش موانع ورود، این زمین بازی به رشد نوآوری و مشارکت جامعه در اکوسیستم MCP و Azure AI Foundry کمک می‌کند.
...
4. افزودن مکانیزم‌های کشینگ برای درخواست‌های پرتکرار  
5. توسعه داشبورد نظارتی  
6. تست با الگوهای مختلف درخواست  

**فناوری‌ها:** انتخاب از بین Python (.NET/Java/Python بسته به ترجیح شما)، Redis برای کشینگ و چارچوب وب ساده برای داشبورد.

### پروژه ۲: سیستم مدیریت پرسش سازمانی

**هدف:** توسعه سیستم مبتنی بر MCP برای مدیریت، نسخه‌بندی و استقرار قالب‌های پرسش در سراسر سازمان.

**نیازمندی‌ها:**  
- ایجاد مخزن متمرکز برای قالب‌های پرسش  
...
**هدف:** ساخت پلتفرمی برای تولید محتوا که با استفاده از MCP نتایج سازگار در انواع مختلف محتوا ارائه دهد.

**نیازمندی‌ها:**  
- پشتیبانی از فرمت‌های مختلف محتوا (مقالات وبلاگ، شبکه‌های اجتماعی، متن‌های بازاریابی)  
- پیاده‌سازی تولید قالب‌بندی‌شده با گزینه‌های سفارشی‌سازی  
...
2. ایجاد قالب برای انواع مختلف محتوا  
3. ساخت خط تولید محتوا  
4. پیاده‌سازی سیستم بازبینی  
5. توسعه سیستم ردیابی شاخص‌ها  
6. ایجاد رابط کاربری برای مدیریت قالب و تولید محتوا  
...
1. **MCP چندرسانه‌ای**  
   ...
   - قالب‌های استاندارد پرسش برای مدالیته‌های مختلف  
...
4. **MCP برای محاسبات لبه‌ای**  
   ...
   - پروتکل‌های بهینه‌شده برای محیط‌های کم‌عرضه‌باند  
...
3. [NLWeb](https://github.com/microsoft/NlWeb) - مجموعه‌ای از پروتکل‌های باز و ابزارهای متن‌باز مرتبط. تمرکز اصلی آن ایجاد لایه بنیادی برای وب هوشمند است  
...
2. [mcp-auth-servers](https://github.com/Azure-Samples/mcp-auth-servers) - سرورهای مرجع MCP که احراز هویت را با مشخصات فعلی پروتکل مدل کانتکست نشان می‌دهند  
3. [remote-mcp-functions](https://github.com/Azure-Samples/remote-mcp-functions) - صفحه فرود برای پیاده‌سازی‌های سرور MCP راه دور در Azure Functions با لینک به مخازن زبان‌های مختلف  
4. [remote-mcp-functions-python](https://github.com/Azure-Samples/remote-mcp-functions-python) - قالب شروع سریع برای ساخت و استقرار سرورهای MCP راه دور سفارشی با Azure Functions
- [مستندات Azure MCP](https://aka.ms/azmcp)
- [مخزن GitHub سرور Playwright MCP](https://github.com/microsoft/playwright-mcp)
...
- [توابع Remote MCP (نمونه‌های Azure)](https://github.com/Azure-Samples/remote-mcp-functions)
- [توابع Remote MCP پایتون (نمونه‌های Azure)](https://github.com/Azure-Samples/remote-mcp-functions-python)
- [توابع Remote MCP دات‌نت (نمونه‌های Azure)](https://github.com/Azure-Samples/remote-mcp-functions-dotnet)
- [توابع Remote MCP تایپ‌اسکریپت (نمونه‌های Azure)](https://github.com/Azure-Samples/remote-mcp-functions-typescript)
- [توابع Remote MCP APIM پایتون (نمونه‌های Azure)](https://github.com/Azure-Samples/remote-mcp-apim-functions-python)
...
3. صنعتی که در مطالعات موردی پوشش داده نشده را بررسی کنید و شرح دهید MCP چگونه می‌تواند چالش‌های خاص آن را حل کند.
4. یکی از جهت‌گیری‌های آینده را کاوش کرده و مفهومی برای یک افزونه جدید MCP جهت پشتیبانی از آن ایجاد کنید.
