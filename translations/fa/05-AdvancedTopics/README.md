...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-17T15:09:49+00:00",
...
این درس به مفاهیم پیشرفته در پیاده‌سازی پروتکل مدل زمینه می‌پردازد و بر یکپارچگی چند‌مدلی، مقیاس‌پذیری، بهترین روش‌های امنیتی و یکپارچگی سازمانی تمرکز دارد. این موضوعات برای ساخت برنامه‌های MCP با درجه تولید که می‌توانند نیازهای پیچیده در محیط‌های سازمانی را برآورده کنند، ضروری هستند.

## اهداف یادگیری

تا پایان این درس، شما قادر خواهید بود:
- قابلیت‌های چند‌مدلی را در چارچوب‌های MCP پیاده‌سازی کنید
- معماری‌های MCP مقیاس‌پذیر برای سناریوهای با تقاضای بالا طراحی کنید
- بهترین روش‌های امنیتی همسو با اصول امنیتی MCP را به کار ببرید
...
برای اطلاعات به‌روز در مورد موضوعات پیشرفته MCP، به موارد زیر مراجعه کنید:
...
MCP نه تنها از تعاملات مبتنی بر متن پشتیبانی می‌کند بلکه قابلیت‌های چند‌مدلی را نیز فراهم می‌کند، که به مدل‌ها اجازه می‌دهد با تصاویر، صوت و انواع دیگر داده‌ها کار کنند.

### معماری برای پشتیبانی چند‌مدلی

پیاده‌سازی‌های چند‌مدلی MCP معمولاً شامل موارد زیر هستند:

1. **پارسرهای خاص مدال**: اجزایی که انواع مختلف رسانه را به فرمت‌هایی تبدیل می‌کنند که مدل می‌تواند پردازش کند
2. **ابزارهای خاص مدال**: ابزارهای ویژه‌ای که برای مدیریت مدال‌های خاص طراحی شده‌اند (تحلیل تصویر، پردازش صوت)
3. **مدیریت زمینه یکپارچه**: سیستمی برای حفظ زمینه در مدال‌های مختلف
4. **تولید پاسخ**: قابلیت تولید پاسخ‌هایی که ممکن است شامل چندین مدال باشند

### مثال چند‌مدلی C#: تحلیل تصویر
...
### مثال چند‌مدلی Python: تولید پاسخ چند‌مدلی
...
زمینه‌های ریشه یک مفهوم اساسی در پروتکل مدل زمینه هستند که لایه‌ای پایدار برای حفظ تاریخچه مکالمه و وضعیت مشترک در درخواست‌ها و جلسات متعدد فراهم می‌کنند.

### درک زمینه‌های ریشه

زمینه‌های ریشه به عنوان ظرف‌هایی عمل می‌کنند که تاریخچه و وضعیت یک سری تعاملات مرتبط را نگه می‌دارند. آنها امکان‌پذیر می‌کنند:

- **پایداری مکالمه**: حفظ مکالمات چند‌نوبتی منسجم
- **مدیریت حافظه**: ذخیره و بازیابی اطلاعات در تعاملات
- **مدیریت وضعیت**: ردیابی پیشرفت در جریان‌های کاری پیچیده
- **اشتراک‌گذاری زمینه**: اجازه دادن به چندین مشتری برای دسترسی به همان وضعیت مکالمه

در MCP، زمینه‌های ریشه دارای این ویژگی‌های کلیدی هستند:

- هر زمینه ریشه دارای یک شناسه منحصر به فرد است
- آنها می‌توانند تاریخچه مکالمه، ترجیحات کاربر و سایر متاداده‌ها را شامل شوند
- آنها می‌توانند در صورت نیاز ایجاد، دسترسی و بایگانی شوند
- آنها از کنترل دسترسی دقیق و مجوزها پشتیبانی می‌کنند

### چرخه حیات زمینه ریشه
...
### مثال .NET: کار با زمینه‌های ریشه
...
### مثال Java: پیاده‌سازی زمینه ریشه
...
### مثال JavaScript: مدیریت زمینه ریشه
...
### مثال Python: زمینه ریشه برای کمک چند‌نوبتی
...
### بهترین روش‌ها برای زمینه‌های ریشه

1. **ایجاد زمینه‌های متمرکز**: زمینه‌های ریشه جداگانه برای اهداف یا دامنه‌های مختلف مکالمه ایجاد کنید تا وضوح حفظ شود.

2. **تنظیم سیاست‌های انقضا**: سیاست‌هایی برای بایگانی یا حذف زمینه‌های قدیمی اجرا کنید تا ذخیره‌سازی مدیریت شود و با سیاست‌های حفظ داده‌ها مطابقت داشته باشد.

3. **ذخیره متاداده‌های مرتبط**: از متاداده زمینه برای ذخیره اطلاعات مهم درباره مکالمه استفاده کنید که ممکن است در آینده مفید باشد.

4. **استفاده مداوم از شناسه‌های زمینه**: پس از ایجاد یک زمینه، از شناسه آن به طور مداوم برای همه درخواست‌های مرتبط استفاده کنید تا تداوم حفظ شود.

5. **تولید خلاصه‌ها**: هنگامی که یک زمینه بزرگ می‌شود، تولید خلاصه‌هایی را در نظر بگیرید تا اطلاعات ضروری را در حالی که اندازه زمینه مدیریت می‌شود، ثبت کنید.

6. **اجرای کنترل دسترسی**: برای سیستم‌های چند‌کاربره، کنترل‌های دسترسی مناسب را اجرا کنید تا حریم خصوصی و امنیت زمینه‌های مکالمه تضمین شود.

7. **مدیریت محدودیت‌های زمینه**: از محدودیت‌های اندازه زمینه آگاه باشید و استراتژی‌هایی برای مدیریت مکالمات بسیار طولانی اجرا کنید.

8. **بایگانی هنگام تکمیل**: زمینه‌ها را هنگامی که مکالمات کامل می‌شوند بایگانی کنید تا منابع آزاد شوند در حالی که تاریخچه مکالمه حفظ می‌شود.

## نمونه‌گیری در پروتکل مدل زمینه

استراتژی‌های نمونه‌گیری برای بهینه‌سازی پاسخ‌های مدل در پیاده‌سازی‌های MCP حیاتی هستند. پیکربندی صحیح نمونه‌گیری می‌تواند به طور چشمگیری کیفیت و عملکرد پاسخ را بهبود بخشد. MCP راهی استاندارد برای کنترل نحوه تولید متن توسط مدل‌ها با پارامترهای خاصی که بر تصادفی بودن، خلاقیت و انسجام تأثیر می‌گذارند، فراهم می‌کند.

### مرور پارامترهای نمونه‌گیری

MCP پارامترهای نمونه‌گیری زیر را تعریف می‌کند که می‌توانند در درخواست‌های مشتری پیکربندی شوند:

| پارامتر | توضیحات | محدوده معمول |
|-----------|-------------|---------------|
...
| `stop_sequences` | دنباله‌های سفارشی که هنگام برخورد با آنها تولید متوقف می‌شود | آرایه‌ای از رشته‌ها |

### نمونه‌گیری دما و Top-K/Top-P

پارامترهای نمونه‌گیری امکان تنظیم دقیق رفتار مدل‌های زبانی را برای دستیابی به تعادل مطلوب بین خروجی‌های قطعی و خلاقانه فراهم می‌کنند.
...
برای برنامه‌هایی که نیاز به خروجی‌های ثابت دارند، نمونه‌گیری قطعی نتایج قابل تکرار را تضمین می‌کند.
...
نمونه‌گیری هوشمندانه پارامترها را بر اساس زمینه و نیازهای هر درخواست تنظیم می‌کند.
...
## مسیریابی در پروتکل مدل زمینه

مسیریابی برای هدایت درخواست‌ها به مدل‌ها، ابزارها یا خدمات مناسب در یک اکوسیستم MCP ضروری است.

### مسیریابی مبتنی بر محتوا

مسیریابی مبتنی بر محتوا درخواست‌ها را بر اساس محتوای درخواست به خدمات تخصصی هدایت می‌کند.
...
تعادل بار بهینه‌سازی استفاده از منابع و تضمین دسترسی بالا برای خدمات MCP را انجام می‌دهد.
...
مسیریابی ابزار اطمینان می‌دهد که تماس‌های ابزار بر اساس زمینه به مناسب‌ترین خدمات هدایت می‌شوند.
...
برای استقرارهای سازمانی، پیاده‌سازی‌های MCP اغلب نیاز دارند که حجم بالایی از درخواست‌ها را با حداقل تأخیر مدیریت کنند.
...
یکپارچگی MCP با سیستم‌های سازمانی مانند Azure OpenAI و Microsoft AI Foundry.

### یکپارچگی Azure OpenAI
...
- پیاده‌سازی‌های چند‌مدلی MCP قابلیت‌های هوش مصنوعی را فراتر از پردازش متن گسترش می‌دهند
- مقیاس‌پذیری برای استقرارهای سازمانی ضروری است و می‌توان از طریق مقیاس‌پذیری افقی و عمودی به آن پرداخت
- اقدامات امنیتی جامع از داده‌ها محافظت می‌کنند و کنترل دسترسی مناسب را تضمین می‌کنند
- یکپارچگی سازمانی با پلتفرم‌هایی مانند Azure OpenAI و Microsoft AI Foundry قابلیت‌های MCP را افزایش می‌دهد
...
1. نیازهای چند‌مدلی را برای مورد استفاده خود شناسایی کنید
2. کنترل‌های امنیتی مورد نیاز برای محافظت از داده‌های حساس را طرح‌ریزی کنید
3. یک معماری مقیاس‌پذیر طراحی کنید که بتواند بارهای متغیر را مدیریت کند
4. نقاط یکپارچگی با سیستم‌های هوش مصنوعی سازمانی را برنامه‌ریزی کنید
5. گلوگاه‌های عملکرد بالقوه و استراتژی‌های کاهش آنها را مستند کنید
...
بعدی: [جامعه و مشارکت‌ها](../06-CommunityContributions/README.md)
