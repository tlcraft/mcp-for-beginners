...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-02T11:57:28+00:00",
...
این درس به بررسی مفاهیم پیشرفته در پیاده‌سازی پروتکل مدل کانتکست می‌پردازد و بر ادغام چندرسانه‌ای، مقیاس‌پذیری، بهترین شیوه‌های امنیتی و یکپارچه‌سازی سازمانی تمرکز دارد. این موضوعات برای ساخت برنامه‌های MCP با کیفیت تولید که قادر به پاسخگویی به نیازهای پیچیده در محیط‌های سازمانی هستند، ضروری‌اند.
...
- بهترین شیوه‌های امنیتی منطبق با اصول امنیتی MCP را به کار ببرید
...
| لینک | عنوان | توضیحات |
|------|-------|-------------|
| [mcp-oauth2-demo](../../../05-AdvancedTopics/mcp-oauth2-demo) | دمو OAuth2 MCP | برنامه ساده Spring Boot که OAuth2 را با MCP هم به عنوان سرور احراز هویت و هم سرور منابع نشان می‌دهد. صدور امن توکن، نقاط پایانی محافظت‌شده، استقرار در Azure Container Apps و ادغام مدیریت API را به نمایش می‌گذارد. |
| [web-search-mcp](../../../05-AdvancedTopics/web-search-mcp) | جستجوی وب MCP | سرور و کلاینت Python MCP که با SerpAPI برای جستجوی زنده وب، اخبار، محصولات و پرسش و پاسخ یکپارچه شده است. ارکستراسیون چندابزاری، ادغام API خارجی و مدیریت خطاهای قوی را نشان می‌دهد. |
...
MCP نه تنها از تعاملات متنی بلکه از قابلیت‌های چندرسانه‌ای نیز پشتیبانی می‌کند و اجازه می‌دهد مدل‌ها با تصاویر، صدا و انواع داده‌های دیگر کار کنند.

### معماری پشتیبانی چندرسانه‌ای

پیاده‌سازی‌های چندرسانه‌ای MCP معمولاً شامل موارد زیر است:

1. **تجزیه‌کننده‌های مخصوص مدیا**: اجزایی که انواع مختلف رسانه را به فرمتی تبدیل می‌کنند که مدل قادر به پردازش آن باشد  
2. **ابزارهای مخصوص مدیا**: ابزارهای ویژه طراحی شده برای مدیریت مدیاهای خاص (تحلیل تصویر، پردازش صدا)  
3. **مدیریت یکپارچه کانتکست**: سیستمی برای حفظ کانتکست در میان مدیاهای مختلف  
4. **تولید پاسخ**: توانایی تولید پاسخ‌هایی که ممکن است شامل چندین مدیا باشد
...
کانتکست‌های ریشه‌ای یک مفهوم اساسی در پروتکل مدل کانتکست هستند که لایه‌ای پایدار برای نگهداری تاریخچه مکالمه و وضعیت مشترک در چندین درخواست و جلسه فراهم می‌کنند.

### درک کانتکست‌های ریشه‌ای

کانتکست‌های ریشه‌ای به عنوان ظرف‌هایی عمل می‌کنند که تاریخچه و وضعیت مجموعه‌ای از تعاملات مرتبط را نگه می‌دارند. آنها امکان می‌دهند:
...
- **اشتراک‌گذاری کانتکست**: اجازه به چندین کلاینت برای دسترسی به وضعیت مکالمه یکسان

در MCP، کانتکست‌های ریشه‌ای ویژگی‌های کلیدی زیر را دارند:

- هر کانتکست ریشه‌ای شناسه منحصربه‌فرد دارد  
- می‌توانند شامل تاریخچه مکالمه، ترجیحات کاربر و متادیتای دیگر باشند  
- امکان ایجاد، دسترسی و آرشیو آنها وجود دارد  
...
### بهترین شیوه‌های کانتکست ریشه‌ای

1. **ایجاد کانتکست‌های متمرکز**: برای اهداف یا حوزه‌های مختلف مکالمه کانتکست‌های ریشه‌ای جداگانه ایجاد کنید تا وضوح حفظ شود.  
2. **تنظیم سیاست‌های انقضا**: سیاست‌هایی برای آرشیو یا حذف کانتکست‌های قدیمی پیاده‌سازی کنید تا مدیریت ذخیره و رعایت سیاست‌های نگهداری داده انجام شود.  
3. **ذخیره متادیتای مرتبط**: از متادیتای کانتکست برای ذخیره اطلاعات مهم مکالمه که ممکن است بعداً مفید باشد استفاده کنید.  
4. **استفاده مداوم از شناسه کانتکست**: پس از ایجاد کانتکست، از شناسه آن برای همه درخواست‌های مرتبط به طور مداوم استفاده کنید تا تداوم حفظ شود.  
5. **تولید خلاصه‌ها**: وقتی کانتکست بزرگ می‌شود، خلاصه‌هایی تولید کنید تا اطلاعات ضروری حفظ و اندازه کانتکست مدیریت شود.  
6. **پیاده‌سازی کنترل دسترسی**: برای سیستم‌های چندکاربره، کنترل‌های دسترسی مناسب برای حفظ حریم خصوصی و امنیت کانتکست‌های مکالمه اعمال کنید.  
7. **مدیریت محدودیت‌های کانتکست**: به محدودیت‌های اندازه کانتکست آگاه باشید و راهکارهایی برای مدیریت مکالمات بسیار طولانی پیاده‌سازی کنید.  
8. **آرشیو پس از اتمام**: پس از پایان مکالمات، کانتکست‌ها را آرشیو کنید تا منابع آزاد شده و تاریخچه مکالمه حفظ شود.

## نمونه‌گیری در پروتکل مدل کانتکست

استراتژی‌های نمونه‌گیری برای بهینه‌سازی پاسخ‌های مدل در پیاده‌سازی‌های MCP حیاتی هستند. پیکربندی مناسب نمونه‌گیری می‌تواند کیفیت و عملکرد پاسخ‌ها را به طور چشمگیری بهبود بخشد. MCP راهی استاندارد برای کنترل نحوه تولید متن توسط مدل‌ها با پارامترهای خاص که بر تصادفی بودن، خلاقیت و انسجام تأثیر می‌گذارند، فراهم می‌کند.

### مرور پارامترهای نمونه‌گیری

MCP پارامترهای نمونه‌گیری زیر را تعریف می‌کند که در درخواست‌های کلاینت قابل تنظیم هستند:
...
| `stop_sequences` | توالی‌های سفارشی که هنگام مواجهه تولید را متوقف می‌کنند | آرایه‌ای از رشته‌ها |
...
نمونه‌گیری هوشمند پارامترها را بر اساس کانتکست و نیازهای هر درخواست تطبیق می‌دهد.
...
مسیریابی مبتنی بر محتوا درخواست‌ها را بر اساس محتوای آنها به خدمات تخصصی هدایت می‌کند.
...
تعادل بار بهینه‌سازی استفاده از منابع و اطمینان از دسترسی بالا برای خدمات MCP را فراهم می‌کند.
...
مسیریابی ابزار اطمینان می‌دهد که تماس‌های ابزار به مناسب‌ترین سرویس بر اساس کانتکست هدایت شوند.
...
نمودار زیر نشان می‌دهد چگونه نمونه‌گیری و مسیریابی در یک معماری جامع MCP با هم کار می‌کنند:
...
برای استقرارهای سازمانی، پیاده‌سازی‌های MCP اغلب باید حجم بالای درخواست‌ها را با کمترین تأخیر مدیریت کنند.
...
### حفاظت داده و حفظ حریم خصوصی

#### مثال حفاظت داده در Python
