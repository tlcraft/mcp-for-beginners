...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T19:33:09+00:00",
...
המעבדה הזו תדריך אותך צעד אחר צעד כיצד ליישם שרת MCP מוכן לייצור באמצעות מסגרת FastMCP. תבנה את מבנה השרת המרכזי, תשלב מסד נתונים, תיצור כלים לגישה לנתונים, ותבסס את התשתית לניתוח קמעונאי מבוסס AI.

## סקירה כללית

שרת MCP הוא הלב של פתרון הניתוח הקמעונאי שלנו. הוא משמש כגשר בין עוזרי AI למסד הנתונים PostgreSQL, ומספק גישה מאובטחת ואינטליגנטית לנתוני העסק באמצעות פרוטוקול סטנדרטי.

המעבדה הזו תלמד אותך כיצד לבנות שרת MCP חזק וניתן להרחבה, תוך שימוש בתבניות ארגוניות ובפרקטיקות הטובות ביותר.

## מטרות למידה

בסיום המעבדה, תוכל:

- **לבנות** שרת FastMCP עם ארכיטקטורה וארגון נכונים  
- **ליישם** שילוב מסד נתונים עם ניהול חיבורים וטיפול בשגיאות  
- **ליצור** כלים MCP לבדיקת סכמת מסד נתונים וביצוע שאילתות  
- **להגדיר** ניהול הקשר של אבטחת רמת שורה (Row Level Security)  
- **להוסיף** תכונות לניטור בריאות ותצפיות  
- **לבדוק** את יישום שרת MCP שלך באופן מקומי ובאמצעות VS Code  

## 📁 מבנה הפרויקט

בואו נבחן את ארגון שרת MCP:
...
### תצורת סביבה (`config.py`)

ראשית, ניצור מערכת תצורה חזקה:
...
- **טעינת משתני סביבה**: תמיכה אוטומטית בקובץ .env  
- **בטיחות סוגים**: אימות Dataclass ורמזי סוגים  
- **ברירות מחדל גמישות**: ברירות מחדל הגיוניות לפיתוח  
- **אימות**: אימות תצורה עם הודעות שגיאה מועילות  
- **אבטחה**: ערכים רגישים רק ממשתני סביבה  

## 🗄️ שכבת שילוב מסד נתונים

### ספק PostgreSQL (`sales_analysis_postgres.py`)

בואו ניישם את שכבת שילוב מסד הנתונים:
...
### תכונות מפתח בשכבת מסד הנתונים

- **ניהול חיבורים**: ניהול משאבים יעיל עם asyncpg  
- **שילוב RLS**: הגדרת הקשר אבטחת רמת שורה באופן אוטומטי  
- **בדיקת סכמות**: גילוי דינמי של סכמת טבלאות  
- **טיפול בשגיאות**: ניהול שגיאות מקיף ורישום  
- **עיצוב שאילתות**: עיצוב תוצאות ידידותי ל-AI  
- **ניטור בריאות**: בדיקות קישוריות מסד נתונים ומצב מאגר  

## 🔧 יישום שרת MCP מרכזי

### שרת FastMCP (`sales_analysis.py`)

עכשיו ניישם את שרת MCP המרכזי:
...
### תכונות מפתח של שרת MCP

- **רישום כלים**: הגדרות כלים דקלרטיביות עם בטיחות סוגים  
- **ניהול הקשר RLS**: חילוץ זהות משתמש והגדרת הקשר באופן אוטומטי  
- **טיפול בשגיאות**: ניהול שגיאות מקיף עם הודעות ידידותיות למשתמש  
- **ניהול מחזור חיים**: הפעלה/כיבוי נכונים עם ניקוי משאבים  
- **ניטור בריאות**: נקודות קצה מובנות לבדיקת בריאות  
- **תמיכה בפיתוח**: טעינה מחדש חמה ויכולות דיבוג  

## 🏥 ניטור בריאות

### יישום בדיקת בריאות (`health_check.py`)
...
2. **בדוק בצ'אט AI**:  
   - פתח את צ'אט AI ב-VS Code  
   - הקלד `#zava` ובחר את השרת שלך  
   - שאל: "אילו טבלאות זמינות?"  
   - שאל: "הראה לי את 5 החנויות המובילות לפי מספר ההזמנות"  

### בדיקות יחידה

צור בדיקות יחידה מקיפות:
...
לאחר השלמת המעבדה, תוכל:

✅ **שרת MCP עובד**: שרת FastMCP עם שילוב מסד נתונים  
✅ **ניהול תצורה**: מערכת תצורה חזקה מבוססת סביבה  
✅ **שכבת מסד נתונים**: שילוב PostgreSQL עם ניהול חיבורים  
✅ **כלי MCP**: כלים לבדיקת סכמות וביצוע שאילתות  
✅ **שילוב RLS**: ניהול הקשר אבטחת רמת שורה  
✅ **ניטור בריאות**: נקודות קצה מקיפות לבדיקת בריאות  
✅ **אסטרטגיית בדיקה**: בדיקה מקומית ובדיקת אינטגרציה עם VS Code  

## 🚀 מה הלאה

המשך עם **[מעבדה 06: פיתוח כלים](../06-Tools/README.md)** כדי:

- להרחיב את אוסף הכלים MCP שלך  
- ליישם תבניות שאילתות מתקדמות  
- להוסיף אימות והמרת נתונים  
- ליצור כלים ניתוחיים מתמחים  

## 📚 משאבים נוספים

### מסגרת FastMCP
- [תיעוד FastMCP](https://github.com/modelcontextprotocol/python-sdk) - מדריך רשמי ל-FastMCP  
- [מפרט MCP](https://modelcontextprotocol.io/docs/) - מפרט הפרוטוקול  
- [מדריך פיתוח כלים](https://modelcontextprotocol.io/docs/tools/) - יצירת כלים MCP  

### שילוב מסד נתונים
- [תיעוד asyncpg](https://magicstack.github.io/asyncpg/current/) - מנהל PostgreSQL אסינכרוני  
- [שיטות עבודה מומלצות לניהול חיבורים](https://www.postgresql.org/docs/current/runtime-config-connection.html) - כוונון PostgreSQL  
- [מדריך אבטחת רמת שורה](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - יישום RLS  

### תבניות FastAPI
- [תיעוד FastAPI](https://fastapi.tiangolo.com/) - הפניה למסגרת אינטרנט  
- [הזרקת תלות](https://fastapi.tiangolo.com/tutorial/dependencies/) - תבניות FastAPI  
- [משימות רקע](https://fastapi.tiangolo.com/tutorial/background-tasks/) - ניהול משימות אסינכרוניות  

---

**הבא**: מוכן להרחיב את הכלים שלך? המשך עם [מעבדה 06: פיתוח כלים](../06-Tools/README.md)
