...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T07:27:24+00:00",
...
- **החלט אילו משאבים לרשום**. לשרת MCP שלך יש משאבים, כלים והנחיות, עליך להחליט אילו מהם לרשום.
- **שלב את הלקוח באפליקציית מארח**. ברגע שתדע את יכולות השרת, עליך לשלב את זה באפליקציית המארח שלך כך שאם משתמש מקליד הנחיה או פקודה אחרת, תתבצע קריאה לפונקציה המתאימה בשרת.
...
כפי שנאמר למעלה, נקדיש זמן להסבר הקוד, ואתם מוזמנים לכתוב את הקוד יחד אם תרצו.

### -1- ייבוא הספריות

נייבא את הספריות הדרושות, נצטרך הפניות ללקוח ולפרוטוקול התקשורת שבחרנו, stdio. stdio הוא פרוטוקול לדברים שמיועדים לרוץ במחשב המקומי שלך. SSE הוא פרוטוקול תקשורת נוסף שנציג בפרקים הבאים, אבל זו האופציה השנייה שלך. לעת עתה, נמשיך עם stdio.
...
לג'אווה, תיצור לקוח שמתחבר לשרת MCP מהתרגיל הקודם. תוך שימוש במבנה פרויקט Java Spring Boot מהפרק [Getting Started with MCP Server](../../../../03-GettingStarted/01-first-server/solution/java), צור מחלקה חדשה בשם `SDKClient` בתיקייה `src/main/java/com/microsoft/mcp/sample/client/` והוסף את הייבוא הבא:
...
- יצרנו מופע של פרוטוקול stdio. שים לב כיצד הוא מגדיר את הפקודה והארגומנטים למציאת והפעלת השרת, כי זה משהו שנצטרך לעשות כשניצור את הלקוח.
...
- אתחלנו אובייקט פרמטרים לשרת, אותו נשתמש כדי להפעיל את השרת ולחבר אליו את הלקוח.
- הגדרנו פונקציה `run` שקוראת ל-`stdio_client` שמתחיל סשן לקוח.
...
- ייבאנו את הספריות הנדרשות.
- יצרנו פרוטוקול stdio ואת הלקוח `mcpClient`. הלקוח ישמש אותנו לרשום ולהפעיל תכונות בשרת MCP.

שים לב, ב-"Arguments" אפשר להצביע או על קובץ *.csproj* או על הקובץ ההרצה.
...
- השתמשנו בפרוטוקול SSE (Server-Sent Events) שמתאים לתקשורת מבוססת HTTP עם שרתי MCP מבוססי Java Spring Boot.

### -3- רשימת תכונות השרת

כעת יש לנו לקוח שיכול להתחבר אם התוכנית תרוץ. עם זאת, הוא לא מציג את התכונות שלו, אז נעשה זאת כעת:
...
- האובייקט `ListToolsResult` מכיל מידע על כל הכלים כולל שמותיהם, תיאורים וסכימות הקלט.

מעולה, עכשיו תפסנו את כל התכונות. עכשיו השאלה היא מתי משתמשים בהן? הלקוח הזה פשוט יחסית, כלומר נצטרך לקרוא במפורש לתכונות כשנרצה אותן. בפרק הבא ניצור לקוח מתקדם יותר שיש לו גישה למודל שפה גדול (LLM) משלו. לעת עתה, נראה כיצד להפעיל את התכונות בשרת:

### -4- הפעלת תכונות

כדי להפעיל תכונות, עלינו לוודא שאנו מציינים את הארגומנטים הנכונים ובמקרים מסוימים את שם מה שאנו מנסים להפעיל.
...
- קוראים למשאב, קוראים למשאב על ידי קריאה ל-`readResource()` עם הפרמטר `uri`. כך זה נראה כנראה בצד השרת:
    ...
    ערך ה-`uri` שלנו `file://example.txt` תואם ל-`file://{name}` בשרת. `example.txt` יוקצה ל-`name`.
...
- מקבלים הנחיה, כדי לקבל הנחיה, קוראים ל-`getPrompt()` עם `name` ו-`arguments`. קוד השרת נראה כך:
    ...
    ולכן קוד הלקוח שלך נראה כך כדי להתאים למה שהוגדר בשרת:
...
ראשית, ודא ששרת MCP שלך רץ בכתובת `http://localhost:8080`. לאחר מכן הרץ את הלקוח:
...
- יכולים להפעיל שרת בזמן שהם מתחילים את עצמם (כמו בפרק זה), אך לקוחות יכולים להתחבר גם לשרתים שרצים.
- דרך מצוינת לבדוק את יכולות השרת לצד חלופות כמו Inspector כפי שתואר בפרק הקודם.
