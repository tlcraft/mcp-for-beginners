...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T20:45:26+00:00",
...
> **טיפ:** בחירת מנגנון ההעברה הנכון משפיעה על ביצועים, מדרגיות וחוויית המשתמש. **Streamable HTTP** מומלץ ליישומים מודרניים, מדרגיים ומוכנים לענן.

שימו לב למנגנוני ההעברה stdio ו-SSE שהוצגו בפרקים הקודמים ואיך Streamable HTTP הוא מנגנון ההעברה הנדון בפרק זה.

## סטרימינג: מושגים ומוטיבציה

הבנת המושגים הבסיסיים והמוטיבציה מאחורי סטרימינג חיונית ליישום מערכות תקשורת בזמן אמת יעילות.

**סטרימינג** היא טכניקה בתכנות רשת המאפשרת שליחה וקבלה של נתונים בחתיכות קטנות, ניתנות לניהול, או כרצף אירועים, במקום להמתין לתגובה מלאה. זה שימושי במיוחד עבור:
...
| עדכוני התקדמות        | נשלחים כחלק מהנתונים         | נשלחים כהודעות התראה          |
| דרישות לקוח           | חייב לעבד סטרים              | חייב לממש מטפל בהודעות         |
...
התראה היא הודעה שנשלחת מהשרת ללקוח כדי ליידע על התקדמות, מצב או אירועים אחרים במהלך פעולה ארוכת טווח. התראות משפרות את השקיפות וחוויית המשתמש.
...
בדוגמה שלמעלה, הכלי `process_files` שולח שלוש התראות ללקוח תוך כדי עיבוד כל קובץ. השיטה `ctx.info()` משמשת לשליחת הודעות מידע.

</details>

בנוסף, כדי לאפשר התראות, ודאו שהשרת משתמש במנגנון העברה סטרימינג (כמו `streamable-http`) ושהלקוח מיישם מטפל בהודעות לעיבוד התראות. כך תוכלו להגדיר את השרת לשימוש במנגנון `streamable-http`:
...
בקוד שלמעלה, הפונקציה `message_handler` בודקת אם ההודעה הנכנסת היא התראה. אם כן, היא מדפיסה את ההתראה; אחרת, מעבדת אותה כהודעת שרת רגילה. שימו לב גם לאופן אתחול `ClientSession` עם `message_handler` לטיפול בהתראות נכנסות.
...
התראות התקדמות הן הודעות בזמן אמת שנשלחות מהשרת ללקוח במהלך פעולות ארוכות. במקום להמתין לסיום התהליך כולו, השרת מעדכן את הלקוח על המצב הנוכחי. זה משפר את השקיפות, חוויית המשתמש ומקל על איתור תקלות.
...
כאשר מיישמים שרתי MCP עם פרוטוקולי HTTP, האבטחה הופכת לנושא מרכזי שדורש תשומת לב מדוקדקת למגוון וקטורי התקפה ומנגנוני הגנה.
...
- **אימות**: ליישם אימות (למשל, מפתחות API, OAuth) בפריסות ייצור.
- **CORS**: להגדיר מדיניות Cross-Origin Resource Sharing (CORS) להגבלת גישה.
...
- Streamable HTTP מציע יכולת סקיילביליות, תאימות ותמיכה עשירה יותר בהתראות מאשר SSE.
...
- **לבצע בדיקות תאימות** עם כלים וזרימות עבודה קיימות.

### שמירת תאימות

מומלץ לשמור על תאימות עם לקוחות SSE קיימים במהלך ההגירה. הנה כמה אסטרטגיות:
...
האבטחה צריכה להיות בראש סדר העדיפויות בעת יישום כל שרת, במיוחד כשמשתמשים בפרוטוקולי HTTP כמו Streamable HTTP ב-MCP.
...
- חקרו שילוב של הזרמת MCP עם מסגרות frontend (React, Vue וכו') לעדכוני UI חיים.
