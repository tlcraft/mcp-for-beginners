...
MCP תומך לא רק באינטראקציות מבוססות טקסט אלא גם ביכולות רב-מודאליות, ומאפשר למודלים לעבוד עם תמונות, קול וסוגי נתונים נוספים.

## מבוא

בשיעור זה תלמדו כיצד לבנות יישום רב-מודאלי.

## מטרות הלמידה

בסיום השיעור תוכלו:

- להבין את הבחירות הרב-מודאליות
- לממש יישום רב-מודאלי.

## ארכיטקטורה לתמיכה רב-מודאלית

מימושי MCP רב-מודאליים כוללים בדרך כלל:

- **מפענחים ספציפיים למודאליות**: רכיבים שממירים סוגי מדיה שונים לפורמטים שהמודל יכול לעבד.
- **כלים ספציפיים למודאליות**: כלים מיוחדים המיועדים לטיפול במודאליות מסוימות (ניתוח תמונות, עיבוד קול)
- **ניהול הקשר מאוחד**: מערכת לשמירת ההקשר בין מודאליות שונות
- **יצירת תגובות**: יכולת לייצר תגובות שיכולות לכלול מספר מודאליות.

## דוגמה רב-מודאלית: ניתוח תמונה

בדוגמה הבאה ננתח תמונה ונחלץ מידע.
...
- יצרנו את `ImageAnalysisTool` שיכול לנתח תמונות באמצעות שירות היפותטי `IImageAnalysisService`.
- הגדרנו את שרת ה-MCP לטיפול בבקשות גדולות יותר ותמיכה בסוגי תוכן של תמונות.
- רשמנו את כלי ניתוח התמונות בשרת.
- מימשנו שיטה להורדת תמונות מכתובת URL וניתוחן בהתאם לסוג המבוקש (אובייקטים, טקסט, פנים וכו').
- החזרנו תוצאות מובנות בפורמט התואם למפרט MCP.

## דוגמה רב-מודאלית: עיבוד קול

עיבוד קול הוא מודאליות נפוצה נוספת ביישומים רב-מודאליים. להלן דוגמה למימוש כלי תמלול קול שיכול לטפל בקבצי קול ולהחזיר תמלולים.
...
- יצרנו את `AudioTranscriptionTool` שיכול לתמלל קבצי קול.
- הגדרנו את הסכמה של הכלי לקבל כתובת URL או נתוני קול מקודדים ב-base64.
- מימשנו את המתודה `execute` לטיפול בעיבוד קול ותמלול.
- הגדרנו את שרת ה-MCP לטיפול בבקשות רב-מודאליות, כולל עיבוד קול ותמונה.
- רשמנו את כלי תמלול הקול בשרת.
- מימשנו שיטה להורדת קבצי קול מכתובת URL או לפענוח נתוני קול מקודדים ב-base64.
- השתמשנו בשירות `AudioProcessor` לטיפול בלוגיקת התמלול בפועל.
- הפעלנו את שרת ה-MCP להאזנה לבקשות.
