...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-17T15:20:27+00:00",
...
השיעור הזה מתמקד ברעיונות מתקדמים ביישום פרוטוקול הקשר מודל, תוך דגש על אינטגרציה רב-מודלית, יכולת הרחבה, נהלי אבטחה מיטביים ואינטגרציה ארגונית. נושאים אלו חיוניים לבניית יישומי MCP ברמת ייצור שיכולים להתמודד עם דרישות מורכבות בסביבות ארגוניות.

## מטרות למידה

בסוף השיעור הזה, תוכלו:
- ליישם יכולות רב-מודליות בתוך מסגרות MCP
- לעצב ארכיטקטורות MCP ניתנות להרחבה לתרחישים בעלי ביקוש גבוה
- ליישם נהלי אבטחה מיטביים התואמים לעקרונות האבטחה של MCP
- לשלב MCP עם מערכות AI ארגוניות ומסגרות
- לייעל ביצועים ואמינות בסביבות ייצור

## מקורות נוספים

למידע המעודכן ביותר על נושאים מתקדמים ב-MCP, עיינו ב:
...
MCP תומך לא רק באינטראקציות מבוססות טקסט אלא גם ביכולות רב-מודליות, המאפשרות למודלים לעבוד עם תמונות, שמע וסוגי נתונים אחרים.

### ארכיטקטורה לתמיכה רב-מודלית

יישומי MCP רב-מודליים כוללים בדרך כלל:

1. **מפרשים ספציפיים למודלים**: רכיבים שממירים סוגי מדיה שונים לפורמטים שהמודל יכול לעבד
2. **כלים ספציפיים למודלים**: כלים מיוחדים שנועדו להתמודד עם מודלים ספציפיים (ניתוח תמונה, עיבוד שמע)
3. **ניהול הקשר מאוחד**: מערכת לשמירה על הקשר בין מודלים שונים
4. **יצירת תגובות**: יכולת לייצר תגובות שעשויות לכלול מספר מודלים

### דוגמה רב-מודלית ב-C#: ניתוח תמונה
...
### דוגמה רב-מודלית ב-Java: עיבוד שמע
...
### דוגמה רב-מודלית ב-Python: יצירת תגובה רב-מודלית
...
הקשרים שורשיים הם מושג יסודי בפרוטוקול הקשר מודל המספק שכבה מתמשכת לשמירה על היסטוריית שיחה ומצב משותף בין בקשות ומפגשים שונים.

### הבנת הקשרים שורשיים

הקשרים שורשיים משמשים כמיכלים המחזיקים את ההיסטוריה והמצב לסדרה של אינטראקציות קשורות. הם מאפשרים:

- **התמדה בשיחה**: שמירה על שיחות מרובות סיבובים עקביות
- **ניהול זיכרון**: אחסון ואחזור מידע בין אינטראקציות
- **ניהול מצב**: מעקב אחר התקדמות בתהליכים מורכבים
- **שיתוף הקשר**: מתן אפשרות למספר לקוחות לגשת לאותו מצב שיחה

ב-MCP, הקשרים שורשיים כוללים את המאפיינים המרכזיים הבאים:

- לכל הקשר שורשי יש מזהה ייחודי
- הם יכולים להכיל היסטוריית שיחה, העדפות משתמש ומטא נתונים נוספים
- ניתן ליצור, לגשת ולארכב אותם לפי הצורך
- הם תומכים בבקרת גישה ועדינות הרשאות
...
### דוגמה ב-.NET: עבודה עם הקשרים שורשיים
...
### דוגמה ב-JavaScript: ניהול הקשר שורשי
...
### דוגמה ב-Python: הקשר שורשי לעזרה מרובת סיבובים
...
### נהלים מיטביים להקשרים שורשיים

1. **יצירת הקשרים ממוקדים**: צרו הקשרים שורשיים נפרדים למטרות שיחה או תחומים שונים כדי לשמור על בהירות.

2. **הגדרת מדיניות תפוגה**: יישמו מדיניות לארכוב או מחיקה של הקשרים ישנים לניהול אחסון ולעמידה במדיניות שמירת נתונים.

3. **אחסון מטא נתונים רלוונטיים**: השתמשו במטא נתונים של הקשר לאחסון מידע חשוב על השיחה שעשוי להיות שימושי מאוחר יותר.

4. **שימוש עקבי במזהי הקשר**: ברגע שנוצר הקשר, השתמשו במזהה שלו באופן עקבי לכל הבקשות הקשורות כדי לשמור על רצף.

5. **יצירת סיכומים**: כאשר הקשר גדל, שקלו ליצור סיכומים כדי ללכוד מידע חיוני תוך ניהול גודל ההקשר.

6. **יישום בקרת גישה**: עבור מערכות מרובות משתמשים, יישמו בקרות גישה נכונות כדי להבטיח פרטיות ואבטחה של הקשרי שיחה.

7. **התמודדות עם מגבלות הקשר**: היו מודעים למגבלות גודל הקשר ויישמו אסטרטגיות להתמודדות עם שיחות ארוכות מאוד.

8. **ארכוב כאשר הושלם**: ארכבו הקשרים כאשר שיחות הושלמו כדי לשחרר משאבים תוך שמירה על היסטוריית השיחה.

## דגימה בפרוטוקול הקשר מודל

אסטרטגיות דגימה הן קריטיות לאופטימיזציה של תגובות מודל ביישומי MCP. הגדרת דגימה נכונה יכולה לשפר באופן דרמטי את איכות התגובה והביצועים. MCP מספק דרך סטנדרטית לשלוט כיצד מודלים מייצרים טקסט עם פרמטרים ספציפיים שמשפיעים על אקראיות, יצירתיות וקוהרנטיות.

### סקירת פרמטרי דגימה

MCP מגדיר את פרמטרי הדגימה הבאים שניתן להגדיר בבקשות לקוח:
...
| `stop_sequences` | רצפים מותאמים אישית שמפסיקים את הייצור כאשר נתקלים בהם | מערך של מחרוזות |

### דגימת טמפרטורה ודגימת Top-K/Top-P

פרמטרי דגימה מאפשרים לכוון את התנהגות מודלי השפה כדי להשיג את האיזון הרצוי בין תוצרים דטרמיניסטיים ויצירתיים.
...
עבור יישומים הדורשים תוצרים עקביים, דגימה דטרמיניסטית מבטיחה תוצאות שניתן לשחזר.
...
דגימה חכמה מתאימה פרמטרים בהתאם להקשר ולדרישות של כל בקשה.
...
## ניתוב בפרוטוקול הקשר מודל

ניתוב הוא חיוני להכוונת בקשות למודלים, כלים או שירותים המתאימים בתוך אקוסיסטם MCP.

### ניתוב מבוסס תוכן

ניתוב מבוסס תוכן מכוון בקשות לשירותים מתמחים בהתבסס על תוכן הבקשה.
...
איזון עומסים אופטימיזציה של ניצול משאבים ומבטיח זמינות גבוהה לשירותי MCP.
...
ניתוב כלים מבטיח שהקריאות לכלים מכוונות לשירות המתאים ביותר בהתבסס על הקשר.
...
התרשים למטה ממחיש כיצד דגימה וניתוב עובדים יחד בארכיטקטורת MCP מקיפה:
...
## יכולת הרחבה וביצועים גבוהים ב-MCP

עבור פריסות ארגוניות, יישומי MCP צריכים לעיתים קרובות להתמודד עם נפחים גבוהים של בקשות עם השהיה מינימלית.

### אסטרטגיות יכולת הרחבה
...
אבטחה היא קריטית ליישומי MCP, במיוחד בסביבות ארגוניות.

### אימות והרשאה

#### אינטגרציה של זהות .NET
...
#### אינטגרציה של אבטחת Spring ב-Java
...
#### דוגמה להגנה על נתונים ב-Python
...
שילוב MCP עם מערכות ארגוניות כמו Azure OpenAI ו-Microsoft AI Foundry.

### אינטגרציה של Azure OpenAI
...
### אינטגרציה של Microsoft AI Foundry
...
### דוגמה לאינטגרציה ארגונית ב-Python
...
- יכולת הרחבה חיונית לפריסות ארגוניות וניתן להתמודד איתה באמצעות הרחבה אופקית ואנכית
...
- יישומי MCP מתקדמים נהנים מארכיטקטורות אופטימליות וניהול משאבים זהיר

## תרגיל

עצבו יישום MCP ברמת ארגון למקרה שימוש ספציפי:

1. זהו דרישות רב-מודליות למקרה השימוש שלכם
2. תארו את בקרות האבטחה הנדרשות להגנה על נתונים רגישים
3. עיצבו ארכיטקטורה ניתנת להרחבה שיכולה להתמודד עם עומס משתנה
4. תכננו נקודות אינטגרציה עם מערכות AI ארגוניות
5. תעדו צווארי בקבוק פוטנציאליים בביצועים ואסטרטגיות להפחתתם
...
הבא: [קהילה ותרומות](../06-CommunityContributions/README.md)

**כתב ויתור**:  
מסמך זה תורגם באמצעות שירות תרגום בינה מלאכותית [Co-op Translator](https://github.com/Azure/co-op-translator). בעוד אנו שואפים לדיוק, יש להיות מודעים לכך שתרגומים אוטומטיים עשויים להכיל שגיאות או אי דיוקים. המסמך המקורי בשפתו המקורית צריך להיחשב כמקור סמכותי. עבור מידע קריטי, מומלץ להשתמש בתרגום מקצועי אנושי. אנו לא אחראים לאי הבנות או לפרשנויות שגויות הנובעות משימוש בתרגום זה.