...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-02T12:37:50+00:00",
...
השיעור הזה בוחן מושגים מתקדמים ביישום פרוטוקול הקשר מודל, עם דגש על אינטגרציה רב-מודאלית, יכולת התרחבות, שיטות אבטחה מיטביות ואינטגרציה ארגונית. נושאים אלה חיוניים לבניית אפליקציות MCP ברמת ייצור שיכולות להתמודד עם דרישות מורכבות בסביבות ארגוניות.

## מטרות הלמידה

בסיום השיעור תוכלו:
- ליישם יכולות רב-מודאליות במסגרת MCP
- לתכנן ארכיטקטורות MCP מתרחבות לתרחישים עם דרישה גבוהה
...
| קישור | כותרת | תיאור |
|-------|--------|--------|
| [mcp-oauth2-demo](../../../05-AdvancedTopics/mcp-oauth2-demo) | הדגמת MCP OAuth2 | אפליקציית Spring Boot מינימלית המציגה OAuth2 עם MCP, הן כשרת הרשאה והן כשרת משאבים. מדגימה הנפקת טוקנים מאובטחת, נקודות קצה מוגנות, פריסת Azure Container Apps ושילוב עם ניהול API. |
| [web-search-mcp](../../../05-AdvancedTopics/web-search-mcp) | חיפוש רשת MCP | שרת ולקוח MCP בפייתון המשולבים עם SerpAPI לחיפוש בזמן אמת באינטרנט, חדשות, מוצרים ושאלות ותשובות. מדגים תיאום כלים רב-מודאליים, אינטגרציה עם API חיצוני וטיפול שגיאות יציב. |

## הפניות נוספות

לקבלת המידע העדכני ביותר בנושאי MCP מתקדמים, יש לעיין ב:
...
MCP תומך לא רק באינטראקציות מבוססות טקסט אלא גם ביכולות רב-מודאליות, המאפשרות למודלים לעבוד עם תמונות, אודיו וסוגי נתונים נוספים.

### ארכיטקטורה לתמיכה רב-מודאלית

יישומי MCP רב-מודאליים כוללים בדרך כלל:

1. **פרסרים ייעודיים למודאליות**: רכיבים הממירים סוגי מדיה שונים לפורמטים שהמודל יכול לעבד  
2. **כלים ייעודיים למודאליות**: כלים מיוחדים לטיפול במודאליות ספציפית (ניתוח תמונה, עיבוד אודיו)  
3. **ניהול הקשר מאוחד**: מערכת לשמירה על הקשר בין מודאליות שונות  
4. **יצירת תגובות**: יכולת לייצר תגובות שיכולות לכלול מספר מודאליות  

### דוגמה ב-C#: ניתוח תמונה
...
### דוגמה בפייתון: יצירת תגובות רב-מודאליות
...
הקשרים שורשיים הם מושג יסודי בפרוטוקול הקשר מודל, המספק שכבה מתמשכת לשמירת היסטוריית שיחות ומצב משותף בין בקשות ומפגשים שונים.

### הבנת הקשרים שורשיים

הקשרים שורשיים משמשים כמכולות האוגרות את ההיסטוריה והמצב של סדרת אינטראקציות קשורות. הם מאפשרים:

- **שמירת שיחה מתמשכת**: שמירה על שיחות רב-סבביות קוהרנטיות  
...
ב-MCP, לקשרים שורשיים מאפיינים מרכזיים אלה:

- לכל קשר שורשי יש מזהה ייחודי  
- הם יכולים להכיל היסטוריית שיחה, העדפות משתמש ומטא-נתונים נוספים  
- ניתן ליצור, לגשת ולארכיב אותם לפי הצורך  
- הם תומכים בשליטה מדויקת בגישה והרשאות  
...
### דוגמה ב-JavaScript: ניהול קשר שורשי
...
### דוגמה בפייתון: קשר שורשי לסיוע רב-סבבי
...
### שיטות עבודה מיטביות לקשרים שורשיים

1. **יצירת הקשרים ממוקדים**: צרו הקשרים שורשיים נפרדים למטרות שיחה או תחומים שונים לשמירת בהירות.  
2. **קביעת מדיניות תפוגה**: יישמו מדיניות לארכוב או מחיקה של הקשרים ישנים לניהול אחסון ועמידה במדיניות שמירת נתונים.  
3. **אחסון מטא-נתונים רלוונטיים**: השתמשו במטא-נתונים של הקשר לאחסון מידע חשוב על השיחה שעשוי להיות שימושי בעתיד.  
4. **שימוש עקבי במזהי הקשר**: לאחר יצירת הקשר, השתמשו במזהה שלו באופן עקבי לכל הבקשות הקשורות לשמירת רצף.  
5. **יצירת סיכומים**: כאשר הקשר גדל, שקלו לייצר סיכומים ללכידת מידע חיוני תוך ניהול גודל ההקשר.  
6. **יישום בקרת גישה**: במערכות רב-משתמש, יש ליישם בקרות גישה נאותות לשמירה על פרטיות ואבטחת הקשרים.  
7. **טיפול במגבלות הקשר**: היו מודעים למגבלות גודל ההקשר ויישמו אסטרטגיות לטיפול בשיחות ארוכות מאוד.  
8. **ארכוב בסיום**: ארכבו הקשרים כאשר השיחות מסתיימות לשחרור משאבים תוך שמירה על היסטוריית השיחה.  

## דגימה בפרוטוקול הקשר מודל

אסטרטגיות דגימה הן קריטיות לאופטימיזציה של תגובות המודל ביישומי MCP. תצורת הדגימה הנכונה יכולה לשפר משמעותית את איכות התגובה והביצועים. MCP מספק דרך סטנדרטית לשלוט כיצד המודלים מייצרים טקסט עם פרמטרים ספציפיים המשפיעים על אקראיות, יצירתיות וקוהרנטיות.
...
| `stop_sequences` | רצפים מותאמים המפסיקים את ההפקה כשהם מתגלים | מערך מחרוזות |

### דגימת טמפרטורה ו-Top-K/Top-P

פרמטרי דגימה מאפשרים כוונון מדויק של התנהגות מודלי השפה להשגת האיזון הרצוי בין תוצאות דטרמיניסטיות ליצירתיות.
...
לאפליקציות שדורשות תוצאות עקביות, דגימה דטרמיניסטית מבטיחה תוצאות שחוזרות על עצמן.
...
דגימה חכמה שמתאימה את הפרמטרים בהתאם להקשר ולדרישות כל בקשה.
...
ניתוב חיוני להפניית בקשות למודלים, כלים או שירותים המתאימים בתוך מערכת MCP.

### ניתוב מבוסס תוכן

ניתוב מבוסס תוכן מפנה בקשות לשירותים מיוחדים בהתאם לתוכן הבקשה.
...
איזון עומסים משפר את ניצול המשאבים ומבטיח זמינות גבוהה לשירותי MCP.
...
ניתוב כלים מבטיח שהקריאות לכלים מופנות לשירות המתאים ביותר בהתאם להקשר.
...
הדיאגרמה הבאה ממחישה כיצד דגימה וניתוב פועלים יחד בארכיטקטורה מקיפה של MCP:
...
## יכולת התרחבות וביצועי MCP גבוהים

בפריסות ארגוניות, יישומי MCP לעיתים צריכים להתמודד עם נפחי בקשות גבוהים עם השהיה מינימלית.
...
#### התרחבות אנכית ואופטימיזציית משאבים
...
אבטחה היא קריטית ביישומי MCP, במיוחד בסביבות ארגוניות.
...
### דוגמה לאינטגרציה ארגונית בפייתון
...
- יכולת התרחבות חיונית לפריסות ארגוניות וניתנת לטיפול באמצעות התרחבות אופקית ואנכית  
- אמצעי אבטחה מקיפים מגנים על נתונים ומבטיחים בקרת גישה נאותה  
- אינטגרציה ארגונית עם פלטפורמות כמו Azure OpenAI ו-Microsoft AI Foundry משפרת את יכולות MCP  
- יישומים מתקדמים של MCP נהנים מארכיטקטורות אופטימליות וניהול משאבים זהיר  

## תרגיל

תכננו יישום MCP ברמת ארגון למקרה שימוש ספציפי:

1. זהו דרישות רב-מודאליות למקרה השימוש שלכם  
2. פרטו את אמצעי האבטחה הדרושים להגנה על נתונים רגישים  
3. תכננו ארכיטקטורה מתרחבת שיכולה להתמודד עם עומסים משתנים  
4. תכננו נקודות אינטגרציה עם מערכות AI ארגוניות  
5. תעדו צווארי בקבוק פוטנציאליים בביצועים ואסטרטגיות הפחתה  
...
**כתב ויתור**:  
מסמך זה תורגם באמצעות שירות תרגום מבוסס בינה מלאכותית [Co-op Translator](https://github.com/Azure/co-op-translator). למרות שאנו שואפים לדיוק, יש לקחת בחשבון כי תרגומים אוטומטיים עלולים להכיל שגיאות או אי-דיוקים. המסמך המקורי בשפתו המקורית הוא המקור הסמכותי. למידע קריטי מומלץ להשתמש בתרגום מקצועי על ידי מתרגם אנושי. אנו לא נושאים באחריות לכל אי-הבנה או פרשנות שגויה הנובעת משימוש בתרגום זה.