...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T16:44:01+00:00",
...
[Model Context Protocol (MCP)](https://gi- **הסכמה מפורשת של המשתמש**: כל גישה לנתונים וביצוע פעולות דורשים אישור מפורש מהמשתמש לפני הביצוע. המשתמשים חייבים להבין בבירור אילו נתונים ייגשו ואילו פעולות יבוצעו, עם שליטה מדויקת בהרשאות ובאישורים.

- **הגנה על פרטיות נתונים**: נתוני המשתמש נחשפים רק עם הסכמה מפורשת וחייבים להיות מוגנים באמצעות בקרות גישה חזקות לאורך כל מחזור האינטראקציה. יש למנוע העברת נתונים לא מורשית ולשמור על גבולות פרטיות מחמירים.

- **בטיחות הפעלת כלים**: כל הפעלת כלי דורשת הסכמה מפורשת של המשתמש עם הבנה ברורה של פונקציונליות הכלי, הפרמטרים וההשפעה הפוטנציאלית. גבולות אבטחה חזקים חייבים למנוע הפעלה לא מכוונת, לא בטוחה או זדונית של כלים.
...
- **ניהול הרשאות**: יישום מערכות הרשאה מדויקות שמאפשרות למשתמשים לשלוט באילו שרתים, כלים ומשאבים ניתן לגשת  
- **אימות ואישור**: שימוש בשיטות אימות מאובטחות (OAuth, מפתחות API) עם ניהול נכון של אסימונים ותוקף  
- **אימות קלט**: אימות כל הפרמטרים וקלטי הנתונים לפי סכמות מוגדרות כדי למנוע התקפות הזרקה  
- **רישום ביקורת**: שמירה על יומנים מקיפים של כל הפעולות לצורך ניטור אבטחה וציות

## סקירה כללית

שיעור זה בוחן את הארכיטקטורה והמרכיבים הבסיסיים שמרכיבים את מערכת Model Context Protocol (MCP). תלמדו על ארכיטקטורת הלקוח-שרת, מרכיבים מרכזיים ומנגנוני תקשורת שמניעים אינטראקציות MCP.
...
מערכת MCP בנויה על מודל לקוח-שרת. מבנה מודולרי זה מאפשר ליישומי AI לתקשר עם כלים, מסדי נתונים, APIs ומשאבים הקשריים בצורה יעילה. בואו נפרק את הארכיטקטורה למרכיבים המרכזיים שלה.

בבסיסו, MCP פועל לפי ארכיטקטורת לקוח-שרת שבה יישום מארח יכול להתחבר למספר שרתים:
...
- **MCP Hosts**: תוכניות כמו VSCode, Claude Desktop, IDEs או כלי AI שרוצים לגשת לנתונים דרך MCP  
- **MCP Clients**: לקוחות פרוטוקול שמחזיקים חיבורים 1:1 עם שרתים  
- **MCP Servers**: תוכניות קלות משקל שכל אחת חושפת יכולות ספציפיות דרך פרוטוקול הקשר המודל הסטנדרטי  
- **מקורות נתונים מקומיים**: קבצים, מסדי נתונים ושירותים במחשב שלך ששרתים של MCP יכולים לגשת אליהם בצורה מאובטחת  
- **שירותים מרוחקים**: מערכות חיצוניות הזמינות דרך האינטרנט ששרתים של MCP יכולים להתחבר אליהן דרך APIs.

פרוטוקול MCP הוא תקן מתפתח המשתמש בגרסאות מבוססות תאריך (פורמט YYYY-MM-DD). גרסת הפרוטוקול הנוכחית היא **2025-06-18**. ניתן לראות את העדכונים האחרונים [במפרט הפרוטוקול](https://modelcontextprotocol.io/specification/2025-06-18/)

### 1. Hosts

בפרוטוקול הקשר המודל (MCP), **Hosts** הם יישומי AI שמשמשים כממשק הראשי דרכו משתמשים מתקשרים עם הפרוטוקול. Hosts מתאמים ומנהלים חיבורים למספר שרתי MCP על ידי יצירת לקוחות MCP ייעודיים לכל חיבור שרת. דוגמאות ל-Hosts כוללות:
...
- **מטפלים בהסכמת המשתמש**: מנהלים אישור משתמש לשיתוף נתונים והפעלת כלים  

### 2. Clients

**Clients** הם מרכיבים חיוניים שמחזיקים חיבורים ייעודיים אחד-על-אחד בין Hosts לשרתי MCP. כל לקוח MCP מופעל על ידי Host כדי להתחבר לשרת MCP ספציפי, מה שמבטיח ערוצי תקשורת מאורגנים ומאובטחים. לקוחות מרובים מאפשרים ל-Hosts להתחבר למספר שרתים בו-זמנית.

**Clients** הם רכיבי מחבר בתוך יישום המארח. הם:
...
- **הפעלת כלים**: מנהלים בקשות להפעלת כלים ממודלים ומעבדים תגובות  
- **עדכונים בזמן אמת**: מטפלים בהתראות ועדכונים בזמן אמת משרתים  
- **עיבוד תגובות**: מעבדים ומעצבים תגובות שרת להצגה למשתמשים  

### 3. Servers

**Servers** הם תוכניות שמספקות הקשר, כלים ויכולות ללקוחות MCP. הם יכולים לפעול באופן מקומי (באותו מחשב כמו ה-Host) או מרחוק (בפלטפורמות חיצוניות), ואחראים לטיפול בבקשות לקוח ולספק תגובות מובנות. שרתים חושפים פונקציונליות ספציפית דרך פרוטוקול הקשר המודל הסטנדרטי.
...
שרתים בפרוטוקול הקשר המודל (MCP) מספקים שלושה **פרימיטיבים** מרכזיים שמגדירים את אבני הבניין הבסיסיות לאינטראקציות עשירות בין לקוחות, Hosts ומודלים שפתיים. פרימיטיבים אלו מציינים את סוגי המידע ההקשרי והפעולות הזמינות דרך הפרוטוקול.
...
- **נתונים הקשריים**: מידע מובנה והקשר לצריכת מודלים AI  
...
**כלים** הם פונקציות ניתנות להפעלה שמודלי AI יכולים לקרוא כדי לבצע פעולות ספציפיות. הם מייצגים את "הפעלים" של מערכת MCP, ומאפשרים למודלים לתקשר עם מערכות חיצוניות:

- **פונקציות ניתנות להפעלה**: פעולות דיסקרטיות שמודלים יכולים לקרוא עם פרמטרים ספציפיים  
...
בפרוטוקול הקשר המודל (MCP), **לקוחות** יכולים לחשוף פרימיטיבים שמאפשרים לשרתים לבקש יכולות נוספות מיישום המארח. פרימיטיבים בצד הלקוח מאפשרים יישומי שרת עשירים ואינטראקטיביים יותר שיכולים לגשת ליכולות מודל AI ואינטראקציות משתמש.
...
- **אינטראקציות LLM חוזרות**: תומך בתרחישים מורכבים שבהם שרתים זקוקים לעזרת AI לעיבוד  
...
- **בקשות קלט משתמש**: שרתים יכולים לבקש מידע נוסף בעת הצורך לביצוע כלי  
...
פרוטוקול הקשר המודל (MCP) מגדיר זרימת מידע מובנית בין Hosts, Clients, Servers ומודלים. הבנת זרימה זו מסייעת להבהיר כיצד בקשות משתמש מעובדות וכיצד כלים ונתונים חיצוניים משולבים בתגובות מודל.
...
- **שימוש במשאב או כלי**  
  ...
  - אם המודל קובע שכלי נדרש (למשל, לשליפת נתונים, ביצוע חישוב או קריאת API), הלקוח שולח בקשת הפעלת כלי לשרת, ומפרט את שם הכלי והפרמטרים.
...
- **יצירת תגובה**  
  הלקוח משלב את תגובות השרת (נתוני משאבים, פלטי כלים וכו') באינטראקציה המודל המתמשכת. המודל משתמש במידע זה כדי ליצור תגובה מקיפה ורלוונטית הקשרית.

- **הצגת תוצאה**  
  המארח מקבל את הפלט הסופי מהלקוח ומציג אותו למשתמש, לעיתים כולל גם את הטקסט שנוצר על ידי המודל
- **ניהול מחזור חיים**: מטפל באתחול חיבורים, משא ומתן על יכולות, וסיום סשן בין לקוחות לשרתים  
- **פרימיטיבים של שרת**: מאפשר לשרתים לספק פונקציונליות בסיסית באמצעות כלים, משאבים ותבניות  
- **פרימיטיבים של לקוח**: מאפשר לשרתים לבקש דגימה מ-LLMs, לקבל קלט ממשתמשים, ולשלוח הודעות לוג  
- **התראות בזמן אמת**: תומך בהתראות אסינכרוניות לעדכונים דינמיים ללא צורך בבדיקה מתמדת  

#### תכונות עיקריות:

- **משא ומתן על גרסת פרוטוקול**: משתמש בגרסאות מבוססות תאריך (YYYY-MM-DD) כדי להבטיח תאימות  
...
1. **תעבורת STDIO**:  
   ...
   - אידיאלי לתהליכים מקומיים באותה מכונה ללא עומס רשת  
   - נפוץ ביישומי שרת MCP מקומיים  

2. **תעבורת HTTP ניתנת לזרימה**:  
   - משתמש ב-HTTP POST להודעות מלקוח לשרת  
   - אופציונלי: Server-Sent Events (SSE) לזרימה משרת ללקוח  
   - מאפשר תקשורת עם שרתים מרוחקים דרך רשתות  
   - תומך באימות HTTP סטנדרטי (אסימוני גישה, מפתחות API, כותרות מותאמות אישית)  
   - MCP ממליץ על OAuth לאימות מאובטח מבוסס אסימונים  

#### הפשטת תעבורה:

שכבת התעבורה מפשטת את פרטי התקשורת משכבת הנתונים, ומאפשרת שימוש באותו פורמט הודעות JSON-RPC 2.0 בכל מנגנוני התעבורה. הפשטה זו מאפשרת לאפליקציות לעבור בין שרתים מקומיים למרוחקים בצורה חלקה.

### שיקולי אבטחה

יישומי MCP חייבים לעמוד בכמה עקרונות אבטחה קריטיים כדי להבטיח אינטראקציות בטוחות, אמינות ומאובטחות בכל פעולות הפרוטוקול:

- **הסכמה ושליטה של משתמש**: משתמשים חייבים לספק הסכמה מפורשת לפני גישה לנתונים או ביצוע פעולות. עליהם להיות בעלי שליטה ברורה על אילו נתונים משותפים ואילו פעולות מאושרות, עם ממשקי משתמש אינטואיטיביים לסקירה ואישור פעילויות.  

- **פרטיות נתונים**: נתוני משתמש צריכים להיחשף רק עם הסכמה מפורשת וחייבים להיות מוגנים באמצעות בקרות גישה מתאימות. יישומי MCP חייבים להגן מפני העברת נתונים לא מורשית ולהבטיח שפרטיות נשמרת לאורך כל האינטראקציות.  
...
על ידי עמידה בעקרונות אבטחה אלו, MCP מבטיח שמירה על אמון המשתמשים, פרטיות ובטיחות בכל אינטראקציות הפרוטוקול תוך מתן אפשרות לאינטגרציות AI מתקדמות.
...
דוגמה זו מדגימה את אותו שרת MCP ורישום כלים כמו בדוגמת .NET לעיל, אך מיושמת ב-Java.
...
בדוגמה זו מוצג כיצד לבנות שרת MCP ב-Python. מוצגות גם שתי דרכים שונות ליצירת כלים.
...
דוגמת JavaScript זו מדגימה כיצד ליצור לקוח MCP שמתחבר לשרת, שולח הנחיה, ומעבד את התגובה כולל כל קריאות כלים שבוצעו.
...
1. **בקרת הרשאות כלים**:  
   לקוחות יכולים לציין אילו כלים מודל רשאי להשתמש בהם במהלך סשן. הדבר מבטיח שרק כלים שאושרו במפורש יהיו נגישים, ומפחית את הסיכון לפעולות לא מכוונות או לא בטוחות. ניתן להגדיר הרשאות באופן דינמי בהתאם להעדפות משתמש, מדיניות ארגונית, או הקשר האינטראקציה.  
...
3. **אימות פרמטרים**:  
   אימות פרמטרים נאכף עבור כל קריאות הכלים. כל כלי מגדיר את סוגי, פורמטי, ומגבלות הפרמטרים הצפויים, והשרת מאמת בקשות נכנסות בהתאם. הדבר מונע קלט פגום או זדוני מלהגיע ליישומי הכלים ועוזר לשמור על שלמות הפעולות.  

4. **הגבלת קצב**:  
   כדי למנוע ניצול יתר ולהבטיח שימוש הוגן במשאבי השרת, שרתי MCP יכולים ליישם הגבלת קצב לקריאות כלים וגישה למשאבים. ניתן להחיל הגבלות קצב לפי משתמש, לפי סשן, או באופן גלובלי, והן עוזרות להגן מפני התקפות מניעת שירות או צריכת משאבים מוגזמת.  

על ידי שילוב מנגנונים אלו, MCP מספק בסיס מאובטח לאינטגרציה של מודלים שפתיים עם כלים ומשאבים חיצוניים, תוך מתן שליטה מדויקת למשתמשים ולמפתחים על גישה ושימוש.
...
#### **הודעות בצד הלקוח**
- **בקשת `sampling/complete`**: שרת מבקש השלמה מ-LLM דרך הלקוח  
- **`elicitation/request`**: שרת מבקש קלט משתמש דרך ממשק הלקוח  
- **הודעות לוג**: שרת שולח הודעות לוג מובנות ללקוח  

#### **הודעות התראה**
- **`notifications/tools/list_changed`**: שרת מודיע ללקוח על שינויים בכלים  
- **`notifications/resources/list_changed`**: שרת מודיע ללקוח על שינויים במשאבים  
- **`notifications/prompts/list_changed`**: שרת מודיע ללקוח על שינויים בתבניות הנחיה  
...
תקשורת מובנית זו מבטיחה אינטראקציות אמינות, ניתנות למעקב, ומורחבות, תוך תמיכה בתרחישים מתקדמים כמו עדכונים בזמן אמת, שרשור כלים, וטיפול חזק בשגיאות.
...
- **פרימיטיבים מרכזיים**: שרתים חושפים כלים (פונקציות ניתנות להפעלה), משאבים (מקורות נתונים), ותבניות (תבניות הנחיה)  
- **פרימיטיבים של לקוח**: שרתים יכולים לבקש דגימה (השלמות LLM), קלט (ממשתמשים), ולוגים מלקוחות  
- **בסיס פרוטוקול**: מבוסס על JSON-RPC 2.0 עם גרסאות מבוססות תאריך (נוכחית: 2025-06-18)  
