...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T15:39:28+00:00",
  "source_file": "11-MCPServerHandsOnLabs/04-Database/README.md",
  "language_code": "ru"
}
-->
# Проектирование базы данных и схема

## 🎯 Что охватывает этот лабораторный практикум

Этот практикум подробно рассматривает проектирование базы данных PostgreSQL для системы розничной торговли Zava. Вы научитесь реализовывать комплексную схему розничной торговли с возможностями векторного поиска, многопользовательским моделированием данных и управлением доступом на уровне строк (Row Level Security, RLS) для изоляции данных.

## Обзор

База данных является основой нашего MCP-сервера, хранящего данные розничной торговли для нескольких магазинов при строгой изоляции данных. Мы используем PostgreSQL с расширением pgvector для обеспечения семантического поиска, позволяя клиентам находить товары с помощью запросов на естественном языке.

Наша схема следует современным многопользовательским шаблонам, а управление доступом на уровне строк гарантирует, что пользователи могут получать доступ только к данным своих авторизованных магазинов. Такой подход обеспечивает безопасность корпоративного уровня при сохранении оптимальной производительности.

## Цели обучения

К концу этого практикума вы сможете:

- **Проектировать** масштабируемые многопользовательские схемы баз данных для розничной торговли
- **Реализовывать** PostgreSQL с pgvector для векторного поиска
- **Настраивать** управление доступом на уровне строк для изоляции данных
- **Генерировать** реалистичные тестовые данные
- **Оптимизировать** производительность базы данных для розничных нагрузок
- **Реализовывать** стратегии резервного копирования и восстановления

## 🗃️ Архитектура базы данных

### PostgreSQL с pgvector

Наша база данных использует корпоративные функции PostgreSQL в сочетании с расширением pgvector для поиска, основанного на ИИ:
...
### Архитектура многопользовательской модели

База данных использует модель многопользовательской архитектуры **общая база данных, общая схема** с управлением доступом на уровне строк:
...
### Таблица магазинов (основная таблица арендаторов)
...
## 🔐 Настройка управления доступом на уровне строк

### Роли базы данных и разрешения
...
### Скрипт для создания реалистичных тестовых данных
...
### Представления для оптимизации запросов
...
После завершения этого практикума вы должны:

✅ **Проектирование многопользовательской базы данных**: Реализовать управление доступом на уровне строк для безопасной изоляции данных  
✅ **Возможности векторного поиска**: Настроить pgvector для семантического поиска товаров  
✅ **Комплексная схема**: Создать готовую к производству схему базы данных для розничной торговли  
✅ **Генерация тестовых данных**: Создать реалистичные тестовые данные для разработки и тестирования  
✅ **Оптимизация производительности**: Настроить индексы и оптимизацию запросов  
✅ **Резервное копирование и восстановление**: Установить надежные стратегии защиты данных  

## 🚀 Что дальше

Продолжите с **[Лабораторного практикума 05: Реализация MCP-сервера](../05-MCP-Server/README.md)**, чтобы:

- Создать сервер FastMCP, который подключается к этой базе данных
- Реализовать инструменты запросов к базе данных для протокола MCP
- Добавить возможности семантического поиска с использованием эмбеддингов
- Настроить пул соединений и обработку ошибок

## 📚 Дополнительные ресурсы

### PostgreSQL и pgvector
- [Документация PostgreSQL](https://www.postgresql.org/docs/) - Полное руководство по PostgreSQL
- [Расширение pgvector](https://github.com/pgvector/pgvector) - Поиск по сходству вектора для PostgreSQL
- [Оптимизация производительности PostgreSQL](https://wiki.postgresql.org/wiki/Performance_Optimization) - Лучшие практики оптимизации

### Архитектура многопользовательской модели
- [Управление доступом на уровне строк](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Документация PostgreSQL по RLS
- [Архитектура многопользовательских данных](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Шаблоны архитектуры Azure
- [Лучшие практики безопасности базы данных](https://www.postgresql.org/docs/current/security.html) - Руководство по безопасности PostgreSQL

### Векторные базы данных
- [Основы векторного поиска](https://www.pinecone.io/learn/vector-database/) - Понимание векторных баз данных
- [Модели эмбеддингов](https://platform.openai.com/docs/guides/embeddings) - Документация OpenAI по эмбеддингам
- [Алгоритм HNSW](https://arxiv.org/abs/1603.09320) - Графы Hierarchical Navigable Small World

---

**Предыдущий**: [Лабораторный практикум 03: Настройка окружения](../03-Setup/README.md)  
**Следующий**: [Лабораторный практикум 05: Реализация MCP-сервера](../05-MCP-Server/README.md)
