...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T15:13:36+00:00",
...
Этот практикум поможет вам реализовать готовый к использованию MCP-сервер с использованием фреймворка FastMCP. Вы создадите основную структуру сервера, интегрируете базу данных, разработаете инструменты для доступа к данным и заложите основу для аналитики розничной торговли на базе ИИ.

## Обзор

MCP-сервер — это сердце нашего решения для аналитики розничной торговли. Он служит мостом между ИИ-ассистентами и базой данных PostgreSQL, обеспечивая безопасный и интеллектуальный доступ к бизнес-данным через стандартизированный протокол.

Этот практикум научит вас создавать надежный и масштабируемый MCP-сервер, следуя корпоративным шаблонам и лучшим практикам.

## Цели обучения

К концу этого практикума вы сможете:

- **Создать** сервер FastMCP с правильной архитектурой и организацией
- **Реализовать** интеграцию с базой данных с использованием пула соединений и обработки ошибок
- **Разработать** инструменты MCP для анализа схемы базы данных и выполнения запросов
- **Настроить** управление контекстом безопасности на уровне строк (Row Level Security)
- **Добавить** функции мониторинга состояния и наблюдаемости
- **Протестировать** реализацию MCP-сервера локально и с помощью VS Code

## 📁 Структура проекта

Рассмотрим организацию MCP-сервера:
...
### Конфигурация окружения (`config.py`)

Сначала создадим надежную систему конфигурации:
...
- **Загрузка переменных окружения**: Автоматическая поддержка .env-файлов
- **Безопасность типов**: Проверка типов и подсказки с использованием dataclass
- **Гибкие значения по умолчанию**: Разумные значения для разработки
- **Валидация**: Проверка конфигурации с полезными сообщениями об ошибках
- **Безопасность**: Чувствительные данные берутся только из переменных окружения

## 🗄️ Слой интеграции с базой данных

### Провайдер PostgreSQL (`sales_analysis_postgres.py`)

Реализуем слой интеграции с базой данных:
...
### Основные особенности слоя базы данных

- **Пул соединений**: Эффективное управление ресурсами с помощью asyncpg
- **Интеграция RLS**: Автоматическая настройка контекста безопасности на уровне строк
- **Анализ схемы**: Динамическое обнаружение структуры таблиц
- **Обработка ошибок**: Полное управление ошибками и логирование
- **Форматирование запросов**: Форматирование результатов, удобное для ИИ
- **Мониторинг состояния**: Проверка подключения к базе данных и состояния пула

## 🔧 Основная реализация MCP-сервера

### Сервер FastMCP (`sales_analysis.py`)

Теперь реализуем основной MCP-сервер:
...
### Основные особенности MCP-сервера

- **Регистрация инструментов**: Декларативное определение инструментов с безопасностью типов
- **Управление контекстом RLS**: Автоматическое извлечение идентичности пользователя и настройка контекста
- **Обработка ошибок**: Полное управление ошибками с удобными для пользователя сообщениями
- **Управление жизненным циклом**: Корректный запуск/остановка с очисткой ресурсов
- **Мониторинг состояния**: Встроенные конечные точки проверки состояния
- **Поддержка разработки**: Горячая перезагрузка и возможности отладки

## 🏥 Мониторинг состояния

### Реализация проверки состояния (`health_check.py`)
...
2. **Проверьте конечные точки состояния**:
   ```bash
   # Basic health check
...
3. **Протестируйте инструменты MCP**:
   ```bash
...
### Интеграционное тестирование в VS Code

1. **Настройте MCP в VS Code**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Тестирование в AI Chat**:
   - Откройте AI Chat в VS Code
   - Введите `#zava` и выберите ваш сервер
   - Спросите: "Какие таблицы доступны?"
   - Спросите: "Покажите топ-5 магазинов по количеству заказов"
...
✅ **Рабочий MCP-сервер**: FastMCP-сервер с интеграцией базы данных  
✅ **Управление конфигурацией**: Надежная конфигурация на основе окружения  
✅ **Слой базы данных**: Интеграция PostgreSQL с пулом соединений  
✅ **Инструменты MCP**: Инструменты для анализа схемы и выполнения запросов  
✅ **Интеграция RLS**: Управление контекстом безопасности на уровне строк  
✅ **Мониторинг состояния**: Полные конечные точки проверки состояния  
✅ **Стратегия тестирования**: Локальное тестирование и интеграция с VS Code  

## 🚀 Что дальше

Продолжите с **[Практикум 06: Разработка инструментов](../06-Tools/README.md)**, чтобы:

- Расширить коллекцию инструментов MCP
- Реализовать сложные шаблоны запросов
- Добавить валидацию и преобразование данных
- Создать специализированные аналитические инструменты

## 📚 Дополнительные ресурсы

### Фреймворк FastMCP
- [Документация FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Официальное руководство FastMCP
- [Спецификация MCP](https://modelcontextprotocol.io/docs/) - Спецификация протокола
- [Руководство по разработке инструментов](https://modelcontextprotocol.io/docs/tools/) - Создание инструментов MCP

### Интеграция с базой данных
- [Документация asyncpg](https://magicstack.github.io/asyncpg/current/) - Асинхронный драйвер PostgreSQL
- [Лучшие практики пула соединений](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Настройка PostgreSQL
- [Руководство по безопасности на уровне строк](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Реализация RLS

### Шаблоны FastAPI
- [Документация FastAPI](https://fastapi.tiangolo.com/) - Справочник по веб-фреймворку
- [Внедрение зависимостей](https://fastapi.tiangolo.com/tutorial/dependencies/) - Шаблоны FastAPI
- [Фоновые задачи](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Управление асинхронными задачами

---

**Далее**: Готовы расширить инструменты? Продолжите с [Практикум 06: Разработка инструментов](../06-Tools/README.md)
