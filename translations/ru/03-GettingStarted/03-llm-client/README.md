...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T23:28:41+00:00",
...
До сих пор вы видели, как создавать сервер и клиент. Клиент мог явно вызывать сервер для получения списка его инструментов, ресурсов и подсказок. Однако это не самый удобный подход. Ваш пользователь живёт в эпоху агентов и ожидает использовать подсказки и общаться с LLM для этого. Для пользователя не важно, используете ли вы MCP для хранения возможностей, но он ожидает взаимодействия на естественном языке. Как же решить эту задачу? Решение — добавить LLM в клиент.

## Обзор

В этом уроке мы сосредоточимся на добавлении LLM в клиент и покажем, как это улучшает опыт пользователя.
...
Давайте разберёмся, какой подход нам нужен. Добавить LLM звучит просто, но как мы это сделаем на самом деле?

Вот как клиент будет взаимодействовать с сервером:

1. Установить соединение с сервером.

1. Получить список возможностей, подсказок, ресурсов и инструментов, сохранить их схему.
...
- Создали метод `run`, отвечающий за логику приложения. Пока он только выводит список инструментов, но скоро мы добавим больше функционала.
...
- Для каждого инструмента вывели имя, описание и его схему. Последняя нам пригодится для вызова инструментов.
...
1. Добавьте следующий код для конвертации ответа MCP сервера в формат инструмента, который понимает LLM:
...
1. Обновим метод `run`, чтобы вывести возможности сервера:
...
1. Затем обновим код клиента, чтобы использовать эту функцию:
    ...
    Здесь мы добавляем вызов `convert_to_llm_tool` для преобразования ответа MCP инструмента в формат, который можно передать LLM.
...
1. Обновим существующий код, чтобы использовать эту функцию:
    ...
    - Обновили функцию для конвертации ответа MCP инструмента в инструмент LLM. Вот ключевая часть:
        ...
        Схема входных данных находится в атрибуте "properties" ответа инструмента, поэтому её нужно извлечь. Затем вызывается `ConvertFrom` с деталями инструмента. Теперь, когда основная работа сделана, посмотрим, как это используется при обработке пользовательских запросов.
...
- Такой подход избавляет от ручной конвертации инструментов — LangChain4j берёт на себя всю сложность преобразования MCP инструментов в формат, совместимый с LLM

Отлично, теперь мы готовы обрабатывать пользовательские запросы, приступим к этому.
...
1. Добавьте метод для вызова нашего LLM:
    ...
    - Вызывает инструмент, если LLM указал на необходимость вызова:
...
1. Покажем код для запроса к LLM:
...
1. Последний шаг — проверим, считает ли LLM, что нужно вызвать функцию:
    ...
    - Для каждого вызова парсим имя и аргументы, вызываем инструмент на MCP сервере через MCP клиент. В конце выводим результаты.
...
Возьмите код из упражнения и расширьте сервер, добавив ещё несколько инструментов. Затем создайте клиента с LLM, как в упражнении, и протестируйте его с разными подсказками, чтобы убедиться, что все инструменты сервера вызываются динамически. Такой способ создания клиента обеспечивает отличный пользовательский опыт, так как пользователи могут использовать подсказки вместо точных команд клиента и не замечать вызовы MCP сервера.
...
- Добавление LLM в клиент обеспечивает более удобный способ взаимодействия пользователей с MCP серверами.
