...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T20:25:10+00:00",
...
- **Обновления прогресса:**
   - Классический HTTP: прогресс передается в основном потоке ответа
...
Итак, вы уже видели рекомендации и сравнения классической потоковой передачи и MCP. Теперь рассмотрим подробнее, как именно можно использовать потоковую передачу в MCP.
...
Безопасность крайне важна при открытии MCP-серверов через HTTP. Streamable HTTP добавляет новые уязвимости и требует тщательной настройки.
...
- **Аутентификация**: В продакшене используйте аутентификацию (например, API-ключи, OAuth).
- **CORS**: Настраивайте политики Cross-Origin Resource Sharing (CORS) для ограничения доступа.
- **HTTPS**: В продакшене используйте HTTPS для шифрования трафика.
...
Для приложений, использующих Server-Sent Events (SSE), переход на Streamable HTTP открывает новые возможности и обеспечивает более устойчивую работу MCP.

### Зачем обновляться?

Есть две веские причины перейти с SSE на Streamable HTTP:

- Streamable HTTP обеспечивает лучшую масштабируемость, совместимость и расширенную поддержку уведомлений по сравнению с SSE.
- Это рекомендуемый транспорт для новых MCP-приложений.

### Шаги миграции

Как перейти с SSE на Streamable HTTP в ваших MCP-приложениях:
...
Рекомендуется сохранять поддержку существующих SSE-клиентов во время миграции. Вот несколько подходов:
...
Обратите внимание на следующие задачи при миграции:

- Обновление всех клиентов
- Учет различий в доставке уведомлений
...
Также рекомендуются следующие практики при реализации безопасности в вашем MCP-стриминговом сервере:
...
При реализации безопасности в MCP-стриминговых серверах вы столкнетесь с такими задачами:
