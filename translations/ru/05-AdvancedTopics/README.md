...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-17T15:08:53+00:00",
...
Этот урок исследует продвинутые концепции реализации Model Context Protocol, сосредотачиваясь на мультимодальной интеграции, масштабируемости, лучших практиках безопасности и интеграции в корпоративную среду. Эти темы важны для создания приложений MCP уровня производства, способных справляться с сложными требованиями в корпоративных условиях.

## Цели обучения

К концу этого урока вы сможете:
- Реализовывать мультимодальные возможности в рамках MCP
- Проектировать масштабируемые архитектуры MCP для сценариев с высоким спросом
...
### Архитектура для поддержки мультимодальности

Реализации мультимодальных MCP обычно включают:

1. **Парсеры, специфичные для модальности**: Компоненты, которые преобразуют различные типы медиа в форматы, которые модель может обработать
2. **Инструменты, специфичные для модальности**: Специальные инструменты, предназначенные для работы с определенными модальностями (анализ изображений, обработка аудио)
3. **Управление единым контекстом**: Система для поддержания контекста между различными модальностями
4. **Генерация ответов**: Возможность создавать ответы, которые могут включать несколько модальностей

### Пример на C#: Анализ изображений
...
### Пример на Java: Обработка аудио
...
### Пример на Python: Генерация мультимодальных ответов
...
Корневые контексты — это фундаментальная концепция в Model Context Protocol, которая предоставляет постоянный слой для поддержания истории разговоров и общего состояния между несколькими запросами и сессиями.

### Понимание корневых контекстов

Корневые контексты служат контейнерами, которые хранят историю и состояние для серии связанных взаимодействий. Они позволяют:

- **Сохранение разговоров**: Поддержание согласованных многоходовых разговоров
...
- **Общий доступ к контексту**: Позволяет нескольким клиентам получать доступ к одному и тому же состоянию разговора

В MCP корневые контексты имеют следующие ключевые характеристики:

- У каждого корневого контекста есть уникальный идентификатор
- Они могут содержать историю разговоров, предпочтения пользователей и другие метаданные
- Они могут быть созданы, доступны и архивированы по мере необходимости
- Они поддерживают детализированный контроль доступа и разрешения
...
### Пример на .NET: Работа с корневыми контекстами
...
### Пример на JavaScript: Управление корневым контекстом
...
### Пример на Python: Корневой контекст для многоходовой помощи
...
1. **Создание фокусированных контекстов**: Создавайте отдельные корневые контексты для разных целей или областей разговоров, чтобы поддерживать ясность.

2. **Установление политик истечения срока действия**: Реализуйте политики для архивирования или удаления старых контекстов, чтобы управлять хранением и соблюдать политики хранения данных.

3. **Хранение релевантных метаданных**: Используйте метаданные контекста для хранения важной информации о разговоре, которая может быть полезна позже.

4. **Последовательное использование идентификаторов контекста**: После создания контекста используйте его идентификатор последовательно для всех связанных запросов, чтобы поддерживать непрерывность.

5. **Генерация резюме**: Когда контекст становится большим, рассмотрите возможность генерации резюме для захвата важной информации при управлении размером контекста.

6. **Реализация контроля доступа**: Для многопользовательских систем реализуйте надлежащий контроль доступа, чтобы обеспечить конфиденциальность и безопасность контекстов разговоров.

7. **Обработка ограничений контекста**: Будьте в курсе ограничений размера контекста и реализуйте стратегии для обработки очень длинных разговоров.

8. **Архивирование по завершении**: Архивируйте контексты по завершении разговоров, чтобы освободить ресурсы, сохраняя при этом историю разговоров.

## Семплирование в Model Context Protocol

Стратегии семплирования критически важны для оптимизации ответов моделей в реализациях MCP. Правильная конфигурация семплирования может значительно улучшить качество и производительность ответов. MCP предоставляет стандартизированный способ управления тем, как модели генерируют текст с определенными параметрами, которые влияют на случайность, креативность и согласованность.

### Обзор параметров семплирования

MCP определяет следующие параметры семплирования, которые могут быть настроены в запросах клиента:
...
| `stop_sequences` | Пользовательские последовательности, которые останавливают генерацию при встрече | Массив строк |

### Семплирование по температуре и Top-K/Top-P

Параметры семплирования позволяют точно настраивать поведение языковых моделей для достижения желаемого баланса между детерминированными и творческими выходами.
...
Для приложений, требующих согласованных выходов, детерминированное семплирование обеспечивает воспроизводимые результаты.
...
### Конфигурация динамического семплирования

Интеллектуальное семплирование адаптирует параметры в зависимости от контекста и требований каждого запроса.
...
Маршрутизация на основе содержимого направляет запросы к специализированным службам на основе содержания запроса.
...
Балансировка нагрузки оптимизирует использование ресурсов и обеспечивает высокую доступность услуг MCP.
...
Маршрутизация инструментов гарантирует, что вызовы инструментов направляются к наиболее подходящей службе в зависимости от контекста.
...
### Архитектура семплирования и маршрутизации в MCP

Диаграмма ниже иллюстрирует, как семплирование и маршрутизация работают вместе в комплексной архитектуре MCP:
...
Для корпоративных внедрений реализации MCP часто должны обрабатывать большие объемы запросов с минимальной задержкой.
...
Безопасность критически важна для реализаций MCP, особенно в корпоративных средах.
...
Интеграция MCP с корпоративными системами, такими как Azure OpenAI и Microsoft AI Foundry.
...
### Интеграция Microsoft AI Foundry
...
### Пример корпоративной интеграции на Python
...
- Масштабируемость важна для корпоративных внедрений и может быть решена через горизонтальное и вертикальное масштабирование
...
Спроектируйте реализацию MCP корпоративного уровня для конкретного сценария использования:

1. Определите мультимодальные требования для вашего сценария использования
2. Очертите необходимые меры безопасности для защиты конфиденциальных данных
...
5. Документируйте потенциальные узкие места производительности и стратегии их смягчения
...
Далее: [Сообщество и вклад](../06-CommunityContributions/README.md)
