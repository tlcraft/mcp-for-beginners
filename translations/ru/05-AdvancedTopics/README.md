...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-02T11:54:40+00:00",
...
|--------|----------|----------|
| [mcp-oauth2-demo](../../../05-AdvancedTopics/mcp-oauth2-demo) | MCP OAuth2 Demo | Минимальное Spring Boot приложение, демонстрирующее OAuth2 с MCP в ролях Authorization и Resource Server. Показывает безопасный выпуск токенов, защищённые эндпоинты, деплоймент в Azure Container Apps и интеграцию с API Management. |
| [web-search-mcp](../../../05-AdvancedTopics/web-search-mcp) | Web Search MCP | Python MCP сервер и клиент, интегрирующиеся с SerpAPI для поиска в реальном времени по вебу, новостям, товарам и вопросам-ответам. Демонстрирует оркестрацию нескольких инструментов, интеграцию внешних API и надёжную обработку ошибок. |
...
Мультимодальные реализации MCP обычно включают:

1. **Парсеры, специфичные для модальностей**: Компоненты, преобразующие разные типы медиа в форматы, которые модель может обработать
2. **Инструменты, специфичные для модальностей**: Специальные инструменты для работы с определёнными модальностями (анализ изображений, обработка аудио)
3. **Единое управление контекстом**: Система для поддержания контекста между разными модальностями
...
Корневые контексты — это фундаментальная концепция Model Context Protocol, обеспечивающая постоянный слой для хранения истории разговоров и общего состояния между множеством запросов и сессий.
...
- **Совместное использование контекста**: Позволяют нескольким клиентам получать доступ к одному и тому же состоянию разговора
...
- Могут содержать историю разговора, пользовательские настройки и другие метаданные
- Могут создаваться, использоваться и архивироваться по мере необходимости
...
### Пример на JavaScript: Управление корневыми контекстами
...
### Пример на Python: Корневой контекст для многошаговой помощи
...
1. **Создавайте специализированные контексты**: Создавайте отдельные корневые контексты для разных целей или областей разговора, чтобы сохранять ясность.

2. **Устанавливайте политики истечения срока**: Реализуйте политики архивирования или удаления старых контекстов для управления хранилищем и соблюдения правил хранения данных.

3. **Храните релевантные метаданные**: Используйте метаданные контекста для хранения важной информации о разговоре, которая может пригодиться позже.

4. **Последовательно используйте ID контекста**: После создания контекста используйте его идентификатор для всех связанных запросов, чтобы сохранить непрерывность.

5. **Генерируйте сводки**: При значительном увеличении размера контекста создавайте сводки, чтобы зафиксировать основную информацию и контролировать размер контекста.

6. **Реализуйте контроль доступа**: Для систем с несколькими пользователями обеспечьте правильные механизмы контроля доступа для защиты приватности и безопасности контекстов.

7. **Учитывайте ограничения контекста**: Имейте в виду ограничения по размеру контекста и внедряйте стратегии для работы с очень длинными разговорами.

8. **Архивируйте по завершении**: Архивируйте контексты по окончании разговоров, чтобы освободить ресурсы и сохранить историю взаимодействия.

## Сэмплирование в Model Context Protocol

Стратегии сэмплирования критичны для оптимизации ответов модели в реализациях MCP. Правильная настройка сэмплирования может значительно улучшить качество и производительность ответов. MCP предоставляет стандартизированный способ управления генерацией текста моделями с параметрами, влияющими на случайность, креативность и связность.

### Обзор параметров сэмплирования

MCP определяет следующие параметры сэмплирования, которые можно настраивать в запросах клиента:
...
| `stop_sequences` | Пользовательские последовательности, при встрече которых генерация останавливается | Массив строк |

### Temperature и Top-K/Top-P сэмплирование

Параметры сэмплирования позволяют тонко настраивать поведение языковых моделей для достижения баланса между детерминированными и креативными ответами.
...
Для приложений, требующих стабильных и воспроизводимых результатов, детерминированное сэмплирование обеспечивает постоянство ответов.
...
Интеллектуальное сэмплирование адаптирует параметры в зависимости от контекста и требований каждого запроса.
...
Маршрутизация на основе содержимого направляет запросы к специализированным сервисам в зависимости от содержания запроса.
...
### Интеллектуальное балансирование нагрузки

Балансировка нагрузки оптимизирует использование ресурсов и обеспечивает высокую доступность MCP-сервисов.
...
Маршрутизация инструментов гарантирует, что вызовы инструментов направляются к наиболее подходящему сервису с учётом контекста.
...
### Архитектура сэмплирования и маршрутизации в MCP

Ниже приведена схема, иллюстрирующая совместную работу сэмплирования и маршрутизации в комплексной архитектуре MCP:
...
Для корпоративных внедрений реализации MCP часто должны обрабатывать большие объёмы запросов с минимальной задержкой.
...
Безопасность критична для реализаций MCP, особенно в корпоративных условиях.
...
- Масштабируемость необходима для корпоративных внедрений и достигается через горизонтальное и вертикальное масштабирование
...
Спроектируйте реализацию MCP промышленного уровня для конкретного сценария:

1. Определите мультимодальные требования для вашего сценария
2. Опишите меры безопасности для защиты чувствительных данных
3. Спроектируйте масштабируемую архитектуру, способную обрабатывать переменную нагрузку
