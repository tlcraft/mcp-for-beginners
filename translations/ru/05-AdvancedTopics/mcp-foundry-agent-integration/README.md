...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T23:50:37+00:00",
...
В этом руководстве показано, как интегрировать серверы Model Context Protocol (MCP) с агентами Azure AI Foundry, что позволяет эффективно организовывать работу инструментов и использовать возможности корпоративного ИИ.
...
mcp_tool = {
    ...
    "server_url": "https://api.example.com/mcp", # Адрес MCP сервера
    "require_approval": "never"                 # Политика одобрения: на данный момент поддерживается только "never"
}
```
...
def create_mcp_agent_example():
    ...
    with project_client:
        # Создание агента с инструментами MCP
        agent = project_client.agents.create_agent(
            model="gpt-4.1-nano", 
...
- Проверьте доступность URL MCP сервера
...
1. **Изучите создание собственных MCP серверов**: создавайте MCP серверы для закрытых источников данных
