...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T22:44:29+00:00",
...
JavaScript SDK обеспечивает легкий и гибкий подход к реализации MCP.
...
Azure API Management — отличный способ обеспечить безопасность MCP-серверов. Идея в том, чтобы разместить Azure API Management перед вашим MCP-сервером и позволить ему обрабатывать такие функции, как:
...
- Azure API Management выступает в роли шлюза и использует политики для направления и управления трафиком.
...
3. Выполните эту команду [azd](https://aka.ms/azd) для создания сервиса управления API, function app (с кодом) и всех необходимых ресурсов Azure
...
5. **Список инструментов**. Кликните по инструменту и выберите **Run Tool**.  

Если все шаги выполнены правильно, вы подключитесь к MCP-серверу и сможете вызвать инструмент.

## MCP-серверы для Azure

[Remote-mcp-functions](https://github.com/Azure-Samples/remote-mcp-functions-dotnet): этот набор репозиториев — шаблон быстрого старта для создания и развертывания кастомных удаленных MCP (Model Context Protocol) серверов с использованием Azure Functions на Python, C# .NET или Node/TypeScript.
...
В репозитории есть все необходимые конфигурационные файлы, исходный код и определения инфраструктуры для быстрого старта с готовой к производству реализацией MCP-сервера.
