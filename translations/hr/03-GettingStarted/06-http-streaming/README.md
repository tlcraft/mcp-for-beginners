...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T17:32:31+00:00",
...
Ovo poglavlje pruža sveobuhvatan vodič za implementaciju sigurnog, skalabilnog i real-time streaminga s Model Context Protocolom (MCP) koristeći HTTPS. Pokriva motivaciju za streaming, dostupne transportne mehanizme, kako implementirati streamable HTTP u MCP-u, najbolje prakse za sigurnost, migraciju sa SSE-a i praktične smjernice za izradu vlastitih MCP aplikacija za streaming.

## Transportni mehanizmi i streaming u MCP-u

Ovaj dio istražuje različite transportne mehanizme dostupne u MCP-u i njihovu ulogu u omogućavanju streaming mogućnosti za real-time komunikaciju između klijenata i servera.
...
- **SSE (Server-Sent Events)**: Omogućuje serverima da šalju real-time ažuriranja klijentima preko HTTP-a. Dobro za web sučelja, ali ograničeno u skalabilnosti i fleksibilnosti.
...
| Transport         | Real-time ažuriranja | Streaming | Skalabilnost | Upotreba                  |
|-------------------|----------------------|-----------|-------------|---------------------------|
| stdio             | Ne                  | Ne        | Niska       | Lokalni CLI alati         |
| SSE               | Da                  | Da        | Srednja     | Web, real-time ažuriranja |
| Streamable HTTP   | Da                  | Da        | Visoka      | Cloud, multi-klijent      |
...
Razumijevanje temeljnih koncepata i motivacije iza streaminga ključno je za implementaciju učinkovitih sustava za real-time komunikaciju.
...
- Koristi Spring Boot-ov reaktivni stack s `Flux` za streaming.
- `ServerSentEvent` omogućuje strukturirani event streaming s tipovima događaja.
...
Razlike između načina na koji streaming funkcionira na "klasičan" način u usporedbi s načinom na koji funkcionira u MCP-u mogu se prikazati ovako:
...
| Upotreba            | Velike datoteke, AI tokeni     | Napredak, logovi, real-time povratne informacije |
...
- **Format poruka:**
  - Klasični HTTP: Plain text dijelovi s novim redovima.
...
Razumijevanje kako streaming funkcionira unutar MCP okvira ključno je za izradu responzivnih aplikacija koje pružaju real-time povratne informacije korisnicima tijekom dugotrajnih operacija.
...
Glavni rezultat i dalje se šalje kao jedan odgovor. Međutim, obavijesti se mogu slati kao zasebne poruke tijekom obrade i na taj način ažurirati klijenta u realnom vremenu. Klijent mora biti sposoban obraditi i prikazati te obavijesti.
...
> [!NOTE]
> Ovisno o korištenom SDK-u, logging može biti omogućen po defaultu ili ga možda trebate eksplicitno omogućiti u konfiguraciji servera.
...
Za implementaciju obavijesti u MCP-u, potrebno je postaviti i server i klijent kako bi mogli obrađivati real-time ažuriranja. Ovo omogućuje vašoj aplikaciji da pruži trenutne povratne informacije korisnicima tijekom dugotrajnih operacija.

### Server-side: Slanje obavijesti

Počnimo sa server stranom. U MCP-u definirate alate koji mogu slati obavijesti dok obrađuju zahtjeve. Server koristi objekt konteksta (obično `ctx`) za slanje poruka klijentu.
...
### Client-side: Primanje obavijesti
...
U prethodnom kodu, funkcija `message_handler` provjerava je li dolazna poruka obavijest. Ako jest, ispisuje obavijest; inače, obrađuje je kao redovnu poruku servera. Također, primijetite kako je `ClientSession` inicijaliziran s `message_handler` za obradu dolaznih obavijesti.
...
U ovom .NET primjeru, funkcija `MessageHandler` provjerava je li dolazna poruka obavijest. Ako jest, ispisuje obavijest; inače, obrađuje je kao redovnu poruku servera. `ClientSession` je inicijaliziran s handlerom poruka putem `ClientSessionOptions`.
...
Ovaj dio objašnjava koncept obavijesti o napretku u MCP-u, zašto su važne i kako ih implementirati koristeći Streamable HTTP. Također ćete pronaći praktični zadatak za jačanje vašeg razumijevanja.

Obavijesti o napretku su real-time poruke koje server šalje klijentu tijekom dugotrajnih operacija. Umjesto čekanja da cijeli proces završi, server ažurira klijenta o trenutnom statusu. Ovo poboljšava transparentnost, korisničko iskustvo i olakšava debugiranje.
...
- **Real-time povratne informacije:** Klijenti mogu prikazati trake napretka ili logove, čineći aplikaciju responzivnom.
...
Kod implementacije MCP servera s HTTP transportima, sigurnost postaje ključna briga koja zahtijeva pažnju na više vektora napada i mehanizama zaštite.

### Pregled

Sigurnost je kritična kada izlažete MCP servere preko HTTP-a. Streamable HTTP uvodi nove površine napada i zahtijeva pažljivu konfiguraciju.

### Ključne točke

- **Validacija Origin headera**: Uvijek validirajte `Origin` header kako biste spriječili DNS rebinding napade.
- **Localhost binding**: Za lokalni razvoj, povežite servere na `localhost` kako biste izbjegli izlaganje javnom internetu.
...
Prilikom implementacije MCP poslužitelja s HTTP-baziranim transportima, sigurnost postaje ključna briga koja zahtijeva pažljivo razmatranje više vektora napada i zaštitnih mehanizama.
...
### Zadatak: Izgradite vlastitu MCP streaming aplikaciju
...
2. Implementirajte klijent s rukovateljem poruka za prikaz obavijesti u stvarnom vremenu.
3. Testirajte svoju implementaciju pokretanjem poslužitelja i klijenta te promatrajte obavijesti.
...
- Pokušajte izgraditi naprednije MCP alate koji koriste streaming za analitiku u stvarnom vremenu, chat ili suradničko uređivanje.
