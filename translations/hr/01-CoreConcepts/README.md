...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T21:55:40+00:00",
...
[Model Context Protocol (MCP)](https://gi- **Izričita korisnička suglasnost**: Svi pristupi podacima i operacije zahtijevaju izričitu korisničku suglasnost prije izvršenja. Korisnici moraju jasno razumjeti koji će podaci biti pristupani i koje će radnje biti izvedene, uz detaljnu kontrolu nad dozvolama i autorizacijama.
...
- **Sigurnost izvršavanja alata**: Svako pokretanje alata zahtijeva izričitu korisničku suglasnost uz jasno razumijevanje funkcionalnosti alata, parametara i potencijalnog utjecaja. Snažne sigurnosne granice moraju spriječiti nenamjerno, nesigurno ili zlonamjerno pokretanje alata.

- **Sigurnost transportnog sloja**: Svi komunikacijski kanali trebaju koristiti odgovarajuće mehanizme enkripcije i autentifikacije. Daljinske veze trebaju implementirati sigurne transportne protokole i pravilno upravljanje vjerodajnicama.
...
- Analizirati ključne značajke koje MCP čine fleksibilnim slojem za integraciju.
...
Ekosustav MCP-a temelji se na modelu klijent-server. Ova modularna struktura omogućuje AI aplikacijama učinkovitu interakciju s alatima, bazama podataka, API-ima i kontekstualnim resursima. Razmotrimo ovu arhitekturu kroz njezine ključne komponente.
...
Serveri u Model Context Protocol-u (MCP) pružaju tri osnovna **primitiva** koji definiraju temeljne gradivne blokove za bogate interakcije između klijenata, domaćina i jezičnih modela. Ovi primitivi specificiraju vrste kontekstualnih informacija i radnji dostupnih putem protokola.
...
**Alati** su izvršne funkcije koje AI modeli mogu pozvati kako bi izvršili specifične radnje. Oni predstavljaju "glagole" MCP ekosustava, omogućujući modelima interakciju s vanjskim sustavima:
...
- **Mogućnosti radnji**: Omogućuju modelima izvođenje stvarnih radnji i dohvaćanje živih podataka

Alati su definirani JSON shemom za validaciju parametara i otkrivaju se putem `tools/list` te izvršavaju putem `tools/call`:
...
U Model Context Protocol-u (MCP), **klijenti** mogu izložiti primitive koji omogućuju serverima da zatraže dodatne mogućnosti od aplikacije domaćina. Ovi primitivi na strani klijenta omogućuju bogatije, interaktivnije implementacije servera koje mogu pristupiti mogućnostima AI modela i interakcijama korisnika.

### Uzorkovanje

**Uzorkovanje** omogućuje serverima da zatraže dovršavanje jezičnog modela od AI aplikacije klijenta. Ovaj primitiv omogućuje serverima pristup LLM mogućnostima bez ugrađivanja vlastitih modelskih ovisnosti:
...
- **Rekurzivne interakcije LLM-a**: Podržava složene scenarije gdje serveri trebaju AI pomoć za obradu
...
Zahtjevi za elicitaciju izrađuju se pomoću metode `elicitation/request` za prikupljanje korisničkog unosa putem sučelja klijenta.
...
Poruke zapisivanja šalju se klijentima kako bi se osigurala transparentnost operacija servera i olakšalo otklanjanje poteškoća.
...
- **Domaćin inicira vezu**  
  Aplikacija domaćin (kao što je IDE ili sučelje za chat) uspostavlja vezu s MCP serverom, obično putem STDIO-a, WebSocket-a ili drugog podržanog transporta.
...
- **Korištenje resursa ili alata**  
  ...
  - Ako model utvrdi da je potreban alat (npr. za dohvaćanje podataka, izvođenje izračuna ili pozivanje API-ja), kl
- **Upravljanje životnim ciklusom**: Upravljanje inicijalizacijom veze, pregovorima o mogućnostima i završetkom sesije između klijenata i poslužitelja  
- **Poslužiteljski primitivni elementi**: Omogućuje poslužiteljima pružanje osnovnih funkcionalnosti putem alata, resursa i predložaka  
- **Klijentski primitivni elementi**: Omogućuje poslužiteljima zahtijevanje uzorkovanja iz LLM-ova, prikupljanje korisničkog unosa i slanje zapisničkih poruka  
- **Obavijesti u stvarnom vremenu**: Podržava asinkrone obavijesti za dinamična ažuriranja bez potrebe za stalnim upitima  
...
- **Stanje sesije**: Održava stanje veze tijekom više interakcija za kontinuitet konteksta  
...
1. **STDIO transport**:  
   ...
   - Često korišteno za lokalne MCP poslužiteljske implementacije  

2. **Streamable HTTP transport**:  
   - Koristi HTTP POST za poruke klijent-poslužitelj  
   - Opcionalni Server-Sent Events (SSE) za strujanje poslužitelj-klijent  
...
Transportni sloj apstrahira detalje komunikacije od podatkovnog sloja, omogućujući isti JSON-RPC 2.0 format poruka za sve transportne mehanizme. Ova apstrakcija omogućuje aplikacijama jednostavno prebacivanje između lokalnih i udaljenih poslužitelja.

### Sigurnosni aspekti

MCP implementacije moraju se pridržavati nekoliko ključnih sigurnosnih principa kako bi osigurale sigurne, pouzdane i zaštićene interakcije u svim operacijama protokola:

- **Korisnički pristanak i kontrola**: Korisnici moraju dati izričit pristanak prije pristupa bilo kojim podacima ili izvođenja operacija. Trebali bi imati jasnu kontrolu nad time koji se podaci dijele i koje su radnje odobrene, uz intuitivna korisnička sučelja za pregled i odobravanje aktivnosti.  

- **Privatnost podataka**: Korisnički podaci smiju se izložiti samo uz izričit pristanak i moraju biti zaštićeni odgovarajućim kontrolama pristupa. MCP implementacije moraju spriječiti neovlašten prijenos podataka i osigurati privatnost tijekom svih interakcija.  

- **Sigurnost alata**: Prije korištenja bilo kojeg alata potreban je izričit pristanak korisnika. Korisnici trebaju jasno razumjeti funkcionalnost svakog alata, a stroge sigurnosne granice moraju se provoditi kako bi se spriječilo nenamjerno ili nesigurno korištenje alata.  

Slijedeći ove sigurnosne principe, MCP osigurava povjerenje korisnika, zaštitu privatnosti i sigurnost tijekom svih interakcija protokola, omogućujući snažne AI integracije.
...
Ovdje je praktičan primjer koda u .NET-u koji pokazuje kako implementirati jednostavan MCP poslužitelj s prilagođenim alatima. Ovaj primjer prikazuje kako definirati i registrirati alate, rukovati zahtjevima i povezati poslužitelj koristeći Model Context Protocol.
...
U ovom primjeru prikazujemo kako izgraditi MCP poslužitelj u Pythonu. Također su prikazana dva različita načina za kreiranje alata.
...
Ovaj primjer prikazuje kreiranje MCP poslužitelja u JavaScriptu i kako registrirati dva alata povezana s vremenskim prognozama.
...
Ovaj JavaScript primjer demonstrira kako kreirati MCP klijent koji se povezuje s poslužiteljem, šalje upit i obrađuje odgovor, uključujući sve pozive alata koji su izvršeni.
...
Kombiniranjem ovih mehanizama, MCP pruža sigurnu osnovu za integraciju jezičnih modela s vanjskim alatima i izvorima podataka, dok korisnicima i programerima omogućuje preciznu kontrolu pristupa i upotrebe.
...
- **Zahtjev `initialize`**: Uspostavlja vezu i pregovara o verziji protokola i mogućnostima  
...
#### **Poruke izvršenja**  
- **Zahtjev `tools/call`**: Izvršava određeni alat s pruženim parametrima  
...
- **Poruke zapisivanja**: Poslužitelj šalje strukturirane zapisničke poruke klijentu  
...
Ova strukturirana komunikacija osigurava pouzdane, sljedive i proširive interakcije koje podržavaju napredne scenarije poput ažuriranja u stvarnom vremenu, ulančavanja alata i robusnog rukovanja pogreškama.
...
- **Sudionici**: Ekosustav uključuje domaćine (AI aplikacije), klijente (konektore protokola) i poslužitelje (pružatelje mogućnosti)  
- **Transportni mehanizmi**: Komunikacija podržava STDIO (lokalno) i Streamable HTTP s opcionalnim SSE (udaljeno)  
- **Osnovni primitivni elementi**: Poslužitelji izlažu alate (izvršne funkcije), resurse (izvore podataka) i predloške (template)  
- **Klijentski primitivni elementi**: Poslužitelji mogu zahtijevati uzorkovanje (LLM dovršetke), prikupljanje unosa (elicitation) i zapisivanje od klijenata  
- **Osnova protokola**: Temeljen na JSON-RPC 2.0 s verzioniranjem temeljenim na datumu (trenutno: 2025-06-18)  
- **Sposobnosti u stvarnom vremenu**: Podržava obavijesti za dinamična ažuriranja i sinkronizaciju u stvarnom vremenu  
