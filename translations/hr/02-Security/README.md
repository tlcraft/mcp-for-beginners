...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T22:05:29+00:00",
...
Model Context Protocol (MCP) donosi snažne nove mogućnosti za aplikacije temeljene na AI-u, ali također uvodi jedinstvene sigurnosne izazove koji nadilaze tradicionalne softverske rizike. MCP sustavi suočavaju se s već poznatim sigurnosnim problemima (sigurno kodiranje, najmanje privilegije, sigurnost opskrbnog lanca) i novim prijetnjama specifičnim za AI, uključujući ubrizgavanje prompta, trovanje alata, otmicu sesija, napade zbunjenog posrednika, ranjivosti prijenosa tokena i dinamičke izmjene sposobnosti.

Ova lekcija istražuje najkritičnije sigurnosne rizike u MCP implementacijama—pokrivajući autentifikaciju, autorizaciju, prekomjerne privilegije, neizravno ubrizgavanje prompta, sigurnost sesija, probleme zbunjenog posrednika, upravljanje tokenima i ranjivosti opskrbnog lanca. Naučit ćete praktične kontrole i najbolje prakse za ublažavanje ovih rizika koristeći Microsoftova rješenja poput Prompt Shields, Azure Content Safety i GitHub Advanced Security za jačanje vaše MCP implementacije.

## Ciljevi učenja

Na kraju ove lekcije, moći ćete:

- **Prepoznati prijetnje specifične za MCP**: Identificirati jedinstvene sigurnosne rizike u MCP sustavima, uključujući ubrizgavanje prompta, trovanje alata, prekomjerne privilegije, otmicu sesija, probleme zbunjenog posrednika, ranjivosti prijenosa tokena i rizike opskrbnog lanca
...
- **Sigurnost transportnog sloja**: Poboljšana podrška za sigurne transportne mehanizme s pravilnim obrascima autentifikacije za lokalne (STDIO) i udaljene (Streamable HTTP) veze
...
- **Kompromitacija OAuth tokena**: Krađa tokena lokalnog MCP poslužitelja omogućuje napadačima da se predstavljaju kao poslužitelji i pristupe uslugama nizvodno
...
- Dnevnici poslužitelja resursa nizvodno pokazuju pogrešno podrijetlo zahtjeva umjesto stvarnih MCP posrednika
- Istrage incidenata i revizije usklađenosti postaju znatno teže

##### Rizici eksfiltracije podataka
- Nevalidirane tvrdnje tokena omogućuju zlonamjernim akterima s ukradenim tokenima korištenje MCP poslužitelja kao proxyja za eksfiltraciju podataka
...
### Sigurnosne kontrole i mitigacije
...
- **Sigurno upravljanje tokenima**: Implementirati [Microsoftove najbolje prakse za validaciju i životni ciklus tokena](https://learn.microsoft.com/en-us/entra/identity-platform/access-tokens)
  - Validirati tvrdnje o publici tokena kako bi odgovarale identitetu MCP poslužitelja
...
- **Zaštićeno pohranjivanje tokena**: Sigurno pohranjivanje tokena uz enkripciju i u mirovanju i u prijenosu
...
- **Kontinuirano praćenje privilegija**: Implementirati stalno praćenje i reviziju pristupa
  ...
  - Brzo rješavanje prekomjernih ili neiskorištenih privilegija
...
**Mehanizmi napada:**
- **Manipulacija metapodataka**: Napadači ubrizgavaju zlonamjerne upute u opise alata, definicije parametara ili primjere upotrebe
...
- **Trovanje kontekstualnog prozora**: Napadi koji manipuliraju velikim kontekstualnim prozorima kako bi sakrili zlonamjerne upute
...
- **Povrede privatnosti**: Izlaganje osobnih identifikacijskih podataka (PII) i povjerljivih poslovnih informacija  
...
Microsoft **AI Prompt Shields** pruža sveobuhvatnu obranu od izravnih i neizravnih napada ubrizgavanja prompta kroz višeslojne sigurnosne mehanizme:
...
2. **Tehnike isticanja**  
   ...
   - Metode transformacije teksta koje poboljšavaju relevantnost modela uz izol
...
- **Upravljanje životnim ciklusom sesija**: Implementirajte pravilno istecanje, rotaciju i poništavanje sesija kako biste smanjili prozore ranjivosti  
...
- **Krađa autorizacijskih kodova**: Postojeći kolačići pristanka mogu uzrokovati da poslužitelji za autorizaciju preskoče ekrane za pristanak, preusmjeravajući kodove na krajnje točke pod kontrolom napadača  
...
**Obavezne kontrole:**
- **Eksplicitni zahtjevi za pristanak**: MCP poslužitelji koji koriste statične ID-ove klijenata **MORAJU** dobiti pristanak korisnika za svakog dinamički registriranog klijenta  
- **Implementacija sigurnosti OAuth 2.1**: Slijedite trenutne najbolje prakse za OAuth sigurnost, uključujući PKCE (Proof Key for Code Exchange) za sve zahtjeve za autorizaciju  
...
- **Eksfiltracija podataka putem poslužitelja**: Nevalidirani tokeni omogućuju zlonamjernim akterima korištenje poslužitelja kao posrednika za neovlašteni pristup podacima  
...
- **Validacija tokena**: MCP poslužitelji **NE SMIJU** prihvaćati tokene koji nisu eksplicitno izdani za MCP poslužitelj  
...
### Proširene komponente AI opskrbnog lanca
...
- **Postupna implementacija**: Koristite strategije progresivne implementacije sa sigurnosnom validacijom u svakoj fazi  
...
- **Kontrole API pristupnika**: Koristite centralizirane API pristupnike za kontrolu i praćenje pristupa vanjskim AI uslugama  
...
- **Sposobnosti povratka**: Mogućnost brzog vraćanja na prethodne poznate dobre verzije AI komponenti  
- **Oporavak od kompromitacije opskrbnog lanca**: Specifični postupci za odgovor na kompromise AI usluga uzvodno  
...
- [Azure API Management kao MCP autentifikacijska pristupna točka](https://techcommunity.microsoft.com/blog/integrationsonazureblog/azure-api-management-your-auth-gateway-for-mcp-servers/4402690)
...
- **[MCP Najbolje Sigurnosne Prakse 2025](./mcp-security-best-practices-2025.md)** - Kompletne najbolje prakse za MCP implementacije  
