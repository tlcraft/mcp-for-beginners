...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T22:03:27+00:00",
...
Model Context Protocol uvodi jedinstvene sigurnosne izazove koji nadilaze tradicionalnu softversku sigurnost. Ove prakse pokrivaju temeljne sigurnosne zahtjeve i MCP-specifične prijetnje, uključujući ubrizgavanje prompta, trovanje alata, otmicu sesija, probleme zbunjenog zamjenika i ranjivosti prijenosa tokena.

### **OBAVEZNI Sigurnosni Zahtjevi**

**Ključni Zahtjevi iz MCP Specifikacije:**

> **NE SMIJE**: MCP poslužitelji **NE SMIJU** prihvaćati tokene koji nisu izričito izdani za MCP poslužitelj  
> 
> **MORA**: MCP poslužitelji koji implementiraju autorizaciju **MORAJU** provjeriti SVE dolazne zahtjeve  
>  
> **NE SMIJE**: MCP poslužitelji **NE SMIJU** koristiti sesije za autentifikaciju  
>
> **MORA**: MCP proxy poslužitelji koji koriste statične ID-ove klijenata **MORAJU** dobiti pristanak korisnika za svakog dinamički registriranog klijenta  
...
**Kontrole Autentifikacije i Autorizacije:**  
   - **Detaljna Revizija Autorizacije**: Provedite sveobuhvatne revizije logike autorizacije MCP poslužitelja kako biste osigurali da samo namijenjeni korisnici i klijenti mogu pristupiti resursima  
   ...
   - **Validacija Publike Tokena**: Uvijek provjerite jesu li tokeni izričito izdani za vaš MCP poslužitelj - nikada ne prihvaćajte tokene iz drugih izvora  
   - **Pravilno Upravljanje Životnim Ciklusom Tokena**: Implementirajte sigurno rotiranje tokena, politike isteka i spriječite napade ponovnog korištenja tokena  
...
**Zaštićeno Pohranjivanje Tokena:**  
   ...
   - Redovito rotirajte vjerodajnice i pratite neovlašteni pristup  
...
**Sigurne Prakse za Sesije:**  
   ...
   - **Veza sa Specifičnim Korisnikom**: Povežite ID-ove sesija s identitetima korisnika koristeći formate poput `<user_id>:<session_id>` kako biste spriječili zloupotrebu sesija među korisnicima  
   - **Upravljanje Životnim Ciklusom Sesija**: Implementirajte pravilno isteknuće, rotaciju i poništavanje kako biste ograničili prozore ranjivosti  
   - **Obavezno HTTPS/TLS**: Obavezno koristite HTTPS za svu komunikaciju kako biste spriječili presretanje ID-ova sesija  
...
**Prevencija Trovanja Alata:**  
   ...
   - **Dinamičko Praćenje Alata**: Pratite ponašanje u stvarnom vremenu i postavite upozorenja za neočekivane obrasce izvršavanja  
   - **Radni Tokovi Odobrenja**: Zahtijevajte izričito odobrenje korisnika za izmjene alata i promjene sposobnosti  

## 4. **Kontrola Pristupa i Dozvole**

**Načelo Najmanjih Privilegija:**  
   - Dodijelite MCP poslužiteljima samo minimalne dozvole potrebne za predviđenu funkcionalnost  
   - Implementirajte kontrolu pristupa temeljenu na ulogama (RBAC) s preciznim dozvolama  
   - Redovito pregledavajte dozvole i kontinuirano pratite eskalaciju privilegija  

**Kontrole Dozvola u Vrijeme Izvršavanja:**  
...
**Implementacija Sigurnosti Sadržaja:**  
   ...
   - **Analiza Ponašanja**: Implementirajte praćenje ponašanja u stvarnom vremenu kako biste otkrili anomalije u MCP poslužitelju i izvršavanju alata  
   - **Sveobuhvatno Logiranje**: Zabilježite sve pokušaje autentifikacije, pozive alata i sigurnosne događaje u sigurnu, nepromjenjivu pohranu  
...
**Siguran Razvojni Cjevovod:**  
   - **GitHub Advanced Security**: Implementirajte skeniranje tajni, analizu ovisnosti i statičku analizu CodeQL  
   - **Sigurnost CI/CD-a**: Integrirajte sigurnosnu validaciju kroz automatizirane cjevovode implementacije  
...
**Implementacija OAuth 2.1:**  
   ...
   - **Izričiti Pristanak**: Dobijte pristanak korisnika za svakog dinamički registriranog klijenta kako biste spriječili napade zbunjenog zamjenika  
   - **Validacija Preusmjeravajućih URI-ja**: Implementirajte strogu validaciju preusmjeravajućih URI-ja i identifikatora klijenata  

**Sigurnost Proxyja:**  
   - Spriječite zaobilaženje autorizacije putem iskorištavanja statičnih ID-ova klijenata  
...
**Sposobnosti Brzog Odgovora:**  
   ...
   - **Forenzičke Sposobnosti**: Detaljni tragovi revizije i logiranje za istragu incidenata  

**Komunikacija i Koordinacija:**  
   - Jasni postupci eskalacije za sigurnosne incidente  
   - Integracija s timovima za odgovor na incidente unutar organizacije  
...
**Regulatorna Usklađenost:**  
   ...
   - Održavajte sveobuhvatnu dokumentaciju za revizije usklađenosti  
...
**Zaštita Aplikacija u Vrijeme Izvršavanja:**  
   ...
   - **Dinamičke Sigurnosne Politike**: Implementirajte sigurnosne politike koje se prilagođavaju trenutnom sigurnosnom krajoliku  
...
**Sveobuhvatna Microsoft Sigurnost:**  
   ...
   - **Azure Sentinel**: Cloud-native SIEM i SOAR mogućnosti za naprednu detekciju prijetnji  
...
**Upravljanje Identitetom i Pristupom:**  
   - **Microsoft Entra ID**: Upravljanje identitetom na razini poduzeća s politikama uvjetnog pristupa  
   - **Privileged Identity Management (PIM)**: Pristup "na zahtjev" i radni tokovi odobrenja za administrativne funkcije  
   - **Zaštita Identiteta**: Pristup temeljen na riziku i automatizirani odgovor na prijetnje  
...
**Održavanje Ažurnosti:**  
   ...
   - **Obavještajni Podaci o Prijetnjama**: Integracija AI-specifičnih izvora prijetnji i indikatora kompromitacije  
   - **Sudjelovanje u Sigurnosnoj Zajednici**: Aktivno sudjelovanje u MCP sigurnosnoj zajednici i programima otkrivanja ranjivosti  

**Adaptivna Sigurnost:**  
   - **Sigurnost Temeljena na Strojnom Učenju**: Koristite detekciju anomalija temeljenu na ML-u za identifikaciju novih obrazaca napada  
   - **Prediktivna Sigurnosna Analitika**: Implementirajte prediktivne modele za proaktivnu identifikaciju prijetnji  
   - **Automatizacija Sigurnosti**: Automatizirano ažuriranje sigurnosnih politika temeljeno na obavještajnim podacima o prijetnjama i promjenama specifikacija  
