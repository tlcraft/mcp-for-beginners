...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:01:56+00:00",
  "source_file": "11-MCPServerHandsOnLabs/05-MCP-Server/README.md",
  "language_code": "hr"
}
-->
# Implementacija MCP poslužitelja

## 🎯 Što ovaj laboratorij pokriva

Ovaj praktični laboratorij vodi vas kroz implementaciju produkcijski spremnog MCP poslužitelja koristeći FastMCP okvir. Izgradit ćete osnovnu strukturu poslužitelja, implementirati integraciju s bazom podataka, kreirati alate za pristup podacima i postaviti temelje za analitiku u maloprodaji temeljenu na umjetnoj inteligenciji.

## Pregled

MCP poslužitelj je srce našeg rješenja za analitiku u maloprodaji. Djeluje kao most između AI asistenata i PostgreSQL baze podataka, pružajući siguran i inteligentan pristup poslovnim podacima putem standardiziranog protokola.

Ovaj laboratorij vas uči kako izgraditi robustan i skalabilan MCP poslužitelj slijedeći korporativne obrasce i najbolje prakse.

## Ciljevi učenja

Na kraju ovog laboratorija, moći ćete:

- **Izgraditi** FastMCP poslužitelj s pravilnom arhitekturom i organizacijom
- **Implementirati** integraciju s bazom podataka koristeći pooliranje konekcija i rukovanje greškama
- **Kreirati** MCP alate za introspekciju sheme baze podataka i izvršavanje upita
- **Konfigurirati** upravljanje kontekstom sigurnosti na razini redaka (Row Level Security)
- **Dodati** značajke za praćenje zdravlja i observabilnost
- **Testirati** implementaciju MCP poslužitelja lokalno i u VS Code-u

## 📁 Struktura projekta

Pogledajmo organizaciju MCP poslužitelja:
...
### Konfiguracija okruženja (`config.py`)

Prvo, kreirajmo robustan sustav konfiguracije:
...
### Ključne značajke konfiguracije

- **Učitavanje varijabli okruženja**: Automatska podrška za .env datoteke
- **Sigurnost tipova**: Validacija dataclass-a i tipovi podataka
- **Fleksibilni zadani parametri**: Razumni zadani parametri za razvoj
- **Validacija**: Validacija konfiguracije s korisnim porukama o greškama
- **Sigurnost**: Osjetljive vrijednosti samo iz varijabli okruženja

## 🗄️ Sloj integracije baze podataka

### PostgreSQL Provider (`sales_analysis_postgres.py`)

Implementirajmo sloj integracije baze podataka:
...
### Ključne značajke sloja baze podataka

- **Pooliranje konekcija**: Učinkovito upravljanje resursima s asyncpg
- **Integracija RLS-a**: Automatsko postavljanje konteksta sigurnosti na razini redaka
- **Introspekcija sheme**: Dinamičko otkrivanje sheme tablica
- **Rukovanje greškama**: Sveobuhvatno upravljanje greškama i logiranje
- **Formatiranje upita**: Rezultati prilagođeni za AI
- **Praćenje zdravlja**: Provjera povezanosti baze podataka i statusa poola

## 🔧 Glavna implementacija MCP poslužitelja

### FastMCP poslužitelj (`sales_analysis.py`)

Sada implementirajmo glavni MCP poslužitelj:
...
### Ključne značajke MCP poslužitelja

- **Registracija alata**: Deklarativne definicije alata s sigurnošću tipova
- **Upravljanje kontekstom RLS-a**: Automatsko izdvajanje identiteta korisnika i postavljanje konteksta
- **Rukovanje greškama**: Sveobuhvatno upravljanje greškama s korisnički prilagođenim porukama
- **Upravljanje životnim ciklusom**: Pravilno pokretanje/zaustavljanje s čišćenjem resursa
- **Praćenje zdravlja**: Ugrađene krajnje točke za provjeru zdravlja
- **Podrška za razvoj**: Hot reload i mogućnosti debugiranja

## 🏥 Praćenje zdravlja

### Implementacija provjere zdravlja (`health_check.py`)
...
2. **Testiraj krajnje točke za zdravlje**:
   ```bash
   # Basic health check
...
### Testiranje integracije u VS Code-u

1. **Konfiguriraj VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Testiraj u AI Chat-u**:
   - Otvori VS Code AI Chat
   - Upitaj: "Koje tablice su dostupne?"
   - Upitaj: "Pokaži mi top 5 trgovina po broju narudžbi"

### Jedinično testiranje

Kreiraj sveobuhvatne jedinične testove:
...
Nakon završetka ovog laboratorija, trebali biste imati:

✅ **Radni MCP poslužitelj**: FastMCP poslužitelj s integracijom baze podataka  
✅ **Upravljanje konfiguracijom**: Robusna konfiguracija temeljena na okruženju  
✅ **Sloj baze podataka**: Integracija PostgreSQL-a s pooliranjem konekcija  
✅ **MCP alati**: Alati za introspekciju sheme i izvršavanje upita  
✅ **Integracija RLS-a**: Upravljanje kontekstom sigurnosti na razini redaka  
✅ **Praćenje zdravlja**: Sveobuhvatne krajnje točke za provjeru zdravlja  
✅ **Strategija testiranja**: Lokalno testiranje i integracija u VS Code-u  

## 🚀 Što dalje

Nastavite s **[Lab 06: Razvoj alata](../06-Tools/README.md)** kako biste:

- Proširili kolekciju MCP alata
- Implementirali napredne obrasce upita
- Dodali validaciju i transformaciju podataka
- Kreirali specijalizirane analitičke alate

## 📚 Dodatni resursi

### FastMCP okvir
- [FastMCP Dokumentacija](https://github.com/modelcontextprotocol/python-sdk) - Službeni vodič za FastMCP
- [MCP Specifikacija](https://modelcontextprotocol.io/docs/) - Specifikacija protokola
- [Vodič za razvoj alata](https://modelcontextprotocol.io/docs/tools/) - Kreiranje MCP alata

### Integracija baze podataka
- [asyncpg Dokumentacija](https://magicstack.github.io/asyncpg/current/) - PostgreSQL async driver
- [Najbolje prakse za pooliranje konekcija](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Podešavanje PostgreSQL-a
- [Vodič za sigurnost na razini redaka](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementacija RLS-a

### FastAPI obrasci
- [FastAPI Dokumentacija](https://fastapi.tiangolo.com/) - Referenca za web okvir
- [Injekcija ovisnosti](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI obrasci
- [Pozadinski zadaci](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Upravljanje asinhronim zadacima

---

**Dalje**: Spremni za proširenje alata? Nastavite s [Lab 06: Razvoj alata](../06-Tools/README.md)
