...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T13:45:29+00:00",
...
To praktyczne laboratorium przeprowadzi Cię przez proces tworzenia gotowego do produkcji serwera MCP przy użyciu frameworka FastMCP. Zbudujesz podstawową strukturę serwera, zaimplementujesz integrację z bazą danych, stworzysz narzędzia do dostępu do danych i ustanowisz fundamenty dla analityki detalicznej wspieranej przez AI.

## Przegląd

Serwer MCP jest sercem naszego rozwiązania analitycznego dla handlu detalicznego. Działa jako pomost między asystentami AI a bazą danych PostgreSQL, zapewniając bezpieczny i inteligentny dostęp do danych biznesowych za pomocą ustandaryzowanego protokołu.

To laboratorium nauczy Cię, jak zbudować solidny, skalowalny serwer MCP, stosując wzorce i najlepsze praktyki na poziomie przedsiębiorstwa.

## Cele nauki

Po ukończeniu tego laboratorium będziesz w stanie:

- **Zbudować** serwer FastMCP z odpowiednią architekturą i organizacją
- **Zaimplementować** integrację z bazą danych z użyciem puli połączeń i obsługi błędów
- **Stworzyć** narzędzia MCP do introspekcji schematu bazy danych i wykonywania zapytań
- **Skonfigurować** zarządzanie kontekstem Row Level Security
- **Dodać** funkcje monitorowania stanu i obserwowalności
- **Przetestować** implementację serwera MCP lokalnie i w VS Code

## 📁 Struktura projektu

Przyjrzyjmy się organizacji serwera MCP:
...
### Konfiguracja środowiska (`config.py`)

Najpierw stwórzmy solidny system konfiguracji:
...
### Kluczowe cechy konfiguracji

- **Ładowanie zmiennych środowiskowych**: Automatyczne wsparcie dla plików .env
- **Bezpieczeństwo typów**: Walidacja za pomocą dataclass i wskazówki typów
- **Elastyczne wartości domyślne**: Sensowne wartości domyślne dla środowiska deweloperskiego
- **Walidacja**: Walidacja konfiguracji z pomocnymi komunikatami o błędach
- **Bezpieczeństwo**: Wartości wrażliwe tylko ze zmiennych środowiskowych

## 🗄️ Warstwa integracji z bazą danych

### Dostawca PostgreSQL (`sales_analysis_postgres.py`)

Zaimplementujmy warstwę integracji z bazą danych:
...
### Kluczowe cechy warstwy bazy danych

- **Pula połączeń**: Efektywne zarządzanie zasobami z asyncpg
- **Integracja RLS**: Automatyczne ustawianie kontekstu Row Level Security
- **Introspekcja schematu**: Dynamiczne odkrywanie schematu tabel
- **Obsługa błędów**: Kompleksowe zarządzanie błędami i logowanie
- **Formatowanie zapytań**: Formatowanie wyników przyjazne dla AI
- **Monitorowanie stanu**: Sprawdzanie łączności z bazą danych i statusu puli

## 🔧 Główna implementacja serwera MCP

### Serwer FastMCP (`sales_analysis.py`)

Teraz zaimplementujmy główny serwer MCP:
...
### Kluczowe cechy serwera MCP

- **Rejestracja narzędzi**: Deklaratywne definicje narzędzi z bezpieczeństwem typów
- **Zarządzanie kontekstem RLS**: Automatyczne wyodrębnianie tożsamości użytkownika i ustawianie kontekstu
- **Obsługa błędów**: Kompleksowe zarządzanie błędami z przyjaznymi komunikatami
- **Zarządzanie cyklem życia**: Odpowiednie uruchamianie/zamykanie z czyszczeniem zasobów
- **Monitorowanie stanu**: Wbudowane punkty końcowe sprawdzania stanu
- **Wsparcie dla deweloperów**: Funkcje hot reload i debugowania

## 🏥 Monitorowanie stanu

### Implementacja sprawdzania stanu (`health_check.py`)
...
2. **Przetestuj punkty końcowe sprawdzania stanu**:
   ```bash
   # Basic health check
...
### Testowanie integracji w VS Code

1. **Skonfiguruj VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Testuj w AI Chat**:
   ...
   - Wpisz `#zava` i wybierz swój serwer
   - Zapytaj: "Jakie tabele są dostępne?"
   - Zapytaj: "Pokaż mi 5 najlepszych sklepów według liczby zamówień"

### Testy jednostkowe

Stwórz kompleksowe testy jednostkowe:
...
✅ **Działający serwer MCP**: Serwer FastMCP z integracją bazy danych  
✅ **Zarządzanie konfiguracją**: Solidna konfiguracja oparta na środowisku  
✅ **Warstwa bazy danych**: Integracja PostgreSQL z pulą połączeń  
✅ **Narzędzia MCP**: Narzędzia do introspekcji schematu i wykonywania zapytań  
✅ **Integracja RLS**: Zarządzanie kontekstem Row Level Security  
✅ **Monitorowanie stanu**: Kompleksowe punkty końcowe sprawdzania stanu  
✅ **Strategia testowania**: Testowanie lokalne i integracja z VS Code  

## 🚀 Co dalej

Kontynuuj z **[Lab 06: Tworzenie narzędzi](../06-Tools/README.md)**, aby:

- Rozszerzyć swoją kolekcję narzędzi MCP
- Zaimplementować zaawansowane wzorce zapytań
- Dodać walidację i transformację danych
- Stworzyć wyspecjalizowane narzędzia analityczne

## 📚 Dodatkowe zasoby

### Framework FastMCP
- [Dokumentacja FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Oficjalny przewodnik FastMCP
- [Specyfikacja MCP](https://modelcontextprotocol.io/docs/) - Specyfikacja protokołu
- [Przewodnik po tworzeniu narzędzi](https://modelcontextprotocol.io/docs/tools/) - Tworzenie narzędzi MCP

### Integracja z bazą danych
- [Dokumentacja asyncpg](https://magicstack.github.io/asyncpg/current/) - Asynchroniczny sterownik PostgreSQL
- [Najlepsze praktyki dotyczące puli połączeń](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Optymalizacja PostgreSQL
- [Przewodnik po Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementacja RLS

### Wzorce FastAPI
- [Dokumentacja FastAPI](https://fastapi.tiangolo.com/) - Odniesienie do frameworka
- [Wstrzykiwanie zależności](https://fastapi.tiangolo.com/tutorial/dependencies/) - Wzorce FastAPI
- [Zadania w tle](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Zarządzanie zadaniami asynchronicznymi

---

**Dalej**: Gotowy na rozszerzenie narzędzi? Kontynuuj z [Lab 06: Tworzenie narzędzi](../06-Tools/README.md)
