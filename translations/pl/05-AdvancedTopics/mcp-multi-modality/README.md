...
MCP wspiera nie tylko interakcje oparte na tekście, ale także możliwości wielomodalne, pozwalając modelom pracować z obrazami, dźwiękiem i innymi typami danych.

## Wprowadzenie

W tej lekcji nauczysz się, jak zbudować aplikację wielomodalną.

## Cele nauki

Po ukończeniu tej lekcji będziesz potrafił:

- Zrozumieć wybory wielomodalne
- Zaimplementować aplikację wielomodalną.

## Architektura wsparcia wielomodalnego

Implementacje MCP wielomodalnego zazwyczaj obejmują:

- **Parsery specyficzne dla modalności**: Komponenty, które konwertują różne typy mediów na formaty, które model może przetwarzać.
- **Narzędzia specyficzne dla modalności**: Specjalne narzędzia zaprojektowane do obsługi konkretnych modalności (analiza obrazów, przetwarzanie dźwięku)
- **Zunifikowane zarządzanie kontekstem**: System utrzymujący kontekst pomiędzy różnymi modalnościami
- **Generowanie odpowiedzi**: Możliwość tworzenia odpowiedzi, które mogą zawierać wiele modalności.

## Przykład wielomodalny: analiza obrazu

W poniższym przykładzie przeanalizujemy obraz i wyodrębnimy z niego informacje.
...
- Stworzyliśmy `ImageAnalysisTool`, który potrafi analizować obrazy za pomocą hipotetycznej usługi `IImageAnalysisService`.
- Skonfigurowaliśmy serwer MCP do obsługi większych żądań i wsparcia typów zawartości obrazów.
- Zarejestrowaliśmy narzędzie do analizy obrazów na serwerze.
- Zaimplementowaliśmy metodę pobierania obrazów z URL i analizowania ich w zależności od żądanego typu (obiekty, tekst, twarze itp.).
- Zwróciliśmy uporządkowane wyniki w formacie zgodnym ze specyfikacją MCP.

## Przykład wielomodalny: przetwarzanie dźwięku

Przetwarzanie dźwięku to kolejna popularna modalność w aplikacjach wielomodalnych. Poniżej znajduje się przykład implementacji narzędzia do transkrypcji dźwięku, które potrafi obsługiwać pliki audio i zwracać transkrypcje.
...
- Stworzyliśmy `AudioTranscriptionTool`, które potrafi transkrybować pliki audio.
- Zdefiniowaliśmy schemat narzędzia, aby akceptowało URL lub dane audio zakodowane w base64.
- Zaimplementowaliśmy metodę `execute` do obsługi przetwarzania dźwięku i transkrypcji.
- Skonfigurowaliśmy serwer MCP do obsługi żądań wielomodalnych, w tym przetwarzania dźwięku i obrazów.
- Zarejestrowaliśmy narzędzie do transkrypcji audio na serwerze.
- Zaimplementowaliśmy metodę pobierania plików audio z URL lub dekodowania danych audio w base64.
- Użyliśmy usługi `AudioProcessor` do obsługi logiki transkrypcji.
- Uruchomiliśmy serwer MCP, aby nasłuchiwał żądań.

### Przykład wielomodalny: generowanie odpowiedzi wielomodalnych
