...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T15:27:12+00:00",
...
生成式 AI 应用是一个重要的进步，因为它们通常允许用户通过自然语言提示与应用互动。然而，随着在此类应用上投入的时间和资源增加，你需要确保能够轻松集成功能和资源，使其易于扩展，支持多模型共存，并处理各种模型的复杂性。简而言之，构建生成式 AI 应用起步容易，但随着应用规模和复杂度的增长，你需要开始定义架构，并很可能依赖一个标准来确保应用的一致性。这正是 MCP 发挥作用、提供标准的地方。
...
除了提供工具，MCP 还促进了知识的访问。它使应用能够通过连接各种数据源，为大型语言模型（LLM）提供上下文。例如，某个 MCP 服务器可能代表公司的文档库，允许代理按需检索相关信息。另一个服务器可能处理特定操作，如发送邮件或更新记录。对代理来说，这些都是它可以使用的工具——有些工具返回数据（知识上下文），有些执行操作。MCP 高效管理这两者。

代理连接到 MCP 服务器后，会通过标准格式自动了解服务器的可用功能和可访问数据。这种标准化支持动态工具的可用性。例如，向代理系统添加新的 MCP 服务器后，其功能即可立即使用，无需进一步定制代理指令。
...
除了基础 MCP 架构外，还有更高级的场景，其中客户端和服务器都包含 LLM，实现更复杂的交互：
