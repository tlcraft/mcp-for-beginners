...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-20T15:58:48+00:00",
...
**模型上下文协议（MCP）** 是一个前沿框架，旨在规范 AI 模型与客户端应用之间的交互。这个开源课程提供了结构化的学习路径，配有实用的代码示例和真实案例，涵盖 C#、Java、JavaScript、TypeScript 和 Python 等主流编程语言。
...
| 章节 | 标题 | 描述 | 链接 |
|--|--|--|--|
| 00 | **MCP 介绍** | 模型上下文协议及其在 AI 流水线中的重要性概述，包括什么是模型上下文协议、为何标准化至关重要，以及实际应用和优势 | [介绍](./00-Introduction/README.md) |
| 01 | **核心概念解析** | 深入探讨 MCP 的核心概念，包括客户端-服务器架构、关键协议组件和消息传递模式 | [核心概念](./01-CoreConcepts/README.md) |
| 02 | **MCP 中的安全性** | 识别基于 MCP 系统中的安全威胁，安全实现的技术和最佳实践 | [安全](./02-Security/readme.md) |
...
| 3.3 | **带有大型语言模型的客户端** | 使用 MCP 协议结合大型语言模型（LLM）搭建客户端 | [带 LLM 的客户端](./03-GettingStarted/03-llm-client/README.md) |
| 3.4 | **用 Visual Studio Code 访问服务器** | 配置 Visual Studio Code 以使用 MCP 协议访问服务器 | [用 VS Code 访问服务器](./03-GettingStarted/04-vscode/README.md) |
| 3.5 | **使用 SSE 创建服务器** | SSE 让我们能将服务器暴露到互联网，本节将帮助你使用 SSE 创建服务器 | [使用 SSE 创建服务器](./03-GettingStarted/05-sse-server/README.md) |
| 3.6 | **使用 AI 工具包** | AI 工具包是管理 AI 和 MCP 工作流的好帮手 | [使用 AI 工具包](./03-GettingStarted/06-aitk/README.md) |
| 3.7 | **测试你的服务器** | 测试是开发过程中重要的一环，本节将帮助你使用多种工具进行测试 | [测试你的服务器](./03-GettingStarted/07-testing/README.md) |
...
| 05 | **MCP 高级主题** | 多模态 AI 工作流与扩展性，安全的扩展策略，企业生态中的 MCP 应用 | [高级主题](./05-AdvancedTopics/README.md) |
| 06 | **社区贡献** | 如何贡献代码和文档，通过 GitHub 协作，社区驱动的改进和反馈 | [社区贡献](./06-CommunityContributions/README.md) |
| 07 | **早期采用的经验分享** | 真实案例及成功经验，构建和部署基于 MCP 的解决方案，趋势与未来路线图 | [经验分享](./07-LessonsFromEarlyAdoption/README.md) |
| 08 | **MCP 最佳实践** | 性能调优与优化，设计容错的 MCP 系统，测试与韧性策略 | [最佳实践](./08-BestPractices/README.md) |
| 09 | **MCP 案例研究** | 深入 MCP 解决方案架构，部署蓝图与集成技巧，带注释的图表和项目演示 | [案例研究](./09-CaseStudy/README.md) |
...
本内容采用 **MIT 许可证** 进行授权。有关条款和条件，请参见 [LICENSE](../../LICENSE)。

## 🤝 贡献指南

本项目欢迎贡献和建议。大多数贡献需要你同意一份贡献者许可协议（CLA），声明你拥有权利且确实授予我们使用你贡献内容的权利。详情请访问 <https://cla.opensource.microsoft.com>。

当你提交拉取请求时，CLA 机器人会自动判断你是否需要提供 CLA，并对 PR 进行相应标记（例如状态检查、评论）。只需按照机器人提供的指示操作即可。所有使用我们 CLA 的仓库，你只需完成一次此流程。

本项目采用了 [Microsoft 开源行为准则](https://opensource.microsoft.com/codeofconduct/)。更多信息请参见 [行为准则常见问题](https://opensource.microsoft.com/codeofconduct/faq/)，或通过 [opencode@microsoft.com](mailto:opencode@microsoft.com) 联系我们，提出任何额外的问题或意见。
...
在本项目的修改版本中使用微软商标或标识，不得引起混淆或暗示微软的赞助。
任何第三方商标或标识的使用均需遵守相应第三方的政策。

**免责声明**：  
本文件使用AI翻译服务[Co-op Translator](https://github.com/Azure/co-op-translator)进行翻译。尽管我们力求准确，但请注意，自动翻译可能存在错误或不准确之处。原始文件的原文版本应被视为权威来源。对于重要信息，建议使用专业人工翻译。因使用本翻译而产生的任何误解或误读，我们概不负责。