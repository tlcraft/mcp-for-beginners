...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-19T21:33:51+00:00",
...
**模型上下文协议（MCP）** 是一个前沿框架，旨在标准化 AI 模型与客户端应用之间的交互。这个开源课程提供了结构化的学习路径，包含实用的代码示例和真实案例，涵盖 C#、Java、JavaScript、TypeScript 和 Python 等主流编程语言。
...
| 章节 | 标题 | 说明 | 链接 |
|--|--|--|--|
| 00 | **MCP 介绍** | 介绍模型上下文协议及其在 AI 流水线中的重要性，包括什么是 MCP、为何标准化重要，以及实际应用场景和优势 | [介绍](./00-Introduction/README.md) |
...
| 03 | **MCP 入门** | 环境搭建与配置，创建基础的 MCP 服务器和客户端，MCP 与现有应用的集成 | [入门](./03-GettingStarted/README.md) |
...
| 3.4 | **使用 Visual Studio Code 连接服务器** | 配置 Visual Studio Code 以通过 MCP 协议连接服务器 | [使用 Visual Studio Code 连接服务器](./03-GettingStarted/04-vscode/README.md) |
| 3.5 | **使用 SSE 创建服务器** | SSE 帮助我们将服务器暴露到互联网，本节将指导你使用 SSE 创建服务器 | [使用 SSE 创建服务器](./03-GettingStarted/05-sse-server/README.md) |
| 3.6 | **使用 AI 工具包** | AI 工具包是一个帮助你管理 AI 和 MCP 工作流的优秀工具 | [使用 AI 工具包](./03-GettingStarted/06-aitk/README.md) |
| 3.7 | **测试你的服务器** | 测试是开发过程中的重要环节，本节将帮助你使用多种工具进行测试 | [测试你的服务器](./03-GettingStarted/07-testing/README.md) |
| 3.8 | **部署你的服务器** | 如何将本地开发的服务器推向生产环境？本节将指导你开发与部署服务器 | [部署你的服务器](./03-GettingStarted/08-deployment/README.md) |
...
| 08 | **MCP 最佳实践** | 性能调优与优化，设计容错的 MCP 系统，测试与弹性策略 | [最佳实践](./08-BestPractices/README.md) |
| 09 | **MCP 案例研究** | 深入解析 MCP 解决方案架构、部署方案与集成技巧，带注释的图示和项目演示 | [案例研究](./09-CaseStudy/README.md) |
...
本项目欢迎贡献和建议。大多数贡献需要您同意一份贡献者许可协议（CLA），声明您拥有并确实授予我们使用您贡献内容的权利。详情请访问 <https://cla.opensource.microsoft.com>。

当您提交拉取请求时，CLA 机器人会自动判断您是否需要提供 CLA，并对 PR 进行相应标注（例如状态检查、评论）。只需按照机器人提供的指引操作即可。您在所有使用我们 CLA 的仓库中只需完成一次此流程。
...
本项目可能包含项目、产品或服务的商标或徽标。微软商标或徽标的授权使用必须遵循并符合  
...
任何第三方商标或徽标的使用均需遵守相应第三方的政策。

**免责声明**：  
本文件由 AI 翻译服务 [Co-op Translator](https://github.com/Azure/co-op-translator) 翻译而成。虽然我们力求准确，但请注意自动翻译可能存在错误或不准确之处。原始文件的母语版本应被视为权威来源。对于重要信息，建议采用专业人工翻译。因使用本翻译所产生的任何误解或误释，我们概不负责。