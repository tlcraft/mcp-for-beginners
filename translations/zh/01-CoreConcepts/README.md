...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T15:48:18+00:00",
...
[Model Context Protocol (MCP)](https://github.com/modelcontextprotocol) 是一个强大且标准化的框架，优化了大型语言模型（LLM）与外部工具、应用程序和数据源之间的通信。本文是一个经过 SEO 优化的指南，将带你深入了解 MCP 的核心概念，确保你理解其客户端-服务器架构、关键组件、通信机制及最佳实现实践。
...
- **本地数据源**：计算机上的文件、数据库和服务，MCP 服务器可以安全访问
...
在模型上下文协议（MCP）中，Hosts 扮演着关键角色，是用户与协议交互的主要界面。Hosts 是启动与 MCP 服务器连接以访问数据、工具和提示的应用或环境。Hosts 的例子包括集成开发环境（IDE）如 Visual Studio Code、AI 工具如 Claude Desktop，或为特定任务定制的代理程序。
...
### 🤖 提示
模型上下文协议（MCP）中的提示包括各种预定义模板和交互模式，旨在简化用户工作流程并增强沟通效果，包括：
...
模型上下文协议（MCP）中的工具是 AI 模型可执行的函数，用于完成特定任务。这些工具旨在通过提供结构化且可靠的操作来增强 AI 模型的能力。关键点包括：

- **供 AI 模型执行的函数**：工具是可执行函数，AI 模型可调用以完成各种任务。
...
- **资源或工具使用**  
  ...
  - 如果模型判断需要工具（如获取数据、执行计算或调用 API），客户端向服务器发送工具调用请求，指定工具名称和参数。
...
MCP（模型上下文协议）基于[JSON-RPC 2.0](https://www.jsonrpc.org/)构建，提供标准化、语言无关的消息格式，用于 Hosts、Clients 和 Servers 之间的通信。该基础确保跨多平台和编程语言的可靠、结构化且可扩展的交互。
...
以下是 MCP 提供的一些额外实用功能和协议扩展，旨在提升开发者体验并支持高级场景：
...
- **用户同意与控制**：在访问任何数据或执行操作前，必须获得用户明确同意。用户应清晰掌控共享数据和授权操作的范围，并通过直观界面审查和批准活动。
- **数据隐私**：用户数据仅在明确同意下暴露，且必须通过适当访问控制保护。MCP 实现需防止未经授权的数据传输，确保隐私在所有交互中得到维护。
...
4. **速率限制**：  
  为防止滥用并确保服务器资源的公平使用，MCP 服务器可以对工具调用和资源访问实施速率限制。速率限制可以按用户、会话或全局应用，有助于防范拒绝服务攻击或过度资源消耗。

通过结合这些机制，MCP 为将语言模型与外部工具和数据源集成提供了安全基础，同时赋予用户和开发者对访问和使用的细粒度控制。
