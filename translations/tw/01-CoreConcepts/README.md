...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-04T16:08:14+00:00",
...
[Model Context Protocol (MCP)](https://github.com/modelcontextprotocol) 是一個強大且標準化的框架，優化大型語言模型（LLMs）與外部工具、應用程式及資料來源之間的溝通。這份經過 SEO 優化的指南將帶你深入了解 MCP 的核心概念，確保你掌握其客戶端-伺服器架構、關鍵組件、通訊機制及實作最佳實踐。

## 概覽

本課程探討構成 Model Context Protocol (MCP) 生態系統的基本架構與組件。你將學習 MCP 的客戶端-伺服器架構、主要組件以及驅動 MCP 互動的通訊機制。
...
- **遠端服務**：透過 API 連接的網際網路外部系統，MCP 伺服器可連線使用

MCP 協議是一個持續演進的標準，你可以在[協議規範](https://modelcontextprotocol.io/specification/2025-06-18/)查看最新更新。

### 1. Hosts

在 Model Context Protocol (MCP) 中，Hosts 扮演使用者與協議互動的主要介面角色。Hosts 是啟動與 MCP 伺服器連線以存取資料、工具和提示的應用程式或環境。Hosts 範例包括整合開發環境（IDE）如 Visual Studio Code、AI 工具如 Claude Desktop，或為特定任務打造的自訂代理程式。
...
Clients 是促進 Hosts 與 MCP 伺服器間互動的重要組件。Clients 作為中介，讓 Hosts 能存取並利用 MCP 伺服器提供的功能。它們在確保 MCP 架構內通訊順暢與資料交換效率上扮演關鍵角色。
...
Servers 負責處理 MCP clients 的請求並提供適當回應。它們管理資料擷取、工具執行及提示產生等多種操作。Servers 確保客戶端與 Hosts 之間的通訊高效且可靠，維護互動過程的完整性。
...
- **專門化對話範本**：為特定對話類型量身訂做的可自訂範本，確保互動具相關性且符合上下文。
...
## 客戶端功能
在 Model Context Protocol (MCP) 中，客戶端向伺服器提供多項關鍵功能，提升協議整體功能與互動性。其中一項重要功能是取樣（Sampling）。
...
- **功能協商**  
  嵌入 Host 的客戶端與伺服器交換彼此支援的功能、工具、資源及協議版本資訊，確保雙方了解本次會話可用的能力。
...
MCP（Model Context Protocol）建立於 [JSON-RPC 2.0](https://www.jsonrpc.org/) 之上，提供標準化、語言無關的訊息格式，用於 Hosts、Clients 與 Servers 間的通訊。此基礎確保跨多元平台與程式語言的可靠、結構化且可擴充互動。

### 主要協議特性

MCP 擴充 JSON-RPC 2.0，加入工具調用、資源存取與提示管理的額外約定。支援多種傳輸層（STDIO、WebSocket、SSE），並實現安全、可擴充且語言無關的元件間通訊。
...
- **工具安全**：調用任何工具前需取得明確使用者同意。使用者應充分了解每個工具的功能，並強制執行嚴格安全邊界，防止非預期或不安全的工具執行。
...
1. **工具權限控制**：  
  用戶端可以指定模型在會話期間允許使用哪些工具。這確保只有明確授權的工具能被存取，降低意外或不安全操作的風險。權限可根據使用者偏好、組織政策或互動情境動態設定。

2. **身份驗證**：  
  伺服器可要求在授予工具、資源或敏感操作的存取權前進行身份驗證。這可能涉及 API 金鑰、OAuth 令牌或其他驗證機制。適當的身份驗證確保只有受信任的用戶端與使用者能調用伺服器端功能。

3. **驗證**：  
  所有工具調用皆強制參數驗證。每個工具定義其參數的預期類型、格式與限制，伺服器會依此驗證傳入請求。此舉防止格式錯誤或惡意輸入進入工具實作，維護操作的完整性。

4. **速率限制**：  
  為防止濫用並確保伺服器資源公平使用，MCP 伺服器可對工具呼叫與資源存取實施速率限制。速率限制可依使用者、會話或全域設定，有助抵禦拒絕服務攻擊或過度資源消耗。
...
這些結構化訊息確保 MCP 工作流程的每個步驟都明確、可追蹤且具擴充性，支持多輪對話、工具串接及健全的錯誤處理等進階場景。
