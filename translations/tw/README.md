...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-15T18:32:46+00:00",
...
**模型上下文協議（MCP）** 是一個前沿框架，旨在標準化 AI 模型與客戶端應用程式之間的互動。這份開源課程提供了結構化的學習路徑，包含實際的程式碼範例和真實世界的使用案例，涵蓋 C#、Java、JavaScript、TypeScript 和 Python 等流行程式語言。
...
此存儲庫包含多種資源，幫助你有效學習和探索：

### 學習指南

一份全面的 [學習指南](./study_guide.md) 可幫助你有效地瀏覽此存儲庫。指南內容包括：
...
準備好迎接兩天深入的技術洞察、社群連結和實作學習，MCP 開發日是一個專注於模型上下文協議（MCP）的虛擬活動——這是一個連接 AI 模型與其依賴工具的新興標準。
你可以透過註冊活動頁面觀看 MCP 開發日：https://aka.ms/mcpdevdays。

#### [第一天：MCP 生產力、開發工具與社群](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

第一天的重點是幫助開發者在開發工作流程中使用 MCP，並慶祝 MCP 社群的卓越成就。我們將與社群成員和合作夥伴（如 Arcade、Block、Okta 和 Neon）一起探討他們如何與 Microsoft 合作，共同打造開放且可擴展的 MCP 生態系統。  
實際範例展示 VS Code、Visual Studio、GitHub Copilot 和流行社群工具  
實用的、以上下文為導向的開發工作流程  
社群主導的會議和洞察  
無論你是剛開始接觸 MCP 還是已經在使用它，第一天將提供啟發和可行的收穫。

#### [第二天：自信地構建 MCP 伺服器](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

第二天適合 MCP 建設者。我們將深入探討構建 MCP 伺服器的實作策略和最佳實踐，以及如何將 MCP 整合到你的 AI 工作流程中。
...
### MCP 訓練營 2025 年 8 月
透過密集的視頻課程學習如何構建 MCP 伺服器、整合 VS Code 並基於 MCP 初學者課程內容在 Azure 上專業部署。掌握一項主要公司已經在使用的技術的實用技能。
...
### 一起學習 MCP 與 C# - 教學系列
一起學習模型上下文協議（MCP），這是一個旨在標準化 AI 模型與客戶端應用之間交互的尖端框架。透過這個適合初學者的課程，我們將向你介紹 MCP 並指導你建立第一個 MCP 伺服器。
