...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-02T09:02:57+00:00",
...
**Model Context Protocol (MCP)** 是一套先進的框架，旨在標準化 AI 模型與客戶端應用程式之間的互動。這份開源課程提供結構化的學習路徑，包含實務程式碼範例與真實案例，涵蓋 C#、Java、JavaScript、TypeScript 和 Python 等主流程式語言。
...
## 歡迎參加 2025 年 7 月 29-30 日 MCP 開發者日

準備好參加為期兩天的深度技術分享、社群交流與實作學習，MCP 開發者日是一場專注於 Model Context Protocol (MCP) 的線上活動 — 這個新興標準正連結 AI 模型與其所依賴的工具。

➡️ [註冊 MCP 開發者日](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

你可以透過活動頁面註冊觀看 MCP 開發者日：https://aka.ms/mcpdevdays。之後即可透過 YouTube 或 Twitch 直播觀看。所有內容均會錄製並於 Microsoft Developer YouTube 頻道上線。示範程式碼也會在 GitHub 上提供。

### 活動資訊
- 日期：7 月 29 日（第一天）及 7 月 30 日（第二天）
- 時間：每日上午 9:00（太平洋標準時間）
- 地點：線上，隨時隨地參加！

#### 第一天：MCP 生產力、開發工具與社群

專注於幫助開發者將 MCP 融入開發流程，並慶祝活躍的 MCP 社群。我們將邀請 Arcade、Block、Okta 和 Neon 等社群成員與合作夥伴，分享他們如何與 Microsoft 合作，打造開放且可擴充的 MCP 生態系。內容涵蓋 VS Code、Visual Studio、GitHub Copilot 及熱門社群工具的實際示範，實務且情境導向的開發流程，以及社群主導的分享與見解。

無論你是 MCP 新手還是已有實作經驗，第一天都會帶給你啟發與具體可行的收穫。

#### 第二天：自信打造 MCP 伺服器

專為 MCP 開發者設計，深入探討 MCP 伺服器的實作策略與最佳實務，以及如何將 MCP 整合進 AI 工作流程。
...
- 以提示為驅動的開發
- 安全最佳實務
- 使用 Functions、ACA 和 API Management 等建構模組
- 登錄對齊與工具（第一方與第三方）

如果你是開發者、工具建置者或 AI 產品策略師，這一天滿載你打造可擴展、安全且具未來性的 MCP 解決方案所需的洞見。

## 🧭 MCP 課程大綱

### Model Context Protocol 基礎  
<details>
  <summary><strong> 課程 1-3：Model Context Protocol 基礎</strong></summary>
...
- **03. MCP 入門**  
  環境設定、基礎伺服器/客戶端與整合。[閱讀更多](./03-GettingStarted/README.md)
</details>

### 建置與部署你的第一個 MCP 伺服器與客戶端，以及實作練習與場景  
<details>
  <summary><strong> 課程 3：建置與部署你的第一個 MCP 伺服器與客戶端</strong></summary>
...
<details>
  <summary><strong> 課程 4-5：實務與進階</strong></summary>

- **04. 實務應用**  
  SDK、除錯、測試、可重用的提示範本。[閱讀更多](./04-PracticalImplementation/README.md)
- **05. MCP 進階議題**  
...
- **5.11. 即時網路搜尋** – [指南](./05-AdvancedTopics/mcp-realtimesearch/README.md)
</details>
...
<details>
  <summary><strong>第 6-9 課：社群、最佳實踐與實驗室</strong></summary>

- **06. 社群貢獻** – [指南](./06-CommunityContributions/README.md)
- **07. 早期採用經驗分享** – [指南](./07-LessonsFromEarlyAdoption/README.md)
- **08. MCP 最佳實踐** – [指南](./08-BestPractices/README.md)
- **09. MCP 案例研究** – [指南](./09-CaseStudy/README.md)
</details>

### Model Context Protocol 與 VScode AI 工具包實作實驗室  
<details>
  <summary><strong>第 10 課：使用 VScode AI 工具包打造 MCP 伺服器實作實驗室</strong></summary>
    
- **10. 精簡 AI 工作流程：使用 AI 工具包建構 MCP 伺服器** – [實作實驗室](./10-StreamliningAIWorkflowsBuildingAnMCPServerWithAIToolkit/README.md)
</details>

## Model Context Protocol 範例專案：使用 Java、C#、JavaScript、TypeScript 與 Python 建立 MCP 計算機專案

### 🧮 MCP 計算機範例專案（Java、C#、JavaScript、TypeScript 與 Python）  
<details>
  <summary><strong>依語言瀏覽程式碼實作</strong></summary>
...
### 💡 MCP 進階範例方案：C#、Java、JavaScript、TypeScript 與 Python 計算機專案  
<details>
...
我們提供了一份完整的[學習指南](./study_guide.md)，協助你有效掌握本資源庫。指南內容包括：
...
本內容採用 **MIT License** 授權。相關條款請參閱 [LICENSE](../../LICENSE)。

## 🤝 貢獻指南

歡迎您對本專案提出貢獻與建議。大部分貢獻需您同意一份
Contributor License Agreement (CLA)，聲明您有權利且確實授權我們
使用您的貢獻。詳細資訊請參閱 <https://cla.opensource.microsoft.com>。

當您提交 pull request 時，CLA 機器人會自動判斷您是否需要簽署
CLA，並在 PR 上標註相應狀態（例如狀態檢查、留言）。請依照機器人指示操作。
您只需在所有使用本 CLA 的資源庫中執行一次。

本專案已採用 [Microsoft Open Source Code of Conduct](https://opensource.microsoft.com/codeofconduct/)。
欲了解更多，請參考 [Code of Conduct FAQ](https://opensource.microsoft.com/codeofconduct/faq/) 或
...
- [初學者的資安](https://github.com/microsoft/Security-101??WT.mc_id=academic-96948-sayoung)
...
- [精通 GitHub Copilot 的 C#/.NET 開發者](https://github.com/microsoft/mastering-github-copilot-for-dotnet-csharp-developers?WT.mc_id=academic-105485-koreyst)
- [選擇你自己的 Copilot 冒險](https://github.com/microsoft/CopilotAdventures?WT.mc_id=academic-105485-koreyst)


## ™️ 商標聲明

本專案可能包含專案、產品或服務的商標或標誌。微軟商標或標誌的授權使用須遵守並依據
[Microsoft 的商標與品牌指引](https://www.microsoft.com/legal/intellectualproperty/trademarks/usage/general)。
在本專案修改版本中使用微軟商標或標誌時，不得引起混淆或暗示微軟贊助。
任何第三方商標或標誌的使用均須遵守該第三方的相關政策。

**免責聲明**：  
本文件係使用 AI 翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。雖然我們致力於確保翻譯的準確性，但請注意，自動翻譯可能包含錯誤或不準確之處。原始文件的母語版本應被視為權威來源。對於重要資訊，建議採用專業人工翻譯。我們不對因使用本翻譯而產生的任何誤解或誤譯負責。