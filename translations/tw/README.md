...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-29T21:25:01+00:00",
...
我們提供了一份完整的 [學習指南](./study_guide.md)，幫助您有效地瀏覽此存放庫。指南內容包括：
...
準備好迎接兩天深入的技術洞察、社群連結以及 MCP 開發日的實作學習，這是一個專注於模型上下文協議（MCP）的虛擬活動——這項新興標準連結了 AI 模型及其依賴的工具。
...
第一天的重點是幫助開發者在開發工作流程中使用 MCP，並慶祝 MCP 社群的卓越成就。我們將與社群成員及合作夥伴（如 Arcade、Block、Okta 和 Neon）一起探討他們如何與 Microsoft 合作，共同打造開放且可擴展的 MCP 生態系統。  
實際範例展示包括 VS Code、Visual Studio、GitHub Copilot 和流行的社群工具  
實用且以上下文為導向的開發工作流程  
社群主導的會議與洞察  
無論您是剛開始接觸 MCP 還是已經在使用它，第一天將提供啟發性內容和可行的建議。

#### [第二天：自信地構建 MCP 伺服器](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

第二天專為 MCP 建設者而設。我們將深入探討構建 MCP 伺服器的實作策略及最佳實踐，並將 MCP 整合到您的 AI 工作流程中。
...
### MCP 訓練營 2025 年 8 月
透過密集的影片課程學習如何建立 MCP 伺服器、整合 VS Code 並根據 MCP 初學者課程內容在 Azure 上專業部署。學習後您將掌握一項主要公司已在使用的技術的實用技能。
...
### 一起學習 MCP 與 C# - 教學系列
一起學習模型上下文協議（MCP），這是一個旨在標準化 AI 模型與客戶端應用程式之間互動的尖端框架。在這個適合初學者的課程中，我們將向您介紹 MCP，並指導您建立第一個 MCP 伺服器。
...
此專案歡迎貢獻與建議。大多數貢獻需要您同意一份
貢獻者授權協議（CLA），聲明您有權且確實授予我們使用您貢獻的權利。詳情請訪問 <https://cla.opensource.microsoft.com>。

當您提交拉取請求時，CLA 機器人會自動判斷您是否需要提供 CLA，並適當地標記 PR（例如，狀態檢查、評論）。只需按照機器人提供的指示操作即可。您只需在所有使用 CLA 的存放庫中執行一次。

此專案採用了 [Microsoft 開源行為準則](https://opensource.microsoft.com/codeofconduct/)。
欲了解更多資訊，請參閱 [行為準則 FAQ](https://opensource.microsoft.com/codeofconduct/faq/) 或
...
此專案可能包含專案、產品或服務的商標或標誌。使用 Microsoft 商標或標誌需遵守並必須符合 [Microsoft 的商標與品牌指南](https://www.microsoft.com/legal/intellectualproperty/trademarks/usage/general)。  
