...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T12:38:09+00:00",
...
這個實作型實驗將指導您使用 FastMCP 框架建立一個可用於生產環境的 MCP 伺服器。您將構建核心伺服器架構、實現資料庫整合、創建資料存取工具，並為 AI 驅動的零售分析奠定基礎。

## 概述

MCP 伺服器是我們零售分析解決方案的核心。它充當 AI 助手與 PostgreSQL 資料庫之間的橋樑，通過標準化協議提供安全且智能的業務資料存取。

本實驗教您如何遵循企業模式和最佳實踐，構建一個穩健且可擴展的 MCP 伺服器。

## 學習目標

完成本實驗後，您將能夠：

- **構建** 一個具有良好架構和組織的 FastMCP 伺服器  
- **實現** 資料庫整合，包括連線池和錯誤處理  
- **創建** MCP 工具，用於資料庫結構檢視和查詢執行  
- **配置** 行級安全性（Row Level Security）上下文管理  
- **新增** 健康監控和可觀察性功能  
- **測試** 您的 MCP 伺服器實作，無論是本地環境還是 VS Code  
...
### PostgreSQL 提供者 (`sales_analysis_postgres.py`)
...
- **工具註冊**：使用類型安全性進行宣告式工具定義  
- **RLS 上下文管理**：自動提取使用者身份並設置上下文  
- **錯誤處理**：全面的錯誤管理，提供使用者友好的訊息  
- **生命週期管理**：正確的啟動/關閉及資源清理  
- **健康監控**：內建健康檢查端點  
- **開發支援**：熱重載和除錯功能  

## 🏥 健康監控

### 健康檢查實作 (`health_check.py`)
...
✅ **可運行的 MCP 伺服器**：具有資料庫整合的 FastMCP 伺服器  
✅ **配置管理**：基於環境的穩健配置系統  
✅ **資料庫層**：具有連線池的 PostgreSQL 整合  
✅ **MCP 工具**：結構檢視和查詢執行工具  
✅ **RLS 整合**：行級安全性上下文管理  
✅ **健康監控**：全面的健康檢查端點  
✅ **測試策略**：本地測試和 VS Code 整合  

## 🚀 下一步

繼續進行 **[實驗 06：工具開發](../06-Tools/README.md)**：
...
### FastMCP 框架
- [FastMCP 文件](https://github.com/modelcontextprotocol/python-sdk) - 官方 FastMCP 指南  
- [MCP 規範](https://modelcontextprotocol.io/docs/) - 協議規範  
- [工具開發指南](https://modelcontextprotocol.io/docs/tools/) - 創建 MCP 工具  

### 資料庫整合
- [asyncpg 文件](https://magicstack.github.io/asyncpg/current/) - PostgreSQL 非同步驅動  
- [連線池最佳實踐](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL 調整  
- [行級安全性指南](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS 實作  

### FastAPI 模式
- [FastAPI 文件](https://fastapi.tiangolo.com/) - Web 框架參考  
- [依賴注入](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI 模式  
- [背景任務](https://fastapi.tiangolo.com/tutorial/background-tasks/) - 非同步任務管理  

---

**下一步**：準備好擴展您的工具了嗎？繼續進行 [實驗 06：工具開發](../06-Tools/README.md)
