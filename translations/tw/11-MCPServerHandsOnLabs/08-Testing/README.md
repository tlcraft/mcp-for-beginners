...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T12:40:38+00:00",
...
本實驗提供全面的指導，幫助您在生產環境中測試與除錯 MCP 伺服器。您將學習如何實施穩健的測試策略、除錯複雜問題，並確保 MCP 伺服器在各種條件下可靠運行。

## 概述

測試 MCP 伺服器需要多層次的方式，包括單元測試、整合測試、效能驗證以及真實場景測試。本實驗涵蓋了從開發到生產監控的完整測試生命週期。

我們的測試策略強調可靠性、安全性和效能，確保您的 MCP 伺服器能夠處理生產工作負載，同時維持資料完整性和使用者體驗品質。
...
- **實施** 全面的單元測試與整合測試套件  
- **設計** MCP 工具和資料庫操作的有效測試策略  
- **除錯** 使用高級除錯技術解決複雜問題  
- **驗證** 在負載下的效能，並進行真實場景測試  
- **監控** 使用有效的警示和可觀測性監控生產系統  
- **自動化** 測試工作流程以支持持續整合  
...
✅ **全面的測試框架**：涵蓋所有元件的單元測試、整合測試和效能測試  
✅ **高級除錯工具**：具備執行追蹤的高級除錯工具  
✅ **效能驗證**：負載測試與可擴展性分析能力  
✅ **安全性測試**：防止 SQL 注入並驗證 RLS  
✅ **監控整合**：效能指標與瓶頸分析  
...
- 設置 VS Code 中的除錯環境  
- 將 MCP 伺服器整合至 VS Code Chat  
- 測試完整的 VS Code 工作流程  

## 📚 其他資源

### 測試框架
- [pytest 文件](https://docs.pytest.org/) - Python 測試框架  
- [AsyncPG 測試](https://magicstack.github.io/asyncpg/current/index.html) - 非同步 PostgreSQL 測試  
- [FastAPI 測試](https://fastapi.tiangolo.com/tutorial/testing/) - API 測試模式  

### 效能測試
- [負載測試最佳實踐](https://docs.python.org/3/library/asyncio.html) - 非同步效能測試  
- [資料庫效能測試](https://www.postgresql.org/docs/current/performance-tips.html) - PostgreSQL 優化  
- [記憶體分析](https://docs.python.org/3/library/tracemalloc.html) - Python 記憶體分析  
