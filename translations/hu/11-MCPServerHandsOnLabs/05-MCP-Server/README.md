...
Ez a gyakorlati labor bemutatja, hogyan lehet egy éles környezetre kész MCP szervert létrehozni a FastMCP keretrendszer segítségével. Megtanulod a szerver alapstruktúrájának felépítését, az adatbázis integráció megvalósítását, adat-hozzáférési eszközök létrehozását, valamint az AI-alapú kiskereskedelmi analitika alapjainak lefektetését.

## Áttekintés

Az MCP szerver a kiskereskedelmi analitikai megoldásunk központi eleme. Ez biztosítja a kapcsolatot az AI asszisztensek és a PostgreSQL adatbázis között, lehetővé téve a biztonságos, intelligens hozzáférést az üzleti adatokhoz egy szabványosított protokollon keresztül.

Ez a labor megtanít arra, hogyan építs egy robusztus, skálázható MCP szervert vállalati minták és legjobb gyakorlatok alapján.

## Tanulási Célok

A labor végére képes leszel:

- **Felépíteni** egy FastMCP szervert megfelelő architektúrával és szervezéssel
- **Megvalósítani** az adatbázis integrációt kapcsolat poolinggal és hibakezeléssel
- **Létrehozni** MCP eszközöket adatbázis séma introspekcióhoz és lekérdezések végrehajtásához
- **Konfigurálni** a sor szintű biztonsági (Row Level Security) kontextuskezelést
- **Hozzáadni** egészségfigyelési és megfigyelési funkciókat
- **Tesztelni** az MCP szerver implementációt helyben és VS Code-ban

## 📁 Projekt Struktúra

Nézzük meg az MCP szerver felépítését:
...
### Környezeti Konfiguráció (`config.py`)

Először hozzunk létre egy robusztus konfigurációs rendszert:
...
- **Környezeti Változók Betöltése**: Automatikus .env fájl támogatás
- **Típusbiztonság**: Dataclass validáció és típusjelölések
- **Rugalmas Alapértelmezések**: Ésszerű alapértelmezések fejlesztéshez
- **Validáció**: Konfiguráció validáció hasznos hibaüzenetekkel
- **Biztonság**: Érzékeny értékek csak környezeti változókból

## 🗄️ Adatbázis Integrációs Réteg

### PostgreSQL Provider (`sales_analysis_postgres.py`)

Valósítsuk meg az adatbázis integrációs réteget:
...
### Főbb Adatbázis Réteg Jellemzők

- **Kapcsolat Pooling**: Erőforrások hatékony kezelése az asyncpg segítségével
- **RLS Integráció**: Automatikus sor szintű biztonsági kontextus beállítás
- **Séma Introspekció**: Dinamikus táblaséma felfedezés
- **Hibakezelés**: Átfogó hibakezelés és naplózás
- **Lekérdezés Formázás**: AI-barát eredményformázás
- **Egészségfigyelés**: Adatbázis kapcsolódási és pool állapot ellenőrzések

## 🔧 MCP Szerver Fő Implementáció

### FastMCP Szerver (`sales_analysis.py`)

Most valósítsuk meg az MCP szerver fő részét:
...
### Főbb MCP Szerver Jellemzők

- **Eszköz Regisztráció**: Deklaratív eszközdefiníciók típusbiztonsággal
- **RLS Kontextus Kezelés**: Automatikus felhasználói identitás kinyerés és kontextus beállítás
- **Hibakezelés**: Átfogó hibakezelés felhasználóbarát üzenetekkel
- **Életciklus Kezelés**: Megfelelő indítás/leállítás erőforrások tisztításával
- **Egészségfigyelés**: Beépített egészségellenőrzési végpontok
- **Fejlesztési Támogatás**: Hot reload és hibakeresési funkciók

## 🏥 Egészségfigyelés

### Egészségellenőrzés Implementáció (`health_check.py`)
...
2. **Teszteld az Egészség Végpontokat**:
   ```bash
   # Basic health check
...
### VS Code Integrációs Tesztelés

1. **Konfiguráld a VS Code MCP-t**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Teszteld az AI Chat-ben**:
   - Nyisd meg a VS Code AI Chat-et
   - Írd be: `#zava` és válaszd ki a szerveredet
   - Kérdezd: "Milyen táblák érhetők el?"
   - Kérdezd: "Mutasd az 5 legjobb boltot rendelési szám alapján"

### Egységtesztelés

Hozz létre átfogó egységteszteket:
...
A labor befejezése után rendelkezel:

✅ **Működő MCP Szerverrel**: FastMCP szerver adatbázis integrációval  
✅ **Konfigurációkezeléssel**: Robusztus környezet-alapú konfigurációval  
✅ **Adatbázis Réteggel**: PostgreSQL integráció kapcsolat poolinggal  
✅ **MCP Eszközökkel**: Séma introspekció és lekérdezés végrehajtási eszközökkel  
✅ **RLS Integrációval**: Sor szintű biztonsági kontextus kezeléssel  
✅ **Egészségfigyeléssel**: Átfogó egészségellenőrzési végpontokkal  
✅ **Tesztelési Stratégiával**: Helyi tesztelés és VS Code integrációval  

## 🚀 Mi a Következő Lépés?

Folytasd a **[Lab 06: Eszközfejlesztés](../06-Tools/README.md)** gyakorlatot, hogy:

- Bővítsd az MCP eszközgyűjteményedet
- Valósíts meg fejlett lekérdezési mintákat
- Adj hozzá adatvalidációt és transzformációt
- Hozz létre speciális analitikai eszközöket

## 📚 További Források

### FastMCP Keretrendszer
- [FastMCP Dokumentáció](https://github.com/modelcontextprotocol/python-sdk) - Hivatalos FastMCP útmutató
- [MCP Specifikáció](https://modelcontextprotocol.io/docs/) - Protokoll specifikáció
- [Eszközfejlesztési Útmutató](https://modelcontextprotocol.io/docs/tools/) - MCP eszközök létrehozása

### Adatbázis Integráció
- [asyncpg Dokumentáció](https://magicstack.github.io/asyncpg/current/) - PostgreSQL aszinkron driver
- [Kapcsolat Pooling Legjobb Gyakorlatok](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL hangolás
- [Sor Szintű Biztonsági Útmutató](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS implementáció

### FastAPI Minták
- [FastAPI Dokumentáció](https://fastapi.tiangolo.com/) - Web keretrendszer referencia
- [Függőség Injektálás](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI minták
- [Háttérfeladatok](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Aszinkron feladatkezelés

---

**Következő**: Készen állsz az eszközök bővítésére? Folytasd a [Lab 06: Eszközfejlesztés](../06-Tools/README.md) gyakorlatot!
