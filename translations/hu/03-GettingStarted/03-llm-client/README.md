...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T14:40:21+00:00",
...
Eddig láthattad, hogyan hozhatsz létre szervert és klienst. A kliens képes volt kifejezetten hívni a szervert, hogy felsorolja annak eszközeit, erőforrásait és promptjait. Ez azonban nem túl praktikus megközelítés. A felhasználód az ügynöki korszakban él, és elvárja, hogy promptokat használjon, valamint természetes nyelven kommunikáljon egy LLM-mel. A felhasználót nem érdekli, hogy MCP-t használsz-e a képességeid tárolására, de elvárja, hogy természetes nyelven tudjon interakcióba lépni. Hogyan oldjuk meg ezt? A megoldás az, hogy egy LLM-et adunk a klienshez.
...
1. Felsorolja a képességeket, promptokat, erőforrásokat és eszközöket, majd elmenti azok sémáját.

1. Hozzáad egy LLM-et, és átadja a mentett képességeket és azok sémáját olyan formátumban, amelyet az LLM megért.

1. Kezeli a felhasználói promptot úgy, hogy átadja azt az LLM-nek, az eszközökkel együtt, amelyeket a kliens felsorolt.
...
- Konfiguráld a Tokened – Adj hozzá egy megjegyzést referenciaként, állíts be lejárati dátumot, és válaszd ki a szükséges jogosultságokat.
- Generáld és másold a Tokened – Kattints a Token generálása gombra, és győződj meg róla, hogy azonnal lemásolod, mivel később nem fogod tudni újra megtekinteni.
...
- Konfiguráltuk az LLM példányt, hogy a GitHub Modelleket használja, a `baseUrl` beállításával az inference API-ra.
...
Ez a kód egy alapvető Rust alkalmazást állít be, amely kapcsolódik egy MCP szerverhez és a GitHub Modellekhez LLM interakciókhoz.
...
Nagyszerű, a következő lépésben felsoroljuk a szerver képességeit.
Adjuk hozzá a következő függvényt a `main.rs` fájlhoz:
...
Ez a függvény fogadja az LLM kliensét, egy üzenetlistát (beleértve a felhasználói promptot), az MCP szerver eszközeit, és kérést küld az LLM-nek, majd visszaadja a választ.

Az LLM válasza egy `choices` tömböt tartalmaz. Feldolgoznunk kell az eredményt, hogy lássuk, vannak-e `tool_calls`. Ez jelzi, hogy az LLM egy adott eszközt szeretne meghívni argumentumokkal. Adja hozzá a következő kódot a `main.rs` fájl aljához, hogy definiáljon egy függvényt az LLM válaszának kezelésére:
...
Ha `tool_calls` jelen van, a függvény kinyeri az eszköz információit, kérést küld az MCP szervernek az eszköz kérésével, és hozzáadja az eredményeket a beszélgetési üzenetekhez. Ezután folytatja a beszélgetést az LLM-mel, és az üzenetek frissülnek az asszisztens válaszával és az eszköz hívás eredményeivel.

Az MCP hívásokhoz szükséges eszköz hívási információk kinyeréséhez adjunk hozzá egy segédfüggvényt, amely mindent kinyer, ami a híváshoz szükséges. Adja hozzá a következő kódot a `main.rs` fájl aljához:
...
Most, hogy minden darab a helyén van, kezelhetjük a kezdeti felhasználói promptot és meghívhatjuk az LLM-et. Frissítse a `main` függvényt az alábbi kóddal:
...
Vegye az eddigi gyakorlat kódját, és építse ki a szervert további eszközökkel. Ezután hozzon létre egy klienst egy LLM-mel, mint a gyakorlatban, és tesztelje különböző promptokkal, hogy megbizonyosodjon arról, hogy az összes szerver eszköz dinamikusan meghívásra kerül. Ez a kliensépítési módszer biztosítja, hogy a végfelhasználó nagyszerű felhasználói élményt kapjon, mivel promptokat használhat, ahelyett, hogy pontos kliens parancsokat kellene megadnia, és nem kell tudnia az MCP szerverről.
