...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T10:28:34+00:00",
  "source_file": "05-AdvancedTopics/mcp-multi-modality/README.md",
  "language_code": "hu"
}
-->
# Többmodális integráció

A többmodális alkalmazások egyre fontosabbá válnak a mesterséges intelligenciában, lehetővé téve gazdagabb interakciókat és összetettebb feladatok elvégzését. A Model Context Protocol (MCP) egy keretrendszert biztosít többmodális alkalmazások építéséhez, amelyek képesek különféle adatokat kezelni, például szöveget, képeket és hangot.

Az MCP nemcsak szövegalapú interakciókat támogat, hanem többmodális képességeket is, így a modellek képesek képekkel, hanggal és más adatfajtákkal dolgozni.

## Bevezetés

Ebben a leckében megtanulod, hogyan építs többmodális alkalmazást.

## Tanulási célok

A lecke végére képes leszel:

- Megérteni a többmodális lehetőségeket
- Megvalósítani egy többmodális alkalmazást.

## Többmodális támogatás architektúrája

A többmodális MCP megvalósítások általában a következőket tartalmazzák:

- **Mód-specifikus elemzők**: Olyan komponensek, amelyek különböző médiaformátumokat alakítanak át a modell által feldolgozható formátumokká.
- **Mód-specifikus eszközök**: Különleges eszközök, amelyek adott modalitások kezelésére szolgálnak (képelemzés, hangfeldolgozás)
- **Egységes kontextuskezelés**: Rendszer, amely fenntartja a kontextust a különböző modalitások között
- **Válaszgenerálás**: Képesség olyan válaszok létrehozására, amelyek több modalitást is tartalmazhatnak.

## Többmodális példa: Képelemzés

Az alábbi példában egy képet elemezünk és információkat nyerünk ki belőle.
...
- Létrehoztunk egy `ImageAnalysisTool`-t, amely képes képeket elemezni egy feltételezett `IImageAnalysisService` segítségével.
- Beállítottuk az MCP szervert, hogy nagyobb kéréseket kezeljen és támogassa a képtartalom típusokat.
- Regisztráltuk a képelemző eszközt a szerveren.
- Megvalósítottunk egy metódust, amely képeket tölt le URL-ről, és a kért típus alapján elemzi azokat (objektumok, szöveg, arcok stb.).
- Strukturált eredményeket adtunk vissza az MCP specifikációnak megfelelő formátumban.

## Többmodális példa: Hangfeldolgozás

A hangfeldolgozás egy másik gyakori modalitás a többmodális alkalmazásokban. Az alábbi példa bemutatja, hogyan valósítható meg egy hangátíró eszköz, amely képes hangfájlokat kezelni és átírásokat visszaadni.
...
- Létrehoztunk egy `AudioTranscriptionTool`-t, amely képes hangfájlokat átírni.
- Meghatároztuk az eszköz sémáját, amely elfogad URL-t vagy base64 kódolt hangadatot.
- Megvalósítottuk az `execute` metódust, amely kezeli a hangfeldolgozást és az átírást.
- Beállítottuk az MCP szervert, hogy kezelje a többmodális kéréseket, beleértve a hang- és képfeldolgozást.
- Regisztráltuk a hangátíró eszközt a szerveren.
- Megvalósítottunk egy metódust, amely hangfájlokat tölt le URL-ről vagy dekódolja a base64 hangadatot.
- Egy `AudioProcessor` szolgáltatást használtunk az átírási logika kezelésére.
- Elindítottuk az MCP szervert, hogy figyelje a kéréseket.

### Többmodális példa: Többmodális válaszgenerálás
