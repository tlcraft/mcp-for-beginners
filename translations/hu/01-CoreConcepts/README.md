...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-04T18:29:11+00:00",
...
Ebben a leckében megismerheted a Model Context Protocol (MCP) ökoszisztéma alapvető architektúráját és összetevőit. Megtanulod a kliens-szerver modellt, a kulcsfontosságú elemeket és a kommunikációs mechanizmusokat, amelyek az MCP interakciókat működtetik.
...
Az MCP ökoszisztéma kliens-szerver modellre épül. Ez a moduláris felépítés lehetővé teszi, hogy az AI alkalmazások hatékonyan kommunikáljanak eszközökkel, adatbázisokkal, API-kkal és kontextuális erőforrásokkal. Nézzük meg részletesen ezt az architektúrát és annak fő összetevőit.
...
- **MCP Clients**: Protokoll kliens alkalmazások, amelyek 1:1 kapcsolatot tartanak fenn a szerverekkel
...
Az MCP protokoll folyamatosan fejlődik, a legfrissebb frissítéseket a [protokoll specifikációban](https://modelcontextprotocol.io/specification/2025-06-18/) találod.
...
A Model Context Protocol (MCP) szerverei alapvető építőelemeket biztosítanak, amelyek gazdag interakciókat tesznek lehetővé a kliensek, hostok és nyelvi modellek között. Ezek a funkciók célja, hogy bővítsék az MCP képességeit strukturált kontextus, eszközök és promptok kínálatával.
...
- **Kontextuális adatok**: Információk és háttér, amelyeket a felhasználók vagy AI modellek döntéshozatalhoz és feladatvégrehajtáshoz használhatnak.
...
- **Helyi fájlok és adatbázisok**: Eszközökön vagy adatbázisokban tárolt adatok, amelyek feldolgozhatók és elemezhetők.
...
- **Előre definiált interakciós minták**: Szabványosított cselekvéssorozatok és válaszok, amelyek következetes és hatékony kommunikációt tesznek lehetővé.
...
Az MCP eszközei olyan funkciók, amelyeket az AI modell végrehajthat bizonyos feladatok elvégzésére. Ezek az eszközök arra szolgálnak, hogy bővítsék az AI modell képességeit strukturált és megbízható műveletek biztosításával. Fő jellemzőik:
...
- **Egyedi név és leírás**: Minden eszköznek saját neve és részletes leírása van, amely elmagyarázza a célját és működését.
...
A Model Context Protocol (MCP) kliensei több kulcsfontosságú funkciót kínálnak a szerverek számára, amelyek növelik a protokoll általános működését és interakcióját. Ezek közül az egyik kiemelkedő funkció a Sampling.
...
- **Rekurzív LLM interakciók**: Ez a funkció lehetővé teszi a Nagy Nyelvi Modellekkel (LLM-ek) való rekurzív interakciókat, támogatva a komplexebb és iteratív feladatfeldolgozást.
...
- **Képességek egyeztetése**  
  A kliens (a hostban beágyazva) és a szerver információt cserélnek a támogatott funkcióikról, eszközeikről, erőforrásaikról és protokoll verzióikról. Ez biztosítja, hogy mindkét fél tisztában legyen a rendelkezésre álló képességekkel az adott munkamenetben.
...
- **Erőforrás vagy eszköz használata**  
  ...
  - Ha a modell úgy ítéli meg, hogy eszközre van szükség (például adat lekéréséhez, számítás elvégzéséhez vagy API híváshoz), a kliens eszköz meghívási kérelmet küld a szervernek, megadva az eszköz nevét és paramétereit.
...
- **Eredmény bemutatása**  
  A host megkapja a kliens végső kimenetét, és megjeleníti a felhasználónak, gyakran a modell által generált szöveget és az eszközvégrehajtás vagy erőforrás lekérdezés eredményeit együtt.
...
Az MCP kiterjeszti a JSON-RPC 2.0-t további konvenciókkal az eszköz meghívás, erőforrás hozzáférés és prompt kezelés terén. Több átviteli réteget támogat (STDIO, WebSocket, SSE), és lehetővé teszi a biztonságos, bővíthető, nyelvfüggetlen kommunikációt az összetevők között.
...
Az alábbiakban néhány további segédeszköz és protokoll kiterjesztés található, amelyeket az MCP
...
4. **Korlátozás (Rate Limiting)**:  
  Az erőforrások visszaélésszerű használatának megakadályozása és a tisztességes hozzáférés biztosítása érdekében az MCP szerverek korlátozhatják az eszközhívások és erőforrás-hozzáférések gyakoriságát. A korlátozások alkalmazhatók felhasználónként, munkamenetenként vagy globálisan, és védelmet nyújtanak a szolgáltatásmegtagadásos támadások vagy túlzott erőforrás-felhasználás ellen.

Ezeknek a mechanizmusoknak a kombinációjával az MCP biztonságos alapot nyújt a nyelvi modellek külső eszközökkel és adatforrásokkal való integrálásához, miközben a felhasználók és fejlesztők részletes kontrollt kapnak a hozzáférés és használat felett.
...
- **Modell válasz**  
  ...
  - Opcionális eszközhívási utasításokat, ha a modell úgy ítéli meg, hogy eszközt kell meghívni
...
- **Eszköz válasz**  
  ...
  - Az eszköz végrehajtásának eredményeit (strukturált adatokat vagy tartalmat)
...
Ezek a strukturált üzenetek biztosítják, hogy az MCP munkafolyamat minden lépése egyértelmű, nyomon követhető és bővíthető legyen, támogatva fejlett forgatókönyveket, mint például a többszörös körös beszélgetések, eszközláncolás és robusztus hibakezelés.
...
- Az eszközök a modellek számára elérhető alapvető funkcióegységek
