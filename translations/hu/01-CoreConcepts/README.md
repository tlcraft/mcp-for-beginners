...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-19T14:52:24+00:00",
...
- **Eszközhasználati Biztonság**: Minden eszköz meghívása explicit felhasználói hozzájárulást igényel, amely biztosítja az eszköz funkcióinak, paramétereinek és lehetséges hatásainak egyértelmű megértését. Robusztus biztonsági határoknak kell megakadályozniuk a nem szándékos, nem biztonságos vagy rosszindulatú eszközhasználatot.
...
- **Bemeneti Érvényesítés**: Minden paraméter és adatbevitel érvényesítése meghatározott sémák szerint, hogy megelőzzük az injekciós támadásokat  
- **Audit Naplózás**: Átfogó naplók fenntartása minden műveletről biztonsági monitorozás és megfelelőség érdekében  

A [Model Context Protocol (MCP)](https://modelcontextprotocol.io/specification/2025-06-18/) egy erőteljes, szabványosított keretrendszer, amely optimalizálja a kommunikációt a Nagy Nyelvi Modellek (LLM-ek) és külső eszközök, alkalmazások, valamint adatforrások között. Ez az útmutató bemutatja az MCP alapfogalmait, biztosítva, hogy megértsd annak kliens-szerver architektúráját, alapvető elemeit, kommunikációs mechanizmusait és implementációs legjobb gyakorlatait.

## Áttekintés

Ez a lecke feltárja az MCP ökoszisztémát alkotó alapvető architektúrát és komponenseket. Megismerheted a kliens-szerver architektúrát, kulcsfontosságú elemeket és kommunikációs mechanizmusokat, amelyek az MCP interakciókat működtetik.
...
Az MCP ökoszisztéma kliens-szerver modellen alapul. Ez a moduláris struktúra lehetővé teszi az AI alkalmazások számára, hogy hatékonyan kommunikáljanak eszközökkel, adatbázisokkal, API-kkal és kontextuális erőforrásokkal. Nézzük meg közelebbről az architektúra alapvető elemeit.
...
- **Távoli Szolgáltatások**: Külső rendszerek, amelyek az interneten keresztül érhetők el, és amelyekhez az MCP szerverek API-kon keresztül csatlakozhatnak.
...
- **Kontextuális Adatok**: Strukturált információ és kontextus az AI modellek számára  
...
A Model Context Protocol (MCP) esetében a **kliensek** primitíveket tehetnek elérhetővé, amelyek lehetővé teszik a szerverek számára, hogy további képességeket kérjenek a host alkalmazástól. Ezek a kliensoldali primitívek gazdagabb, interaktívabb szerver implementációkat tesznek lehetővé, amelyek hozzáférhetnek az AI modellek képességeihez és a felhasználói interakciókhoz.

### Mintavételezés

A **Mintavételezés** lehetővé teszi a szerverek számára, hogy nyelvi modell kiegészítéseket kérjenek a kliens AI alkalmazásától. Ez a primitív lehetővé teszi a szerverek számára, hogy hozzáférjenek az LLM képességeihez anélkül, hogy saját modellfüggőségeket ágyaznának be:
...
A mintavétele
- **Életciklus-kezelés**: Kezeli a kapcsolat inicializálását, képességek egyeztetését és a munkamenet lezárását a kliensek és szerverek között  
- **Szerver primitívek**: Lehetővé teszi a szerverek számára, hogy alapvető funkciókat biztosítsanak eszközök, erőforrások és sablonok segítségével  
...
- **Felhasználói beleegyezés és irányítás**: A felhasználóknak kifejezett beleegyezést kell adniuk, mielőtt bármilyen adatot elérnének vagy műveletet végrehajtanának. Egyértelmű irányítást kell biztosítani számukra arról, hogy milyen adatokat osztanak meg, és mely műveletek engedélyezettek, intuitív felhasználói felületekkel támogatva az aktivitások áttekintését és jóváhagyását.  
...
Ezeknek a biztonsági elveknek a követésével az MCP biztosítja a felhasználói bizalom, adatvédelem és biztonság fenntartását minden protokoll interakció során, miközben lehetővé teszi a hatékony AI-integrációkat.

## Kódpéldák: Kulcskomponensek

Az alábbiakban népszerű programozási nyelveken találhatók kódpéldák, amelyek bemutatják, hogyan lehet megvalósítani az MCP szerver kulcskomponenseit és eszközeit.

### .NET példa: Egyszerű MCP szerver létrehozása eszközökkel

Ez egy gyakorlati .NET kódpélda, amely bemutatja, hogyan lehet egy egyszerű MCP szervert megvalósítani egyedi eszközökkel. A példa bemutatja, hogyan lehet eszközöket definiálni és regisztrálni, kéréseket kezelni, és a szervert csatlakoztatni a Model Context Protocol segítségével.
...
Az MCP több beépített koncepciót és mechanizmust tartalmaz a protokoll során a biztonság és jogosultságkezelés biztosítására:
...
3. **Érvényesítés**:  
   Az összes eszközhívás paraméterének érvényesítése kötelező. Minden eszköz meghatározza a paraméterek elvárt típusait, formátumait és korlátait, és a szerver ennek megfelelően érvényesíti a bejövő kéréseket. Ez megakadályozza a hibás vagy rosszindulatú bemenetek elérését az eszközmegvalósításokhoz, és segít fenntartani a műveletek integritását.  

4. **Sebességkorlátozás**:  
   Az MCP szerverek sebességkorlátozást alkalmazhatnak az eszközhívásokra és az erőforrás-hozzáférésre az erőforrások túlzott használatának vagy visszaélések megelőzése érdekében. A sebességkorlátozások alkalmazhatók felhasználónként, munkamenetenként vagy globálisan, és segítenek megvédeni a szervereket a szolgáltatásmegtagadási támadásoktól vagy túlzott erőforrás-felhasználástól.  
...
Az MCP kommunikáció strukturált **JSON-RPC 2.0** üzeneteket használ a gazdagépek, kliensek és szerverek közötti világos és megbízható interakciók megkönnyítésére. A protokoll különböző művelettípusokhoz specifikus üzenetmintákat határoz meg:

### Alapvető üzenettípusok:

#### **Inicializációs üzenetek**
- **`initialize` kérés**: Kapcsolatot hoz létre, és egyezteti a protokoll verzióját és képességeit  
...
#### **Felderítési üzenetek**
- **`tools/list` kérés**: A szerver elérhető eszközeinek felfedezése  
- **`resources/list` kérés**: Elérhető erőforrások (adatforrások) listázása  
- **`prompts/list` kérés**: Elérhető sablonok lekérése  
...
Ez a strukturált kommunikáció biztosítja a megbízható, nyomon követhető és bővíthető interakciókat, támogatva az olyan fejlett forgatókönyveket, mint a valós idejű frissítések, eszközláncolás és robusztus hibakezelés.
