...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-07T08:24:32+00:00",
  "source_file": "03-GettingStarted/07-aitk/README.md",
  "language_code": "ja"
}
-->
# AI Toolkit拡張機能を使用したVisual Studio Codeでのサーバー利用

AIエージェントを構築する際、賢い応答を生成するだけでなく、エージェントに行動を起こす能力を与えることが重要です。そこで登場するのがModel Context Protocol (MCP)です。MCPは、エージェントが外部ツールやサービスに一貫した方法でアクセスできるようにする仕組みです。まるでエージェントを実際に使えるツールボックスに接続するようなものです。

例えば、エージェントを計算機MCPサーバーに接続したとします。すると、エージェントは「47×89は何？」といったプロンプトを受け取るだけで数学の計算を実行できるようになります。ロジックをハードコーディングしたり、カスタムAPIを構築する必要はありません。

## 概要

このレッスンでは、Visual Studio Codeの[AI Toolkit](https://aka.ms/AIToolkit)拡張機能を使用して、計算機MCPサーバーをエージェントに接続する方法を学びます。これにより、エージェントは自然言語を通じて加算、減算、乗算、除算などの数学的操作を実行できるようになります。

AI Toolkitは、Visual Studio Code向けの強力な拡張機能で、エージェント開発を効率化します。AIエンジニアは、生成AIモデルをローカルまたはクラウドで開発・テストすることで、AIアプリケーションを簡単に構築できます。この拡張機能は、現在利用可能な主要な生成モデルのほとんどをサポートしています。
...
では、エージェントの能力を向上させるために、MCPを通じて外部ツールを活用する設定を始めましょう！
...
この演習では、Visual Studio Code内でAI Toolkitを使用して、MCPサーバーのツールを備えたAIエージェントを構築、実行、拡張します。

### -0- 事前準備: OpenAI GPT-4oモデルをMy Modelsに追加する

この演習では**GPT-4o**モデルを使用します。エージェントを作成する前に、このモデルを**My Models**に追加してください。

1. **AI Toolkit**拡張機能を**Activity Bar**から開きます。
...
1. **+ Add**をクリックしてモデルを**My Models**リストに追加します。**GitHubでホストされている**モデルを選択したことを確認してください。
1. **Activity Bar**で**OpenAI GPT-4o**モデルがリストに表示されていることを確認します。

### -1- エージェントを作成する

**Agent (Prompt) Builder**を使用すると、独自のAIエージェントを作成してカスタマイズできます。このセクションでは、新しいエージェントを作成し、会話を動かすモデルを割り当てます。
...
1. 名前に**Calculator Agent**を入力し、**Enter**を押します。
1. **Agent (Prompt) Builder**で、**Model**フィールドに**OpenAI GPT-4o (via GitHub)**モデルを選択します。

### -2- エージェントのシステムプロンプトを作成する

エージェントの骨組みができたら、その性格と目的を定義します。このセクションでは、**Generate system prompt**機能を使用して、エージェントの意図する動作を記述します。この場合、計算機エージェントとしての役割を設定し、モデルにシステムプロンプトを書かせます。
...
エージェントのシステムプロンプトを定義したら、その動作と応答を導く準備が整います。次に、エージェントに実用的な能力を装備します。このセクションでは、加算、減算、乗算、除算を実行するツールを備えた計算機MCPサーバーを作成します。このサーバーにより、エージェントは自然言語プロンプトに応じてリアルタイムで数学的操作を実行できるようになります。
...
エージェントにツールが追加されたので、実際に使用してみましょう！このセクションでは、エージェントにプロンプトを送信し、計算機MCPサーバーの適切なツールを活用できるかどうかをテスト・検証します。

1. `F5`キーを押してMCPサーバーのデバッグを開始します。**Agent (Prompt) Builder**が新しいエディタタブで開きます。サーバーのステータスはターミナルで確認できます。
1. **Agent (Prompt) Builder**の**User prompt**フィールドに以下のプロンプトを入力します：`私は3つのアイテムをそれぞれ$25で購入し、$20の割引を使用しました。いくら支払いましたか？`
1. **Run**ボタンをクリックしてエージェントの応答を生成します。
1. エージェントの出力を確認します。モデルは**$55**を支払ったと結論付けるはずです。
1. 以下が発生するはずです：
    - エージェントが計算を補助するために**multiply**と**subtract**ツールを選択します。
    - **multiply**ツールにそれぞれ`a`と`b`の値が割り当てられます。
    - **subtract**ツールにそれぞれ`a`と`b`の値が割り当てられます。
...
1. エージェントのテストが終了したら、**ターミナル**で**CTRL/CMD+C**を入力してサーバーを停止します。

## 課題

**server.py**ファイルに新しいツールエントリ（例: 数値の平方根を返す）を追加してみてください。エージェントが新しいツール（または既存のツール）を活用する必要があるプロンプトを送信してください。新しく追加したツールを読み込むためにサーバーを再起動することを忘れないでください。
...
- AI Toolkitには、カスタムツールの作成を簡素化するテンプレート（例: Python MCPサーバーテンプレート）が含まれています。
