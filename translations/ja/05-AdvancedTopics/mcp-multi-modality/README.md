...
MCPはテキストベースの対話だけでなく、画像や音声などのマルチモーダル機能もサポートしており、モデルが多様なデータタイプを扱えるようにします。

## はじめに

このレッスンでは、マルチモーダルアプリケーションの作り方を学びます。
...
マルチモーダルMCPの実装には通常、以下が含まれます：

- **モーダル固有のパーサー**：異なるメディアタイプをモデルが処理可能な形式に変換するコンポーネント
- **モーダル固有のツール**：特定のモダリティ（画像解析、音声処理など）を扱うための専用ツール
- **統合コンテキスト管理**：異なるモダリティ間でコンテキストを維持するシステム
- **レスポンス生成**：複数のモダリティを含むレスポンスを生成する機能

## マルチモーダル例：画像解析

以下の例では、画像を解析して情報を抽出します。
...
- 仮想の `IImageAnalysisService` を使って画像を解析できる `ImageAnalysisTool` を作成しました。
- MCPサーバーを大きなリクエストと画像コンテンツタイプに対応するよう設定しました。
- 画像解析ツールをサーバーに登録しました。
- URLから画像をダウンロードし、要求されたタイプ（オブジェクト、テキスト、顔など）に基づいて解析するメソッドを実装しました。
- MCP仕様に準拠した形式で構造化された結果を返しました。

## マルチモーダル例：音声処理

音声処理もマルチモーダルアプリケーションでよく使われるモダリティです。以下は音声ファイルを扱い、文字起こしを返すツールの実装例です。
...
- 音声ファイルの文字起こしができる `AudioTranscriptionTool` を作成しました。
- ツールのスキーマをURLまたはbase64エンコードされた音声データのいずれかを受け入れるよう定義しました。
- 音声処理と文字起こしを行う `execute` メソッドを実装しました。
- MCPサーバーを音声や画像処理を含むマルチモーダルリクエストに対応するよう設定しました。
- 音声文字起こしツールをサーバーに登録しました。
- URLから音声ファイルをダウンロードするか、base64音声データをデコードするメソッドを実装しました。
- 実際の文字起こしロジックを処理する `AudioProcessor` サービスを使用しました。
- MCPサーバーを起動してリクエストを待ち受けました。

### マルチモーダル例：マルチモーダルレスポンス生成
