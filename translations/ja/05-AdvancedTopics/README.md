...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-02T12:07:20+00:00",
...
このレッスンでは、Model Context Protocolの実装における高度な概念を探ります。マルチモーダル統合、スケーラビリティ、セキュリティのベストプラクティス、エンタープライズ統合に焦点を当てています。これらのトピックは、複雑な要件を持つエンタープライズ環境で動作する本番レベルのMCPアプリケーションを構築するために不可欠です。
...
| リンク | タイトル | 説明 |
|------|-------|-------------|
| [mcp-oauth2-demo](../../../05-AdvancedTopics/mcp-oauth2-demo) | MCP OAuth2 デモ | OAuth2を利用したMCPの最小限のSpring Bootアプリ。認可サーバーとリソースサーバーの両方として機能し、安全なトークン発行、保護されたエンドポイント、Azure Container Appsへのデプロイ、API Managementとの統合を示します。 |
| [web-search-mcp](../../../05-AdvancedTopics/web-search-mcp) | Web Search MCP | SerpAPIと連携したPythonのMCPサーバーとクライアント。リアルタイムのウェブ検索、ニュース、商品検索、Q&Aを統合し、マルチツールのオーケストレーション、外部API統合、堅牢なエラーハンドリングを実演します。 |
...
MCPはテキストベースのやり取りだけでなく、画像、音声、その他のデータタイプにも対応したマルチモーダル機能をサポートしています。

### マルチモーダル対応のアーキテクチャ

マルチモーダルMCPの実装には通常以下が含まれます：

1. **モーダル固有のパーサー**：異なるメディアタイプをモデルが処理可能な形式に変換するコンポーネント
2. **モーダル固有のツール**：特定のモダリティ（画像解析、音声処理）を扱う専用ツール
3. **統合コンテキスト管理**：異なるモダリティ間でコンテキストを維持するシステム
4. **レスポンス生成**：複数のモダリティを含むレスポンスを生成する能力
...
### Python マルチモーダル例：マルチモーダルレスポンス生成
...
ルートコンテキストは、Model Context Protocolの基本的な概念であり、複数のリクエストやセッションにわたって会話履歴や共有状態を保持する永続的なレイヤーを提供します。

### ルートコンテキストの理解

ルートコンテキストは、一連の関連するやり取りの履歴や状態を保持するコンテナとして機能します。これにより以下が可能になります：

- **会話の持続性**：一貫した複数ターンの会話を維持
- **メモリ管理**：やり取りを通じて情報の保存・取得
- **状態管理**：複雑なワークフローの進捗を追跡
- **コンテキスト共有**：複数のクライアントが同じ会話状態にアクセス可能

MCPにおけるルートコンテキストの主な特徴：

- 各ルートコンテキストは一意の識別子を持つ
- 会話履歴、ユーザー設定、その他のメタデータを含むことができる
...
1. **目的別にコンテキストを分ける**：異なる会話目的やドメインごとに別々のルートコンテキストを作成し、明確さを保つ。
2. **有効期限ポリシーを設定する**：古いコンテキストをアーカイブまたは削除するポリシーを実装し、ストレージ管理やデータ保持ポリシーに対応する。
3. **関連メタデータを保存する**：会話に関する重要な情報を後で活用できるよう、コンテキストのメタデータに保存する。
4. **コンテキストIDを一貫して使用する**：コンテキスト作成後は、そのIDを関連するすべてのリクエストで使い続け、連続性を維持する。
5. **要約を生成する**：コンテキストが大きくなった場合は、重要な情報をまとめる要約を作成し、サイズ管理を行う。
6. **アクセス制御を実装する**：複数ユーザーが利用するシステムでは、プライバシーとセキュリティを確保するために適切なアクセス制御を実施する。
7. **コンテキストの制限に対応する**：コンテキストサイズの制限を理解し、非常に長い会話に対処する戦略を立てる。
8. **完了時にアーカイブする**：会話が終了したらコンテキストをアーカイブし、リソースを解放しつつ履歴を保存する。

## Model Context Protocolにおけるサンプリング

サンプリング戦略は、MCP実装でモデルの応答を最適化するために重要です。適切なサンプリング設定は、応答の質とパフォーマンスを大幅に向上させます。MCPは、ランダム性、創造性、一貫性に影響を与える特定のパラメーターを用いてテキスト生成を制御する標準的な方法を提供します。

### サンプリングパラメーターの概要

MCPは、クライアントリクエストで設定可能な以下のサンプリングパラメーターを定義しています：
...
| `stop_sequences` | 生成を停止させるカスタムシーケンス | 文字列の配列 |

### TemperatureとTop-K/Top-Pサンプリング

サンプリングパラメーターは、言語モデルの挙動を微調整し、決定論的な出力と創造的な出力のバランスを取ることができます。
...
一貫した出力が求められるアプリケーションでは、決定論的サンプリングにより再現可能な結果を保証します。
...
インテリジェントなサンプリングは、リクエストのコンテキストや要件に応じてパラメーターを適応的に調整します。
...
## Model Context Protocolにおけるルーティング

ルーティングは、MCPエコシステム内でリクエストを適切なモデル、ツール、サービスに振り分けるために不可欠です。

### コンテンツベースルーティング

コンテンツベースルーティングは、リクエストの内容に応じて専門サービスへ振り分けます。
...
ロードバランシングは、リソースの効率的利用と高可用性を実現します。
...
ツール呼び出しをコンテキストに基づいて最適なサービスに振り分けます。
...
以下の図は、包括的なMCPアーキテクチャにおけるサンプリングとルーティングの連携を示しています：
...
エンタープライズ展開では、MCP実装は大量のリクエストを低遅延で処理する必要があります。
...
セキュリティは、特にエンタープライズ環境でのMCP実装において重要です。
...
MCPをAzure OpenAIやMicrosoft AI Foundryなどのエンタープライズシステムと統合します。
...
- マルチモーダルMCP実装はテキスト処理を超えたAI能力を拡張する
- スケーラビリティはエンタープライズ展開に不可欠であり、水平・垂直スケーリングで対応可能
- 包括的なセキュリティ対策によりデータ保護と適切なアクセス制御を実現
- Azure OpenAIやMicrosoft AI Foundryなどのプラットフォームとの統合がMCPの能力を強化
...
**免責事項**:  
本書類はAI翻訳サービス「Co-op Translator」（https://github.com/Azure/co-op-translator）を使用して翻訳されています。正確性には努めておりますが、自動翻訳には誤りや不正確な部分が含まれる可能性があります。原文の言語によるオリジナル文書が正式な情報源とみなされるべきです。重要な情報については、専門の人間による翻訳を推奨します。本翻訳の利用により生じたいかなる誤解や誤訳についても、当方は責任を負いかねます。