...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T16:29:01+00:00",
...
Questo laboratorio pratico ti guida nell'implementazione di un server MCP pronto per la produzione utilizzando il framework FastMCP. Costruirai la struttura principale del server, integrerai il database, creerai strumenti per l'accesso ai dati e stabilirai le basi per l'analisi retail basata sull'intelligenza artificiale.

## Panoramica

Il server MCP è il cuore della nostra soluzione di analisi retail. Funziona come un ponte tra gli assistenti AI e il database PostgreSQL, fornendo un accesso sicuro e intelligente ai dati aziendali attraverso un protocollo standardizzato.

Questo laboratorio ti insegna a costruire un server MCP robusto e scalabile seguendo modelli aziendali e migliori pratiche.

## Obiettivi di Apprendimento

Alla fine di questo laboratorio, sarai in grado di:

- **Costruire** un server FastMCP con un'architettura e un'organizzazione adeguate
- **Implementare** l'integrazione del database con pooling di connessioni e gestione degli errori
- **Creare** strumenti MCP per introspezione dello schema del database ed esecuzione di query
- **Configurare** la gestione del contesto di Sicurezza a Livello di Riga (Row Level Security)
- **Aggiungere** funzionalità di monitoraggio della salute e osservabilità
- **Testare** l'implementazione del server MCP localmente e con VS Code

## 📁 Struttura del Progetto

Esaminiamo l'organizzazione del server MCP:
...
### Configurazione dell'Ambiente (`config.py`)

Per prima cosa, creiamo un sistema di configurazione robusto:
...
### Caratteristiche Chiave della Configurazione

- **Caricamento Variabili d'Ambiente**: Supporto automatico per file .env
- **Sicurezza dei Tipi**: Validazione con dataclass e suggerimenti di tipo
- **Default Flessibili**: Valori predefiniti sensati per lo sviluppo
- **Validazione**: Validazione della configurazione con messaggi di errore utili
- **Sicurezza**: Valori sensibili solo dalle variabili d'ambiente

## 🗄️ Livello di Integrazione del Database

### Provider PostgreSQL (`sales_analysis_postgres.py`)

Implementiamo il livello di integrazione del database:
...
### Caratteristiche Chiave del Livello Database

- **Pooling di Connessioni**: Gestione efficiente delle risorse con asyncpg
- **Integrazione RLS**: Impostazione automatica del contesto di Sicurezza a Livello di Riga
- **Introspezione dello Schema**: Scoperta dinamica dello schema delle tabelle
- **Gestione degli Errori**: Gestione completa degli errori e logging
- **Formattazione delle Query**: Formattazione dei risultati compatibile con AI
- **Monitoraggio della Salute**: Controllo della connettività del database e dello stato del pool

## 🔧 Implementazione Principale del Server MCP

### Server FastMCP (`sales_analysis.py`)

Ora implementiamo il server MCP principale:
...
### Caratteristiche Chiave del Server MCP

- **Registrazione degli Strumenti**: Definizioni dichiarative degli strumenti con sicurezza dei tipi
- **Gestione del Contesto RLS**: Estrazione automatica dell'identità utente e impostazione del contesto
- **Gestione degli Errori**: Gestione completa degli errori con messaggi utili per l'utente
- **Gestione del Ciclo di Vita**: Avvio/arresto corretto con pulizia delle risorse
- **Monitoraggio della Salute**: Endpoint di controllo della salute integrati
- **Supporto allo Sviluppo**: Ricarica automatica e capacità di debug

## 🏥 Monitoraggio della Salute

### Implementazione del Controllo della Salute (`health_check.py`)
...
2. **Testa gli Endpoint di Salute**:
   ```bash
   # Basic health check
...
### Test di Integrazione con VS Code
...
2. **Testa nella Chat AI**:
   - Apri la Chat AI di VS Code
   - Scrivi `#zava` e seleziona il tuo server
   - Chiedi: "Quali tabelle sono disponibili?"
   - Chiedi: "Mostrami i 5 negozi principali per numero di ordini"

### Test Unitari

Crea test unitari completi:
...
✅ **Server MCP Funzionante**: Server FastMCP con integrazione del database  
✅ **Gestione della Configurazione**: Configurazione robusta basata sull'ambiente  
✅ **Livello Database**: Integrazione PostgreSQL con pooling di connessioni  
✅ **Strumenti MCP**: Strumenti per introspezione dello schema ed esecuzione di query  
✅ **Integrazione RLS**: Gestione del contesto di Sicurezza a Livello di Riga  
✅ **Monitoraggio della Salute**: Endpoint di controllo della salute completi  
✅ **Strategia di Test**: Test locale e integrazione con VS Code  

## 🚀 Cosa Fare Dopo

Continua con **[Lab 06: Sviluppo degli Strumenti](../06-Tools/README.md)** per:

- Espandere la tua collezione di strumenti MCP
- Implementare modelli avanzati di query
- Aggiungere validazione e trasformazione dei dati
- Creare strumenti di analisi specializzati

## 📚 Risorse Aggiuntive

### Framework FastMCP
- [Documentazione FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Guida ufficiale FastMCP
- [Specifiche MCP](https://modelcontextprotocol.io/docs/) - Specifiche del protocollo
- [Guida allo Sviluppo degli Strumenti](https://modelcontextprotocol.io/docs/tools/) - Creazione di strumenti MCP

### Integrazione del Database
- [Documentazione asyncpg](https://magicstack.github.io/asyncpg/current/) - Driver PostgreSQL asincrono
- [Best Practices per il Pooling di Connessioni](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Ottimizzazione PostgreSQL
- [Guida alla Sicurezza a Livello di Riga](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementazione RLS

### Modelli FastAPI
- [Documentazione FastAPI](https://fastapi.tiangolo.com/) - Riferimento al framework web
- [Iniezione di Dipendenze](https://fastapi.tiangolo.com/tutorial/dependencies/) - Modelli FastAPI
- [Attività in Background](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Gestione di attività asincrone

---

**Prossimo**: Pronto per espandere i tuoi strumenti? Continua con [Lab 06: Sviluppo degli Strumenti](../06-Tools/README.md)
