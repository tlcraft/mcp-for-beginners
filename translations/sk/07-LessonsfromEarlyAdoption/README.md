...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T15:24:03+00:00",
...
- Poháňa schopnosti GitHub Copilot Coding Agent s prehliadaním webu
...
- Znížené manuálne testovacie úsilie a zlepšené pokrytie testov pre webové aplikácie
...
- Poháňa schopnosti GitHub Copilot pre prehliadanie webu
...
**Výsledky:**  
- Znížený čas na hodnotu pre podnikové AI projekty vďaka pripravenému, súladnému MCP serverovému platformu
...
- Zlepšená kvalita kódu s najlepšími postupmi Azure SDK a aktuálnymi autentifikačnými vzormi
...
MCP (Model Context Protocol) je vznikajúci protokol pre chatboty a AI asistentov na interakciu s nástrojmi. Každý NLWeb inštancia je tiež MCP server, ktorý podporuje jednu základnú metódu, ask, ktorá sa používa na kladenie otázok webovej stránke v prirodzenom jazyku. Vrátená odpoveď využíva schema.org, široko používanú slovnú zásobu na opis webových dát. Voľne povedané, MCP je NLWeb tak, ako Http je k HTML. NLWeb kombinuje protokoly, formáty Schema.org a ukážkový kód na pomoc stránkam rýchlo vytvárať tieto endpointy, čo prospieva ľuďom prostredníctvom konverzačných rozhraní a strojom prostredníctvom prirodzenej interakcie agent-agent.

Existujú dve odlišné komponenty NLWeb:
- Protokol, veľmi jednoduchý na začiatok, na rozhranie so stránkou v prirodzenom jazyku a formát, využívajúci json a schema.org pre vrátenú odpoveď. Viac informácií nájdete v dokumentácii k REST API.
- Priama implementácia (1), ktorá využíva existujúce označenie, pre stránky, ktoré môžu byť abstrahované ako zoznamy položiek (produkty, recepty, atrakcie, recenzie, atď.). Spolu so sadou widgetov užívateľského rozhrania môžu stránky ľahko poskytovať konverzačné rozhrania k ich obsahu. Viac informácií o tom, ako to funguje, nájdete v dokumentácii k životnému cyklu chatovacieho dotazu.
...
**Kľúčové vlastnosti:**
- Komplexný prístup k AI ekosystému Azure, vrátane katalógov modelov a správy nasadenia
...
- Plynulá integrácia s Azure AI službami pre pokročilé scenáre
...
- Urýchlené prijatie AI pri zachovaní kontroly nad komplexnými procesmi riadenými agentmi
...
- Implementácia verzovania a schvaľovacích pracovných tokov
- Vývoj schopností testovania šablón so vzorovými vstupmi
- Zavedenie prístupových práv na základe rolí
- Vytvorenie API na načítanie a nasadzovanie šablón

**Kroky implementácie:**

1. Návrh databázovej schémy na ukladanie šablón
2. Vytvorenie základného API pre CRUD operácie so šablónami
3. Implementácia systému verzovania
4. Vývoj schvaľovacieho pracovného toku
5. Vytvorenie testovacieho rámca
6. Vytvorenie jednoduchého webového rozhrania na správu
7. Integrácia s MCP serverom

**Technológie:** Vami zvolený backendový framework, SQL alebo NoSQL databáza a frontendový framework pre rozhranie na správu.
...
5. **Regulačné rámce**
   ...
   - Štandardizované audítne stopy a rozhrania na vysvetliteľnosť
   - Integrácia s novými rámcami pre správu AI
...
3. [remote-mcp-functions](https://github.com/Azure-Samples/remote-mcp-functions) - Stránka pre vzdialené implementácie MCP serverov v Azure Functions
