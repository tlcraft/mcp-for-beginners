...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T21:47:01+00:00",
  "source_file": "11-MCPServerHandsOnLabs/05-MCP-Server/README.md",
  "language_code": "sk"
}
-->
# Implementácia MCP servera

## 🎯 Čo tento lab pokrýva

Tento praktický lab vás prevedie implementáciou produkčne pripraveného MCP servera pomocou frameworku FastMCP. Vytvoríte základnú štruktúru servera, implementujete integráciu s databázou, vytvoríte nástroje na prístup k dátam a položíte základy pre analytiku maloobchodu poháňanú AI.

## Prehľad

MCP server je jadrom nášho riešenia pre analytiku maloobchodu. Funguje ako most medzi AI asistentmi a databázou PostgreSQL, poskytujúc bezpečný a inteligentný prístup k obchodným dátam prostredníctvom štandardizovaného protokolu.

Tento lab vás naučí, ako vybudovať robustný a škálovateľný MCP server podľa podnikových vzorov a osvedčených postupov.

## Ciele učenia

Na konci tohto labu budete schopní:

- **Vytvoriť** FastMCP server so správnou architektúrou a organizáciou
- **Implementovať** integráciu databázy s poolingom pripojení a spracovaním chýb
- **Vytvoriť** MCP nástroje na introspekciu schémy databázy a vykonávanie dotazov
- **Konfigurovať** správu kontextu Row Level Security
- **Pridať** funkcie monitorovania zdravia a pozorovateľnosti
- **Otestovať** implementáciu MCP servera lokálne a pomocou VS Code

## 📁 Štruktúra projektu

Pozrime sa na organizáciu MCP servera:
...
### Konfigurácia prostredia (`config.py`)

Najprv vytvorme robustný systém konfigurácie:
...
### Kľúčové vlastnosti konfigurácie

- **Načítanie premenných prostredia**: Automatická podpora súborov .env
- **Typová bezpečnosť**: Validácia pomocou dataclass a typové hinty
- **Flexibilné predvolené hodnoty**: Rozumné predvolené hodnoty pre vývoj
- **Validácia**: Validácia konfigurácie s užitočnými chybovými správami
- **Bezpečnosť**: Citlivé hodnoty iba z premenných prostredia

## 🗄️ Vrstva integrácie databázy

### Poskytovateľ PostgreSQL (`sales_analysis_postgres.py`)

Implementujme vrstvu integrácie databázy:
...
### Kľúčové vlastnosti vrstvy databázy

- **Pooling pripojení**: Efektívne riadenie zdrojov pomocou asyncpg
- **Integrácia RLS**: Automatické nastavenie kontextu Row Level Security
- **Introspekcia schémy**: Dynamické objavovanie schémy tabuľky
- **Spracovanie chýb**: Komplexné spracovanie chýb a logovanie
- **Formátovanie dotazov**: Výsledky priateľské pre AI
- **Monitorovanie zdravia**: Kontrola konektivity databázy a stavu poolu

## 🔧 Hlavná implementácia MCP servera

### FastMCP Server (`sales_analysis.py`)

Teraz implementujme hlavný MCP server:
...
### Kľúčové vlastnosti MCP servera

- **Registrácia nástrojov**: Deklaratívne definície nástrojov s typovou bezpečnosťou
- **Správa kontextu RLS**: Automatická extrakcia identity používateľa a nastavenie kontextu
- **Spracovanie chýb**: Komplexné spracovanie chýb s užívateľsky prívetivými správami
- **Správa životného cyklu**: Správne spustenie/zastavenie s čistením zdrojov
- **Monitorovanie zdravia**: Zabudované endpointy na kontrolu zdravia
- **Podpora vývoja**: Hot reload a možnosti ladenia

## 🏥 Monitorovanie zdravia

### Implementácia kontroly zdravia (`health_check.py`)
...
### Testovanie integrácie vo VS Code
...
2. **Testujte v AI chate**:
   - Otvorte AI chat vo VS Code
   - Napíšte `#zava` a vyberte svoj server
   - Spýtajte sa: "Aké tabuľky sú dostupné?"
   - Spýtajte sa: "Ukáž mi top 5 obchodov podľa počtu objednávok"

### Jednotkové testovanie

Vytvorte komplexné jednotkové testy:
...
✅ **Funkčný MCP server**: FastMCP server s integráciou databázy  
✅ **Správu konfigurácie**: Robustnú konfiguráciu založenú na prostredí  
✅ **Vrstva databázy**: Integráciu PostgreSQL s poolingom pripojení  
✅ **MCP nástroje**: Nástroje na introspekciu schémy a vykonávanie dotazov  
✅ **Integráciu RLS**: Správu kontextu Row Level Security  
✅ **Monitorovanie zdravia**: Komplexné endpointy na kontrolu zdravia  
✅ **Testovaciu stratégiu**: Lokálne testovanie a integráciu vo VS Code  

## 🚀 Čo ďalej

Pokračujte s **[Lab 06: Vývoj nástrojov](../06-Tools/README.md)**, kde:

- Rozšírite svoju kolekciu MCP nástrojov
- Implementujete pokročilé vzory dotazov
- Pridáte validáciu a transformáciu dát
- Vytvoríte špecializované analytické nástroje

## 📚 Dodatočné zdroje

### Framework FastMCP
- [Dokumentácia FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Oficiálny sprievodca FastMCP
- [Špecifikácia MCP](https://modelcontextprotocol.io/docs/) - Špecifikácia protokolu
- [Sprievodca vývojom nástrojov](https://modelcontextprotocol.io/docs/tools/) - Tvorba MCP nástrojov

### Integrácia databázy
- [Dokumentácia asyncpg](https://magicstack.github.io/asyncpg/current/) - Asynchrónny ovládač PostgreSQL
- [Najlepšie postupy pre pooling pripojení](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Optimalizácia PostgreSQL
- [Sprievodca Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementácia RLS

### Vzory FastAPI
- [Dokumentácia FastAPI](https://fastapi.tiangolo.com/) - Referencia webového frameworku
- [Dependency Injection](https://fastapi.tiangolo.com/tutorial/dependencies/) - Vzory FastAPI
- [Background Tasks](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Správa asynchrónnych úloh

---

**Ďalej**: Pripravení rozšíriť svoje nástroje? Pokračujte s [Lab 06: Vývoj nástrojov](../06-Tools/README.md)
