...
MCP podporuje nielen textové interakcie, ale aj multi-modálne schopnosti, čo umožňuje modelom pracovať s obrázkami, zvukom a ďalšími typmi dát.

## Úvod

V tejto lekcii sa naučíte, ako vytvoriť multi-modálnu aplikáciu.

## Ciele učenia

Na konci tejto lekcie budete vedieť:

- Pochopiť možnosti multi-modálnych riešení
- Implementovať multi-modálnu aplikáciu.

## Architektúra pre podporu multi-modálnosti

Implementácie MCP pre multi-modálne aplikácie zvyčajne zahŕňajú:

- **Parsery špecifické pre modalitu**: Komponenty, ktoré prevádzajú rôzne typy médií do formátov, ktoré model dokáže spracovať.
- **Nástroje špecifické pre modalitu**: Špeciálne nástroje určené na spracovanie konkrétnych modalít (analýza obrázkov, spracovanie zvuku)
- **Jednotné riadenie kontextu**: Systém na udržiavanie kontextu naprieč rôznymi modalitami
- **Generovanie odpovedí**: Schopnosť vytvárať odpovede, ktoré môžu obsahovať viacero modalít.

## Príklad multi-modálnej aplikácie: Analýza obrázkov

V nasledujúcom príklade budeme analyzovať obrázok a extrahovať z neho informácie.
...
- Vytvorili `ImageAnalysisTool`, ktorý dokáže analyzovať obrázky pomocou hypotetickej služby `IImageAnalysisService`.
- Nakonfigurovali MCP server tak, aby zvládal väčšie požiadavky a podporoval obsahové typy obrázkov.
- Zaregistrovali nástroj na analýzu obrázkov na serveri.
- Implementovali metódu na sťahovanie obrázkov z URL a ich analýzu podľa požadovaného typu (objekty, text, tváre a pod.).
- Vrátili štruktúrované výsledky vo formáte vyhovujúcom špecifikácii MCP.

## Príklad multi-modálnej aplikácie: Spracovanie zvuku

Spracovanie zvuku je ďalšou bežnou modalitou v multi-modálnych aplikáciách. Nižšie je príklad implementácie nástroja na prepis zvukových súborov, ktorý dokáže spracovať audio súbory a vrátiť prepis.
...
- Vytvorili `AudioTranscriptionTool`, ktorý dokáže prepisovať zvukové súbory.
- Definovali schému nástroja tak, aby prijímal buď URL, alebo base64 kódované audio dáta.
- Implementovali metódu `execute` na spracovanie zvuku a prepis.
- Nakonfigurovali MCP server na spracovanie multi-modálnych požiadaviek vrátane spracovania zvuku a obrázkov.
- Zaregistrovali nástroj na prepis zvuku na serveri.
- Implementovali metódu na sťahovanie zvukových súborov z URL alebo dekódovanie base64 audio dát.
- Použili službu `AudioProcessor` na samotnú logiku prepisu.
- Spustili MCP server, ktorý počúva na požiadavky.

### Príklad multi-modálnej aplikácie: Generovanie multi-modálnych odpovedí
