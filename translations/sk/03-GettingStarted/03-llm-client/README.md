...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T15:37:00+00:00",
...
Vytvorenie tokenu GitHub je jednoduchý proces. Tu je postup:

- Prejdite do nastavení GitHub – Kliknite na svoj profilový obrázok v pravom hornom rohu a vyberte Nastavenia.
- Prejdite do Nastavení vývojára – Posuňte sa nadol a kliknite na Nastavenia vývojára.
...
- **Nastavili HTTP transport**: Použitím Server-Sent Events (SSE) na pripojenie k serveru MCP.
- **Vytvorili klienta MCP**: Ktorý bude spracovávať komunikáciu so serverom.
- **Použili vstavanú podporu MCP v LangChain4j**: Ktorá zjednodušuje integráciu medzi LLM a servermi MCP.

#### Rust

Tento príklad predpokladá, že máte spustený server MCP založený na Ruste. Ak ho nemáte, vráťte sa k lekcii [01-first-server](../01-first-server/README.md) a vytvorte server.

Keď máte svoj server MCP v Ruste, otvorte terminál a prejdite do rovnakého adresára ako server. Potom spustite nasledujúci príkaz na vytvorenie nového projektu klienta LLM:
...
> [!NOTE]
> Neexistuje oficiálna knižnica Rust pre OpenAI, avšak `async-openai` je [knižnica udržiavaná komunitou](https://platform.openai.com/docs/libraries/rust#rust), ktorá sa bežne používa.
...
Tento kód nastavuje základnú aplikáciu Rust, ktorá sa pripojí k serveru MCP a GitHub Models na interakcie s LLM.
...
- Vytvorili metódu `run`, ktorá je zodpovedná za spracovanie toku našej aplikácie. Zatiaľ iba získava zoznam nástrojov, ale čoskoro pridáme viac.
...
- Získanie zoznamu zdrojov a nástrojov a ich vypísanie. Pre nástroje tiež uvádzame `inputSchema`, ktorý použijeme neskôr.
...
- Získali zoznam nástrojov dostupných na serveri MCP.
- Pre každý nástroj uviedli názov, popis a jeho schému. Posledné z nich použijeme na volanie nástrojov neskôr.
...
- Tento prístup abstrahuje manuálny proces získavania a konverzie nástrojov.

#### Rust

Získanie nástrojov zo servera MCP sa vykonáva pomocou metódy `list_tools`. Vo vašej funkcii `main`, po nastavení klienta MCP, pridajte nasledujúci kód:
...
Ďalším krokom po získaní schopností servera je ich konverzia do formátu, ktorému LLM rozumie. Keď to urobíme, môžeme tieto schopnosti poskytnúť ako nástroje pre LLM.
...
1. Pridajte nasledujúci kód na konverziu odpovede zo servera MCP na formát nástroja, ktorý môže LLM použiť:
    ...
    Vyššie uvedený kód berie odpoveď zo servera MCP a konvertuje ju na definíciu nástroja, ktorú LLM rozumie.

1. Aktualizujte metódu `run`, aby získala schopnosti servera:
...
1. Ďalej aktualizujeme kód klienta, aby využíval túto funkciu takto:
...
1. Pridajme kód na konverziu odpovede nástroja MCP na niečo, čo LLM rozumie:
...
- Definovali funkčnosť, ktorá vytvára `FunctionDefinition`, ktorý sa odovzdáva `ChatCompletionsDefinition`. Posledné z nich je niečo, čo LLM rozumie.
...
1. Pozrime sa, ako môžeme aktualizovať existujúci kód, aby využíval túto funkciu vyššie:
    ...
    - Aktualizovali funkciu na konverziu odpovede nástroja MCP na nástroj LLM. Poďme zvýrazniť pridaný kód:
        ...
        Vstupná schéma je súčasťou odpovede nástroja, ale na atribúte "properties", takže ju musíme extrahovať. Okrem toho teraz voláme `ConvertFrom` s podrobnosťami o nástroji. Teraz, keď sme urobili ťažkú prácu, pozrime sa, ako to všetko spolu funguje, keď spracujeme používateľskú výzvu ďalej.
...
V tejto časti kódu spracujeme požiadavky používateľa.
...
1. Pridajte metódu, ktorá sa bude používať na volanie nášho LLM:
    ...
    - Metóda berie odpoveď LLM a kontroluje, aké nástroje majú byť volané, ak vôbec:
...
1. Ďalej pridajme funkciu, ktorá bude volať LLM:
    ...
    - Potom kontrolujeme výsledok, aby sme zistili, aké funkcie by sme mali volať, ak vôbec.
...
1. Ukážme kód na vykonanie požiadavky na výzvu LLM:
...
1. Jeden posledný krok, pozrime sa, či LLM považuje za potrebné volať funkciu:
    ...
    - Pre každý nástrojový hovor sme analyzovali názov a argumenty a zavolali nástroj na serveri MCP pomocou klienta MCP. Nakoniec sme vytlačili výsledky.
...
Pridajme nasledujúcu funkciu do vášho súboru `main.rs`:
...
Odpoveď z LLM bude obsahovať pole `choices`. Budeme musieť spracovať výsledok, aby sme zistili, či sú prítomné nejaké `tool_calls`. To nám dá vedieť, že LLM žiada o zavolanie konkrétneho nástroja s argumentmi. Pridajte nasledujúci kód na koniec vášho súboru `main.rs`, aby ste definovali funkciu na spracovanie odpovede LLM:
...
Na extrahovanie informácií o volaní nástroja, ktoré LLM vracia pre volania MCP, pridáme ďalšiu pomocnú funkciu na extrakciu všetkého potrebného na vykonanie volania. Pridajte nasledujúci kód na koniec vášho súboru `main.rs`:
...
Keď máme všetky časti pripravené, môžeme teraz spracovať počiatočné používateľské zadanie a zavolať LLM. Aktualizujte svoju funkciu `main`, aby obsahovala nasledujúci kód:
...
Týmto sa odošle dotaz na LLM s počiatočným používateľským zadaním, ktoré žiada o súčet dvoch čísel, a spracuje odpoveď na dynamické spracovanie volaní nástrojov.

Skvelé, zvládli ste to!

## Zadanie

Vezmite kód z cvičenia a rozšírte server o ďalšie nástroje. Potom vytvorte klienta s LLM, ako v cvičení, a otestujte ho s rôznymi zadaniami, aby ste sa uistili, že všetky nástroje vášho servera sú dynamicky volané. Tento spôsob vytvárania klienta zabezpečí, že koncový používateľ bude mať skvelý používateľský zážitok, pretože bude môcť používať zadania namiesto presných príkazov klienta a nebude si musieť uvedomovať, že sa volá nejaký server MCP.
