...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T20:28:58+00:00",
...
Klienti sú vlastné aplikácie alebo skripty, ktoré komunikujú priamo s MCP serverom za účelom požiadania o zdroje, nástroje a výzvy. Na rozdiel od použitia inšpekčného nástroja, ktorý poskytuje grafické rozhranie na interakciu so serverom, písanie vlastného klienta umožňuje programovateľné a automatizované interakcie. To umožňuje vývojárom integrovať schopnosti MCP do vlastných pracovných postupov, automatizovať úlohy a vytvárať riešenia prispôsobené konkrétnym potrebám.
...
- Pripojiť a otestovať klienta s MCP serverom, aby ste sa uistili, že server funguje podľa očakávania.
...
- **Integrovať klienta do hostiteľskej aplikácie**. Keď poznáte schopnosti servera, musíte ich integrovať do hostiteľskej aplikácie tak, aby sa pri zadaní výzvy alebo iného príkazu používateľom spustila príslušná funkcia servera.
...
A to je všetko, klient, ktorý dokáže komunikovať s MCP serverom.

V ďalšej časti cvičenia si rozoberieme každý úryvok kódu a vysvetlíme, čo sa deje.

## Cvičenie: Písanie klienta

Ako sme už povedali, venujme si čas vysvetleniu kódu a pokojne si kódujte spolu s nami, ak chcete.
...
- Vytvorili inštanciu stdio prenosu. Všimnite si, ako špecifikuje príkaz a argumenty na nájdenie a spustenie servera, pretože to budeme musieť urobiť pri vytváraní klienta.
...
- Vytvorili vstupný bod, kde poskytujeme metódu `run` funkcii `asyncio.run`.
...
Poznámka: V časti "Arguments" môžete buď ukázať na *.csproj*, alebo na spustiteľný súbor.
...
Teraz máme klienta, ktorý sa dokáže pripojiť, ak sa program spustí. Avšak, ešte nezobrazuje zoznam funkcií, takže to urobme ďalej:
...
- Čítali zdroj, voláme zdroj pomocou `readResource()` so špecifikovaním `uri`. Na strane servera to vyzerá pravdepodobne takto:
...
Prípadne môžete spustiť kompletný projekt klienta poskytnutý v priečinku riešenia `03-GettingStarted\02-client\solution\java`:
...
V [predchádzajúcej sekcii](../../../../03-GettingStarted/01-first-server) ste sa naučili, ako vytvoriť jednoduchý MCP server s Rustom. Môžete na tom pokračovať alebo si pozrieť tento odkaz na ďalšie príklady MCP serverov založených na Rust: [Príklady MCP serverov](https://github.com/modelcontextprotocol/rust-sdk/tree/main/examples/servers)
...
1. **Prejdite do priečinka vášho preferovaného jazyka**:
...
2. **Postupujte podľa pokynov v README** v každom priečinku pre:
...
- ✅ **Vyhľadávanie servera** (nástroje, zdroje, výzvy, kde je to vhodné)
...
Podrobné informácie o spustení a prispôsobení týchto príkladov nájdete tu: **[📖 Dokumentácia kompletných príkladov](./complete_examples.md)**
...
- Môžu byť použité na vyhľadávanie aj využívanie funkcií na serveri.
- Môžu spustiť server pri svojom vlastnom spustení (ako v tejto kapitole), ale klienti sa môžu pripojiť aj k už bežiacim serverom.
- Sú skvelým spôsobom testovania schopností servera vedľa alternatív, ako je Inspector, ktorý bol opísaný v predchádzajúcej kapitole.
