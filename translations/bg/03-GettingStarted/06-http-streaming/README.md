...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T16:36:46+00:00",
  "source_file": "03-GettingStarted/06-http-streaming/README.md",
  "language_code": "bg"
}
-->
# HTTPS Streaming с Model Context Protocol (MCP)

Тази глава предоставя изчерпателно ръководство за внедряване на сигурно, мащабируемо и реално време стрийминг с Model Context Protocol (MCP), използвайки HTTPS. Тя обхваща мотивацията за стрийминг, наличните транспортни механизми, как да се реализира стрийминг HTTP в MCP, най-добрите практики за сигурност, миграция от SSE и практически насоки за изграждане на собствени MCP приложения за стрийминг.

## Транспортни механизми и стрийминг в MCP

Тази секция разглежда различните транспортни механизми, налични в MCP, и тяхната роля в осигуряването на стрийминг възможности за комуникация в реално време между клиенти и сървъри.
...
- **SSE (Server-Sent Events)**: Позволява на сървърите да изпращат актуализации в реално време към клиентите чрез HTTP. Добър за уеб интерфейси, но ограничен в мащабируемост и гъвкавост.
...
Ето прост пример за това как може да се реализира стрийминг:
...
| Актуализации за напредък | Изпращат се като части от данни | Изпращат се като известия         |
...
Има някои препоръки, когато става въпрос за избор между имплементиране на класически стрийминг (като крайна точка, която показахме по-горе, използвайки `/stream`) и избор на стрийминг чрез MCP.

- **За прости нужди от стрийминг:** Класически HTTP стрийминг е по-лесен за имплементиране и достатъчен за основни нужди.
...
Препоръчително е да поддържате съвместимост с наличните SSE клиенти по време на процеса на миграция. Ето някои стратегии:
...
- **Валидиране на Origin заглавката**: Винаги валидирайте заглавката `Origin`, за да предотвратите DNS rebinding атаки.
...
- **Автентикация**: Реализирайте автентикация (например API ключове, OAuth) за производствени среди.
- **CORS**: Конфигурирайте политики за Cross-Origin Resource Sharing (CORS), за да ограничите достъпа.
- **HTTPS**: Използвайте HTTPS в производствени среди за криптиране на трафика.

### Най-добри практики

Освен това, ето някои най-добри практики, които да следвате при внедряване на сигурност в MCP стрийминг сървър:

- Никога не се доверявайте на входящи заявки без валидиране.
- Логвайте и наблюдавайте всички достъпи и грешки.
- Редовно актуализирайте зависимостите, за да коригирате уязвимости в сигурността.
...
**Сценарий:**
Създайте MCP сървър и клиент, където сървърът обработва списък с елементи (например файлове или документи) и изпраща известие за всеки обработен елемент. Клиентът трябва да показва всяко известие веднага щом пристигне.

**Стъпки:**

1. Реализирайте инструмент за сървър, който обработва списък и изпраща известия за всеки елемент.
2. Реализирайте клиент с обработчик на съобщения, който показва известия в реално време.
...
За да продължите вашето пътешествие с MCP стрийминг и да разширите знанията си, този раздел предоставя допълнителни ресурси и предложени следващи стъпки за създаване на по-сложни приложения.
...
- Опитайте да създадете по-сложни MCP инструменти, които използват стрийминг за анализи в реално време, чат или съвместно редактиране.
- Разгледайте интегрирането на MCP стрийминг с фронтенд рамки (React, Vue и др.) за актуализации на потребителския интерфейс в реално време.
- Следва: [Използване на AI Toolkit за VSCode](../07-aitk/README.md)
