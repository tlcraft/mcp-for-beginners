...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T21:20:58+00:00",
...
- **Интегриране на клиента в хост приложение**. След като разберете възможностите на сървъра, трябва да го интегрирате във вашето хост приложение, така че когато потребител въведе подсказка или друга команда, съответната функция на сървъра да бъде извикана.
...
Както беше споменато по-горе, нека отделим време за обяснение на кода, и ако желаете, кодирайте заедно с нас.
...
- Създадохме клиент, като му зададохме име и версия.
...
Забележка: В "Arguments" можете да посочите или *.csproj* файла, или изпълнимия файл.
...
В основната функция, след инициализиране на клиента, можем да инициализираме сървъра и да изброим някои от неговите функции.
...
- Четем ресурс, като извикваме `readResource()` и задаваме `uri`. Ето как изглежда това от страна на сървъра:
    ...
    Нашата стойност на `uri` `file://example.txt` съответства на `file://{name}` на сървъра. `example.txt` ще бъде съпоставен с `name`.
...
- **Скриптове за изграждане и изпълнение** за лесна настройка и изпълнение.
