...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T16:35:05+00:00",
...
Когато създавате AI агент, не става въпрос само за генериране на интелигентни отговори; важно е също така да дадете на вашия агент възможност да предприема действия. Тук влиза в действие Model Context Protocol (MCP). MCP улеснява достъпа на агентите до външни инструменти и услуги по последователен начин. Представете си го като свързване на вашия агент към инструментариум, който той *наистина* може да използва.

Да кажем, че свързвате агент към MCP сървър за калкулатор. Изведнъж вашият агент може да извършва математически операции само чрез получаване на заявка като „Колко е 47 умножено по 89?“—без нужда от твърдо кодирана логика или създаване на персонализирани API.
...
- Да конфигурирате настройките на агент, за да му позволите да открива и използва инструменти, предоставени от MCP сървър.
...
> [!WARNING]
> Забележка за потребителите на macOS. В момента разследваме проблем, засягащ инсталирането на зависимости на macOS. В резултат на това потребителите на macOS няма да могат да завършат този урок в момента. Ще актуализираме инструкциите веднага щом бъде налично решение. Благодарим ви за търпението и разбирането!
...
### -2- Създайте системен промпт за агента

След като сте създали структурата на агента, е време да дефинирате неговата личност и цел. В този раздел ще използвате функцията **Generate system prompt**, за да опишете предназначението на агента—в този случай агент за калкулатор—и ще позволите на модела да напише системния промпт вместо вас.

1. За секцията **Prompts** кликнете върху бутона **Generate system prompt**. Този бутон отваря генератора на промпт, който използва AI за генериране на системен промпт за агента.
...
1. Кликнете върху бутона **Generate**. Ще се появи известие в долния десен ъгъл, потвърждаващо, че системният промпт се генерира. След като генерирането на промпт приключи, промптът ще се появи в полето **System prompt** на **Agent (Prompt) Builder**.
1. Прегледайте **System prompt** и го модифицирайте, ако е необходимо.

### -3- Създайте MCP сървър

След като сте дефинирали системния промпт на вашия агент—насочвайки неговото поведение и отговори—е време да оборудвате агента с практически възможности. В този раздел ще създадете MCP сървър за калкулатор с инструменти за изпълнение на събиране, изваждане, умножение и деление. Този сървър ще позволи на вашия агент да извършва математически операции в реално време в отговор на заявки на естествен език.
