...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T21:17:13+00:00",
...
Досега видяхте как да създадете сървър и клиент. Клиентът можеше изрично да извика сървъра, за да изброи неговите инструменти, ресурси и подсказки. Въпреки това, този подход не е много практичен. Вашият потребител живее в ерата на агентите и очаква да използва подсказки и да комуникира с LLM, за да го направи. За потребителя няма значение дали използвате MCP за съхранение на вашите възможности, но те очакват да взаимодействат чрез естествен език. Как да решим това? Решението е да добавим LLM към клиента.
...
Първо, ще трябва да добавите зависимостите LangChain4j към вашия файл `pom.xml`. Добавете тези зависимости, за да активирате интеграцията с MCP и поддръжката на GitHub Models:
...
- **Добавили зависимостите LangChain4j**: Необходими за интеграция с MCP, официалния клиент на OpenAI и поддръжка на GitHub Models.
- **Импортирали библиотеките LangChain4j**: За интеграция с MCP и функционалност на OpenAI чат модела.
...
- **Създали MCP клиент**: Който ще обработва комуникацията със сървъра.
...
Добавете следните зависимости към вашия файл `Cargo.toml`:
...
- Създали метод `run`, отговорен за обработката на потока на приложението. Засега той само изброява инструментите, но скоро ще добавим още.
...
- Изброяване на ресурси и инструменти и отпечатване на тях. За инструментите също изброяваме `inputSchema`, който ще използваме по-късно.
...
1. Нека видим как можем да актуализираме съществуващ код, за да се възползваме от тази функция:
    ...
    - Актуализирали функцията, за да конвертира отговора от MCP инструмент в LLM инструмент. Нека подчертаем добавения код:
...
- Използвали `AiServices` от LangChain4j, за да свържем автоматично LLM с провайдъра на MCP инструменти.
...
За да конвертираме отговора от MCP инструмент във формат, който LLM може да разбере, ще добавим помощна функция, която форматира списъка с инструменти. Добавете следния код към вашия файл `main.rs` под функцията `main`. Това ще бъде извикано при заявки към LLM:
