...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T11:35:19+00:00",
...
- Конфигурирахме нашия LLM екземпляр да използва GitHub Models, като зададохме `baseUrl` към inference API.
...
- Създадохме метод `run`, отговорен за управлението на потока на приложението. До момента той само изброява инструментите, но скоро ще добавим още.
...
- Провайдърът на инструменти обработва вътрешно конверсията между MCP инструментални схеми и формата на инструментите в LangChain4j
- Този подход абстрахира ръчния процес на изброяване и конверсия на инструментите
...
1. Добавете следния код, за да конвертирате отговора от MCP сървъра във формат на инструмент, който LLM може да използва:
    ...
    Горният код приема отговор от MCP сървъра и го конвертира във формат на дефиниция на инструмент, който LLM разбира.
...
1. Нека актуализираме метода `run`, за да изброим възможностите на сървъра:
    ...
    В горния код актуализирахме метода `run`, за да премине през резултата и за всяка позиция да извика `openAiToolAdapter`.
...
1. След това, нека актуализираме кода на клиента, за да използва тази функция по следния начин:
...
- Актуализирахме функцията, за да конвертира отговора от MCP инструмента в LLM инструмент. Нека подчертаем добавения код:
...
- Използвахме `AiServices` на LangChain4j, за да свържем автоматично LLM с MCP провайдъра на инструменти
...
1. Нека покажем код за извършване на LLM заявка с подсказка:
    ...
    - Създадохме обект с опции, задаващ модел и инструменти.
...
- Този подход осигурява безпроблемно потребителско изживяване, при което потребителите не трябва да знаят за задкулисната реализация на MCP
...
Вземете кода от упражнението и разширете сървъра с още инструменти. След това създайте клиент с LLM, както в упражнението, и го тествайте с различни подсказки, за да се уверите, че всички инструменти на сървъра се извикват динамично. Този начин на създаване на клиент осигурява отлично потребителско изживяване, тъй като потребителите могат да използват подсказки вместо точни клиентски команди и не се налага да знаят за извикването на MCP сървър.
