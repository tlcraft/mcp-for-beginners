...
>
> **ТРЯБВА**: MCP прокси сървърите, използващи статични клиентски идентификатори, **ТРЯБВА** да получат съгласие от потребителя за всеки динамично регистриран клиент  
...
**Контроли за автентикация и авторизация:**
   - **Стриктен преглед на авторизацията**: Провеждайте цялостни одити на логиката за авторизация на MCP сървъра, за да гарантирате, че само предназначените потребители и клиенти имат достъп до ресурсите  
   - **Интеграция с външни доставчици на идентичност**: Използвайте утвърдени доставчици на идентичност като Microsoft Entra ID вместо да внедрявате персонализирана автентикация  
   ...
   - **Правилен жизнен цикъл на токените**: Внедрете сигурна ротация на токените, политики за изтичане и предотвратяване на повторно използване на токени  

**Защитено съхранение на токени:**
   - Използвайте Azure Key Vault или подобни сигурни хранилища за всички тайни  
   - Внедрете криптиране на токените както при съхранение, така и при пренос  
...
**Сигурни практики за сесии:**
   ...
   - **Обвързване със специфичен потребител**: Обвържете идентификаторите на сесии с идентичностите на потребителите, използвайки формати като `<user_id>:<session_id>`, за да предотвратите злоупотреба между потребители  
   - **Управление на жизнения цикъл на сесиите**: Внедрете правилно изтичане, ротация и анулиране, за да ограничите прозорците на уязвимост  
...
## 3. **Защита от специфични за ИИ заплахи** 🤖

**Защита от инжектиране на команди:**
   - **Microsoft Prompt Shields**: Използвайте AI Prompt Shields за напреднало откриване и филтриране на злонамерени инструкции  
   - **Санитизация на входните данни**: Проверявайте и пречиствайте всички входни данни, за да предотвратите атаки чрез инжектиране и проблеми с объркан заместник  
...
**Предотвратяване на отравяне на инструменти:**
   - **Валидиране на метаданни на инструменти**: Внедрете проверки за целостта на дефинициите на инструменти и наблюдавайте за неочаквани промени  
   - **Динамично наблюдение на инструменти**: Наблюдавайте поведението по време на изпълнение и настройте известия за неочаквани модели на изпълнение  
...
**Принцип на минималните привилегии:**
   ...
   - Внедрете контрол на достъпа, базиран на роли (RBAC), с фино настроени разрешения  
...
**Внедряване на сигурност на съдържанието:**
   ...
   - **Анализ на поведението**: Внедрете мониторинг на поведението по време на изпълнение, за да откривате аномалии в MCP сървъра и изпълнението на инструменти  
...
**Проверка на компонентите:**
   ...
   - **Проверка на произхода**: Проверявайте произхода, лицензирането и целостта на моделите, източниците на данни и външните услуги  
...
**Сигурност на разработката:**
   - **GitHub Advanced Security**: Внедрете сканиране за тайни, анализ на зависимости и статичен анализ с CodeQL  
   - **Сигурност на CI/CD**: Интегрирайте проверки за сигурност в автоматизираните процеси за внедряване  
   - **Цялост на артефактите**: Внедрете криптографска проверка за внедрените артефакти и конфигурации  

## 7. **OAuth сигурност и предотвратяване на объркан заместник**

**Внедряване на OAuth 2.1:**
   - **PKCE внедряване**: Използвайте Proof Key for Code Exchange (PKCE) за всички заявки за авторизация  
   ...
   - **Валидиране на URI за пренасочване**: Внедрете стриктно валидиране на URI за пренасочване и идентификатори на клиенти  

**Сигурност на проксито:**
   - Предотвратете заобикаляне на авторизацията чрез експлоатация на статични клиентски идентификатори  
   - Внедрете правилни процеси за съгласие за достъп до API на трети страни  
...
**Бързи възможности за реакция:**
   - **Автоматизирана реакция**: Внедрете автоматизирани системи за ротация на идентификационни данни и ограничаване на заплахите  
   - **Процедури за връщане назад**: Способност за бързо връщане към известни добри конфигурации и компоненти  
   - **Форензични възможности**: Подробни одитни следи и логове за разследване на инциденти  
...
**Регулаторно съответствие:**
   - Уверете се, че MCP внедренията отговарят на специфичните за индустрията изисквания (GDPR, HIPAA, SOC 2)  
   - Внедрете контроли за класификация на данни и поверителност за обработка на AI данни  
...
**Управление на промените:**
   ...
   - Контрол на версиите и процеси за одобрение на промени в конфигурацията  
...
**Защита на приложенията по време на изпълнение:**
   ...
   - **Мониторинг на производителността на приложенията**: Наблюдавайте за аномалии в производителността, които могат да индикират атаки  
   - **Динамични политики за сигурност**: Внедрете политики за сигурност, които се адаптират според текущия пейзаж на заплахите  
...
**Управление на идентичност и достъп:**
   ...
   - **Privileged Identity Management (PIM)**: Достъп "точно навреме" и процеси за одобрение за административни функции  
   - **Identity Protection**: Условен достъп, базиран на риска, и автоматизирана реакция на заплахи  
...
**Адаптивна сигурност:**
   ...
   - **Предиктивна аналитика за сигурност**: Внедрете предиктивни модели за проактивно идентифициране на заплахи  
...
- [NIST Рамка за управление на рисковете при AI](https://www.nist.gov/itl/ai-risk-management-framework)  

### **Ръководства за внедряване**
- [Azure API Management MCP Authentication Gateway](https://techcommunity.microsoft.com/blog/integrationsonazureblog/azure-api
