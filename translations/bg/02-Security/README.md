...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T16:26:31+00:00",
...
- **Идентифициране на специфични за MCP заплахи**: Разпознаване на уникални рискове за сигурността в MCP системи, включително инжектиране на команди, отравяне на инструменти, прекомерни разрешения, отвличане на сесии, проблеми с объркан заместник, уязвимости при предаване на токени и рискове във веригата за доставки
...
- **Използване на решения за сигурност на Microsoft**: Разбиране и внедряване на Microsoft Prompt Shields, Azure Content Safety и GitHub Advanced Security за защита на MCP натоварвания
- **Валидиране на сигурността на инструментите**: Разпознаване на важността на валидирането на метаданни на инструменти, мониторинг за динамични промени и защита срещу индиректни атаки чрез инжектиране на команди
- **Интегриране на добри практики**: Комбиниране на утвърдени основи за сигурност (сигурно кодиране, укрепване на сървъри, нулево доверие) със специфични за MCP контроли за цялостна защита
...
Спецификацията на MCP значително е еволюирала в подхода си към автентикация и авторизация:

- **Първоначален подход**: Ранните спецификации изискваха разработчиците да внедряват персонализирани сървъри за автентикация, като MCP сървърите действаха като OAuth 2.0 Authorization Servers, управляващи автентикацията на потребителите директно
...
- **Компрометиране на OAuth токени**: Кражба на токени от локални MCP сървъри позволява на атакуващите да се представят за сървъри и да получат достъп до услуги надолу по веригата
...
- MCP сървъри и API-та надолу по веригата прилагат критични контроли за сигурност (ограничаване на скоростта, валидиране на заявки, мониторинг на трафика), които зависят от правилната валидиране на токени
...
- MCP сървъри не могат да различат клиенти, използващи токени, издадени нагоре по веригата, нарушавайки одитните следи
- Логовете на ресурсните сървъри надолу по веригата показват подвеждащи източници на заявки вместо действителните MCP сървъри посредници
...
- **Сигурно управление на токени**: Внедряване на [най-добрите практики на Microsoft за валидиране и жизнен цикъл на токени](https://learn.microsoft.com/en-us/entra/identity-platform/access-tokens)
  ...
  - Предотвратяване на атаки чрез повторно използване на токени и неоторизирана употреба
...
- **Непрекъснат мониторинг на разрешенията**: Внедряване на постоянен одит и мониторинг на достъпа
  - Наблюдение на модели на използване на разрешения за аномалии
  - Бързо отстраняване на прекомерни или неизползвани привилегии

## Специфични за AI заплахи за сигурността

### Атаки чрез инжектиране на команди и манипулация на инструменти
...
**Индиректното инжектиране на команди** представлява една от най-критичните уязвимости в AI системи, активирани чрез MCP. Атакуващите вграждат злонамерени инструкции в външно съдържание—документи, уеб страници, имейли или източници на данни—които AI системите впоследствие обработват като легитимни команди.
...
#### **Атаки чрез отравяне на инструменти**
...
**Механизми на атака:**
- **Манипулация на метаданни**: Атакуващите вграждат злонамерени инструкции в описанията на инструменти, дефинициите на параметри или примери за употреба
- **Невидими инструкции**: Скритите команди в метаданните на инструменти, които се обработват от AI модели, но са невидими за човешки потребители
...
- **Заобикаляне на съгласие чрез бисквитки**: Предишно удостоверяване на потребителя създава бисквитки за съгласие, които нападателите експлоатират чрез злонамерени заявки за авторизация с изработени URI за пренасочване  
- **Кражба на код за авторизация**: Съществуващите бисквитки за съгласие могат да накарат сървърите за авторизация да пропуснат екрани за съгласие, пренасочвайки кодове към контролирани от нападателя крайни точки  
...
- **Изискване за изрично съгласие**: MCP прокси сървърите, използващи статични идентификатори на клиенти, **ТРЯБВА** да получат съгласие от потребителя за всеки динамично регистриран клиент  
- **Имплементация на OAuth 2.1 сигурност**: Следвайте актуалните най-добри практики за сигурност на OAuth, включително PKCE (Proof Key for Code Exchange) за всички заявки за авторизация  
...
- **Заобикаляне на контроли**: Директното използване на токени от клиент към API заобикаля критични контроли като ограничаване на скоростта, валидация и мониторинг  
- **Компрометиране на одитната следа**: Токените, издадени нагоре по веригата, правят идентификацията на клиента невъзможна, нарушавайки възможностите за разследване на инциденти  
...
- **Разширяване на атаки между услуги**: Компрометираните токени, приети от множество услуги, позволяват странично движение  
...
- **Проверка на аудиторията**: Винаги проверявайте дали претенциите за аудитория на токените съвпадат с идентичността на MCP сървъра  
...
- **Източници на тренировъчни данни**: Набори от данни, използвани за обучение и фина настройка на модели  

### Всеобхватна стратегия за сигурност на веригата за доставки

#### **Проверка на компонентите и доверие**
- **Валидация на произхода**: Проверявайте произхода, лицензирането и целостта на всички AI компоненти преди интеграция  
...
- **Сканиране на зависимости**: Постоянно наблюдение за уязвимости във всички софтуерни и AI зависимости  
...
- **Възстановяване при пробив във веригата за доставки**: Специфични процедури за реакция при компрометиране на AI услуги нагоре по веригата  
...
- **[Най-добри практики за сигурност на MCP 2025](./mcp-security-best-practices-2025.md)** - Пълни най-добри практики за внедряване на MCP
...
- **[Контроли за сигурност на MCP 2025](./mcp-security-controls-2025.md)** - Най-новите контроли и техники за сигурност при внедряване на MCP
- **[Бърз справочник за най-добри практики на MCP](./mcp-best-practices.md)** - Бърз справочник за основни практики за сигурност на MCP
