...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-19T16:48:12+00:00",
  "source_file": "01-CoreConcepts/README.md",
  "language_code": "bg"
}
-->
# Основни концепции на MCP: Овладяване на протокола за контекст на модела за AI интеграция
...
- **Сигурност на транспортния слой**: Всички комуникационни канали трябва да използват подходящи механизми за криптиране и автентикация. Отдалечените връзки трябва да имплементират сигурни транспортни протоколи и правилно управление на идентификационни данни.
...
- **Автентикация и авторизация**: Използвайте сигурни методи за автентикация (OAuth, API ключове) с правилно управление на токени и изтичане.  
- **Валидация на входните данни**: Валидирайте всички параметри и входни данни според дефинирани схеми, за да предотвратите атаки чрез инжектиране.
- **Одитни логове**: Поддържайте подробни логове на всички операции за мониторинг на сигурността и съответствие.

## Преглед

Този урок разглежда основната архитектура и компонентите, които съставляват екосистемата на Model Context Protocol (MCP). Ще научите за клиент-сървърната архитектура, ключовите компоненти и механизмите за комуникация, които задвижват взаимодействията в MCP.
...
Екосистемата на MCP е изградена върху клиент-сървър модел. Тази модулна структура позволява AI приложенията да взаимодействат с инструменти, бази данни, API-та и контекстуални ресурси ефективно. Нека разгледаме тази архитектура в нейните основни компоненти.

В основата си MCP следва клиент-сървърна архитектура, където хост приложението може да се свърже с множество сървъри:
...
- **Среда за разработка**: IDE-та и редактори на код с MCP интеграция.  
...
- **Налагат сигурност**: Контролират разрешенията, ограниченията за сигурност и автентикацията.
...
- **Управление на състоянието**: Поддържат състоянието на сесията и обработват взаимодействия със състояние, когато е необходимо.
...
**Подсказките** са многократно използваеми шаблони, които помагат за структуриране на взаимодействията с езиковите модели. Те предоставят стандартизирани модели на взаимодействие и шаблони за работни потоци:
...
**Семплирането** позволява на сървърите да заявяват завършвания от езиковия модел чрез AI приложението на клиента. Този примитив позволява на сървърите да достъпват възможностите на LLM без да включват свои собствени зависимости от модели:
...
Семплирането се инициира чрез метода `sampling/complete`, където сървърите изпращат заявки за завършване към клиентите.

### Събиране на информация  

**Събирането на информация** позволява на сървърите да заявяват допълнителна информация или потвърждение от потребителите чрез интерфейса на клиента:
...
- **Договаряне на версия на протокола**: Използва версия, базирана на дата (ГГГГ-ММ-ДД), за да осигури съвместимост  
...
2. **HTTP транспорт с възможност за стрийминг**:  
   ...
   - Поддържа стандартна HTTP автентикация (bearer tokens, API ключове, персонализирани хедъри)  
   - MCP препоръчва OAuth за сигурна автентикация, базирана на токени  

#### Абстракция на транспорта:

Транспортният слой абстрахира детайлите на комуникацията от слоя с данни, позволявайки използването на един и същ JSON-RPC 2.0 формат на съобщенията за всички транспортни механизми. Тази абстракция позволява на приложенията безпроблемно да превключват между локални и отдалечени сървъри.

### Съображения за сигурност

Реализациите на MCP трябва да спазват няколко критични принципа за сигурност, за да осигурят безопасни, надеждни и сигурни взаимодействия във всички операции на протокола:

- **Съгласие и контрол на потребителя**: Потребителите трябва да дадат изрично съгласие, преди да бъде достъпен какъвто и да е вид данни или да бъдат извършени операции. Те трябва да имат ясен контрол върху това какви данни се споделят и кои действия са разрешени, подкрепени от интуитивни интерфейси за преглед и одобрение на дейностите.  

- **Поверителност на данните**: Данните на потребителите трябва да бъдат достъпни само с изрично съгласие и трябва да бъдат защитени чрез подходящи контроли за достъп. Реализациите на MCP трябва да предотвратяват неоторизирано предаване на данни и да гарантират, че поверителността се запазва през всички взаимодействия.  

- **Безопасност на инструментите**: Преди да бъде извикан какъвто и да е инструмент, се изисква изрично съгласие от потребителя. Потребителите трябва да имат ясно разбиране за функционалността на всеки инструмент, а също така трябва да бъдат наложени строги граници за сигурност, за да се предотврати нежелано или опасно изпълнение на инструменти.  
...
Ето практически пример на .NET, който демонстрира как да се реализира прост MCP сървър с персонализирани инструменти. Примерът показва как да се дефинират и регистрират инструменти, да се обработват заявки и да се свърже сървърът с Model Context Protocol.
...
Този пример демонстрира същия MCP сървър и регистрация на инструменти, както в примера на .NET, но реализиран на Java.
...
Този пример на JavaScript демонстрира как да се създаде MCP клиент, който се свързва със сървър, изпраща заявка и обработва отговора, включително всички извикани инструменти.
...
- **`tools/call` Заявка**: Изпълнява конкретен инструмент с предоставените параметри  
...
- **Транспортни механизми**: Комуникацията поддържа STDIO (локално) и HTTP с възможност за стрийминг и опционални SSE (отдалечено)  
...
- **Клиентски примитиви**: Сървърите могат да изискват извадки (LLM завършвания), събиране на вход (потребителски вход) и логове от клиенти  
...
4. Как моделът може да използва този инструмент за решаване на проблеми на потребителите  
