...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T16:16:33+00:00",
...
- **Одитиране и логове**: Поддържайте подробни логове на всички операции за мониторинг на сигурността и съответствие.
...
## Архитектура на MCP: Подробен поглед
...
- **Отдалечени услуги**: Външни системи, достъпни през интернет, до които MCP сървърите могат да се свържат чрез API-та.
...
Сървърите на MCP могат да предоставят всяка комбинация от следните три основни примитива:
...
**Подсказките** са многократно използваеми шаблони, които помагат за структуриране на взаимодействията с езиковите модели. Те предоставят стандартизирани модели за взаимодействие и шаблони за работни потоци:
...
**Семплирането** позволява на сървърите да заявяват завършвания от езиковия модел на клиента. Този примитив позволява на сървърите да достъпват възможностите на LLM без да включват свои собствени зависимости от модела:
...
- **Рекурсивни взаимодействия с LLM**: Поддържа сложни сценарии, при които сървърите се нуждаят от помощ от AI за обработка.
...
- **Докладване на грешки**: Предоставя подробен контекст за грешки и диагностична информация.
- **Одитни следи**: Създава подробни логове на операциите и решенията на сървъра.

Лог съобщенията се изпращат към клиентите, за да предоставят прозрачност в операциите на сървъра и да улеснят дебъгването.

## Поток на информация в MCP

Model Context Protocol (MCP) деф
...
- **Сесии с поддържане на състояние**: Поддържа състоянието на връзката през множество взаимодействия за осигуряване на контекст  
...
2. **Streamable HTTP транспорт**:
   ...
   - Поддържа стандартна HTTP автентикация (bearer токени, API ключове, персонализирани хедъри)  
...
- **Безопасност на инструментите**: Преди да бъде извикан какъвто и да е инструмент, е необходимо изрично съгласие от потребителя. Потребителите трябва да имат ясно разбиране за функционалността на всеки инструмент, а строгите граници за сигурност трябва да предотвратяват нежелано или опасно изпълнение на инструменти.
...
Тук е представен практически пример за .NET, който демонстрира как да се реализира прост MCP сървър с персонализирани инструменти. Примерът показва как да се дефинират и регистрират инструменти, да се обработват заявки и да се свърже сървърът чрез Model Context Protocol.
...
Този пример на JavaScript демонстрира как да се създаде MCP клиент, който се свързва със сървър, изпраща заявка и обработва отговора, включително всички извиквания на инструменти.
...
- **`prompts/list` Заявка**: Извлича наличните шаблони  
...
- **`prompts/get` Заявка**: Извлича шаблон с опционални параметри  
...
- **Основни примитиви**: Сървърите предоставят инструменти (изпълними функции), ресурси (източници на данни) и шаблони (templates)  
...
- **Основи на протокола**: Изграден върху JSON-RPC 2.0 с базирано на дата версиониране (текуща версия: 2025-06-18)  
