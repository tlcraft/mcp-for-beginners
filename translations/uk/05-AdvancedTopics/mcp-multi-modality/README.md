...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T13:02:48+00:00",
  "source_file": "05-AdvancedTopics/mcp-multi-modality/README.md",
  "language_code": "uk"
}
-->
# Інтеграція мультимодальності

Мультимодальні застосунки стають дедалі важливішими в штучному інтелекті, забезпечуючи більш насичену взаємодію та виконання складніших завдань. Протокол контексту моделі (MCP) надає основу для створення мультимодальних застосунків, які можуть працювати з різними типами даних, такими як текст, зображення та аудіо.

MCP підтримує не лише текстові взаємодії, а й мультимодальні можливості, дозволяючи моделям працювати з зображеннями, аудіо та іншими типами даних.

## Вступ

У цьому уроці ви навчитеся створювати мультимодальний застосунок.

## Цілі навчання

До кінця цього уроку ви зможете:

- Розуміти мультимодальні варіанти
- Реалізувати мультимодальний застосунок.

## Архітектура підтримки мультимодальності

Реалізації MCP для мультимодальності зазвичай включають:

- **Парсери, специфічні для модальності**: Компоненти, які перетворюють різні типи медіа у формати, які модель може обробляти.
- **Інструменти, специфічні для модальності**: Спеціальні інструменти для роботи з певними модальностями (аналіз зображень, обробка аудіо)
- **Уніфіковане управління контекстом**: Система для підтримки контексту між різними модальностями
- **Генерація відповідей**: Можливість створювати відповіді, що можуть включати кілька модальностей.

## Приклад мультимодальності: Аналіз зображень

У наведеному нижче прикладі ми проаналізуємо зображення та витягнемо інформацію.
...
У попередньому прикладі ми:

- Створили `ImageAnalysisTool`, який може аналізувати зображення за допомогою гіпотетичного `IImageAnalysisService`.
- Налаштували сервер MCP для обробки більших запитів і підтримки типів контенту зображень.
- Зареєстрували інструмент аналізу зображень на сервері.
- Реалізували метод для завантаження зображень за URL та їх аналізу залежно від запитаного типу (об’єкти, текст, обличчя тощо).
- Повернули структуровані результати у форматі, що відповідає специфікації MCP.

## Приклад мультимодальності: Обробка аудіо

Обробка аудіо — ще одна поширена модальність у мультимодальних застосунках. Нижче наведено приклад реалізації інструменту транскрипції аудіо, який може обробляти аудіофайли та повертати транскрипції.
...
- Створили `AudioTranscriptionTool`, який може транскрибувати аудіофайли.
- Визначили схему інструменту для прийому або URL, або аудіоданих у форматі base64.
- Реалізували метод `execute` для обробки аудіо та транскрипції.
- Налаштували сервер MCP для обробки мультимодальних запитів, включно з обробкою аудіо та зображень.
- Зареєстрували інструмент транскрипції аудіо на сервері.
- Реалізували метод для завантаження аудіофайлів за URL або декодування аудіо у форматі base64.
- Використали сервіс `AudioProcessor` для виконання логіки транскрипції.
- Запустили сервер MCP для прийому запитів.

### Приклад мультимодальної відповіді: Генерація мультимодальної відповіді
