...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-14T03:52:02+00:00",
...
У цьому розділі показано, як створити реального AI-агента, який інтегрується з зовнішніми API, обробляє різноманітні типи даних, керує помилками та координує роботу кількох інструментів — усе це у форматі, готовому до використання в продакшені. Ви дізнаєтесь:

- **Інтеграція з зовнішніми API, що потребують автентифікації**
- **Обробка різних типів даних з кількох кінцевих точок**
...
У цьому уроці ви навчитесь створювати просунутий MCP-сервер і клієнт, які розширюють можливості LLM за допомогою актуальних веб-даних у реальному часі через SerpAPI. Це важливий навик для розробки динамічних AI-агентів, які можуть отримувати актуальну інформацію з інтернету.
...
### Особливості
- **Приклади коду**: містить мовно-специфічні блоки коду для Python (легко розширювані на інші мови) з використанням розгортальних секцій для зручності
...
- Ключ SerpAPI (зареєструйтесь на [SerpAPI](https://serpapi.com/) — доступний безкоштовний тариф)
...
2. Створіть файл `.env` у корені проєкту з вашим ключем SerpAPI:
...
- **Звичайний режим**: запускає автоматизовані тести, які перевіряють усі інструменти та їх відповіді. Це корисно для швидкої перевірки роботи сервера та інструментів.
- **Інтерактивний режим**: запускає меню, де можна вручну вибирати інструменти, вводити власні запити та бачити результати в реальному часі. Ідеально підходить для дослідження можливостей сервера та експериментів з різними запитами.
...
У цьому контексті "тестовий скрипт" означає власну програму на Python, яку ви пишете як клієнт MCP-сервера. Це не формальний юніт-тест, а скрипт, що дозволяє програмно підключатися до сервера, викликати будь-який інструмент із потрібними параметрами та переглядати результати. Цей підхід корисний для:
...
Ви можете швидко перевірити нові запити, налагодити поведінку інструментів або використати цей скрипт як основу для більш складної автоматизації. Нижче наведено приклад використання MCP Python SDK для створення такого скрипта:
...
Або в інтерактивному режимі виберіть `qna` у меню та введіть запитання, коли буде запропоновано.

**Параметри:**
- `question` (рядок): запитання, на яке потрібно знайти відповідь
...
Цей розділ містить фрагменти коду та посилання на реалізації сервера і клієнта.
...
Перед початком розробки ознайомтеся з важливими просунутими концепціями, які зустрічатимуться в цьому розділі. Розуміння їх допоможе вам краще слідувати матеріалу, навіть якщо ви раніше з ними не стикалися:
...
- **Валідація параметрів**: це перевірка, що всі вхідні дані для інструментів правильні та безпечні для використання. Включає встановлення значень за замовчуванням і перевірку типів, що допомагає уникнути помилок і непорозумінь.

Цей розділ допоможе вам діагностувати та вирішувати поширені проблеми, з якими ви можете зіткнутися під час роботи з MCP-сервером для веб-пошуку. Якщо ви натрапите на помилки або несподівану поведінку, цей розділ з усунення несправностей надасть рішення для найпоширеніших проблем. Ознайомтеся з цими порадами перед тим, як звертатися за додатковою допомогою — вони часто швидко вирішують проблеми.

## Усунення несправностей

Під час роботи з MCP-сервером для веб-пошуку іноді можуть виникати проблеми — це нормально при розробці з використанням зовнішніх API та нових інструментів. Цей розділ містить практичні рішення найпоширеніших проблем, щоб ви могли швидко повернутися до роботи. Якщо ви зіткнулися з помилкою, почніть тут: наведені нижче поради розглядають проблеми, з якими найчастіше стикаються користувачі, і часто можуть вирішити вашу проблему без додаткової допомоги.
...
2. **Помилки типу Module not found**
...
3. **Проблеми з підключенням**
   - Якщо ви отримуєте помилку на кшталт `Error during client execution`, це часто означає, що клієнт не може підключитися до сервера або сервер не працює як очікується. Перевірте, що клієнт і сервер сумісні за версіями, а файл `server.py` знаходиться у правильній директорії і запущений. Перезапуск сервера і клієнта також може допомогти.

4. **Помилки SerpAPI**
   - Повідомлення `Search API returned error status: 401` означає, що ваш ключ SerpAPI відсутній, неправильний або прострочений. Перейдіть у свій кабінет SerpAPI, перевірте ключ і оновіть файл `.env`, якщо потрібно. Якщо ключ правильний, але помилка залишається, перевірте, чи не вичерпано безкоштовний ліміт.

### Режим налагодження (Debug Mode)

За замовчуванням додаток логуватиме лише важливу інформацію. Якщо
