...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:03:00+00:00",
...
Цей практикум допоможе вам реалізувати готовий до використання MCP-сервер за допомогою фреймворку FastMCP. Ви створите основну структуру сервера, інтегруєте базу даних, розробите інструменти для доступу до даних і закладете основу для аналітики роздрібної торгівлі на базі штучного інтелекту.

## Огляд

MCP-сервер є центральним елементом нашого рішення для аналітики роздрібної торгівлі. Він виступає мостом між AI-асистентами та базою даних PostgreSQL, забезпечуючи безпечний і розумний доступ до бізнес-даних через стандартизований протокол.

Цей практикум навчить вас створювати надійний і масштабований MCP-сервер, дотримуючись корпоративних шаблонів і найкращих практик.

## Цілі навчання

Після завершення цього практикуму ви зможете:

- **Створити** сервер FastMCP з правильною архітектурою та організацією
- **Інтегрувати** базу даних із пулінгом з'єднань і обробкою помилок
- **Розробити** MCP-інструменти для інспекції схеми бази даних і виконання запитів
- **Налаштувати** управління контекстом Row Level Security
- **Додати** функції моніторингу стану та спостереження
- **Протестувати** реалізацію MCP-сервера локально та за допомогою VS Code

## 📁 Структура проєкту

Розглянемо організацію MCP-сервера:
...
### Конфігурація середовища (`config.py`)

Спочатку створимо надійну систему конфігурації:
...
### Основні особливості конфігурації

- **Завантаження змінних середовища**: Автоматична підтримка файлів .env
- **Безпека типів**: Валідація через dataclass і підказки типів
- **Гнучкі значення за замовчуванням**: Розумні налаштування для розробки
- **Валідація**: Перевірка конфігурації з корисними повідомленнями про помилки
- **Безпека**: Чутливі значення лише зі змінних середовища

## 🗄️ Шар інтеграції бази даних

### Провайдер PostgreSQL (`sales_analysis_postgres.py`)

Реалізуємо шар інтеграції бази даних:
...
### Основні особливості шару бази даних

- **Пулінг з'єднань**: Ефективне управління ресурсами за допомогою asyncpg
- **Інтеграція RLS**: Автоматичне налаштування контексту Row Level Security
- **Інспекція схеми**: Динамічне виявлення схеми таблиць
- **Обробка помилок**: Комплексне управління помилками та логування
- **Форматування запитів**: Форматування результатів, зручне для AI
- **Моніторинг стану**: Перевірка підключення до бази даних і стану пулу

## 🔧 Основна реалізація MCP-сервера

### Сервер FastMCP (`sales_analysis.py`)

Тепер реалізуємо основний MCP-сервер:
...
### Основні особливості MCP-сервера

- **Реєстрація інструментів**: Декларативні визначення інструментів із безпекою типів
- **Управління контекстом RLS**: Автоматичне вилучення ідентичності користувача та налаштування контексту
- **Обробка помилок**: Комплексне управління помилками з дружніми повідомленнями
- **Управління життєвим циклом**: Правильний запуск/зупинка з очищенням ресурсів
- **Моніторинг стану**: Вбудовані кінцеві точки перевірки стану
- **Підтримка розробки**: Гаряче перезавантаження та можливості налагодження

## 🏥 Моніторинг стану

### Реалізація перевірки стану (`health_check.py`)
...
2. **Перевірте кінцеві точки стану**:
   ```bash
   # Basic health check
...
### Тестування інтеграції у VS Code

1. **Налаштуйте VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Тестуйте в AI Chat**:
   - Відкрийте AI Chat у VS Code
   - Введіть `#zava` і виберіть ваш сервер
   - Запитайте: "Які таблиці доступні?"
   - Запитайте: "Покажіть мені топ-5 магазинів за кількістю замовлень"

### Юніт-тестування

Створіть комплексні юніт-тести:
...
✅ **Робочий MCP-сервер**: сервер FastMCP із інтеграцією бази даних  
✅ **Управління конфігурацією**: Надійна конфігурація на основі середовища  
✅ **Шар бази даних**: Інтеграція PostgreSQL із пулінгом з'єднань  
✅ **MCP-інструменти**: Інструменти для інспекції схеми та виконання запитів  
✅ **Інтеграція RLS**: Управління контекстом Row Level Security  
✅ **Моніторинг стану**: Комплексні кінцеві точки перевірки стану  
✅ **Стратегія тестування**: Локальне тестування та інтеграція у VS Code  

## 🚀 Що далі

Продовжуйте з **[Практикум 06: Розробка інструментів](../06-Tools/README.md)**, щоб:

- Розширити колекцію MCP-інструментів
- Реалізувати складні шаблони запитів
- Додати перевірку та трансформацію даних
- Створити спеціалізовані аналітичні інструменти

## 📚 Додаткові ресурси

### Фреймворк FastMCP
- [Документація FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Офіційний посібник FastMCP
- [Специфікація MCP](https://modelcontextprotocol.io/docs/) - Специфікація протоколу
- [Посібник з розробки інструментів](https://modelcontextprotocol.io/docs/tools/) - Створення MCP-інструментів

### Інтеграція бази даних
- [Документація asyncpg](https://magicstack.github.io/asyncpg/current/) - Асинхронний драйвер PostgreSQL
- [Найкращі практики пулінгу з'єднань](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Налаштування PostgreSQL
- [Посібник з Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Реалізація RLS

### Шаблони FastAPI
- [Документація FastAPI](https://fastapi.tiangolo.com/) - Довідник веб-фреймворку
- [Ін'єкція залежностей](https://fastapi.tiangolo.com/tutorial/dependencies/) - Шаблони FastAPI
- [Фонові завдання](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Управління асинхронними завданнями

---

**Далі**: Готові розширити ваші інструменти? Продовжуйте з [Практикум 06: Розробка інструментів](../06-Tools/README.md)
