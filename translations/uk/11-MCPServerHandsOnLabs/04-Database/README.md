...
-->
# Проектування бази даних і схема

## 🎯 Що охоплює цей лабораторний практикум

Цей практикум глибоко занурюється в проектування бази даних PostgreSQL для системи Zava Retail. Ви навчитеся реалізовувати комплексну схему роздрібної торгівлі з можливостями пошуку за векторами, багатоклієнтським моделюванням даних і Row Level Security (RLS) для ізоляції даних.

## Огляд

База даних є основою нашого MCP-сервера, зберігаючи дані роздрібної торгівлі для кількох магазинів із суворою ізоляцією даних. Ми використовуємо PostgreSQL із розширенням pgvector для забезпечення семантичного пошуку, що дозволяє клієнтам знаходити товари за допомогою запитів природною мовою.

Наша схема дотримується сучасних багатоклієнтських шаблонів із Row Level Security, що гарантує доступ користувачів лише до даних їхніх авторизованих магазинів. Такий підхід забезпечує безпеку корпоративного рівня, зберігаючи оптимальну продуктивність.

## Цілі навчання

До кінця цього практикуму ви зможете:

- **Проектувати** масштабовані багатоклієнтські схеми баз даних для роздрібної торгівлі
- **Реалізовувати** PostgreSQL із pgvector для пошуку за векторами
- **Налаштовувати** Row Level Security для ізоляції даних
- **Генерувати** реалістичні тестові дані
- **Оптимізувати** продуктивність бази даних для навантажень роздрібної торгівлі
- **Реалізовувати** стратегії резервного копіювання та відновлення

## 🗃️ Архітектура бази даних

### PostgreSQL із pgvector

Наша база даних використовує корпоративні функції PostgreSQL у поєднанні з розширенням pgvector для пошуку, що базується на штучному інтелекті:
...
### Архітектура багатоклієнтської моделі

База даних використовує модель багатоклієнтності **спільна база даних, спільна схема** із Row Level Security:
...
## 📊 Основне проектування схеми

### Таблиця магазинів (Tenant Master)
...
### Таблиця продуктів із категоріями
...
### Таблиця векторних представлень продуктів
...
## 🔐 Налаштування Row Level Security

### Ролі бази даних і дозволи
...
### Скрипт для реалістичних тестових даних
...
✅ **Проектування багатоклієнтської бази даних**: Реалізувати Row Level Security для безпечної ізоляції даних  
✅ **Можливості пошуку за векторами**: Налаштувати pgvector для семантичного пошуку продуктів  
✅ **Комплексна схема**: Створити готову до виробництва схему бази даних для роздрібної торгівлі  
✅ **Генерація тестових даних**: Створити реалістичні тестові дані для розробки та тестування  
✅ **Оптимізація продуктивності**: Налаштувати індекси та оптимізацію запитів  
✅ **Резервне копіювання та відновлення**: Впровадити надійні стратегії захисту даних  

## 🚀 Що далі

Продовжуйте з **[Lab 05: Реалізація MCP-сервера](../05-MCP-Server/README.md)**, щоб:

- Побудувати сервер FastMCP, який підключається до цієї бази даних
- Реалізувати інструменти запитів до бази даних для протоколу MCP
- Додати можливості семантичного пошуку за допомогою векторних представлень
- Налаштувати пулінг з'єднань і обробку помилок

## 📚 Додаткові ресурси

### PostgreSQL і pgvector
- [Документація PostgreSQL](https://www.postgresql.org/docs/) - Повний довідник PostgreSQL
- [Розширення pgvector](https://github.com/pgvector/pgvector) - Пошук за схожістю вектора для PostgreSQL
- [Оптимізація продуктивності PostgreSQL](https://wiki.postgresql.org/wiki/Performance_Optimization) - Найкращі практики оптимізації

### Архітектура багатоклієнтської моделі
- [Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Документація PostgreSQL RLS
- [Архітектура багатоклієнтських даних](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Шаблони архітектури Azure
- [Найкращі практики безпеки баз даних](https://www.postgresql.org/docs/current/security.html) - Посібник із безпеки PostgreSQL

### Векторні бази даних
- [Основи пошуку за векторами](https://www.pinecone.io/learn/vector-database/) - Розуміння векторних баз даних
- [Моделі векторних представлень](https://platform.openai.com/docs/guides/embeddings) - Документація OpenAI про векторні представлення
- [Алгоритм HNSW](https://arxiv.org/abs/1603.09320) - Графи Hierarchical Navigable Small World

---

**Попередній**: [Lab 03: Налаштування середовища](../03-Setup/README.md)  
**Наступний**: [Lab 05: Реалізація MCP-сервера](../05-MCP-Server/README.md)
