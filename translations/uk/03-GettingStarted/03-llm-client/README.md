...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-19T19:23:50+00:00",
...
- Налаштуйте свій токен – Додайте примітку для довідки, встановіть дату закінчення терміну дії та виберіть необхідні області (permissions).
...
- Налаштували екземпляр LLM для використання моделей GitHub, встановивши `baseUrl`, щоб вказати на API інференції.
...
- **Створили клієнта MCP**: Для обробки зв’язку із сервером.
...
Після того, як у вас буде сервер MCP на основі Rust, відкрийте термінал і перейдіть до тієї ж директорії, що й сервер. Потім виконайте наступну команду, щоб створити новий проєкт клієнта LLM:
...
- Створили `McpToolProvider`, який автоматично виявляє та реєструє всі інструменти із сервера MCP.
- Провайдер інструментів обробляє перетворення між схемами інструментів MCP і форматом інструментів LangChain4j внутрішньо.
- Цей підхід абстрагує процес ручного переліку та перетворення інструментів.
...
1. Додайте наступний код для перетворення відповіді від сервера MCP у формат інструментів, зрозумілий для LLM:
...
1. Спочатку створимо наступну функцію-конвертер:
    ...
    У функції `convert_to_llm_tools` ми беремо відповідь інструменту MCP і перетворюємо її у формат, зрозумілий для LLM.
...
- Визначили функціональність, яка створює `FunctionDefinition`, що передається до `ChatCompletionsDefinition`. Останнє є тим, що розуміє LLM.
...
1. Подивимося, як оновити існуючий код, щоб скористатися цією функцією:
    ...
    - Оновили функцію для перетворення відповіді інструменту MCP на інструмент LLM. Ось виділений код, який ми додали:
        ...
        Схема вводу є частиною відповіді інструменту, але знаходиться в атрибуті "properties", тому нам потрібно її витягти. Далі ми викликаємо `ConvertFrom` із деталями інструменту. Тепер, коли ми виконали основну роботу, давайте подивимося, як усе це об'єднується, коли ми обробляємо запит користувача.
...
- Фреймворк автоматично обробляє перетворення схем інструментів і виклики функцій у фоновому режимі.
- Цей підхід усуває необхідність ручного перетворення інструментів — LangChain4j обробляє всю складність перетворення інструментів MCP у формат, сумісний із LLM.

#### Rust

Щоб перетворити відповідь інструменту MCP у формат, зрозумілий для LLM, ми додамо допоміжну функцію, яка форматуватиме список інструментів. Додайте наступний код до вашого файлу `main.rs` під функцією `main`. Це буде викликано під час запитів до LLM:
...
Чудово, тепер ми готові обробляти будь-які запити користувачів, тож давайте перейдемо до цього.
...
Відповідь від LLM міститиме масив `choices`. Нам потрібно обробити результат, щоб перевірити, чи присутні `tool_calls`. Це дозволяє зрозуміти, що LLM запитує виклик певного інструменту з аргументами. Додайте наступний код до кінця вашого файлу `main.rs`, щоб визначити функцію для обробки відповіді LLM:
...
Якщо `tool_calls` присутні, функція витягує інформацію про інструмент, викликає сервер MCP із запитом до інструменту і додає результати до повідомлень розмови. Потім вона продовжує розмову з LLM, а повідомлення оновлюються відповіддю асистента та результатами виклику інструменту.

Щоб витягнути інформацію про виклик інструменту, яку LLM повертає для викликів MCP, ми додамо ще одну допоміжну функцію, яка витягує все необхідне для здійснення виклику. Додайте наступний код до кінця вашого файлу `main.rs`:
...
Це здійснить запит до LLM із початковим запитом користувача, який просить знайти суму двох чисел, і обробить відповідь для динамічного управління викликами інструментів.

Чудово, ви це зробили!

## Завдання

Візьміть код із вправи та розробіть сервер із додатковими інструментами. Потім створіть клієнт із LLM, як у вправі, і протестуйте його з різними запитами, щоб переконатися, що всі ваші серверні інструменти викликаються динамічно. Такий спосіб створення клієнта забезпечує чудовий досвід для кінцевого користувача, оскільки він може використовувати запити, а не точні команди клієнта, і бути не в курсі викликів сервера MCP.
