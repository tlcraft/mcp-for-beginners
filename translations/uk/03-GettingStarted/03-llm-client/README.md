...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T13:04:53+00:00",
...
1. Отримати список можливостей, підказок, ресурсів і інструментів, та зберегти їхню схему.
...
- Виберіть Personal Access Tokens – Клікніть на Personal access tokens, а потім Generate new token.
- Налаштуйте свій токен – Додайте примітку для довідки, встановіть дату закінчення дії та виберіть необхідні області доступу (дозволи).
...
- Створили метод `run`, який відповідає за логіку нашого додатку. Поки що він лише виводить список інструментів, але незабаром ми додамо більше.
...
- Виведення ресурсів і інструментів. Для інструментів ми також виводимо `inputSchema`, який використаємо пізніше.
...
Наступний крок після отримання можливостей сервера — конвертувати їх у формат, який розуміє LLM. Після цього ми можемо надати ці можливості як інструменти нашому LLM.
...
1. Оновимо метод `run`, щоб виводити можливості сервера:
...
1. Оновимо існуючий код, щоб скористатися цією функцією:
    ...
    - Оновили функцію для конвертації відповіді MCP інструменту у інструмент LLM. Виділимо доданий код:
        ...
        Схема вхідних даних є частиною відповіді інструменту, але у властивості "properties", тому її потрібно витягти. Далі ми викликаємо `ConvertFrom` з деталями інструменту. Тепер, коли ми зробили основну роботу, подивимось, як це працює при обробці підказки користувача.
...
1. Останній крок — перевірити, чи LLM вважає, що потрібно викликати функцію:
    ...
    - Для кожного виклику інструменту розібрали ім’я та аргументи і викликали інструмент на MCP сервері за допомогою MCP клієнта. Врешті-решт вивели результати.
...
- Фреймворк LangChain4j автоматично обробляє:
  - Конвертацію підказок користувача у виклики інструментів, коли це потрібно
...
- Цей підхід забезпечує безшовний досвід користувача, де користувачам не потрібно знати про внутрішню реалізацію MCP
