...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-11T10:57:49+00:00",
...
1. A continuación, actualicemos nuestro código de cliente para aprovechar esta función de la siguiente manera:
...
1. A continuación, agreguemos la función que llamará al LLM:
...
Agregaremos múltiples llamadas al LLM, así que definamos una función que manejará la llamada al LLM. Añade la siguiente función a tu archivo `main.rs`:
...
Esta función toma el cliente LLM, una lista de mensajes (incluido el mensaje del usuario), herramientas del servidor MCP, y envía una solicitud al LLM, devolviendo la respuesta.
...
Si hay `tool_calls` presentes, se extrae la información de la herramienta, se llama al servidor MCP con la solicitud de la herramienta y se añaden los resultados a los mensajes de la conversación. Luego, se continúa la conversación con el LLM y los mensajes se actualizan con la respuesta del asistente y los resultados de la llamada a la herramienta.
...
- Añadir un LLM a tu cliente proporciona una mejor manera para que los usuarios interactúen con los servidores MCP.
