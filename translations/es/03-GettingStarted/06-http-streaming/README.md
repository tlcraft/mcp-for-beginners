...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-17T21:51:35+00:00",
...
- **Streamable HTTP**: Transporte de transmisión moderno basado en HTTP, que soporta notificaciones y mejor escalabilidad. Recomendado para la mayoría de escenarios de producción y en la nube.
...
Comprender los conceptos fundamentales y la motivación detrás de la transmisión es esencial para implementar sistemas de comunicación en tiempo real efectivos.
...
- Cómputos de larga duración donde se quiere mantener informado al usuario.

Esto es lo que necesitas saber sobre la transmisión a alto nivel:

- Los datos se entregan de forma progresiva, no todos a la vez.
...
There are some things we recommend when it comes to choosing between implementing classical streaming (as an endpoint we showed you above using `/stream`) en lugar de elegir transmisión vía MCP.
...
Bien, ya has visto algunas recomendaciones y comparaciones sobre la diferencia entre la transmisión clásica y la transmisión en MCP. Ahora entremos en detalle sobre cómo puedes aprovechar la transmisión en MCP.
...
En MCP, la transmisión no se trata de enviar la respuesta principal en fragmentos, sino de enviar **notificaciones** al cliente mientras una herramienta procesa una solicitud. Estas notificaciones pueden incluir actualizaciones de progreso, registros u otros eventos.
...
Para que el registro funcione, el servidor necesita habilitarlo como una característica/capacidad de esta forma:
...
> [!NOTE]
> Dependiendo del SDK utilizado, el registro podría estar habilitado por defecto, o puede que necesites activarlo explícitamente en la configuración del servidor.
...
Para implementar notificaciones en MCP, necesitas configurar tanto el servidor como el cliente para manejar actualizaciones en tiempo real. Esto permite que tu aplicación proporcione retroalimentación inmediata a los usuarios durante operaciones prolongadas.

### Lado servidor: Enviar notificaciones

Comencemos con el lado servidor. En MCP, defines herramientas que pueden enviar notificaciones mientras procesan solicitudes. El servidor usa el objeto contexto (usualmente `ctx`) para enviar mensajes al cliente.
...
- **Depuración y monitoreo más fáciles:** Desarrolladores y usuarios pueden ver dónde un proceso puede estar lento o bloqueado.
...
### Tarea: Construye tu Propia Aplicación MCP con Transmisión

**Escenario:**
Construye un servidor y cliente MCP donde el servidor procese una lista de ítems (por ejemplo, archivos o documentos) y envíe una notificación por cada ítem procesado. El cliente debe mostrar cada notificación conforme llegue.
...
Para continuar tu camino con la transmisión MCP y ampliar tu conocimiento, esta sección ofrece recursos adicionales y pasos sugeridos para construir aplicaciones más avanzadas.

### Lecturas Adicionales

- [Microsoft: Introducción a la Transmisión HTTP](https://learn.microsoft.com/aspnet/core/fundamentals/http-requests?view=aspnetcore-8.0&WT.mc_id=%3Fwt.mc_id%3DMVP_452430#streaming)
...
- Explora la integración de la transmisión MCP con frameworks frontend (React, Vue, etc.) para actualizaciones en vivo de la UI.
