...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T12:23:50+00:00",
...
Večmodalne aplikacije postajajo vse pomembnejše v umetni inteligenci, saj omogočajo bogatejše interakcije in zahtevnejše naloge. Protokol Model Context (MCP) ponuja okvir za gradnjo večmodalnih aplikacij, ki lahko obdelujejo različne vrste podatkov, kot so besedilo, slike in zvok.

MCP podpira ne le interakcije na osnovi besedila, temveč tudi večmodalne zmogljivosti, ki omogočajo modelom delo s slikami, zvokom in drugimi vrstami podatkov.

## Uvod

V tej lekciji se boste naučili, kako zgraditi večmodalno aplikacijo.

## Cilji učenja

Do konca te lekcije boste znali:

- Razumeti večmodalne možnosti
- Implementirati večmodalno aplikacijo.

## Arhitektura za podporo večmodalnosti

Večmodalne implementacije MCP običajno vključujejo:

- **Parserje specifične za modaliteto**: Komponente, ki pretvarjajo različne vrste medijev v formate, ki jih model lahko obdela.
- **Orodja specifična za modaliteto**: Posebna orodja, namenjena obdelavi določenih modalitet (analiza slik, obdelava zvoka)
- **Enotno upravljanje konteksta**: Sistem za vzdrževanje konteksta med različnimi modalitetami
- **Generiranje odzivov**: Zmožnost ustvarjanja odzivov, ki lahko vključujejo več modalitet.

## Večmodalni primer: analiza slike

V spodnjem primeru bomo analizirali sliko in iz nje izvlekli informacije.
...
- Ustvarili `ImageAnalysisTool`, ki lahko analizira slike z uporabo hipotetične storitve `IImageAnalysisService`.
- Konfigurirali MCP strežnik za obdelavo večjih zahtev in podporo vsebinam slik.
- Registrirali orodje za analizo slik na strežniku.
- Implementirali metodo za prenos slik z URL-ja in njihovo analizo glede na zahtevano vrsto (predmeti, besedilo, obrazi itd.).
- Vrnil strukturirane rezultate v formatu, skladnem s specifikacijo MCP.

## Večmodalni primer: obdelava zvoka

Obdelava zvoka je še ena pogosta modaliteta v večmodalnih aplikacijah. Spodaj je primer, kako implementirati orodje za prepis zvoka, ki lahko obdeluje zvočne datoteke in vrača prepise.
...
- Ustvarili `AudioTranscriptionTool`, ki lahko prepisuje zvočne datoteke.
- Določili shemo orodja, ki sprejema bodisi URL ali base64 kodirane zvočne podatke.
- Implementirali metodo `execute` za obdelavo zvoka in prepisovanje.
- Konfigurirali MCP strežnik za obdelavo večmodalnih zahtev, vključno z obdelavo zvoka in slik.
- Registrirali orodje za prepis zvoka na strežniku.
- Implementirali metodo za prenos zvočnih datotek z URL-ja ali dekodiranje base64 zvočnih podatkov.
- Uporabili storitev `AudioProcessor` za dejansko logiko prepisovanja.
- Zagnali MCP strežnik, da posluša zahteve.

### Večmodalni primer: generiranje večmodalnih odzivov
