...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-19T18:04:57+00:00",
...
- **Upravljanje dovoljenj**: Implementirajte sisteme z natančno določenimi dovoljenji, ki uporabnikom omogočajo nadzor nad tem, kateri strežniki, orodja in viri so dostopni.
...
- **Pogajanje o zmogljivostih**: Med inicializacijo se dogovorijo o podprtih funkcijah in različicah protokola s strežniki.
...
- **Obdelava zahtevkov**: Sprejemanje in izvajanje klicev orodij, zahtevkov za vire in pozive odjemalcev.
- **Zagotavljanje konteksta**: Zagotavljanje kontekstualnih informacij in podatkov za izboljšanje odgovorov modela.
- **Upravljanje stanja**: Vzdrževanje stanja seje in obravnava interakcij, ki zahtevajo stanje.
...
**Vzorčenje** omogoča strežnikom, da zahtevajo dopolnitve jezikovnega modela iz AI aplikacije odjemalca. Ta primitiv omogoča strežnikom dostop do zmogljivosti LLM brez lastnih odvisnosti od modela:
...
- **AI na pobudo strežnika**: Omogoča strežnikom avtonomno generiranje vsebin z uporabo AI modela odjemalca.
...
- **Potrditveni dialogi**: Zahteva uporabnikovo odobritev za občutljive ali pomembne operacije.
- **Interaktivni delovni tokovi**: Omogoča strežnikom ustvarjanje korak-po-korak interakcij z uporabnikom.
- **Zbiranje dinamičnih parametrov**: Zbira manjkajoče ali izbirne parametre med izvajanjem orodij.
...
- **Uporabniška zahteva**  
  Uporabnik komunicira z gostiteljem (npr. vnese poziv ali ukaz).
...
- **Strežniške prvine**: Omogoča strežnikom zagotavljanje osnovne funkcionalnosti prek orodij, virov in predlog
- **Odjemalske prvine**: Omogoča strežnikom zahtevanje vzorčenja iz LLM-jev, pridobivanje uporabniškega vnosa in pošiljanje dnevniških sporočil
...
S spoštovanjem teh varnostnih načel MCP zagotavlja zaupanje, zasebnost in varnost uporabnikov pri vseh interakcijah protokola, hkrati pa omogoča zmogljive integracije AI.
...
1. **Nadzor dovoljenj za orodja**:  
   Odjemalci lahko določijo, katera orodja model lahko uporablja med sejo. To zagotavlja, da so dostopna le izrecno odobrena orodja, kar zmanjšuje tveganje nenamernih ali nevarnih operacij. Dovoljenja je mogoče dinamično konfigurirati glede na uporabniške preference, organizacijske politike ali kontekst interakcije.
...
- **`prompts/get` Zahteva**: Pridobi predlogo poziva z opcionalnimi parametri
...
- **Zahtevami**: Vključujejo `id`, `method` in opcionalne `params`
- **Odgovori**: Vključujejo `id` in bodisi `result` ali `error`  
- **Obvestili**: Vključujejo `method` in opcionalne `params` (brez `id` ali pričakovanega odgovora)
...
- **Osnovne prvine**: Strežniki izpostavljajo orodja (izvedljive funkcije), vire (podatkovne vire) in predloge (template)
- **Odjemalske prvine**: Strežniki lahko zahtevajo vzorčenje (LLM dokončanja), pridobivanje (uporabniški vnos) in dnevniško beleženje od odjemalcev
