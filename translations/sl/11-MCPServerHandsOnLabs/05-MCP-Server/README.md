...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:02:19+00:00",
  "source_file": "11-MCPServerHandsOnLabs/05-MCP-Server/README.md",
  "language_code": "sl"
}
-->
# Implementacija MCP strežnika

## 🎯 Kaj zajema ta laboratorij

Ta praktični laboratorij vas vodi skozi implementacijo produkcijsko pripravljenega MCP strežnika z uporabo ogrodja FastMCP. Zgradili boste osnovno strukturo strežnika, izvedli integracijo z bazo podatkov, ustvarili orodja za dostop do podatkov in vzpostavili temelje za analitiko v trgovini, ki jo poganja umetna inteligenca.

## Pregled

MCP strežnik je srce naše rešitve za analitiko v trgovini. Deluje kot most med AI pomočniki in PostgreSQL bazo podatkov, zagotavlja varen in inteligenten dostop do poslovnih podatkov prek standardiziranega protokola.

Ta laboratorij vas uči, kako zgraditi robusten in razširljiv MCP strežnik, ki sledi vzorcem in najboljšim praksam za podjetja.

## Cilji učenja

Do konca tega laboratorija boste sposobni:

- **Zgraditi** FastMCP strežnik z ustrezno arhitekturo in organizacijo
- **Izvesti** integracijo z bazo podatkov z uporabo povezovalnih bazenov in obravnavo napak
- **Ustvariti** MCP orodja za introspekcijo sheme baze podatkov in izvajanje poizvedb
- **Konfigurirati** upravljanje konteksta varnosti na ravni vrstic (Row Level Security)
- **Dodati** funkcije za spremljanje zdravja in opazovanje
- **Testirati** implementacijo MCP strežnika lokalno in z uporabo VS Code

## 📁 Struktura projekta

Poglejmo organizacijo MCP strežnika:
...
### Konfiguracija okolja (`config.py`)

Najprej ustvarimo robusten sistem konfiguracije:
...
### Ključne značilnosti konfiguracije

- **Nalaganje spremenljivk okolja**: Samodejna podpora za datoteke .env
- **Tipna varnost**: Validacija dataclass in namigi tipov
- **Prilagodljivi privzeti parametri**: Smiselni privzeti parametri za razvoj
- **Validacija**: Validacija konfiguracije z uporabnimi sporočili o napakah
- **Varnost**: Občutljive vrednosti samo iz spremenljivk okolja

## 🗄️ Plast za integracijo z bazo podatkov

### PostgreSQL ponudnik (`sales_analysis_postgres.py`)

Izvedimo plast za integracijo z bazo podatkov:
...
### Ključne značilnosti plasti baze podatkov

- **Povezovalni bazeni**: Učinkovito upravljanje virov z asyncpg
- **Integracija RLS**: Samodejno nastavljanje konteksta varnosti na ravni vrstic
- **Introspekcija sheme**: Dinamično odkrivanje sheme tabel
- **Obravnava napak**: Celovito upravljanje napak in beleženje
- **Formatiranje poizvedb**: Rezultati, prijazni za AI
- **Spremljanje zdravja**: Preverjanje povezljivosti baze podatkov in stanja bazenov

## 🔧 Glavna implementacija MCP strežnika

### FastMCP strežnik (`sales_analysis.py`)

Zdaj izvedimo glavni MCP strežnik:
...
### Ključne značilnosti MCP strežnika

- **Registracija orodij**: Deklarativne definicije orodij z varnostjo tipov
- **Upravljanje konteksta RLS**: Samodejno pridobivanje identitete uporabnika in nastavljanje konteksta
- **Obravnava napak**: Celovito upravljanje napak z uporabniku prijaznimi sporočili
- **Upravljanje življenjskega cikla**: Pravilno zaganjanje/ustavljanje z čiščenjem virov
- **Spremljanje zdravja**: Vgrajene končne točke za preverjanje zdravja
- **Podpora za razvoj**: Samodejno ponovno nalaganje in zmožnosti za odpravljanje napak

## 🏥 Spremljanje zdravja

### Implementacija preverjanja zdravja (`health_check.py`)
...
2. **Testirajte končne točke za zdravje**:
   ```bash
   # Basic health check
...
### Testiranje integracije z VS Code

1. **Konfigurirajte VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Testirajte v AI klepetu**:
   - Odprite VS Code AI klepet
   - Vnesite `#zava` in izberite svoj strežnik
   - Vprašajte: "Katere tabele so na voljo?"
   - Vprašajte: "Pokaži mi top 5 trgovin po številu naročil"

### Enotestiranje

Ustvarite celovite enotestne primere:
...
✅ **Delujoč MCP strežnik**: FastMCP strežnik z integracijo baze podatkov  
✅ **Upravljanje konfiguracije**: Robustna konfiguracija, ki temelji na okolju  
✅ **Plast baze podatkov**: Integracija PostgreSQL z povezovalnimi bazeni  
✅ **MCP orodja**: Orodja za introspekcijo sheme in izvajanje poizvedb  
✅ **Integracija RLS**: Upravljanje konteksta varnosti na ravni vrstic  
✅ **Spremljanje zdravja**: Celovite končne točke za preverjanje zdravja  
✅ **Strategija testiranja**: Lokalno testiranje in integracija z VS Code  

## 🚀 Kaj sledi

Nadaljujte z **[Lab 06: Razvoj orodij](../06-Tools/README.md)**, da:

- Razširite svojo zbirko MCP orodij
- Izvedete napredne vzorce poizvedb
- Dodate validacijo in transformacijo podatkov
- Ustvarite specializirana analitična orodja

## 📚 Dodatni viri

### FastMCP ogrodje
- [Dokumentacija FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Uradni vodič za FastMCP
- [Specifikacija MCP](https://modelcontextprotocol.io/docs/) - Specifikacija protokola
- [Vodič za razvoj orodij](https://modelcontextprotocol.io/docs/tools/) - Ustvarjanje MCP orodij

### Integracija baze podatkov
- [Dokumentacija asyncpg](https://magicstack.github.io/asyncpg/current/) - Asinhroni gonilnik za PostgreSQL
- [Najboljše prakse za povezovalne bazene](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Nastavitve PostgreSQL
- [Vodič za varnost na ravni vrstic](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementacija RLS

### Vzorci FastAPI
- [Dokumentacija FastAPI](https://fastapi.tiangolo.com/) - Referenca za spletno ogrodje
- [Odvisnostne injekcije](https://fastapi.tiangolo.com/tutorial/dependencies/) - Vzorci FastAPI
- [Ozadna opravila](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Upravljanje asinhronih opravil

---

**Naprej**: Pripravljeni na razširitev orodij? Nadaljujte z [Lab 06: Razvoj orodij](../06-Tools/README.md)
