...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-13T02:20:18+00:00",
...
Όταν δημιουργείς έναν AI agent, δεν πρόκειται μόνο για τη δημιουργία έξυπνων απαντήσεων· είναι επίσης σημαντικό να δίνεις στον agent τη δυνατότητα να αναλαμβάνει δράση. Εδώ έρχεται το Model Context Protocol (MCP). Το MCP διευκολύνει την πρόσβαση των agents σε εξωτερικά εργαλεία και υπηρεσίες με έναν συνεπή τρόπο. Σκέψου το σαν να συνδέεις τον agent σου σε ένα κουτί εργαλείων που μπορεί *πραγματικά* να χρησιμοποιήσει.

Ας πούμε ότι συνδέεις έναν agent με τον MCP server του calculator. Ξαφνικά, ο agent μπορεί να εκτελεί μαθηματικές πράξεις απλά λαμβάνοντας ένα prompt όπως «Πόσο κάνει 47 επί 89;»—χωρίς να χρειάζεται να γράψεις λογική ή να φτιάξεις ειδικά APIs.

## Επισκόπηση

Αυτό το μάθημα καλύπτει πώς να συνδέσεις έναν calculator MCP server με έναν agent χρησιμοποιώντας την επέκταση [AI Toolkit](https://aka.ms/AIToolkit) στο Visual Studio Code, επιτρέποντας στον agent σου να εκτελεί μαθηματικές πράξεις όπως πρόσθεση, αφαίρεση, πολλαπλασιασμό και διαίρεση μέσω φυσικής γλώσσας.

Το AI Toolkit είναι μια ισχυρή επέκταση για το Visual Studio Code που απλοποιεί την ανάπτυξη agents. Οι AI Engineers μπορούν εύκολα να δημιουργούν εφαρμογές AI αναπτύσσοντας και δοκιμάζοντας γενετικά μοντέλα AI—τοπικά ή στο cloud. Η επέκταση υποστηρίζει τα περισσότερα από τα βασικά γενετικά μοντέλα που είναι διαθέσιμα σήμερα.

*Σημείωση*: Το AI Toolkit υποστηρίζει προς το παρόν Python και TypeScript.

## Μαθησιακοί Στόχοι

Στο τέλος αυτού του μαθήματος, θα μπορείς να:

- Καταναλώνεις έναν MCP server μέσω του AI Toolkit.
- Διαμορφώνεις μια agent configuration ώστε να μπορεί να ανακαλύπτει και να χρησιμοποιεί εργαλεία που παρέχονται από τον MCP server.
- Χρησιμοποιείς εργαλεία MCP μέσω φυσικής γλώσσας.

## Προσέγγιση

Ακολουθεί μια γενική εικόνα της προσέγγισης:

- Δημιουργία ενός agent και ορισμός του system prompt.
- Δημιουργία ενός MCP server με εργαλεία calculator.
...
### -0- Προβήμα, πρόσθεσε το μοντέλο OpenAI GPT-4o στα My Models

Η άσκηση χρησιμοποιεί το μοντέλο **GPT-4o**. Πρέπει να προστεθεί στα **My Models** πριν τη δημιουργία του agent.
...
1. Άνοιξε την επέκταση **AI Toolkit** από το **Activity Bar**.
1. Στην ενότητα **Catalog**, επίλεξε **Models** για να ανοίξεις το **Model Catalog**. Η επιλογή **Models** ανοίγει το **Model Catalog** σε νέα καρτέλα του editor.
1. Στη γραμμή αναζήτησης του **Model Catalog**, γράψε **OpenAI GPT-4o**.
1. Πάτησε **+ Add** για να προσθέσεις το μοντέλο στη λίστα **My Models**. Βεβαιώσου ότι έχεις επιλέξει το μοντέλο που είναι **Hosted by GitHub**.
1. Στο **Activity Bar**, επιβεβαίωσε ότι το μοντέλο **OpenAI GPT-4o** εμφανίζεται στη λίστα.

### -1- Δημιουργία agent

Ο **Agent (Prompt) Builder** σου επιτρέπει να δημιουργήσεις και να προσαρμόσεις τους δικούς σου agents με AI. Σε αυτή την ενότητα, θα δημιουργήσεις έναν νέο agent και θα του αναθέσεις ένα μοντέλο που θα τροφοδοτεί τη συνομιλία.
...
1. Άνοιξε την επέκταση **AI Toolkit** από το **Activity Bar**.
1. Στην ενότητα **Tools**, επίλεξε **Agent (Prompt) Builder**. Η επιλογή ανοίγει το **Agent (Prompt) Builder** σε νέα καρτέλα editor.
...
1. Στο **Agent (Prompt) Builder**, στο πεδίο **Model**, επίλεξε το μοντέλο **OpenAI GPT-4o (via GitHub)**.

### -2- Δημιουργία system prompt για τον agent

Τώρα που ο agent έχει τη βασική δομή, ήρθε η ώρα να ορίσεις την προσωπικότητα και το σκοπό του. Σε αυτή την ενότητα, θα χρησιμοποιήσεις τη λειτουργία **Generate system prompt** για να περιγράψεις τη συμπεριφορά του agent—στην προκειμένη περίπτωση, έναν calculator agent—και το μοντέλο θα δημιουργήσει το system prompt για σένα.
...
1. Στην ενότητα **Prompts**, πάτησε το κουμπί **Generate system prompt**. Αυτό ανοίγει τον prompt builder που χρησιμοποιεί AI για να δημιουργήσει το system prompt του agent.
...
1. Πάτησε το κουμπί **Generate**. Θα εμφανιστεί ειδοποίηση κάτω δεξιά που επιβεβαιώνει ότι το system prompt δημιουργείται. Όταν ολοκληρωθεί, το prompt θα εμφανιστεί στο πεδίο **System prompt** του **Agent (Prompt) Builder**.
1. Δες το **System prompt** και τροποποίησέ το αν χρειάζεται.

### -3- Δημιουργία MCP server

Τώρα που έχεις ορίσει το system prompt του agent—που καθοδηγεί τη συμπεριφορά και τις απαντήσεις του—ήρθε η ώρα να τον εξοπλίσεις με πρακτικές δυνατότητες. Σε αυτή την ενότητα, θα δημιουργήσεις έναν calculator MCP server με εργαλεία για πρόσθεση, αφαίρεση, πολλαπλασιασμό και διαίρεση. Αυτός ο server θα επιτρέψει στον agent να εκτελεί μαθηματικούς υπολογισμούς σε πραγματικό χρόνο, απαντώντας σε prompts φυσικής γλώσσας.
...
Το AI Toolkit διαθέτει πρότυπα για να διευκολύνει τη δημιουργία του δικού σου MCP server. Θα χρησιμοποιήσουμε το πρότυπο Python για τη δημιουργία του calculator MCP server.
...
1. Θα ανοίξει νέο παράθυρο Visual Studio Code. Επίλεξε **Yes, I trust the authors**.
1. Μέσω του τερματικού (**Terminal** > **New Terminal**), δημιούργησε ένα virtual environment: `python -m venv .venv`
1. Μέσω του τερματικού, ενεργοποίησε το virtual environment:
    1. Windows - `.venv\Scripts\activate`
    1. macOS/Linux - `source venv/bin/activate`
1. Μέσω του τερματικού, εγκατέστησε τις εξαρτήσεις: `pip install -e .[dev]`
1. Στην προβολή **Explorer** του **Activity Bar**, άνοιξε τον φάκελο **src** και επίλεξε το αρχείο **server.py** για να το ανοίξεις στον editor.
...
### -4- Τρέξε τον agent με τον calculator MCP server

Τώρα που ο agent έχει εργαλεία, ήρθε η ώρα να τα χρησιμοποιήσεις! Σε αυτή την ενότητα, θα στείλεις prompts στον agent για να δοκιμάσεις και να επιβεβαιώσεις αν ο agent χρησιμοποιεί το σωστό εργαλείο από τον calculator MCP server.
...
Θα τρέξεις τον calculator MCP server στον τοπικό σου υπολογιστή ως MCP client μέσω του **Agent Builder**.

1. Πάτησε `F5` to start debugging the MCP server. The **Agent (Prompt) Builder** will open in a new editor tab. The status of the server is visible in the terminal.
...
1. Here's a breakdown of what should occur:
    ...
    - Η απάντηση από κάθε εργαλείο εμφανίζεται στην αντίστοιχη **Tool Response**.
    - Το τελικό αποτέλεσμα του μοντέλου εμφανίζεται στο **Model Response**.
1. Στείλε επιπλέον prompts για να δοκιμάσεις τον agent. Μπορείς να τροποποιήσεις το υπάρχον prompt στο πεδίο **User prompt** κάνοντας κλικ μέσα και αντικαθιστώντας το.
1. Όταν τελειώσεις τις δοκιμές, μπορείς να σταματήσεις τον server μέσω του **τερματικού** με **CTRL/CMD+C**.

## Ανάθεση

Πρόσθεσε ένα επιπλέον εργαλείο στο αρχείο **server.py** (π.χ. να επιστρέφει την τετραγωνική ρίζα ενός αριθμού). Στείλε νέα prompts που θα απαιτούν από τον agent να χρησιμοποιήσει το νέο εργαλείο (ή τα υπάρχοντα). Φρόντισε να κάνεις επανεκκίνηση του server για να φορτωθούν τα νέα εργαλεία.
...
- Η επέκταση AI Toolkit είναι ένας εξαιρετικός client που σου επιτρέπει να καταναλώνεις MCP Servers και τα εργαλεία τους.
- Μπορείς να προσθέτεις νέα εργαλεία σε MCP servers, επεκτείνοντας τις δυνατότητες του agent ώστε να καλύπτουν τις εξελισσόμενες ανάγκες.
...
## Τι Ακολουθεί
- Επόμενο: [Testing & Debugging](/03-GettingStarted/08-testing/README.md)
