...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-02T10:15:20+00:00",
...
Αυτός ο οδηγός δείχνει πώς να ενσωματώσετε διακομιστές Model Context Protocol (MCP) με τους agents του Azure AI Foundry, επιτρέποντας ισχυρή ορχήστρωση εργαλείων και δυνατότητες AI για επιχειρήσεις.

## Εισαγωγή

Το Model Context Protocol (MCP) είναι ένα ανοιχτό πρότυπο που επιτρέπει στις εφαρμογές AI να συνδέονται με ασφάλεια με εξωτερικές πηγές δεδομένων και εργαλεία. Όταν ενσωματώνεται με το Azure AI Foundry, το MCP δίνει τη δυνατότητα στους agents να έχουν πρόσβαση και να αλληλεπιδρούν με διάφορες εξωτερικές υπηρεσίες, APIs και πηγές δεδομένων με έναν τυποποιημένο τρόπο.

Αυτή η ενσωμάτωση συνδυάζει την ευελιξία του οικοσυστήματος εργαλείων του MCP με το ισχυρό πλαίσιο agent του Azure AI Foundry, προσφέροντας λύσεις AI επιπέδου επιχείρησης με εκτεταμένες δυνατότητες παραμετροποίησης.

**Note:** Αν θέλετε να χρησιμοποιήσετε MCP στο Azure AI Foundry Agent Service, προς το παρόν υποστηρίζονται μόνο οι εξής περιοχές: westus, westus2, uaenorth, southindia και switzerlandnorth
...
- **Τυποποιημένη Ενσωμάτωση**: Ομοιόμορφη διεπαφή μεταξύ διαφορετικών εργαλείων και υπηρεσιών
- **Ασφάλεια**: Ασφαλείς μηχανισμοί πιστοποίησης και εξουσιοδότησης
- **Ευελιξία**: Υποστήριξη διαφόρων πηγών δεδομένων, APIs και προσαρμοσμένων εργαλείων
...
Πρώτα, ρυθμίστε τις μεταβλητές περιβάλλοντος και τις εξαρτήσεις σας:
...
with project_client:
    ...
    print(f"Δημιουργήθηκε ο agent, ID agent: {agent.id}")
```
...
mcp_tool = {
    ...
    "server_url": "https://api.example.com/mcp", # Σημείο πρόσβασης MCP διακομιστή
    "require_approval": "never"                 # Πολιτική έγκρισης: προς το παρόν υποστηρίζεται μόνο "never" 
}
```
...
def create_mcp_agent_example():
    ...
    with project_client:
        ...
        print(f"Δημιουργήθηκε νήμα, ID νήματος: {thread.id}")
        ...
        print(f"Δημιουργήθηκε μήνυμα, ID μηνύματος: {message.id}")
        ...
        for step in run_steps:
            ...
            if step.type == "tool_calls":
                print("Λεπτομέρειες κλήσης εργαλείων:")
...
- Επαληθεύστε ότι η διεύθυνση URL του MCP διακομιστή είναι προσβάσιμη
- Ελέγξτε τα διαπιστευτήρια πιστοποίησης
- Βεβαιωθείτε για τη δικτυακή συνδεσιμότητα

### 2. Αποτυχίες Κλήσεων Εργαλείων
- Επανεξετάστε τα επιχειρήματα και τη μορφοποίηση των εργαλείων
- Ελέγξτε τις απαιτήσεις του συγκεκριμένου διακομιστή
...
1. **Εξερευνήστε Προσαρμοσμένους MCP Διακομιστές**: Δημιουργήστε δικούς σας MCP διακομιστές για ιδιωτικές πηγές δεδομένων
