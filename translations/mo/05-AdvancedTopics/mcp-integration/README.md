...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T23:58:23+00:00",
...
Azure OpenAI 提供存取 GPT-4 等強大 AI 模型的能力。將 MCP 與 Azure OpenAI 整合，讓你在享有 MCP 工具協調彈性的同時，使用這些模型。
...
- 建立 `GetCompletionWithToolsAsync` 方法以取得支援工具的完成結果。
- 處理回應中的工具呼叫。
...
- 將 Azure ML 資料型別對應到 JSON schema 型別以供工具註冊使用。
- 採用非同步程式設計，處理可能耗時的操作，如 ML 管線執行與模型註冊。
