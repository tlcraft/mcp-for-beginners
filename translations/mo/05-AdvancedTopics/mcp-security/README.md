...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T14:34:08+00:00",
...
> **目前標準**：本指南反映 [MCP 規範 2025-06-18](https://spec.modelcontextprotocol.io/specification/2025-06-18/) 的安全要求以及官方 [MCP 安全最佳實踐](https://modelcontextprotocol.io/specification/2025-06-18/basic/security_best_practices)。

安全性對於 MCP 的實施至關重要，尤其是在企業環境中。本高級指南探討了針對生產環境 MCP 部署的全面安全實踐，涵蓋了傳統安全問題以及 MCP 特有的 AI 威脅。

## 簡介

模型上下文協議（MCP）帶來了超越傳統軟體安全的獨特安全挑戰。隨著 AI 系統獲得工具、數據和外部服務的訪問權限，新的攻擊向量也隨之出現，包括提示注入、工具污染、會話劫持、混淆代理問題以及令牌傳遞漏洞。

本課程探討基於最新 MCP 規範（2025-06-18）、Microsoft 安全解決方案以及企業安全模式的高級安全實施。
...
- **防止 AI 特有攻擊**：使用 Microsoft Prompt Shields 和 Azure Content Safety 防範提示注入、工具污染和會話劫持
- **應用企業安全**：為生產環境 MCP 部署實施全面的日誌記錄、監控和事件響應  
- **保護工具執行**：設計具有適當隔離和資源控制的沙盒執行環境
- **解決 MCP 漏洞**：識別並減輕混淆代理問題、令牌傳遞漏洞和供應鏈風險
- **整合 Microsoft 安全**：利用 Azure 安全服務和 GitHub 高級安全功能進行全面保護
...
現代 MCP 實施受益於規範向外部身份提供者委派的演進，這顯著改善了安全狀態，相較於自定義身份驗證實施。

### **Microsoft Entra ID 集成**

最新 MCP 規範（2025-06-18）允許委派至外部身份提供者，例如 Microsoft Entra ID，提供企業級安全功能：
...
### 使用 Java Spring Security 與 OAuth 2.1 的集成

遵循 MCP 規範要求的 OAuth 2.1 安全模式的增強 Spring Security 實施：
...
### **使用 Azure Application Insights 的全面日誌記錄**
...
- **整合的安全狀態**：統一的身份、基礎設施和應用安全
- **高級 AI 保護**：針對 AI 特有威脅的專用防禦  
- **企業合規性**：內建支持監管要求和行業標準
- **威脅情報**：全球威脅情報集成以進行主動保護
- **可擴展架構**：企業級擴展同時保持安全控制
...
> **安全提示**：本高級實施指南反映了當前 MCP 規範（2025-06-18）的要求。在實施這些控制時，請始終核對最新的官方文檔並考慮您的特定安全需求和威脅模型。
