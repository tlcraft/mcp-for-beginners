...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-17T15:28:43+00:00",
...
قم بتمكين خادم التفويض وخادم الموارد عن طريق تعريف سلسلة فلتر الأمان. على سبيل المثال:
...
هذا يكشف `/.well-known/openid-configuration`, which APIM can use for metadata. Finally, you may want to customize the JWT **audience** claim so that APIM’s `<audiences>` تحقق سوف يمر. على سبيل المثال، أضف مخصصًا للرمز:
...
In Azure APIM, add an inbound policy that uses the `<validate-jwt>` سياسة للتحقق من الرموز المميزة الواردة ضد خادم التفويض Spring الخاص بك. للحصول على إعداد بسيط، يمكنك استخدام عنوان URL لبيانات OpenID Connect. مثال مقتطف السياسة:
...
**References:** See Spring Authorization Server docs for default endpoints ([Configuration Model :: Spring Authorization Server](https://docs.spring.io/spring-authorization-server/reference/configuration-model.html#:~:text=public%20static%20Builder%20builder%28%29%20,oauth2%2Fauthorize)) and OIDC configuration ([Configuration Model :: Spring Authorization Server](https://docs.spring.io/spring-authorization-server/reference/configuration-model.html#:~:text=.securityMatcher%28authorizationServerConfigurer.getEndpointsMatcher%28%29%29%20.with%28authorizationServerConfigurer%2C%20%28authorizationServer%29%20,%29%3B%20return%20http.build)); see Microsoft APIM docs for `validate-jwt` أمثلة ([مرجع سياسة إدارة واجهة برمجة التطبيقات Azure - التحقق من صحة JWT | تعلم Microsoft](https://learn.microsoft.com/en-us/azure/api-management/validate-jwt-policy#:~:text=Microsoft%20Entra%20ID%20single%20tenant,token%20validation)); ووثائق Azure Container Apps للنشر والشهادات ([نشر تطبيقات Java Spring Boot على Azure Container Apps - Java على Azure | تعلم Microsoft](https://learn.microsoft.com/en-us/azure/developer/java/identity/deploy-spring-boot-to-azure-container-apps#:~:text=Now%20you%20can%20deploy%20your,CLI%20command)) ([أسماء النطاقات المخصصة والشهادات المدارة المجانية في Azure Container Apps | تعلم Microsoft](https://learn.microsoft.com/en-us/azure/container-apps/custom-domains-managed-certificates#:~:text=Free%20certificate%20requirements)).

It seems there might be a typo in your request, as "mo" is not recognized as a language code. Could you please clarify the language you would like the text translated into? If you meant "Mongolian," please confirm, and I'll be happy to assist!