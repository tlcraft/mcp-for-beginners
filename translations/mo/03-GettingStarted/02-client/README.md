...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T09:42:06+00:00",
...
客戶端是自訂的應用程式或腳本，直接與 MCP Server 通訊以請求資源、工具和提示。與使用檢查器工具（提供圖形介面與伺服器互動）不同，撰寫自己的客戶端可實現程式化和自動化的互動。這讓開發者能將 MCP 功能整合到自己的工作流程中，自動化任務，並打造符合特定需求的客製化解決方案。
...
- **將客戶端整合到主機應用程式**。了解伺服器功能後，需將其整合到主機應用程式中，讓使用者輸入提示或指令時，能呼叫對應的伺服器功能。
...
Java 部分，你會建立一個客戶端連接到前面練習中的 MCP 伺服器。使用 [Getting Started with MCP Server](../../../../03-GettingStarted/01-first-server/solution/java) 中相同的 Java Spring Boot 專案結構，在 `src/main/java/com/microsoft/mcp/sample/client/` 資料夾新增一個名為 `SDKClient` 的 Java 類別，並加入以下匯入：
...
現在我們有一個能連接的客戶端，但還沒列出功能，接著來做這件事：
...
很好，現在我們已取得所有功能。那麼什麼時候使用它們呢？這個客戶端很簡單，必須明確呼叫功能。下一章我們會建立更進階的客戶端，內建大型語言模型（LLM）。目前先看看如何呼叫伺服器功能：

### -4- 呼叫功能

呼叫功能時，需確保指定正確參數，有時還要指定要呼叫的名稱。
...
確保 MCP 伺服器運行於 `http://localhost:8080`，然後執行客戶端：
...
這裡有一個伺服器可供你使用，請透過客戶端呼叫它，並嘗試為伺服器新增更多功能，使其更有趣。
...
**解答資料夾**包含完整且可直接執行的客戶端實作，示範本教學涵蓋的所有概念。每個解答包含獨立的客戶端與伺服器程式碼專案。
...
完整文件與逐步指引請參考：**[📖 解答文件](./solution/README.md)**

## 🎯 完整範例

我們提供了涵蓋本教學所有程式語言的完整可運作客戶端實作。這些範例展示上述所有功能，可作為參考或起點。
...
詳細文件與自訂說明請參考：**[📖 完整範例文件](./complete_examples.md)**
