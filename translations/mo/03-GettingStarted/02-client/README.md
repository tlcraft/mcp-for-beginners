...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T00:00:47+00:00",
...
客戶端是自訂的應用程式或腳本，直接與 MCP Server 通訊以請求資源、工具和提示。與使用檢查器工具提供的圖形介面互動不同，撰寫自己的客戶端可以進行程式化和自動化的互動。這讓開發者能將 MCP 功能整合到自己的工作流程中，自動化任務，並打造符合特定需求的客製化解決方案。
...
- **決定要列出哪些資源**。你的 MCP Server 提供資源、工具和提示，你需要決定要列出哪些。
- **將客戶端整合到主機應用程式中**。了解伺服器的功能後，需將其整合到主機應用程式中，讓使用者輸入提示或其他指令時，能呼叫對應的伺服器功能。
...
接下來的練習部分，我們會逐段解析程式碼並說明其運作。

## 練習：撰寫客戶端

如前所述，讓我們慢慢說明程式碼，歡迎你跟著一起寫。

### -1- 匯入函式庫

先匯入所需的函式庫，我們需要引用客戶端和選定的傳輸協定 stdio。stdio 是用於本機執行的協定。SSE 是另一種傳輸協定，未來章節會介紹，但目前先用 stdio。
...
Java 部分，你將建立一個客戶端，連接到前一練習中的 MCP Server。使用 [Getting Started with MCP Server](../../../../03-GettingStarted/01-first-server/solution/java) 中相同的 Java Spring Boot 專案結構，在 `src/main/java/com/microsoft/mcp/sample/client/` 資料夾建立一個名為 `SDKClient` 的新 Java 類別，並加入以下匯入：
...
注意，「Arguments」中可以指定 *.csproj* 或執行檔。
...
現在我們有一個能連接的客戶端，但還沒列出伺服器功能，接下來來做這件事：
...
很好，現在我們已取得所有功能。那麼什麼時候使用它們呢？這個客戶端相當簡單，必須明確呼叫功能才能使用。下一章我們會建立更進階的客戶端，內建大型語言模型（LLM）。現在先來看看如何呼叫伺服器功能：
...
先確保 MCP Server 正在 `http://localhost:8080` 運行，然後執行客戶端：
...
或者，你也可以執行解決方案資料夾 `03-GettingStarted\02-client\solution\java` 中提供的完整客戶端專案：
...
在這個作業中，你將運用所學建立自己的客戶端。

這裡有一個伺服器可供你使用，需透過客戶端程式碼呼叫，試著為伺服器新增更多功能，讓它更有趣。
...
- 可用來發現並呼叫伺服器上的功能。
- 可在啟動自身時啟動伺服器（如本章示範），也能連接到已運行的伺服器。
- 是測試伺服器功能的好方法，與前章提到的 Inspector 工具互為補充。
...
- 下一章：[建立具備 LLM 的客戶端](../03-llm-client/README.md)
