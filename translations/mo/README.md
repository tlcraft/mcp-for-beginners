...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T15:02:32+00:00",
...
| 5.3 | OAuth2 示範 | 實作 OAuth2 驗證 | [指南](./05-AdvancedTopics/mcp-oauth2-demo/README.md) |
...
| **模組 11：MCP 伺服器實作實驗室** | | | |
| 11 | MCP 伺服器資料庫整合 | 針對 PostgreSQL 整合的完整 13 實驗學習路徑 | [實驗](./11-MCPServerHandsOnLabs/README.md) |
| 11.1 | 簡介 | MCP 與資料庫整合及零售分析使用案例的概述 | [實驗 00](./11-MCPServerHandsOnLabs/00-Introduction/README.md) |
...
提供了一份全面的 [學習指南](./study_guide.md)，幫助您有效瀏覽此存儲庫。指南包括：
...
準備好參加為期兩天的深入技術洞察、社群連結及實作學習的 MCP 開發日活動，這是一個專注於模型上下文協議（MCP）的虛擬活動——這是一個連接 AI 模型及其依賴工具的新興標準。
您可以通過註冊活動頁面觀看 MCP 開發日：https://aka.ms/mcpdevdays。

#### [第一天：MCP 生產力、開發工具及社群](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

第一天旨在幫助開發者將 MCP 融入其開發工作流程，並慶祝 MCP 社群的卓越成就。我們將與社群成員及合作夥伴（如 Arcade、Block、Okta 和 Neon）一起探討他們如何與 Microsoft 合作，共同塑造開放且可擴展的 MCP 生態系統。  
實際演示涵蓋 VS Code、Visual Studio、GitHub Copilot 及流行社群工具  
實用的上下文驅動開發工作流程  
社群主導的會議及洞察  
...
第二天專為 MCP 構建者設計。我們將深入探討構建 MCP 伺服器及將 MCP 集成到 AI 工作流程中的實現策略及最佳實踐。

#### 主題包括：

- 構建 MCP 伺服器及將其集成到代理體驗中
- 驅動式開發
- 安全性最佳實踐
...
### 一起學習 MCP 與 C# - 教程系列
一起學習模型上下文協議（MCP），這是一個旨在標準化 AI 模型與客戶端應用之間交互的前沿框架。在這個適合初學者的課程中，我們將向您介紹 MCP 並指導您創建第一個 MCP 伺服器。
...
此內容基於 **MIT 授權**。有關條款及條件，請參閱 [LICENSE](../../LICENSE)。

## 🤝 貢獻指南

此項目歡迎貢獻及建議。大多數貢獻需要您同意一份
貢獻者授權協議（CLA），聲明您有權並實際授予我們使用您貢獻的權利。詳情請訪問 <https://cla.opensource.microsoft.com>。

當您提交拉取請求時，CLA 機器人將自動判斷您是否需要提供 CLA 並適當地裝飾 PR（例如，狀態檢查、評論）。只需按照機器人提供的指示操作即可。您只需在所有使用 CLA 的存儲庫中完成一次。

此項目採用了 [Microsoft 開源行為準則](https://opensource.microsoft.com/codeofconduct/)。
有關更多信息，請參閱 [行為準則 FAQ](https://opensource.microsoft.com/codeofconduct/faq/) 或
聯繫 [opencode@microsoft.com](mailto:opencode@microsoft.com) 提出其他問題或評論。
...
此專案可能包含專案、產品或服務的商標或標誌。經授權使用 Microsoft 的商標或標誌時，必須遵守並符合 [Microsoft 的商標與品牌準則](https://www.microsoft.com/legal/intellectualproperty/trademarks/usage/general)。  
在此專案的修改版本中使用 Microsoft 的商標或標誌時，不得引起混淆或暗示 Microsoft 的贊助。  
