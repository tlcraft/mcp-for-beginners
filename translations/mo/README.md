...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-02T08:59:54+00:00",
...
**Model Context Protocol (MCP)** 是一個先進的框架，旨在標準化 AI 模型與客戶端應用程式之間的互動。這個開源課程提供結構化的學習路徑，包含實務程式範例與真實應用案例，涵蓋熱門程式語言如 C#、Java、JavaScript、TypeScript 及 Python。
...
## 參加 MCP Dev Days 2025 年 7 月 29-30 日

準備好迎接為期兩天的深入技術分享、社群交流與實作學習吧！MCP Dev Days 是一場專注於 Model Context Protocol（MCP）的線上活動，這個新興標準串連 AI 模型與其所依賴的工具。

➡️ [註冊 MCP Dev Days](https://developer.microsoft.com/en-us/reactor/series/S-1563/)

你可於活動頁面 https://aka.ms/mcpdevdays 註冊，屆時能透過 YouTube 或 Twitch 直播參與。所有內容皆會錄製，活動結束後可在 Microsoft Developer YouTube 頻道觀看。示範程式碼也會在 GitHub 上提供。
...
專注於協助開發者將 MCP 融入工作流程，並慶祝活躍的 MCP 社群。我們邀請到 Arcade、Block、Okta 和 Neon 等社群成員與合作夥伴，一同分享他們如何與微軟合作打造開放且可擴充的 MCP 生態系。內容包含：

實際案例展示 VS Code、Visual Studio、GitHub Copilot 與熱門社群工具的整合  
以情境為導向的開發流程  
社群主導的議程與見解  
無論你是 MCP 新手或已在使用，第一天都會帶給你靈感與可執行的收穫。

#### 第二天：自信打造 MCP 伺服器

專為 MCP 開發者設計，深入探討建立 MCP 伺服器的實作策略與最佳實踐，以及如何將 MCP 整合至 AI 工作流程。
...
- 以提示驅動的開發  
- 安全性最佳實踐  
- 使用 Functions、ACA 及 API Management 等組件  
- 註冊表對齊與工具（第一方與第三方）

若你是開發者、工具建構者或 AI 產品策略師，這一天充滿你打造可擴展、安全且未來導向 MCP 解決方案所需的見解。
...
- **01. 核心概念說明**  
  深入探討 MCP 核心概念。 [閱讀更多](./01-CoreConcepts/README.md)
- **02. MCP 安全性**  
...
### 建立與部署你的第一個 MCP 伺服器與客戶端，並進行實作練習與情境演練  
<details>
...
- **3.4. 使用 Visual Studio Code 連接伺服器** – [指南](./03-GettingStarted/04-vscode/README.md)
...
- **04. 實務實作**  
  SDK、除錯、測試與可重複使用的提示範本。 [閱讀更多](./04-PracticalImplementation/README.md)
- **05. MCP 進階主題**  
  多模態 AI、擴充性、企業應用。 [閱讀更多](./05-AdvancedTopics/README.md)
...
- **5.7. 擴充** – [指南](./05-AdvancedTopics/mcp-scaling/README.md)
...
- **5.12. 使用 Entra ID 認證的 Model Context Protocol 伺服器** – [指南](./05-AdvancedTopics/mcp-security-entra/README.md)
- **5.13. Model Context Protocol (MCP) 與 Azure AI Foundry 的整合** – [指南](./05-AdvancedTopics/mcp-foundry-agent-integration/README.md)
</details>

### Model Context Protocol 最佳實踐  
<details>
  <summary><strong>第6-9課：社群、最佳實踐與實驗室</strong></summary>
...
### 使用 AI Toolkit for VScode 進行 Model Context Protocol 實作實驗室  
<details>
  <summary><strong>第10課：使用 AI Toolkit for VScode 建立 MCP 伺服器實作實驗室</strong></summary>
    
- **10. 精簡 AI 工作流程：使用 AI Toolkit 建立 MCP 伺服器** – [實作實驗室](./10-StreamliningAIWorkflowsBuildingAnMCPServerWithAIToolkit/README.md)
</details>

## Model Context Protocol 範例專案：使用 Java、C#、JavaScript、TypeScript 及 Python 建立 MCP 計算機專案

### 🧮 Java、C#、JavaScript、TypeScript 與 Python MCP 計算機範例專案  
<details>
...
### 💡 MCP 進階範例解決方案：C#、Java、JavaScript、TypeScript 與 Python 計算機專案  
<details>
...
我們提供一份完整的[學習指南](./study_guide.md)協助您有效瀏覽此資源庫。指南內容包含：
...
貢獻者授權協議（CLA），聲明您擁有並實際授權我們使用您的貢獻。詳情請參閱 <https://cla.opensource.microsoft.com>。

當您提交 pull request 時，CLA 機器人會自動判斷是否需要您提供 CLA，並適當標示 PR（如狀態檢查、留言）。請依照機器人指示操作。您只需在使用我們 CLA 的所有資源庫中完成一次。

本專案採用 [Microsoft 開放原始碼行為守則](https://opensource.microsoft.com/codeofconduct/)。更多資訊請參閱 [行為守則常見問題](https://opensource.microsoft.com/codeofconduct/faq/) 或
聯絡 [opencode@microsoft.com](mailto:opencode@microsoft.com) 提出任何疑問或意見。
...
此專案可能包含專案、產品或服務的商標或標誌。授權使用 Microsoft 商標或標誌需遵守並依據
[Microsoft 的商標與品牌指南](https://www.microsoft.com/legal/intellectualproperty/trademarks/usage/general)。
在修改版本中使用 Microsoft 商標或標誌不得造成混淆或暗示 Microsoft 的贊助。
任何第三方商標或標誌的使用皆須遵循該第三方的相關政策。
