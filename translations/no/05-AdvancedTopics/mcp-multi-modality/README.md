...
MCP støtter ikke bare tekstbaserte interaksjoner, men også multi-modale funksjoner, slik at modeller kan jobbe med bilder, lyd og andre datatyper.

## Introduksjon

I denne leksjonen vil du lære hvordan du bygger en multi-modale applikasjon.

## Læringsmål

Etter denne leksjonen skal du kunne:

- Forstå valg innen multi-modale løsninger
- Implementere en multi-modale app.

## Arkitektur for multi-modal støtte

Multi-modale MCP-implementasjoner involverer vanligvis:

- **Modal-spesifikke parser**: Komponenter som konverterer ulike mediatyper til formater modellen kan behandle.
- **Modal-spesifikke verktøy**: Spesialverktøy designet for å håndtere spesifikke modaliteter (bildeanalyse, lydbehandling)
- **Enhetlig kontekststyring**: System for å opprettholde kontekst på tvers av ulike modaliteter
- **Responsgenerering**: Evne til å generere svar som kan inkludere flere modaliteter.

## Multi-modalt eksempel: Bildeanalyse

I eksempelet under skal vi analysere et bilde og hente ut informasjon.
...
- Opprettet et `ImageAnalysisTool` som kan analysere bilder ved hjelp av en hypotetisk `IImageAnalysisService`.
- Konfigurert MCP-serveren til å håndtere større forespørsler og støtte bildeinnholdstyper.
- Registrert bildeanalyseverktøyet med serveren.
- Implementert en metode for å laste ned bilder fra en URL og analysere dem basert på forespurt type (objekter, tekst, ansikter osv.).
- Returnert strukturerte resultater i et format som er i samsvar med MCP-spesifikasjonen.

## Multi-modalt eksempel: Lydbehandling

Lydbehandling er en annen vanlig modalitet i multi-modale applikasjoner. Nedenfor er et eksempel på hvordan man kan implementere et verktøy for lydtranskripsjon som kan håndtere lydfiler og returnere transkripsjoner.
...
- Opprettet et `AudioTranscriptionTool` som kan transkribere lydfiler.
- Definert verktøyets skjema for å akseptere enten en URL eller base64-kodet lyddata.
- Implementert `execute`-metoden for å håndtere lydbehandling og transkripsjon.
- Konfigurert MCP-serveren til å håndtere multi-modale forespørsler, inkludert lyd- og bildebehandling.
- Registrert lydtranskripsjonsverktøyet med serveren.
- Implementert en metode for å laste ned lydfiler fra en URL eller dekode base64-lyddata.
- Brukt en `AudioProcessor`-tjeneste for å håndtere selve transkripsjonslogikken.
- Startet MCP-serveren for å lytte etter forespørsler.

### Multi-modalt eksempel: Generering av multi-modale svar
