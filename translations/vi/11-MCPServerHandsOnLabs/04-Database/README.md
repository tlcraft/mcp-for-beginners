...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T20:00:18+00:00",
  "source_file": "11-MCPServerHandsOnLabs/04-Database/README.md",
  "language_code": "vi"
}
-->
# Thiết Kế Cơ Sở Dữ Liệu và Lược Đồ

## 🎯 Nội Dung Của Bài Thực Hành Này

Bài thực hành này đi sâu vào thiết kế cơ sở dữ liệu PostgreSQL cho hệ thống bán lẻ Zava. Bạn sẽ học cách triển khai một lược đồ bán lẻ toàn diện với khả năng tìm kiếm vector, mô hình dữ liệu đa tenant, và Row Level Security (RLS) để cô lập dữ liệu.

## Tổng Quan

Cơ sở dữ liệu là nền tảng của máy chủ MCP, lưu trữ dữ liệu bán lẻ từ nhiều cửa hàng trong khi duy trì sự cô lập dữ liệu nghiêm ngặt. Chúng tôi sử dụng PostgreSQL với tiện ích mở rộng pgvector để kích hoạt khả năng tìm kiếm ngữ nghĩa, cho phép khách hàng tìm sản phẩm bằng các truy vấn ngôn ngữ tự nhiên.

Lược đồ của chúng tôi tuân theo các mô hình đa tenant hiện đại với Row Level Security đảm bảo người dùng chỉ có thể truy cập dữ liệu từ các cửa hàng được ủy quyền. Cách tiếp cận này cung cấp bảo mật cấp doanh nghiệp trong khi vẫn duy trì hiệu suất tối ưu.

## Mục Tiêu Học Tập

Sau khi hoàn thành bài thực hành này, bạn sẽ có thể:

- **Thiết kế** lược đồ cơ sở dữ liệu bán lẻ đa tenant có khả năng mở rộng
- **Triển khai** PostgreSQL với pgvector để tìm kiếm vector
- **Cấu hình** Row Level Security để cô lập dữ liệu
- **Tạo** dữ liệu mẫu thực tế để kiểm thử
- **Tối ưu hóa** hiệu suất cơ sở dữ liệu cho khối lượng công việc bán lẻ
- **Triển khai** chiến lược sao lưu và khôi phục

## 🗃️ Kiến Trúc Cơ Sở Dữ Liệu

### PostgreSQL với pgvector

Cơ sở dữ liệu của chúng tôi tận dụng các tính năng doanh nghiệp của PostgreSQL kết hợp với tiện ích mở rộng pgvector để tìm kiếm dựa trên AI:
...
Cơ sở dữ liệu sử dụng mô hình đa tenant **cơ sở dữ liệu chia sẻ, lược đồ chia sẻ** với Row Level Security:
...
## 📊 Thiết Kế Lược Đồ Cốt Lõi

### Bảng Stores (Tenant Master)
...
### Vai Trò và Quyền Hạn Cơ Sở Dữ Liệu
...
### Script Dữ Liệu Kiểm Thử Thực Tế
...
✅ **Thiết Kế Cơ Sở Dữ Liệu Đa Tenant**: Triển khai Row Level Security để cô lập dữ liệu an toàn  
✅ **Khả Năng Tìm Kiếm Vector**: Cấu hình pgvector để tìm kiếm ngữ nghĩa sản phẩm  
✅ **Lược Đồ Toàn Diện**: Tạo lược đồ cơ sở dữ liệu bán lẻ sẵn sàng cho sản xuất  
✅ **Tạo Dữ Liệu Mẫu**: Xây dựng dữ liệu kiểm thử thực tế cho phát triển và kiểm thử  
✅ **Tối Ưu Hiệu Suất**: Cấu hình chỉ mục và tối ưu hóa truy vấn  
✅ **Sao Lưu và Khôi Phục**: Thiết lập chiến lược bảo vệ dữ liệu mạnh mẽ  

## 🚀 Tiếp Theo

Tiếp tục với **[Lab 05: Triển Khai Máy Chủ MCP](../05-MCP-Server/README.md)** để:

- Xây dựng máy chủ FastMCP kết nối với cơ sở dữ liệu này
- Triển khai các công cụ truy vấn cơ sở dữ liệu cho giao thức MCP
- Thêm khả năng tìm kiếm ngữ nghĩa bằng embeddings
- Cấu hình pooling kết nối và xử lý lỗi

## 📚 Tài Nguyên Bổ Sung

### PostgreSQL & pgvector
- [Tài Liệu PostgreSQL](https://www.postgresql.org/docs/) - Tham khảo đầy đủ về PostgreSQL
- [Tiện Ích Mở Rộng pgvector](https://github.com/pgvector/pgvector) - Tìm kiếm tương đồng vector cho PostgreSQL
- [Tối Ưu Hiệu Suất PostgreSQL](https://wiki.postgresql.org/wiki/Performance_Optimization) - Các thực hành tối ưu hóa tốt nhất

### Kiến Trúc Đa Tenant
- [Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Tài liệu PostgreSQL về RLS
- [Kiến Trúc Dữ Liệu Đa Tenant](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Các mẫu kiến trúc Azure
- [Thực Hành Tốt Nhất Về Bảo Mật Cơ Sở Dữ Liệu](https://www.postgresql.org/docs/current/security.html) - Hướng dẫn bảo mật PostgreSQL

### Cơ Sở Dữ Liệu Vector
- [Kiến Thức Cơ Bản Về Tìm Kiếm Vector](https://www.pinecone.io/learn/vector-database/) - Hiểu về cơ sở dữ liệu vector
- [Mô Hình Embedding](https://platform.openai.com/docs/guides/embeddings) - Tài liệu về embeddings của OpenAI
- [Thuật Toán HNSW](https://arxiv.org/abs/1603.09320) - Đồ thị Hierarchical Navigable Small World

---

**Trước**: [Lab 03: Cài Đặt Môi Trường](../03-Setup/README.md)  
**Tiếp Theo**: [Lab 05: Triển Khai Máy Chủ MCP](../05-MCP-Server/README.md)
