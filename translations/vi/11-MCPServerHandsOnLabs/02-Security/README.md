...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T19:46:44+00:00",
  "source_file": "11-MCPServerHandsOnLabs/02-Security/README.md",
  "language_code": "vi"
}
-->
# Bảo mật và Đa Người Dùng

## 🎯 Nội dung của bài thực hành này

Bài thực hành này cung cấp hướng dẫn toàn diện về cách triển khai bảo mật cấp doanh nghiệp và đa người dùng cho các máy chủ MCP. Bạn sẽ học cách thiết kế hệ thống an toàn, tuân thủ quy định để bảo vệ dữ liệu nhạy cảm trong ngành bán lẻ, đồng thời cho phép các mô hình truy cập linh hoạt giữa nhiều người dùng.

## Tổng quan

Bảo mật là yếu tố quan trọng trong các ứng dụng bán lẻ xử lý dữ liệu khách hàng, thông tin thanh toán và trí tuệ kinh doanh. Bài thực hành này bao gồm toàn bộ kiến trúc bảo mật từ xác thực và cấp quyền đến cách ly dữ liệu và giám sát tuân thủ.

Chúng tôi triển khai chiến lược bảo mật nhiều lớp, kết hợp các dịch vụ nhận dạng của Azure, Row Level Security của PostgreSQL, các kiểm soát ở cấp ứng dụng và ghi nhật ký kiểm tra toàn diện để tạo ra một nền tảng mạnh mẽ và tuân thủ quy định.

## Mục tiêu học tập

Sau khi hoàn thành bài thực hành này, bạn sẽ có thể:

- **Triển khai** Row Level Security cấp doanh nghiệp để cách ly dữ liệu đa người dùng
- **Thiết kế** các mô hình xác thực và cấp quyền an toàn với Azure
- **Cấu hình** ghi nhật ký kiểm tra toàn diện để đáp ứng yêu cầu tuân thủ
- **Áp dụng** chiến lược bảo mật nhiều lớp trên tất cả các tầng ứng dụng
- **Xác thực** các triển khai bảo mật thông qua kiểm tra hệ thống
- **Giám sát** các sự kiện bảo mật và phản ứng với các mối đe dọa tiềm ẩn

## 🔐 Kiến trúc bảo mật đa người dùng

### Tổng quan về các lớp bảo mật
...
### Mô hình đa người dùng

Triển khai của chúng tôi sử dụng mô hình **Cơ sở dữ liệu chung, Schema chung** với Row Level Security:

**Lợi ích:**
- Tối ưu hóa chi phí sử dụng tài nguyên
- Đơn giản hóa việc bảo trì và cập nhật
- Cách ly dữ liệu mạnh mẽ thông qua RLS
- Dễ dàng tuân thủ với các nhật ký kiểm tra

**Hạn chế:**
- Yêu cầu thiết kế chính sách RLS cẩn thận
- Thay đổi schema ảnh hưởng đến tất cả người dùng
- Cần có quy trình sao lưu/phục hồi mạnh mẽ

## 🛡️ Triển khai Row Level Security
...
### Danh sách kiểm tra kiểm tra thâm nhập
...
✅ **Bảo mật đa người dùng**: Triển khai Row Level Security để cách ly dữ liệu hoàn toàn  
✅ **Xác thực Azure**: Tích hợp Azure Entra ID với xác thực JWT  
✅ **Cấp quyền dựa trên vai trò**: Cấu hình hệ thống vai trò và quyền phân cấp  
✅ **Ghi nhật ký kiểm tra toàn diện**: Thiết lập theo dõi và giám sát sự kiện bảo mật  
✅ **Kiểm tra bảo mật**: Triển khai các bài kiểm tra xác thực bảo mật tự động  
✅ **Giám sát mối đe dọa**: Tạo hệ thống phát hiện và cảnh báo sự kiện bảo mật theo thời gian thực  

## 🚀 Tiếp theo

Tiếp tục với **[Bài thực hành 03: Cài đặt môi trường](../03-Setup/README.md)** để:

- Cấu hình môi trường phát triển với các nguyên tắc bảo mật tốt nhất
- Thiết lập các dịch vụ Azure cho xác thực và giám sát
- Triển khai kết nối cơ sở dữ liệu an toàn và quản lý bí mật
- Xác thực cấu hình bảo mật trong môi trường phát triển

## 📚 Tài liệu bổ sung

### Bảo mật Azure
- [Tài liệu Azure Entra ID](https://docs.microsoft.com/azure/active-directory/) - Hướng dẫn đầy đủ về nền tảng nhận dạng
- [Azure Key Vault](https://docs.microsoft.com/azure/key-vault/) - Dịch vụ quản lý bí mật
- [Các nguyên tắc và thực hành bảo mật Azure](https://docs.microsoft.com/azure/security/fundamentals/best-practices-and-patterns) - Hướng dẫn bảo mật

### Bảo mật cơ sở dữ liệu
- [PostgreSQL Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Tài liệu chính thức về RLS
- [Danh sách kiểm tra bảo mật cơ sở dữ liệu](https://www.postgresql.org/docs/current/security.html) - Hướng dẫn bảo mật PostgreSQL
- [Mô hình cơ sở dữ liệu đa người dùng](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Các mô hình kiến trúc

### Kiểm tra bảo mật
- [Hướng dẫn kiểm tra OWASP](https://owasp.org/www-project-web-security-testing-guide/) - Kiểm tra bảo mật toàn diện
- [Các nguyên tắc bảo mật JWT](https://tools.ietf.org/html/rfc8725) - Cân nhắc bảo mật JWT
- [Kiểm tra bảo mật API](https://owasp.org/www-project-api-security/) - Kiểm tra bảo mật dành riêng cho API

---

**Trước**: [Bài thực hành 01: Các khái niệm kiến trúc cốt lõi](../01-Architecture/README.md)  
**Tiếp theo**: [Bài thực hành 03: Cài đặt môi trường](../03-Setup/README.md)
