...
Bài thực hành này hướng dẫn bạn triển khai một máy chủ MCP sẵn sàng cho sản xuất bằng cách sử dụng framework FastMCP. Bạn sẽ xây dựng cấu trúc máy chủ cốt lõi, tích hợp cơ sở dữ liệu, tạo công cụ truy cập dữ liệu và thiết lập nền tảng cho phân tích bán lẻ dựa trên AI.

## Tổng quan

Máy chủ MCP là trung tâm của giải pháp phân tích bán lẻ của chúng tôi. Nó đóng vai trò như một cầu nối giữa các trợ lý AI và cơ sở dữ liệu PostgreSQL, cung cấp quyền truy cập an toàn, thông minh vào dữ liệu kinh doanh thông qua một giao thức chuẩn hóa.

Bài thực hành này sẽ hướng dẫn bạn xây dựng một máy chủ MCP mạnh mẽ, có khả năng mở rộng, tuân theo các mẫu và thực tiễn tốt nhất trong doanh nghiệp.

## Mục tiêu học tập

Sau khi hoàn thành bài thực hành này, bạn sẽ có thể:

- **Xây dựng** máy chủ FastMCP với kiến trúc và tổ chức hợp lý
- **Triển khai** tích hợp cơ sở dữ liệu với quản lý kết nối và xử lý lỗi
- **Tạo** công cụ MCP để khám phá cấu trúc cơ sở dữ liệu và thực thi truy vấn
- **Cấu hình** quản lý ngữ cảnh bảo mật cấp hàng (Row Level Security - RLS)
- **Thêm** các tính năng giám sát sức khỏe và khả năng quan sát
- **Kiểm tra** triển khai máy chủ MCP của bạn cục bộ và với VS Code

## 📁 Cấu trúc dự án

Hãy cùng xem xét cách tổ chức máy chủ MCP:
...
### Cấu hình môi trường (`config.py`)

Đầu tiên, hãy tạo một hệ thống cấu hình mạnh mẽ:
...
### Các tính năng chính của cấu hình

- **Tải biến môi trường**: Hỗ trợ tự động tệp .env
- **An toàn kiểu dữ liệu**: Xác thực dataclass và gợi ý kiểu dữ liệu
- **Mặc định linh hoạt**: Giá trị mặc định hợp lý cho môi trường phát triển
- **Xác thực**: Xác thực cấu hình với thông báo lỗi hữu ích
- **Bảo mật**: Chỉ lấy các giá trị nhạy cảm từ biến môi trường

## 🗄️ Lớp tích hợp cơ sở dữ liệu

### Nhà cung cấp PostgreSQL (`sales_analysis_postgres.py`)

Hãy triển khai lớp tích hợp cơ sở dữ liệu:
...
### Các tính năng chính của lớp cơ sở dữ liệu

- **Quản lý kết nối**: Quản lý tài nguyên hiệu quả với asyncpg
- **Tích hợp RLS**: Tự động thiết lập ngữ cảnh bảo mật cấp hàng
- **Khám phá cấu trúc**: Tự động khám phá cấu trúc bảng
- **Xử lý lỗi**: Quản lý lỗi toàn diện và ghi nhật ký
- **Định dạng truy vấn**: Định dạng kết quả thân thiện với AI
- **Giám sát sức khỏe**: Kiểm tra kết nối cơ sở dữ liệu và trạng thái pool

## 🔧 Triển khai chính máy chủ MCP

### Máy chủ FastMCP (`sales_analysis.py`)

Bây giờ hãy triển khai máy chủ MCP chính:
...
### Các tính năng chính của máy chủ MCP

- **Đăng ký công cụ**: Định nghĩa công cụ khai báo với an toàn kiểu dữ liệu
- **Quản lý ngữ cảnh RLS**: Tự động trích xuất danh tính người dùng và thiết lập ngữ cảnh
- **Xử lý lỗi**: Quản lý lỗi toàn diện với thông báo thân thiện với người dùng
- **Quản lý vòng đời**: Khởi động/tắt máy đúng cách với dọn dẹp tài nguyên
- **Giám sát sức khỏe**: Điểm cuối kiểm tra sức khỏe tích hợp
- **Hỗ trợ phát triển**: Tải lại nóng và khả năng gỡ lỗi

## 🏥 Giám sát sức khỏe

### Triển khai kiểm tra sức khỏe (`health_check.py`)
...
2. **Kiểm tra các điểm cuối sức khỏe**:
   ```bash
   # Basic health check
...
### Kiểm tra tích hợp với VS Code

1. **Cấu hình VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Kiểm tra trong AI Chat**:
   - Mở AI Chat trong VS Code
   - Gõ `#zava` và chọn máy chủ của bạn
   - Hỏi: "Có những bảng nào?"
   - Hỏi: "Hiển thị 5 cửa hàng hàng đầu theo số lượng đơn hàng"
...
✅ **Máy chủ MCP hoạt động**: Máy chủ FastMCP với tích hợp cơ sở dữ liệu  
✅ **Quản lý cấu hình**: Hệ thống cấu hình mạnh mẽ dựa trên môi trường  
✅ **Lớp cơ sở dữ liệu**: Tích hợp PostgreSQL với quản lý kết nối  
✅ **Công cụ MCP**: Công cụ khám phá cấu trúc và thực thi truy vấn  
✅ **Tích hợp RLS**: Quản lý ngữ cảnh bảo mật cấp hàng  
✅ **Giám sát sức khỏe**: Điểm cuối kiểm tra sức khỏe toàn diện  
✅ **Chiến lược kiểm tra**: Kiểm tra cục bộ và tích hợp với VS Code  

## 🚀 Tiếp theo

Tiếp tục với **[Bài thực hành 06: Phát triển công cụ](../06-Tools/README.md)** để:

- Mở rộng bộ sưu tập công cụ MCP của bạn
- Triển khai các mẫu truy vấn nâng cao
- Thêm xác thực và chuyển đổi dữ liệu
- Tạo các công cụ phân tích chuyên biệt

## 📚 Tài liệu bổ sung

### Framework FastMCP
- [Tài liệu FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Hướng dẫn chính thức về FastMCP
- [Đặc tả MCP](https://modelcontextprotocol.io/docs/) - Đặc tả giao thức
- [Hướng dẫn phát triển công cụ](https://modelcontextprotocol.io/docs/tools/) - Tạo công cụ MCP

### Tích hợp cơ sở dữ liệu
- [Tài liệu asyncpg](https://magicstack.github.io/asyncpg/current/) - Trình điều khiển PostgreSQL async
- [Thực tiễn tốt nhất về quản lý kết nối](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Tinh chỉnh PostgreSQL
- [Hướng dẫn bảo mật cấp hàng](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Triển khai RLS

### Mẫu FastAPI
- [Tài liệu FastAPI](https://fastapi.tiangolo.com/) - Tham khảo framework web
- [Tiêm phụ thuộc](https://fastapi.tiangolo.com/tutorial/dependencies/) - Mẫu FastAPI
- [Nhiệm vụ nền](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Quản lý nhiệm vụ async

---

**Tiếp theo**: Sẵn sàng mở rộng công cụ của bạn? Tiếp tục với [Bài thực hành 06: Phát triển công cụ](../06-Tools/README.md)
