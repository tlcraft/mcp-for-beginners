...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-18T17:06:55+00:00",
...
[Model Context Protocol (MCP)](https://gi- **Sự Đồng Ý Rõ Ràng của Người Dùng**: Mọi truy cập dữ liệu và thao tác đều yêu cầu sự chấp thuận rõ ràng từ người dùng trước khi thực hiện. Người dùng phải hiểu rõ dữ liệu nào sẽ được truy cập và hành động nào sẽ được thực hiện, với quyền kiểm soát chi tiết về các quyền và sự cho phép.

- **Bảo Vệ Quyền Riêng Tư Dữ Liệu**: Dữ liệu người dùng chỉ được tiết lộ khi có sự đồng ý rõ ràng và phải được bảo vệ bằng các biện pháp kiểm soát truy cập mạnh mẽ trong suốt vòng đời tương tác. Các triển khai phải ngăn chặn việc truyền dữ liệu trái phép và duy trì ranh giới quyền riêng tư nghiêm ngặt.
...
- **Bảo Mật Lớp Vận Chuyển**: Tất cả các kênh giao tiếp nên sử dụng các cơ chế mã hóa và xác thực phù hợp. Các kết nối từ xa nên triển khai các giao thức vận chuyển an toàn và quản lý thông tin xác thực đúng cách.

#### Hướng Dẫn Triển Khai:

- **Quản Lý Quyền**: Triển khai các hệ thống quyền chi tiết cho phép người dùng kiểm soát máy chủ, công cụ và tài nguyên nào có thể truy cập
- **Xác Thực & Ủy Quyền**: Sử dụng các phương pháp xác thực an toàn (OAuth, API keys) với quản lý token và thời hạn hợp lý  
- **Xác Thực Đầu Vào**: Xác thực tất cả các tham số và dữ liệu đầu vào theo các schema đã định nghĩa để ngăn chặn các cuộc tấn công chèn mã
...
- Thu được những hiểu biết thực tiễn thông qua các ví dụ mã trong .NET, Java, Python, và JavaScript.

## Kiến Trúc MCP: Phân Tích Sâu

Hệ sinh thái MCP được xây dựng trên mô hình client-server. Cấu trúc mô-đun này cho phép các ứng dụng AI tương tác với công cụ, cơ sở dữ liệu, API, và các tài nguyên ngữ cảnh một cách hiệu quả. Hãy phân tích kiến trúc này thành các thành phần cốt lõi.
...
- **MCP Hosts**: Các chương trình như VSCode, Claude Desktop, IDEs, hoặc công cụ AI muốn truy cập dữ liệu thông qua MCP
- **MCP Clients**: Các client giao thức duy trì kết nối 1:1 với các server
- **MCP Servers**: Các chương trình nhẹ cung cấp các khả năng cụ thể thông qua giao thức Model Context Protocol tiêu chuẩn
...
- **Ứng Dụng Tùy Chỉnh**: Các công cụ và tác nhân AI được xây dựng theo mục đích
...
- **Quản Lý Trạng Thái**: Duy trì trạng thái phiên và xử lý các tương tác có trạng thái khi cần thiết
...
Tài nguyên được xác định bằng URIs và hỗ trợ khám phá thông qua các phương thức `resources/list` và truy xuất qua `resources/read`:
...
**Lời Nhắc** là các mẫu có thể tái sử dụng giúp cấu trúc các tương tác với mô hình ngôn ngữ. Chúng cung cấp các mẫu tương tác tiêu chuẩn và quy trình làm việc theo mẫu:

- **Tương Tác Dựa Trên Mẫu**: Tin nhắn được cấu trúc trước và các khởi đầu hội thoại
- **Mẫu Quy Trình Làm Việc**: Các chuỗi tiêu chuẩn cho các nhiệm vụ và tương tác phổ biến
- **Ví Dụ Few-shot**: Các mẫu dựa trên ví dụ để hướng dẫn mô hình
- **Lời Nhắc Hệ Thống**: Lời nhắc nền tảng xác định hành vi và ngữ cảnh của mô hình
...
**Sampling** cho phép servers yêu cầu các hoàn thành mô hình ngôn ngữ từ ứng dụng AI của client. Nguyên thủy này cho phép servers truy cập các khả năng LLM mà không cần tích hợp các phụ thuộc mô hình riêng:
...
Sampling được khởi tạo thông qua phương thức `sampling/complete`, nơi servers gửi yêu cầu hoàn thành đến clients.

### Elicitation  

**Elicitation** cho phép servers yêu cầu thông tin bổ sung hoặc xác nhận từ người dùng thông qua giao diện client:

- **Yêu Cầu Đầu Vào Người Dùng**: Servers có thể yêu cầu thông tin bổ sung khi cần thiết để thực thi công cụ
- **Hộp Thoại Xác Nhận**: Yêu cầu sự chấp thuận của người dùng cho các thao tác nhạy cảm hoặc có tác động
...
Yêu cầu elicitation được thực hiện bằng phương thức `elicitation/request` để thu thập đầu vào người dùng thông qua giao diện của client.
...
- **Host Khởi Tạo Kết Nối**  
  Ứng dụng host (chẳng hạn như IDE hoặc giao diện chat) thiết lập kết nối đến một server MCP, thường thông qua STDIO, WebSocket, hoặc một phương thức vận chuyển được hỗ trợ khác.
...
- **Sử Dụng Tài Nguyên hoặc Công Cụ**  
  ...
  - Nếu mô hình xác định rằng cần một công cụ (ví dụ: để lấy dữ liệu, thực hiện tính toán, hoặc gọi API), client gửi yêu cầu gọi công cụ đến server, chỉ định tên công cụ và các tham số.
...
- **Tạo Phản Hồi**  
  Client tích hợp các phản hồi của server (dữ liệu tài nguyên, đầu ra công cụ, v.v.) vào tương tác mô hình đang diễn ra. Mô hình sử dụng thông tin này để tạo một phản hồi toàn diện và phù hợp với ngữ cảnh.

- **Trình Bày Kết Quả**  
  Host nhận đầu ra cuối cùng từ client và trình bày nó cho người dùng, thường bao gồm cả văn bản được tạo bởi mô hình và bất kỳ kết quả nào từ các thao tác công cụ hoặc tìm kiếm tài nguyên.

Luồng này cho phép MCP hỗ trợ các ứng dụng AI
...
- **Nguyên thủy máy khách**: Cho phép máy chủ yêu cầu lấy mẫu từ LLM, thu thập đầu vào từ người dùng, và gửi thông báo nhật ký  
...
**Lớp truyền tải** quản lý các kênh giao tiếp, định dạng thông điệp, và xác thực giữa các thành viên MCP:

#### Các cơ chế truyền tải được hỗ trợ:

1. **Truyền tải STDIO**:  
   - Sử dụng luồng đầu vào/đầu ra tiêu chuẩn để giao tiếp trực tiếp giữa các quy trình  
   - Tối ưu cho các quy trình cục bộ trên cùng một máy mà không có chi phí mạng  
...
Lớp truyền tải trừu tượng hóa chi tiết giao tiếp khỏi lớp dữ liệu, cho phép sử dụng cùng định dạng thông điệp JSON-RPC 2.0 trên tất cả các cơ chế truyền tải. Sự trừu tượng hóa này cho phép ứng dụng chuyển đổi dễ dàng giữa máy chủ cục bộ và máy chủ từ xa.
...
- **Sự đồng ý và kiểm soát của người dùng**: Người dùng phải cung cấp sự đồng ý rõ ràng trước khi bất kỳ dữ liệu nào được truy cập hoặc hoạt động nào được thực hiện. Họ cần có quyền kiểm soát rõ ràng về dữ liệu được chia sẻ và các hành động được ủy quyền, được hỗ trợ bởi giao diện người dùng trực quan để xem xét và phê duyệt hoạt động.

- **Bảo mật dữ liệu**: Dữ liệu người dùng chỉ được tiết lộ khi có sự đồng ý rõ ràng và phải được bảo vệ bằng các kiểm soát truy cập thích hợp. Các triển khai MCP phải bảo vệ chống lại việc truyền dữ liệu trái phép và đảm bảo rằng quyền riêng tư được duy trì trong tất cả các tương tác.

- **An toàn công cụ**: Trước khi sử dụng bất kỳ công cụ nào, cần có sự đồng ý rõ ràng từ người dùng. Người dùng cần hiểu rõ chức năng của từng công cụ, và các ranh giới bảo mật mạnh mẽ phải được thực thi để ngăn chặn việc thực thi công cụ không mong muốn hoặc không an toàn.
...
Trong ví dụ này, chúng tôi trình bày cách xây dựng máy chủ MCP bằng Python. Bạn cũng được giới thiệu hai cách khác nhau để tạo công cụ.
...
Ví dụ JavaScript này trình bày cách tạo máy khách MCP kết nối với máy chủ, gửi lời nhắc, và xử lý phản hồi bao gồm bất kỳ cuộc gọi công cụ nào đã được thực hiện.
...
1. **Kiểm soát quyền công cụ**:  
   Máy khách có thể chỉ định công cụ nào mô hình được phép sử dụng trong một phiên. Điều này đảm bảo rằng chỉ các công cụ được ủy quyền rõ ràng mới có thể truy cập, giảm nguy cơ hoạt động không mong muốn hoặc không an toàn. Quyền có thể được cấu hình động dựa trên sở thích của người dùng, chính sách tổ chức, hoặc ngữ cảnh tương tác.
...
3. **Xác thực tham số**:  
   Xác thực tham số được thực thi cho tất cả các cuộc gọi công cụ. Mỗi công cụ định nghĩa các loại, định dạng, và ràng buộc mong đợi cho các tham số của nó, và máy chủ xác thực các yêu cầu đến tương ứng. Điều này ngăn chặn đầu vào sai định dạng hoặc độc hại tiếp cận các triển khai công cụ và giúp duy trì tính toàn vẹn của hoạt động.

4. **Giới hạn tốc độ**:  
   Để ngăn chặn lạm dụng và đảm bảo sử dụng công bằng tài nguyên máy chủ, các máy chủ MCP có thể triển khai giới hạn tốc độ cho các cuộc gọi công cụ và truy cập tài nguyên. Giới hạn tốc độ có thể được áp dụng theo người dùng, theo phiên, hoặc toàn cầu, và giúp bảo vệ chống lại các cuộc tấn công từ chối dịch vụ hoặc tiêu thụ tài nguyên quá mức.
...
Giao tiếp MCP sử dụng các thông điệp **JSON-RPC 2.0** có cấu trúc để tạo điều kiện tương tác rõ ràng và đáng tin cậy giữa các máy chủ, máy khách, và máy chủ. Giao thức định nghĩa các mẫu thông điệp cụ thể cho các loại hoạt động khác nhau:
...
- **Thông điệp nhật ký**: Máy chủ gửi thông điệp nhật ký có cấu trúc đến máy khách  
...
- **`notifications/prompts/list_changed`**: Máy chủ thông báo cho máy khách về thay đổi mẫu lời nhắc  
...
- **Thông điệp thông báo**: Bao gồm `method` và các `params` tùy chọn (không có `id` hoặc phản hồi được mong đợi)  

Cách giao tiếp có cấu trúc này đảm bảo tương tác đáng tin cậy, có thể truy vết, và có thể mở rộng, hỗ trợ các kịch bản nâng cao như cập nhật thời gian thực, chuỗi công cụ, và xử lý lỗi mạnh mẽ.

## Những điểm chính

- **Kiến trúc**: MCP sử dụng kiến trúc máy khách-máy chủ, nơi các máy chủ quản lý nhiều kết nối máy khách đến máy chủ  
- **Thành phần**: Hệ sinh thái bao gồm máy chủ (ứng dụng AI), máy khách (kết nối giao thức), và máy chủ (nhà cung cấp khả năng)  
...
- **Nguyên thủy máy khách**: Máy chủ có thể yêu cầu lấy mẫu (hoàn thành LLM), thu thập đầu vào (người dùng), và nhật ký từ máy khách  
- **Nền tảng giao thức**: Dựa trên JSON-RPC 2.0 với phiên bản dựa trên ngày (hiện tại: 2025-06-18)  
- **Khả năng thời gian thực**: Hỗ trợ thông báo để cập nhật động và đồng bộ hóa thời gian thực  
