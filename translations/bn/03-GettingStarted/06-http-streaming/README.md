...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-10T16:03:24+00:00",
...
- **SSE (Server-Sent Events)**: সার্ভারকে HTTP এর মাধ্যমে ক্লায়েন্টকে রিয়েল-টাইম আপডেট পাঠানোর সুযোগ দেয়। ওয়েব UI এর জন্য ভালো, কিন্তু স্কেলেবিলিটি এবং নমনীয়তায় সীমাবদ্ধ।
...
নিচে একটি সহজ উদাহরণ দেওয়া হলো কিভাবে স্ট্রিমিং বাস্তবায়ন করা যায়:
...
- **যোগাযোগের প্যাটার্ন:**
   - ক্লাসিক HTTP স্ট্রিমিং: ডেটা চাঙ্কে পাঠানোর জন্য সাধারণ চাঙ্কড ট্রান্সফার এনকোডিং ব্যবহার করে
...
- **ক্লায়েন্ট বাস্তবায়ন:**
   ...
   - MCP: বিভিন্ন ধরনের মেসেজ প্রক্রিয়াকরণের জন্য উন্নত মেসেজ হ্যান্ডলার সহ ক্লায়েন্ট
...
- **জটিল, ইন্টারেক্টিভ অ্যাপ্লিকেশনের জন্য:** MCP স্ট্রিমিং আরও কাঠামোবদ্ধ পদ্ধতি প্রদান করে, সমৃদ্ধ মেটাডেটা এবং নোটিফিকেশন ও চূড়ান্ত ফলাফলের পৃথকীকরণ সহ।
...
এখন পর্যন্ত আপনি ক্লাসিক স্ট্রিমিং এবং MCP স্ট্রিমিংয়ের মধ্যে কিছু সুপারিশ এবং তুলনা দেখেছেন। আসুন বিস্তারিতভাবে জানি কিভাবে MCP-তে স্ট্রিমিং ব্যবহার করবেন।

MCP ফ্রেমওয়ার্কের মধ্যে স্ট্রিমিং কিভাবে কাজ করে তা বোঝা জরুরি, যাতে দীর্ঘমেয়াদী অপারেশনের সময় ব্যবহারকারীদের রিয়েল-টাইম ফিডব্যাক প্রদান করে প্রতিক্রিয়াশীল অ্যাপ্লিকেশন তৈরি করা যায়।
...
প্রধান ফলাফল এখনও একক রেসপন্স হিসেবে পাঠানো হয়। তবে, প্রক্রিয়াকরণের সময় আলাদা মেসেজ হিসেবে নোটিফিকেশন পাঠানো হয় এবং এর মাধ্যমে ক্লায়েন্টকে রিয়েল-টাইমে আপডেট করা হয়। ক্লায়েন্টকে এই নোটিফিকেশনগুলো গ্রহণ ও প্রদর্শন করতে সক্ষম হতে হবে।
...
| warning   | সতর্কতা শর্তাবলী               | অব্যবহৃত ফিচার ব্যবহারের সতর্কতা |
...
MCP-তে নোটিফিকেশন বাস্তবায়নের জন্য সার্ভার এবং ক্লায়েন্ট উভয়কেই রিয়েল-টাইম আপডেট পরিচালনার জন্য প্রস্তুত করতে হবে। এর ফলে দীর্ঘমেয়াদী অপারেশনের সময় ব্যবহারকারীদের অবিলম্বে প্রতিক্রিয়া প্রদান সম্ভব হয়।
...
অতিরিক্তভাবে, নোটিফিকেশন সক্ষম করতে, নিশ্চিত করুন আপনার সার্ভার স্ট্রিমিং ট্রান্সপোর্ট (যেমন `streamable-http`) ব্যবহার করছে এবং ক্লায়েন্ট মেসেজ হ্যান্ডলার বাস্তবায়ন করেছে। নিচে দেখানো হলো কিভাবে সার্ভার `streamable-http` ট্রান্সপোর্ট ব্যবহার করবে:
...
.NET MCP সার্ভারে নোটিফিকেশন সক্ষম করতে, নিশ্চিত করুন আপনি স্ট্রিমিং ট্রান্সপোর্ট ব্যবহার করছেন:
...
ক্লায়েন্টকে একটি মেসেজ হ্যান্ডলার বাস্তবায়ন করতে হবে যা আসা নোটিফিকেশনগুলো প্রক্রিয়া ও প্রদর্শন করবে।
...
উপরের কোডে, `message_handler` ফাংশন চেক করে আসা মেসেজটি নোটিফিকেশন কিনা। যদি হয়, এটি নোটিফিকেশন প্রিন্ট করে; অন্যথায়, এটি সাধারণ সার্ভার মেসেজ হিসেবে প্রক্রিয়া করে। এছাড়াও লক্ষ্য করুন `ClientSession` কে `message_handler` দিয়ে ইনিশিয়ালাইজ করা হয়েছে যাতে আসা নোটিফিকেশনগুলো হ্যান্ডল করা যায়।
...
এই .NET উদাহরণে, `MessageHandler` ফাংশন চেক করে আসা মেসেজটি নোটিফিকেশন কিনা। যদি হয়, এটি নোটিফিকেশন প্রিন্ট করে; অন্যথায়, এটি সাধারণ সার্ভার মেসেজ হিসেবে প্রক্রিয়া করে। `ClientSession` মেসেজ হ্যান্ডলার সহ `ClientSessionOptions` এর মাধ্যমে ইনিশিয়ালাইজ করা হয়েছে।

</details>

নোটিফিকেশন সক্ষম করতে, নিশ্চিত করুন আপনার সার্ভার স্ট্রিমিং ট্রান্সপোর্ট (যেমন `streamable-http`) ব্যবহার করছে এবং ক্লায়েন্ট মেসেজ হ্যান্ডলার বাস্তবায়ন করেছে।

## প্রগ্রেস নোটিফিকেশন এবং পরিস্থিতি

এই অংশে MCP-তে প্রগ্রেস নোটিফিকেশনের ধারণা, এর গুরুত্ব এবং Streamable HTTP ব্যবহার করে কিভাবে তা বাস্তবায়ন করবেন তা ব্যাখ্যা করা হয়েছে। এছাড়াও একটি ব্যবহারিক অ্যাসাইনমেন্ট রয়েছে যা আপনার বোঝাপড়া মজবুত করবে।
...
- সব অ্যাক্সেস এবং ত্রুটি লগ ও মনিটর করুন।
...
বর্তমানে Server-Sent Events (SSE) ব্যবহারকারী অ্যাপ্লিকেশনগুলোর জন্য Streamable HTTP তে মাইগ্রেশন MCP বাস্তবায়নের জন্য উন্নত ক্ষমতা এবং দীর্ঘমেয়াদী টেকসইতা প্রদান করে।

### কেন আপগ্রেড করবেন?

SSE থেকে Streamable HTTP তে আপগ্রেড করার দুটি প্রধান কারণ আছে:
...
আপনার MCP অ্যাপ্লিকেশনগুলোতে SSE থেকে Streamable HTTP তে মাইগ্রেট করার পদ্ধতি:

- সার্ভার কোড আপডেট করুন যাতে `mcp.run()` এ `transport="streamable-http"` ব্যবহার হয়।
- ক্লায়েন্ট কোড আপডেট করুন যাতে SSE ক্লায়েন্টের পরিবর্তে `streamablehttp_client` ব্যবহার হয়।
...
- SSE এবং Streamable HTTP উভয় ট্রান্সপোর্ট আলাদা এন্ডপয়েন্টে চালিয়ে সমর্থন করতে পারেন।
...
- সব ক্লায়েন্ট আপডেট করা নিশ্চিত করা
...
2. একটি ক্লায়েন্ট তৈরি করুন যার মধ্যে একটি মেসেজ হ্যান্ডলার থাকবে যা রিয়েল টাইমে নোটিফিকেশন দেখাবে।
