...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-11T10:41:14+00:00",
...
- **적절한 라이브러리 가져오기**. 이전에 사용한 라이브러리와 동일하지만 다른 구조를 사용합니다.
- **클라이언트 인스턴스화**. 클라이언트 인스턴스를 생성하고 선택한 전송 방법에 연결합니다.
- **나열할 리소스 결정하기**. MCP 서버에는 리소스, 도구, 프롬프트가 포함되어 있으므로 나열할 항목을 결정해야 합니다.
...
다음 연습 섹션에서 각 코드 스니펫을 분해하고 무엇이 진행되고 있는지 설명해 보겠습니다.

## 연습: 클라이언트 작성하기

앞서 말했듯이, 코드를 설명하는 데 시간을 할애하고 싶다면 직접 코딩해도 좋습니다.

### -1- 라이브러리 가져오기

필요한 라이브러리를 가져옵니다. 클라이언트와 선택한 전송 프로토콜(stdio)에 대한 참조가 필요합니다. stdio는 로컬 머신에서 실행되도록 설계된 프로토콜입니다. SSE는 다른 전송 프로토콜로, 이후 챕터에서 보여드릴 예정이지만 또 다른 옵션입니다. 지금은 stdio로 계속 진행하겠습니다.
...
- stdio 전송 인스턴스를 생성했습니다. 서버를 찾고 시작하는 방법을 지정하는 명령과 args를 확인하세요. 이는 클라이언트를 생성할 때 필요합니다.
...
- MCP 서버가 실행될 `http://localhost:8080`을 가리키는 SSE 전송을 설정하는 main 메서드를 생성했습니다.
...
이제 프로그램이 실행될 경우 연결할 수 있는 클라이언트를 생성했습니다. 하지만 실제로 기능을 나열하지는 않았으므로 이를 다음으로 진행해 보겠습니다:
...
좋습니다. 이제 모든 기능을 캡처했습니다. 이제 질문은 언제 이를 사용할 것인가입니다. 이 클라이언트는 매우 간단하며, 서버 기능을 호출하려면 명시적으로 호출해야 합니다. 다음 챕터에서는 자체 대형 언어 모델(LLM)을 갖춘 더 고급 클라이언트를 생성할 것입니다. 지금은 서버의 기능을 호출하는 방법을 살펴보겠습니다:
...
- 서버 도구는 특정 매개변수 이름(예: 수학 연산을 위한 "a", "b")을 기대합니다.
...
이 프로젝트를 확인하여 [프롬프트와 리소스 추가](https://github.com/modelcontextprotocol/csharp-sdk/blob/main/samples/EverythingServer/Program.cs) 방법을 확인하세요.
...
[이전 섹션](../../../../03-GettingStarted/01-first-server)에서 Rust로 간단한 MCP 서버를 생성하는 방법을 배웠습니다. 이를 계속 구축하거나 Rust 기반 MCP 서버 예제를 확인하려면 다음 링크를 참조하세요: [MCP Server Examples](https://github.com/modelcontextprotocol/rust-sdk/tree/main/examples/servers)

## 솔루션

**솔루션 폴더**에는 이 튜토리얼에서 다룬 모든 개념을 보여주는 완전한 실행 가능한 클라이언트 구현이 포함되어 있습니다. 각 솔루션에는 클라이언트와 서버 코드가 별도의 독립된 프로젝트로 구성되어 있습니다.
...
2. **완전한 예제 파일을 검토**하여 전체 구현을 이해하세요.
3. [`complete_examples.md`](./complete_examples.md)의 지침에 따라 **예제를 실행**하세요.
4. **특정 사용 사례에 맞게 수정 및 확장**하세요.

이 예제를 실행하고 사용자 정의하는 방법에 대한 자세한 문서는 다음을 참조하세요: **[📖 완전한 예제 문서](./complete_examples.md)**
...
두 접근 방식 모두 유용합니다. **솔루션 폴더**는 완전한 프로젝트에, **완전한 예제**는 학습 및 참고용으로 사용하세요.

## 주요 요점

이 장의 주요 요점은 클라이언트에 대한 다음 내용입니다:

- 서버의 기능을 검색하고 호출하는 데 사용할 수 있습니다.
- (이 장에서처럼) 클라이언트가 시작될 때 서버를 시작할 수 있지만, 실행 중인 서버에 연결할 수도 있습니다.
