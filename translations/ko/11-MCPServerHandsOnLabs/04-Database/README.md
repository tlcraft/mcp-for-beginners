...
이 실습에서는 Zava Retail 시스템을 위한 PostgreSQL 데이터베이스 설계에 대해 깊이 탐구합니다. 벡터 검색 기능, 멀티 테넌트 데이터 모델링, 데이터 격리를 위한 Row Level Security(RLS)를 포함한 포괄적인 소매 스키마를 구현하는 방법을 배우게 됩니다.

## 개요

데이터베이스는 MCP 서버의 기반으로, 여러 매장의 소매 데이터를 저장하면서 엄격한 데이터 격리를 유지합니다. 우리는 PostgreSQL과 pgvector 확장을 사용하여 고객이 자연어 쿼리를 통해 제품을 검색할 수 있는 의미 검색 기능을 제공합니다.

우리의 스키마는 현대적인 멀티 테넌트 패턴을 따르며, Row Level Security를 통해 사용자가 승인된 매장의 데이터만 액세스할 수 있도록 보장합니다. 이 접근 방식은 엔터프라이즈급 보안을 제공하면서도 최적의 성능을 유지합니다.

## 학습 목표

이 실습을 완료하면 다음을 수행할 수 있습니다:

- **설계**: 확장 가능한 멀티 테넌트 소매 데이터베이스 스키마 설계
- **구현**: 벡터 검색을 위한 PostgreSQL과 pgvector 설정
- **구성**: 데이터 격리를 위한 Row Level Security 설정
- **생성**: 테스트를 위한 현실적인 샘플 데이터 생성
- **최적화**: 소매 워크로드를 위한 데이터베이스 성능 최적화
- **구현**: 백업 및 복구 전략
...
데이터베이스는 Row Level Security를 사용하는 **공유 데이터베이스, 공유 스키마** 멀티 테넌시 모델을 사용합니다:
...
✅ **벡터 검색 기능**: 의미 있는 제품 검색을 위한 pgvector 설정  
✅ **포괄적인 스키마**: 프로덕션 준비가 된 소매 데이터베이스 스키마 생성  
...
**[Lab 05: MCP 서버 구현](../05-MCP-Server/README.md)**을 계속 진행하여:

- 이 데이터베이스에 연결하는 FastMCP 서버 구축
- MCP 프로토콜을 위한 데이터베이스 쿼리 도구 구현
- Embeddings를 사용한 의미 검색 기능 추가
- 연결 풀링 및 오류 처리 구성

## 📚 추가 자료

### PostgreSQL & pgvector
- [PostgreSQL 문서](https://www.postgresql.org/docs/) - PostgreSQL의 완전한 참조 자료
...
- [Embedding 모델](https://platform.openai.com/docs/guides/embeddings) - OpenAI Embeddings 문서
- [HNSW 알고리즘](https://arxiv.org/abs/1603.09320) - 계층적 탐색 가능한 소형 세계 그래프

---

**이전**: [Lab 03: 환경 설정](../03-Setup/README.md)  
**다음**: [Lab 05: MCP 서버 구현](../05-MCP-Server/README.md)
