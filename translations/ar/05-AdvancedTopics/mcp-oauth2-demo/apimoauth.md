...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-17T15:27:43+00:00",
  "source_file": "05-AdvancedTopics/mcp-oauth2-demo/apimoauth.md",
  "language_code": "ar"
}
-->
# نشر تطبيق Spring AI MCP إلى Azure Container Apps

([تأمين خوادم Spring AI MCP باستخدام OAuth2](https://spring.io/blog/2025/04/02/mcp-server-oauth2)) *الشكل: خادم Spring AI MCP مؤمن باستخدام Spring Authorization Server. يقوم الخادم بإصدار رموز الوصول للعملاء والتحقق منها في الطلبات الواردة (المصدر: مدونة Spring) ([تأمين خوادم Spring AI MCP باستخدام OAuth2](https://spring.io/blog/2025/04/02/mcp-server-oauth2#:~:text=,server%20with%20the%20MCP%20inspector)).* لنشر خادم Spring MCP، قم ببنائه كحاوية واستخدم Azure Container Apps مع الدخول الخارجي. على سبيل المثال، باستخدام Azure CLI يمكنك تشغيل:
...
قم بتمكين خادم التفويض وخادم الموارد عن طريق تحديد سلسلة مرشحات الأمان. على سبيل المثال:
...
هذا يكشف عن `/.well-known/openid-configuration`, which APIM can use for metadata. Finally, you may want to customize the JWT **audience** claim so that APIM’s `<audiences>` سيتجاوز الفحص. على سبيل المثال، أضف مخصصًا للرمز:
...
هذا يضمن أن الرموز تحمل `"aud": ["mcp-client"]`, matching the client ID or scope expected by APIM. 
...
In Azure APIM, add an inbound policy that uses the `<validate-jwt>` سياسة لفحص JWTs الواردة ضد خادم التفويض الخاص بـ Spring. لإعداد بسيط، يمكنك استخدام عنوان URL لبيانات OpenID Connect. مقتطف سياسة المثال:
...
**References:** See Spring Authorization Server docs for default endpoints ([Configuration Model :: Spring Authorization Server](https://docs.spring.io/spring-authorization-server/reference/configuration-model.html#:~:text=public%20static%20Builder%20builder%28%29%20,oauth2%2Fauthorize)) and OIDC configuration ([Configuration Model :: Spring Authorization Server](https://docs.spring.io/spring-authorization-server/reference/configuration-model.html#:~:text=.securityMatcher%28authorizationServerConfigurer.getEndpointsMatcher%28%29%29%20.with%28authorizationServerConfigurer%2C%20%28authorizationServer%29%20,%29%3B%20return%20http.build)); see Microsoft APIM docs for `validate-jwt` أمثلة ([مرجع سياسة إدارة API لـ Azure - validate-jwt | Microsoft Learn](https://learn.microsoft.com/en-us/azure/api-management/validate-jwt-policy#:~:text=Microsoft%20Entra%20ID%20single%20tenant,token%20validation)); ومستندات Azure Container Apps للنشر والشهادات ([نشر تطبيقات Java Spring Boot إلى Azure Container Apps - Java على Azure | Microsoft Learn](https://learn.microsoft.com/en-us/azure/developer/java/identity/deploy-spring-boot-to-azure-container-apps#:~:text=Now%20you%20can%20deploy%20your,CLI%20command)) ([أسماء النطاقات المخصصة والشهادات المدارة المجانية في Azure Container Apps | Microsoft Learn](https://learn.microsoft.com/en-us/azure/container-apps/custom-domains-managed-certificates#:~:text=Free%20certificate%20requirements)).
