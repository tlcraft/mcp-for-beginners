...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-02T11:56:07+00:00",
...
تستكشف هذه الدرس مفاهيم متقدمة في تنفيذ بروتوكول سياق النموذج، مع التركيز على التكامل متعدد الوسائط، القابلية للتوسع، أفضل ممارسات الأمان، وتكامل المؤسسات. هذه المواضيع ضرورية لبناء تطبيقات MCP بمستوى إنتاجي قادرة على التعامل مع متطلبات معقدة في بيئات المؤسسات.
...
|--------|---------|-------|
| [mcp-oauth2-demo](../../../05-AdvancedTopics/mcp-oauth2-demo) | عرض توضيحي MCP OAuth2 | تطبيق Spring Boot بسيط يعرض OAuth2 مع MCP، كخادم تفويض وموارد. يوضح إصدار رموز آمن، نقاط نهاية محمية، نشر Azure Container Apps، وتكامل إدارة API. |
| [web-search-mcp](../../../05-AdvancedTopics/web-search-mcp) | بحث ويب MCP | خادم وعميل Python MCP يتكامل مع SerpAPI للبحث في الويب، الأخبار، المنتجات، والأسئلة والأجوبة في الوقت الحقيقي. يوضح تنسيق أدوات متعددة، تكامل API خارجي، وتعامل قوي مع الأخطاء. |
...
يدعم MCP ليس فقط التفاعلات النصية بل أيضًا قدرات متعددة الوسائط، مما يسمح للنماذج بالعمل مع الصور، الصوت، وأنواع بيانات أخرى.

### هيكلية دعم متعدد الوسائط

عادةً ما تتضمن تطبيقات MCP متعددة الوسائط:

1. **محللات خاصة بالنمط**: مكونات تحول أنواع الوسائط المختلفة إلى صيغ يمكن للنموذج معالجتها
2. **أدوات خاصة بالنمط**: أدوات مصممة للتعامل مع أنماط معينة (تحليل الصور، معالجة الصوت)
3. **إدارة سياق موحدة**: نظام للحفاظ على السياق عبر أنماط متعددة
4. **توليد الاستجابات**: القدرة على توليد استجابات قد تشمل وسائط متعددة

### مثال C# متعدد الوسائط: تحليل الصور
...
### مثال Java متعدد الوسائط: معالجة الصوت
...
### مثال Python متعدد الوسائط: توليد استجابات متعددة الوسائط
...
سياقات الجذر هي مفهوم أساسي في بروتوكول سياق النموذج توفر طبقة دائمة للحفاظ على سجل المحادثة والحالة المشتركة عبر عدة طلبات وجلسات.

### فهم سياقات الجذر

تعمل سياقات الجذر كحاويات تحمل التاريخ والحالة لسلسلة من التفاعلات ذات الصلة. وهي تمكّن من:
...
- يمكن أن يحتوي على سجل المحادثة، تفضيلات المستخدم، وبيانات وصفية أخرى
- يمكن إنشاؤه، الوصول إليه، وأرشفته حسب الحاجة
- يدعم التحكم الدقيق في الوصول والصلاحيات

### دورة حياة سياقات الجذر
...
### مثال JavaScript: إدارة سياقات الجذر
...
### مثال Python: سياق جذر للمساعدة متعددة الأدوار
...
2. **تعيين سياسات انتهاء الصلاحية**: نفذ سياسات لأرشفة أو حذف السياقات القديمة لإدارة التخزين والامتثال لسياسات الاحتفاظ بالبيانات.

3. **تخزين البيانات الوصفية ذات الصلة**: استخدم بيانات وصفية للسياق لتخزين معلومات مهمة عن المحادثة قد تكون مفيدة لاحقًا.

4. **استخدام معرفات السياق باستمرار**: بمجرد إنشاء سياق، استخدم معرفه باستمرار لجميع الطلبات ذات الصلة للحفاظ على الاستمرارية.

5. **توليد ملخصات**: عندما يكبر السياق، فكر في توليد ملخصات لالتقاط المعلومات الأساسية مع إدارة حجم السياق.

6. **تنفيذ التحكم في الوصول**: في الأنظمة متعددة المستخدمين، نفذ ضوابط وصول مناسبة لضمان خصوصية وأمان سياقات المحادثة.

7. **التعامل مع قيود السياق**: كن على دراية بقيود حجم السياق ونفذ استراتيجيات للتعامل مع المحادثات الطويلة جدًا.

8. **الأرشفة عند الاكتمال**: أرشف السياقات عند اكتمال المحادثات لتحرير الموارد مع الحفاظ على سجل المحادثة.

## أخذ العينات في بروتوكول سياق النموذج

استراتيجيات أخذ العينات مهمة لتحسين استجابات النموذج في تطبيقات MCP. يمكن لتكوين أخذ العينات الصحيح تحسين جودة الاستجابة والأداء بشكل كبير. يوفر MCP طريقة موحدة للتحكم في كيفية توليد النماذج للنص مع معلمات محددة تؤثر على العشوائية، الإبداع، والتماسك.

### نظرة عامة على معلمات أخذ العينات

يعرف MCP معلمات أخذ العينات التالية التي يمكن تكوينها في طلبات العميل:
...
| `stop_sequences` | تسلسلات مخصصة توقف التوليد عند مواجهتها | مصفوفة من السلاسل |

### أخذ عينات درجة الحرارة وTop-K/Top-P
...
للتطبيقات التي تتطلب مخرجات متسقة، يضمن أخذ العينات الحتمي نتائج قابلة للتكرار.
...
التوجيه ضروري لتوجيه الطلبات إلى النماذج، الأدوات، أو الخدمات المناسبة داخل نظام MCP.

### التوجيه المعتمد على المحتوى

يوجه التوجيه المعتمد على المحتوى الطلبات إلى خدمات متخصصة بناءً على محتوى الطلب.
...
تحسن موازنة الحمل استخدام الموارد وتضمن توافر عالي لخدمات MCP.
...
يضمن توجيه الأدوات توجيه مكالمات الأدوات إلى الخدمة الأنسب بناءً على السياق.
...
يوضح المخطط أدناه كيف يعمل أخذ العينات والتوجيه معًا في هيكلية MCP شاملة:
...
لتطبيقات المؤسسات، غالبًا ما تحتاج تطبيقات MCP إلى التعامل مع أحجام كبيرة من الطلبات مع تقليل زمن الاستجابة.
...
الأمان أمر حاسم في تطبيقات MCP، خصوصًا في بيئات المؤسسات.
...
- القابلية للتوسع ضرورية لنشر المؤسسات ويمكن تحقيقها من خلال التوسع الأفقي والرأسي
- توفر إجراءات أمان شاملة حماية للبيانات وضمان تحكم مناسب في الوصول
...
3. صمم هيكلية قابلة للتوسع يمكنها التعامل مع أحمال متغيرة
...
- [5.1 عرض MCP OAuth2](./mcp-oauth2-demo/README.md)
