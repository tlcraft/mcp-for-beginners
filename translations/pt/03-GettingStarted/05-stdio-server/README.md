...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-26T19:30:16+00:00",
...
O transporte stdio é um dos dois tipos de transporte suportados na especificação atual do MCP (2025-06-18). Veja como funciona:
...
- O servidor NÃO DEVE escrever nada em `stdout` que não seja uma mensagem válida do MCP.
- O cliente NÃO DEVE escrever nada na `stdin` do servidor que não seja uma mensagem válida do MCP.
...
- Criamos uma instância de servidor com configuração e capacidades básicas.
...
- Criamos uma instância de servidor usando o SDK MCP.
...
Neste laboratório, criaremos um servidor MCP simples usando o transporte stdio recomendado. Este servidor irá expor ferramentas que os clientes podem chamar usando o protocolo Model Context Protocol.
...
- Agora substituído por HTTP Streamable para cenários baseados na web.
...
2. **Criar uma instância de servidor** - Definir o servidor com suas capacidades.
...
O servidor será iniciado e aguardará entrada via stdin. Ele comunica-se usando mensagens JSON-RPC através do transporte stdio.
...
Agora que você aprendeu como construir servidores MCP com o transporte stdio, pode explorar tópicos mais avançados:
