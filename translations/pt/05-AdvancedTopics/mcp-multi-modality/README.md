...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-16T21:57:59+00:00",
...
As aplicações multi-modais estão a tornar-se cada vez mais importantes na IA, permitindo interações mais ricas e tarefas mais complexas. O Model Context Protocol (MCP) fornece uma estrutura para construir aplicações multi-modais que podem lidar com vários tipos de dados, como texto, imagens e áudio.

O MCP suporta não só interações baseadas em texto, mas também capacidades multi-modais, permitindo que os modelos trabalhem com imagens, áudio e outros tipos de dados.

## Introdução

Nesta lição, vais aprender a construir uma aplicação multi-modal.

## Objetivos de Aprendizagem

No final desta lição, serás capaz de:

- Compreender as opções multi-modais
- Implementar uma aplicação multi-modal.

## Arquitetura para Suporte Multi-Modal

As implementações MCP multi-modais normalmente envolvem:

- **Analisadores Específicos para Modalidades**: Componentes que convertem diferentes tipos de media em formatos que o modelo consegue processar.
- **Ferramentas Específicas para Modalidades**: Ferramentas especiais desenhadas para lidar com modalidades específicas (análise de imagens, processamento de áudio)
- **Gestão Unificada de Contexto**: Sistema para manter o contexto através das diferentes modalidades
- **Geração de Respostas**: Capacidade para gerar respostas que podem incluir múltiplas modalidades.

## Exemplo Multi-Modal: Análise de Imagem

No exemplo abaixo, vamos analisar uma imagem e extrair informação.
...
No exemplo anterior, nós:

- Criámos uma `ImageAnalysisTool` que pode analisar imagens usando um hipotético `IImageAnalysisService`.
- Configurámos o servidor MCP para lidar com pedidos maiores e suportar tipos de conteúdo de imagem.
- Registámos a ferramenta de análise de imagens no servidor.
- Implementámos um método para descarregar imagens a partir de uma URL e analisá-las com base no tipo solicitado (objetos, texto, rostos, etc.).
- Retornámos resultados estruturados num formato compatível com a especificação MCP.

## Exemplo Multi-Modal: Processamento de Áudio

O processamento de áudio é outra modalidade comum em aplicações multi-modais. Abaixo está um exemplo de como implementar uma ferramenta de transcrição de áudio que pode lidar com ficheiros de áudio e devolver transcrições.
...
- Criámos uma `AudioTranscriptionTool` que pode transcrever ficheiros de áudio.
- Definimos o esquema da ferramenta para aceitar uma URL ou dados de áudio codificados em base64.
- Implementámos o método `execute` para lidar com o processamento de áudio e transcrição.
- Configurámos o servidor MCP para lidar com pedidos multi-modais, incluindo processamento de áudio e imagem.
- Registámos a ferramenta de transcrição de áudio no servidor.
- Implementámos um método para descarregar ficheiros de áudio a partir de uma URL ou decodificar dados de áudio em base64.
- Usámos um serviço `AudioProcessor` para tratar a lógica real da transcrição.
- Iniciámos o servidor MCP para escutar pedidos.
