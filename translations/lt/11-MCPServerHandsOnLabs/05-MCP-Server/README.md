...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:03:20+00:00",
  "source_file": "11-MCPServerHandsOnLabs/05-MCP-Server/README.md",
  "language_code": "lt"
}
-->
# MCP Serverio Įgyvendinimas

## 🎯 Ką Apima Ši Laboratorija

Ši praktinė laboratorija padės jums sukurti gamybai paruoštą MCP serverį naudojant FastMCP sistemą. Jūs sukursite pagrindinę serverio struktūrą, įgyvendinsite duomenų bazės integraciją, sukursite įrankius duomenų prieigai ir sukursite pagrindą AI pagrįstai mažmeninės prekybos analitikai.

## Apžvalga

MCP serveris yra mūsų mažmeninės prekybos analitikos sprendimo šerdis. Jis veikia kaip tiltas tarp AI asistentų ir PostgreSQL duomenų bazės, užtikrindamas saugią ir intelektualią prieigą prie verslo duomenų per standartizuotą protokolą.

Ši laboratorija moko, kaip sukurti patikimą ir mastelį palaikantį MCP serverį, laikantis įmonės modelių ir geriausios praktikos.

## Mokymosi Tikslai

Laboratorijos pabaigoje jūs galėsite:

- **Sukurti** FastMCP serverį su tinkama architektūra ir organizacija
- **Įgyvendinti** duomenų bazės integraciją su ryšio baseinais ir klaidų valdymu
- **Sukurti** MCP įrankius duomenų bazės schemos introspekcijai ir užklausų vykdymui
- **Konfigūruoti** eilučių lygio saugumo (RLS) konteksto valdymą
- **Pridėti** sveikatos stebėjimo ir stebimumo funkcijas
- **Testuoti** savo MCP serverio įgyvendinimą lokaliai ir su VS Code

## 📁 Projekto Struktūra

Pažvelkime į MCP serverio organizaciją:
...
### Aplinkos Konfigūracija (`config.py`)

Pirmiausia sukurkime patikimą konfigūracijos sistemą:
...
### Pagrindinės Konfigūracijos Funkcijos

- **Aplinkos Kintamųjų Įkėlimas**: Automatinė .env failų palaikymas
- **Tipų Sauga**: Dataclass validacija ir tipų užuominos
- **Lankstūs Numatytieji Parametrai**: Protingi numatytieji parametrai vystymui
- **Validacija**: Konfigūracijos validacija su naudingomis klaidų žinutėmis
- **Saugumas**: Jautrios vertės tik iš aplinkos kintamųjų

## 🗄️ Duomenų Bazės Integracijos Sluoksnis

### PostgreSQL Tiekėjas (`sales_analysis_postgres.py`)

Įgyvendinkime duomenų bazės integracijos sluoksnį:
...
### Pagrindinės Duomenų Bazės Sluoksnio Funkcijos

- **Ryšio Baseinai**: Efektyvus resursų valdymas naudojant asyncpg
- **RLS Integracija**: Automatinis eilučių lygio saugumo konteksto nustatymas
- **Schemos Introspekcija**: Dinaminis lentelių schemos aptikimas
- **Klaidų Valdymas**: Išsamus klaidų valdymas ir logavimas
- **Užklausų Formatavimas**: AI draugiškas rezultatų formatavimas
- **Sveikatos Stebėjimas**: Duomenų bazės ryšio ir baseino būklės patikrinimai

## 🔧 Pagrindinis MCP Serverio Įgyvendinimas

### FastMCP Serveris (`sales_analysis.py`)

Dabar įgyvendinkime pagrindinį MCP serverį:
...
### Pagrindinės MCP Serverio Funkcijos

- **Įrankių Registracija**: Deklaratyvūs įrankių apibrėžimai su tipų sauga
- **RLS Konteksto Valdymas**: Automatinis vartotojo identiteto ištraukimas ir konteksto nustatymas
- **Klaidų Valdymas**: Išsamus klaidų valdymas su vartotojui draugiškomis žinutėmis
- **Gyvavimo Ciklo Valdymas**: Tinkamas paleidimas/uždarymas su resursų valymu
- **Sveikatos Stebėjimas**: Įmontuoti sveikatos patikrinimo galiniai taškai
- **Vystymo Palaikymas**: Karštas perkrovimas ir derinimo galimybės

## 🏥 Sveikatos Stebėjimas

### Sveikatos Patikrinimo Įgyvendinimas (`health_check.py`)
...
2. **Testuokite Sveikatos Galinius Taškus**:
   ```bash
   # Basic health check
...
### VS Code Integracijos Testavimas

1. **Konfigūruokite VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Testuokite AI Pokalbyje**:
   - Atidarykite VS Code AI Pokalbį
   - Įveskite `#zava` ir pasirinkite savo serverį
   - Klauskite: "Kokios lentelės yra prieinamos?"
   - Klauskite: "Parodykite 5 geriausias parduotuves pagal užsakymų skaičių"

### Vienetų Testavimas

Sukurkite išsamius vienetų testus:
...
Baigę šią laboratoriją, turėtumėte turėti:

✅ **Veikiantį MCP Serverį**: FastMCP serverį su duomenų bazės integracija  
✅ **Konfigūracijos Valdymą**: Patikimą aplinkos pagrindu veikiančią konfigūraciją  
✅ **Duomenų Bazės Sluoksnį**: PostgreSQL integraciją su ryšio baseinais  
✅ **MCP Įrankius**: Schemos introspekcijos ir užklausų vykdymo įrankius  
✅ **RLS Integraciją**: Eilučių lygio saugumo konteksto valdymą  
✅ **Sveikatos Stebėjimą**: Išsamius sveikatos patikrinimo galinius taškus  
✅ **Testavimo Strategiją**: Lokalus testavimas ir VS Code integracija  

## 🚀 Kas Toliau

Tęskite su **[Laboratorija 06: Įrankių Kūrimas](../06-Tools/README.md)**, kad:

- Išplėstumėte savo MCP įrankių kolekciją
- Įgyvendintumėte pažangius užklausų modelius
- Pridėtumėte duomenų validaciją ir transformaciją
- Sukurtumėte specializuotus analitikos įrankius

## 📚 Papildomi Ištekliai

### FastMCP Sistema
- [FastMCP Dokumentacija](https://github.com/modelcontextprotocol/python-sdk) - Oficialus FastMCP vadovas
- [MCP Specifikacija](https://modelcontextprotocol.io/docs/) - Protokolo specifikacija
- [Įrankių Kūrimo Vadovas](https://modelcontextprotocol.io/docs/tools/) - MCP įrankių kūrimas

### Duomenų Bazės Integracija
- [asyncpg Dokumentacija](https://magicstack.github.io/asyncpg/current/) - PostgreSQL asinchroninis tvarkyklė
- [Ryšio Baseinų Geriausia Praktika](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL optimizavimas
- [Eilučių Lygio Saugumo Vadovas](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS įgyvendinimas

### FastAPI Modeliai
- [FastAPI Dokumentacija](https://fastapi.tiangolo.com/) - Web sistemos nuoroda
- [Priklausomybių Injekcija](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI modeliai
- [Fono Užduotys](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Asinchroninių užduočių valdymas

---

**Toliau**: Pasiruošę plėsti savo įrankius? Tęskite su [Laboratorija 06: Įrankių Kūrimas](../06-Tools/README.md)
