...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-06-13T01:40:34+00:00",
...
Um mecanismo de transporte define como os dados são trocados entre cliente e servidor. O MCP suporta múltiplos tipos de transporte para atender diferentes ambientes e necessidades:

- **stdio**: Entrada/saída padrão, adequado para ferramentas locais e baseadas em CLI. Simples, mas não indicado para web ou nuvem.
- **SSE (Server-Sent Events)**: Permite que servidores enviem atualizações em tempo real para clientes via HTTP. Bom para interfaces web, mas com limitações em escalabilidade e flexibilidade.
...
Veja a tabela abaixo para entender as diferenças entre esses mecanismos de transporte:
...
> **Dica:** A escolha do transporte certo impacta desempenho, escalabilidade e experiência do usuário. **Streamable HTTP** é recomendado para aplicações modernas, escaláveis e preparadas para nuvem.
...
- Computações longas onde se deseja manter o usuário informado.
...
As razões para usar streaming são:

- Os usuários recebem feedback imediato, não apenas no final.
...
Este exemplo demonstra um servidor enviando uma série de mensagens ao cliente conforme elas ficam disponíveis, ao invés de esperar todas as mensagens estarem prontas.

**Como funciona:**
- O servidor entrega cada mensagem conforme ela está pronta.
- O cliente recebe e imprime cada pedaço conforme chega.

**Requisitos:**
- O servidor deve usar uma resposta streaming (ex: `StreamingResponse` in FastAPI).
...
Ok, você já viu algumas recomendações e comparações sobre a diferença entre streaming clássico e streaming no MCP. Vamos detalhar exatamente como você pode aproveitar o streaming no MCP.

Entender como o streaming funciona dentro do framework MCP é essencial para construir aplicações responsivas que forneçam feedback em tempo real aos usuários durante operações longas.

No MCP, streaming não é sobre enviar a resposta principal em pedaços, mas sim enviar **notificações** ao cliente enquanto uma ferramenta processa uma requisição. Essas notificações podem incluir atualizações de progresso, logs ou outros eventos.
...
Por exemplo, o cliente deve enviar uma notificação assim que a conexão inicial com o servidor for estabelecida.
...
> [!NOTE]
> Dependendo do SDK usado, o logging pode estar habilitado por padrão, ou pode ser necessário ativá-lo explicitamente na configuração do servidor.

Existem diferentes tipos de notificações:

| Nível     | Descrição                     | Exemplo de Uso                  |
|-----------|-------------------------------|--------------------------------|
| debug     | Informações detalhadas para debug | Pontos de entrada/saída de função |
| info      | Mensagens informativas gerais  | Atualizações de progresso       |
| notice    | Eventos normais, mas importantes | Alterações de configuração      |
...
Para implementar notificações no MCP, você precisa configurar os lados servidor e cliente para lidar com atualizações em tempo real. Isso permite que sua aplicação forneça feedback imediato aos usuários durante operações longas.
...
To enable notifications, ensure your server uses a streaming transport (like `streamable-http`) e seu cliente implementa um manipulador para processar notificações.

## Notificações de Progresso & Cenários

Esta seção explica o conceito de notificações de progresso no MCP, por que são importantes e como implementá-las usando Streamable HTTP. Você também encontrará um exercício prático para reforçar o entendimento.

Notificações de progresso são mensagens em tempo real enviadas do servidor para o cliente durante operações longas. Em vez de esperar o processo todo terminar, o servidor mantém o cliente atualizado sobre o status atual. Isso melhora a transparência, a experiência do usuário e facilita o debug.
...
- **Melhora a experiência do usuário:** Usuários veem atualizações conforme o trabalho avança, não só no final.
- **Feedback em tempo real:** Clientes podem exibir barras de progresso ou logs, deixando o app mais responsivo.
- **Facilita debug e monitoramento:** Desenvolvedores e usuários podem ver onde um processo está lento ou travado.
...
### Pontos-Chave
- **Validação do Header Origin**: Sempre valide o cabeçalho `Origin` header to prevent DNS rebinding attacks.
...
Garanta que você trate os seguintes desafios durante a migração:

- Garantir que todos os clientes estejam atualizados.
- Lidar com diferenças na entrega das notificações.

### Exercício: Construa sua Própria Aplicação MCP com Streaming
...
Para continuar sua jornada com streaming MCP e expandir seus conhecimentos, esta seção fornece recursos adicionais e sugestões de próximos passos para construir aplicações mais avançadas.
