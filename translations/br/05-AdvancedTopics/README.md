...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-05-17T15:15:21+00:00",
...
- Aplicar práticas de segurança alinhadas com os princípios de segurança do MCP
...
Para obter as informações mais atualizadas sobre tópicos avançados de MCP, consulte:
...
O MCP suporta não apenas interações baseadas em texto, mas também capacidades multimodais, permitindo que modelos trabalhem com imagens, áudio e outros tipos de dados.

### Arquitetura para Suporte Multimodal

Implementações multimodais de MCP geralmente envolvem:

1. **Analisadores Específicos para Modalidade**: Componentes que convertem diferentes tipos de mídia em formatos que o modelo pode processar
2. **Ferramentas Específicas para Modalidade**: Ferramentas especiais projetadas para lidar com modalidades específicas (análise de imagens, processamento de áudio)
3. **Gerenciamento de Contexto Unificado**: Sistema para manter o contexto em diferentes modalidades
4. **Geração de Respostas**: Capacidade de gerar respostas que podem incluir múltiplas modalidades

### Exemplo em C#: Análise de Imagem
...
### Exemplo em Java: Processamento de Áudio
...
### Exemplo em Python: Geração de Respostas Multimodais
...
Contextos raiz são um conceito fundamental no Model Context Protocol que fornecem uma camada persistente para manter o histórico de conversas e o estado compartilhado em várias solicitações e sessões.

### Compreendendo os Contextos Raiz

Os contextos raiz servem como contêineres que mantêm o histórico e o estado para uma série de interações relacionadas. Eles permitem:

- **Persistência de Conversação**: Manter conversas coerentes de múltiplas interações
- **Gerenciamento de Memória**: Armazenar e recuperar informações em interações
- **Gerenciamento de Estado**: Acompanhar o progresso em fluxos de trabalho complexos
- **Compartilhamento de Contexto**: Permitir que vários clientes acessem o mesmo estado de conversa

No MCP, os contextos raiz têm estas características principais:

- Cada contexto raiz tem um identificador único
- Podem conter histórico de conversas, preferências do usuário e outros metadados
...
### Exemplo em .NET: Trabalhando com Contextos Raiz
...
### Exemplo em Java: Implementação de Contexto Raiz
...
### Exemplo em JavaScript: Gerenciamento de Contexto Raiz
...
### Exemplo em Python: Contexto Raiz para Assistência de Múltiplas Interações
...
1. **Crie Contextos Focados**: Crie contextos raiz separados para diferentes propósitos ou domínios de conversação para manter a clareza.

2. **Defina Políticas de Expiração**: Implemente políticas para arquivar ou excluir contextos antigos para gerenciar armazenamento e cumprir políticas de retenção de dados.

3. **Armazene Metadados Relevantes**: Use metadados de contexto para armazenar informações importantes sobre a conversa que podem ser úteis posteriormente.

4. **Use IDs de Contexto Consistentemente**: Uma vez criado um contexto, use seu ID de forma consistente para todas as solicitações relacionadas para manter a continuidade.

5. **Gere Resumos**: Quando um contexto cresce muito, considere gerar resumos para capturar informações essenciais enquanto gerencia o tamanho do contexto.

6. **Implemente Controle de Acesso**: Para sistemas multiusuário, implemente controles de acesso adequados para garantir a privacidade e segurança dos contextos de conversação.

7. **Lide com Limitações de Contexto**: Esteja ciente das limitações de tamanho de contexto e implemente estratégias para lidar com conversas muito longas.

8. **Archive Quando Completo**: Arquive contextos quando as conversas estiverem completas para liberar recursos enquanto preserva o histórico da conversa.

## Amostragem no Model Context Protocol

Estratégias de amostragem são críticas para otimizar as respostas do modelo em implementações MCP. A configuração de amostragem correta pode melhorar dramaticamente a qualidade e o desempenho das respostas. O MCP fornece uma maneira padronizada de controlar como os modelos geram texto com parâmetros específicos que influenciam aleatoriedade, criatividade e coerência.

### Visão Geral dos Parâmetros de Amostragem

O MCP define os seguintes parâmetros de amostragem que podem ser configurados em solicitações de cliente:
...
| `stop_sequences` | Sequências personalizadas que interrompem a geração quando encontradas | Array de strings |

### Amostragem por Temperatura e Top-K/Top-P

Os parâmetros de amostragem permitem ajustar o comportamento dos modelos de linguagem para alcançar o equilíbrio desejado entre saídas determinísticas e criativas.
...
Para aplicações que requerem saídas consistentes, a amostragem determinística garante resultados reproduzíveis.
...
A amostragem inteligente adapta os parâmetros com base no contexto e nos requisitos de cada solicitação.
...
O roteamento baseado em conteúdo direciona solicitações para serviços especializados com base no conteúdo da solicitação.
...
O balanceamento de carga otimiza a utilização de recursos e garante alta disponibilidade para serviços MCP.
...
O roteamento de ferramentas garante que chamadas de ferramentas sejam direcionadas ao serviço mais apropriado com base no contexto.
...
## Escalabilidade e Alto Desempenho no MCP

Para implantações empresariais, as implementações do MCP frequentemente precisam lidar com altos volumes de solicitações com latência mínima.
...
#### Escalonamento Vertical e Otimização de Recursos
...
A segurança é crítica para implementações do MCP, especialmente em ambientes empresariais.
...
Integrando o MCP com sistemas empresariais como Azure OpenAI e Microsoft AI Foundry.
...
### Exemplo de Integração Empresarial em Python
...
- Escalabilidade é essencial para implantações empresariais e pode ser abordada através de escalonamento horizontal e vertical
- Medidas de segurança abrangentes protegem dados e garantem controle de acesso adequado
- Integração empresarial com plataformas como Azure OpenAI e Microsoft AI Foundry aprimora as capacidades do MCP
...
Próximo: [Comunidade e Contribuições](../06-CommunityContributions/README.md)
