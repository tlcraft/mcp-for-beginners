...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T01:07:21+00:00",
...
Aplicações multi-modais estão se tornando cada vez mais importantes em IA, permitindo interações mais ricas e tarefas mais complexas. O Model Context Protocol (MCP) oferece uma estrutura para construir aplicações multi-modais que podem lidar com vários tipos de dados, como texto, imagens e áudio.

O MCP suporta não apenas interações baseadas em texto, mas também capacidades multi-modais, permitindo que os modelos trabalhem com imagens, áudio e outros tipos de dados.

## Introdução

Nesta lição, você aprenderá como construir uma aplicação multi-modal.

## Objetivos de Aprendizagem

Ao final desta lição, você será capaz de:

- Entender as opções multi-modais
- Implementar uma aplicação multi-modal.

## Arquitetura para Suporte Multi-Modal

Implementações multi-modais do MCP geralmente envolvem:

- **Parsers Específicos para Modalidades**: Componentes que convertem diferentes tipos de mídia em formatos que o modelo pode processar.
- **Ferramentas Específicas para Modalidades**: Ferramentas especiais projetadas para lidar com modalidades específicas (análise de imagens, processamento de áudio)
- **Gerenciamento Unificado de Contexto**: Sistema para manter o contexto entre diferentes modalidades
- **Geração de Respostas**: Capacidade de gerar respostas que podem incluir múltiplas modalidades.

## Exemplo Multi-Modal: Análise de Imagem

No exemplo abaixo, vamos analisar uma imagem e extrair informações.
...
- Criamos uma `ImageAnalysisTool` que pode analisar imagens usando um hipotético `IImageAnalysisService`.
- Configuramos o servidor MCP para lidar com requisições maiores e suportar tipos de conteúdo de imagem.
- Registramos a ferramenta de análise de imagem no servidor.
- Implementamos um método para baixar imagens de uma URL e analisá-las com base no tipo solicitado (objetos, texto, rostos, etc.).
- Retornamos resultados estruturados em um formato compatível com a especificação MCP.
...
- Criamos uma `AudioTranscriptionTool` que pode transcrever arquivos de áudio.
- Definimos o esquema da ferramenta para aceitar uma URL ou dados de áudio codificados em base64.
- Implementamos o método `execute` para lidar com o processamento e transcrição de áudio.
- Configuramos o servidor MCP para lidar com requisições multi-modais, incluindo processamento de áudio e imagem.
- Registramos a ferramenta de transcrição de áudio no servidor.
- Implementamos um método para baixar arquivos de áudio de uma URL ou decodificar dados de áudio em base64.
- Usamos um serviço `AudioProcessor` para lidar com a lógica real da transcrição.
- Iniciamos o servidor MCP para escutar as requisições.
