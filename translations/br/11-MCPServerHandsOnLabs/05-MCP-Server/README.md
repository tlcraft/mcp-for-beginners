...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T16:28:40+00:00",
...
Este laboratório prático orienta você na implementação de um servidor MCP pronto para produção usando o framework FastMCP. Você construirá a estrutura principal do servidor, integrará o banco de dados, criará ferramentas para acesso a dados e estabelecerá a base para análises de varejo impulsionadas por IA.

## Visão Geral

O servidor MCP é o coração da nossa solução de análise de varejo. Ele atua como uma ponte entre assistentes de IA e o banco de dados PostgreSQL, fornecendo acesso seguro e inteligente aos dados empresariais por meio de um protocolo padronizado.

Este laboratório ensina você a construir um servidor MCP robusto e escalável, seguindo padrões empresariais e melhores práticas.

## Objetivos de Aprendizado

Ao final deste laboratório, você será capaz de:

- **Construir** um servidor FastMCP com arquitetura e organização adequadas
- **Implementar** integração com banco de dados, incluindo pooling de conexões e tratamento de erros
- **Criar** ferramentas MCP para introspecção de esquema de banco de dados e execução de consultas
- **Configurar** gerenciamento de contexto de Segurança em Nível de Linha (Row Level Security - RLS)
- **Adicionar** recursos de monitoramento de saúde e observabilidade
- **Testar** sua implementação do servidor MCP localmente e com o VS Code

## 📁 Estrutura do Projeto

Vamos examinar a organização do servidor MCP:
...
Primeiro, vamos criar um sistema de configuração robusto:
...
### Principais Recursos de Configuração

- **Carregamento de Variáveis de Ambiente**: Suporte automático para arquivos .env
- **Segurança de Tipos**: Validação com dataclass e dicas de tipo
- **Padrões Flexíveis**: Padrões sensatos para desenvolvimento
- **Validação**: Validação de configuração com mensagens de erro úteis
- **Segurança**: Valores sensíveis apenas a partir de variáveis de ambiente

## 🗄️ Camada de Integração com Banco de Dados

### Provedor PostgreSQL (`sales_analysis_postgres.py`)

Vamos implementar a camada de integração com o banco de dados:
...
### Principais Recursos da Camada de Banco de Dados

- **Pooling de Conexões**: Gerenciamento eficiente de recursos com asyncpg
- **Integração RLS**: Configuração automática de contexto de Segurança em Nível de Linha
- **Introspecção de Esquema**: Descoberta dinâmica de esquema de tabelas
- **Tratamento de Erros**: Gerenciamento abrangente de erros e registro de logs
- **Formatação de Consultas**: Resultados formatados para IA
- **Monitoramento de Saúde**: Verificação de conectividade e status do pool de conexões

## 🔧 Implementação Principal do Servidor MCP

### Servidor FastMCP (`sales_analysis.py`)

Agora vamos implementar o servidor MCP principal:
...
### Principais Recursos do Servidor MCP

- **Registro de Ferramentas**: Definições declarativas de ferramentas com segurança de tipos
- **Gerenciamento de Contexto RLS**: Extração automática de identidade do usuário e configuração de contexto
- **Tratamento de Erros**: Gerenciamento abrangente de erros com mensagens amigáveis ao usuário
- **Gerenciamento de Ciclo de Vida**: Inicialização/encerramento adequados com limpeza de recursos
- **Monitoramento de Saúde**: Endpoints de verificação de saúde integrados
- **Suporte ao Desenvolvimento**: Recursos de recarregamento automático e depuração

## 🏥 Monitoramento de Saúde

### Implementação de Verificação de Saúde (`health_check.py`)
...
2. **Teste no Chat de IA**:
   - Abra o Chat de IA no VS Code
   - Digite `#zava` e selecione seu servidor
   - Pergunte: "Quais tabelas estão disponíveis?"
   - Pergunte: "Mostre-me as 5 principais lojas por número de pedidos"
...
Após concluir este laboratório, você deverá ter:

✅ **Servidor MCP Funcional**: Servidor FastMCP com integração ao banco de dados  
✅ **Gerenciamento de Configuração**: Configuração robusta baseada em ambiente  
✅ **Camada de Banco de Dados**: Integração com PostgreSQL e pooling de conexões  
✅ **Ferramentas MCP**: Ferramentas de introspecção de esquema e execução de consultas  
✅ **Integração RLS**: Gerenciamento de contexto de Segurança em Nível de Linha  
✅ **Monitoramento de Saúde**: Endpoints abrangentes de verificação de saúde  
✅ **Estratégia de Testes**: Testes locais e integração com VS Code  

## 🚀 O que vem a seguir

Continue com **[Lab 06: Desenvolvimento de Ferramentas](../06-Tools/README.md)** para:

- Expandir sua coleção de ferramentas MCP
- Implementar padrões avançados de consulta
- Adicionar validação e transformação de dados
- Criar ferramentas de análise especializadas

## 📚 Recursos Adicionais

### Framework FastMCP
- [Documentação do FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Guia oficial do FastMCP
- [Especificação MCP](https://modelcontextprotocol.io/docs/) - Especificação do protocolo
- [Guia de Desenvolvimento de Ferramentas](https://modelcontextprotocol.io/docs/tools/) - Criando ferramentas MCP

### Integração com Banco de Dados
- [Documentação do asyncpg](https://magicstack.github.io/asyncpg/current/) - Driver assíncrono para PostgreSQL
- [Melhores Práticas de Pooling de Conexões](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Ajustes para PostgreSQL
- [Guia de Segurança em Nível de Linha](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementação de RLS

### Padrões FastAPI
- [Documentação do FastAPI](https://fastapi.tiangolo.com/) - Referência do framework web
- [Injeção de Dependências](https://fastapi.tiangolo.com/tutorial/dependencies/) - Padrões do FastAPI
- [Tarefas em Segundo Plano](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Gerenciamento de tarefas assíncronas

---

**Próximo**: Pronto para expandir suas ferramentas? Continue com [Lab 06: Desenvolvimento de Ferramentas](../06-Tools/README.md)
