...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-07T08:22:30+00:00",
...
*Note*: L'AI Toolkit prend actuellement en charge Python et TypeScript.
...
> [!WARNING]
> Note pour les utilisateurs de macOS. Nous enquêtons actuellement sur un problème affectant l'installation des dépendances sur macOS. En conséquence, les utilisateurs de macOS ne pourront pas terminer ce tutoriel pour le moment. Nous mettrons à jour les instructions dès qu'une solution sera disponible. Merci pour votre patience et votre compréhension !

Dans cet exercice, vous allez construire, exécuter et améliorer un agent d'IA avec des outils provenant d'un serveur MCP dans Visual Studio Code en utilisant l'AI Toolkit.
...
1. Dans la section **Catalogue**, sélectionnez **Modèles** pour ouvrir le **Catalogue de modèles**. La sélection de **Modèles** ouvre le **Catalogue de modèles** dans un nouvel onglet de l'éditeur.
...
1. Ouvrez l'extension **AI Toolkit** depuis la **Barre d'activité**.
1. Dans la section **Outils**, sélectionnez **Générateur d'agents (Prompt)**. La sélection de **Générateur d'agents (Prompt)** ouvre le **Générateur d'agents (Prompt)** dans un nouvel onglet de l'éditeur.
...
1. Dans la section **Prompts**, cliquez sur le bouton **Générer un prompt système**. Ce bouton ouvre le générateur de prompt qui utilise l'IA pour générer un prompt système pour l'agent.
...
1. Voici un aperçu de ce qui devrait se produire :
...
- L'AI Toolkit inclut des modèles (par exemple, des modèles de serveurs MCP Python) pour simplifier la création d'outils personnalisés.
