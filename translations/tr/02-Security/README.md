...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-04T17:19:21+00:00",
...
Bu ders, MCP ile ilişkili en önemli güvenlik risklerini—kimlik doğrulama, yetkilendirme, aşırı izinler, dolaylı prompt enjeksiyonu ve tedarik zinciri zayıflıkları dahil—inceler ve bunları azaltmak için uygulanabilir kontroller ve en iyi uygulamalar sunar. Ayrıca, MCP uygulamanızı güçlendirmek için Microsoft çözümleri olan Prompt Shields, Azure Content Safety ve GitHub Advanced Security’den nasıl yararlanacağınızı öğreneceksiniz. Bu kontrolleri anlayıp uygulayarak, bir güvenlik ihlalinin olasılığını önemli ölçüde azaltabilir ve yapay zeka sistemlerinizin sağlam ve güvenilir kalmasını sağlayabilirsiniz.
...
> **Note:** Aşağıdaki bilgiler **29 Mayıs 2025** tarihi itibarıyla doğrudur. MCP protokolü sürekli gelişmektedir ve gelecekteki uygulamalar yeni kimlik doğrulama desenleri ve kontroller getirebilir. En güncel bilgiler ve rehberlik için her zaman [MCP Specification](https://spec.modelcontextprotocol.io/), resmi [MCP GitHub deposu](https://github.com/modelcontextprotocol) ve [güvenlik en iyi uygulama sayfası](https://modelcontextprotocol.io/specification/draft/basic/security_best_practices) kaynaklarına başvurun.
...
- Yerel MCP sunucusunda OAuth token hırsızlığı. Çalınan token, MCP sunucusunu taklit etmek ve tokenin ait olduğu hizmetten kaynaklara ve verilere erişmek için kullanılabilir.

#### Token Passthrough  
Token passthrough, yetkilendirme spesifikasyonunda açıkça yasaktır çünkü aşağıdaki güvenlik risklerini beraberinde getirir:

#### Güvenlik Kontrolü Atlatma  
MCP Sunucusu veya alt API’ler, token hedef kitlesi veya diğer kimlik bilgisi kısıtlamalarına bağlı olarak hız sınırlaması, istek doğrulama veya trafik izleme gibi önemli güvenlik kontrolleri uygulayabilir. İstemciler, MCP sunucusunun tokenleri doğru şekilde doğrulamadan veya tokenlerin doğru hizmet için verildiğinden emin olmadan doğrudan alt API’lerle token kullanabilirse, bu kontroller atlanır.

#### Hesap Verebilirlik ve Denetim Sorunları  
MCP Sunucusu, istemciler yukarı akışta verilen erişim tokeni ile çağrı yaptığında MCP İstemcilerini tanımlayamaz veya ayırt edemez.  
Alt Kaynak Sunucusunun günlükleri, tokenleri ileten MCP sunucusu yerine farklı bir kaynaktan ve farklı bir kimlikle gelen istekler olarak görünebilir.  
Her iki faktör de olay incelemesini, kontrolleri ve denetimi zorlaştırır.  
MCP Sunucusu, tokenlerin iddialarını (örneğin roller, ayrıcalıklar veya hedef kitle) veya diğer meta verileri doğrulamadan tokenleri geçirirse, çalınan tokeni elinde bulunduran kötü niyetli bir aktör sunucuyu veri sızıntısı için vekil olarak kullanabilir.
...
Token, uygun doğrulama olmadan birden fazla hizmet tarafından kabul edilirse, bir hizmeti ele geçiren saldırgan tokeni diğer bağlı hizmetlere erişmek için kullanabilir.
...
**MCP sunucuları, açıkça MCP sunucusu için verilmemiş tokenleri KABUL ETMEMELİDİR**

- **Yetkilendirme Mantığını Gözden Geçirin ve Sertleştirin:** MCP sunucunuzun yetkilendirme uygulamasını dikkatlice denetleyin, sadece amaçlanan kullanıcıların ve istemcilerin hassas kaynaklara erişebildiğinden emin olun. Pratik rehberlik için [Azure API Management Your Auth Gateway For MCP Servers | Microsoft Community Hub](https://techcommunity.microsoft.com/blog/integrationsonazureblog/azure-api-management-your-auth-gateway-for-mcp-servers/4402690) ve [Using Microsoft Entra ID To Authenticate With MCP Servers Via Sessions - Den Delimarsky](https://den.dev/blog/mcp-server-auth-entra-id-session/) kaynaklarına bakabilirsiniz.
- **Güvenli Token Uygulamalarını Zorunlu Kılın:** Erişim tokenlerinin kötüye kullanımını önlemek ve token tekrar oynatma veya hırsızlık riskini azaltmak için [Microsoft’un token doğrulama ve ömür süresi en iyi uygulamalarını](https://learn.microsoft.com/en-us/entra/identity-platform/access-tokens) takip edin.
- **Token Depolamayı Koruyun:** Tokenleri her zaman güvenli şekilde depolayın ve hem dinlenme hem de iletim sırasında şifreleme kullanın. Uygulama ipuçları için [Use secure token storage and encrypt tokens](https://youtu.be/uRdX37EcCwg?si=6fSChs1G4glwXRy2) videosuna göz atabilirsiniz.

# MCP sunucuları için aşırı izinler

### Sorun Tanımı  
MCP sunucularına eriştikleri hizmet/kaynak üzerinde aşırı izinler verilmiş olabilir. Örneğin, bir AI satış uygulamasının parçası olan bir MCP sunucusu, kurumsal veri deposuna bağlanıyorsa, sadece satış verilerine erişim izni olmalı, depodaki tüm dosyalara erişim hakkı olmamalıdır. En az ayrıcalık ilkesi (en eski güvenlik ilkelerinden biri) gereği, hiçbir kaynak, yürütmesi gereken görevler için gerekli olandan fazla izin almamalıdır. AI bu alanda esnek olabilmesi için gereken kesin izinleri tanımlamak zor olduğundan ekstra zorluk yaratır.
...
### Azaltıcı Kontroller  
- **En Az Ayrıcalık İlkesini Uygulayın:** MCP sunucusuna sadece gerekli görevleri yerine getirmesi için minimum izinleri verin. Bu izinleri düzenli olarak gözden geçirip güncelleyerek gereksiz izinlerin önüne geçin. Detaylı rehber için [Secure least-privileged access](https://learn.microsoft.com/entra/identity-platform/secure-least-privileged-access) sayfasına bakabilirsiniz.  
...
- **Prompt Enjeksiyonu Saldırıları:** Saldırganlar, yapay zekanın istenmeyen işlemler yapmasına veya hassas verileri sızdırmasına neden olan kötü niyetli talimatları promptlara veya dış içeriklere gömer. Daha fazla bilgi: [Prompt Injection](https://simonwillison.net/2025/Apr/9/mcp-prompt-injection/)
...
1.  **Tespit ve Filtreleme:** Prompt Shields, belgeler, web sayfaları veya e-postalar gibi dış içeriklere gömülü kötü niyetli talimatları tespit etmek ve filtrelemek için gelişmiş makine öğrenimi algoritmaları ve doğal dil işleme teknikleri kullanır.
...
3.  **Sınırlandırıcılar ve Veri İşaretleme:** Sistem mesajına eklenen sınırlandırıcılar, girdi metninin yerini açıkça belirtir ve AI sisteminin kullanıcı girdilerini potansiyel zararlı dış içerikten ayırmasına yardımcı olur. Veri işaretleme, güvenilir ve güvenilmez verilerin sınırlarını vurgulamak için özel işaretler kullanarak bu kavramı genişletir.
...
AI prompt shields hakkında daha fazla bilgi için [Prompt Shields dokümantasyonuna](https://learn.microsoft.com/azure/ai-services/content-safety/concepts/jailbreak-detection) göz atabilirsiniz.
...
Microsoft ayrıca tüm ürünler için dahili olarak kapsamlı tedarik zinciri güvenliği uygulamaları yürütmektedir. Daha fazla bilgi için [The Journey to Secure the Software Supply Chain at Microsoft](https://devblogs.microsoft.com/engineering-at-microsoft/the-journey-to-secure-the-software-supply-chain-at-microsoft/) sayfasını inceleyebilirsiniz.
...
- Sunucu sertleştirme — mümkün olduğunda MFA kullanımı, yamaların güncel tutulması, erişim için üçüncü taraf kimlik sağlayıcı entegrasyonu vb.
- Cihazlar, altyapı ve uygulamaların yamalarla güncel tutulması
- Güvenlik izleme — yapay zeka uygulaması (MCP istemci/sunucuları dahil) için günlük kaydı ve izleme uygulayarak bu günlüklerin merkezi bir SIEM’e gönderilmesi ve anormal aktivitelerin tespiti
...
- MCP, prompt enjeksiyonu, araç zehirlenmesi ve aşırı izinler gibi yeni riskler getirir; bu riskler hem geleneksel hem de yapay zekaya özgü kontroller gerektirir.
- Güçlü kimlik doğrulama, yetkilendirme ve token yönetimi uygulamalarını kullanın; mümkünse Microsoft Entra ID gibi dış kimlik sağlayıcılarından yararlanın.
...
- Yapay zeka tedarik zincirinizdeki tüm bileşenlere — modeller, gömme servisleri ve bağlam sağlayıcılar dahil — kod bağımlılıklarıyla aynı titizlikle yaklaşın.
- Gelişen MCP spesifikasyonlarını takip edin ve güvenli standartların şekillenmesine katkıda bulunmak için topluluğa katılın.
