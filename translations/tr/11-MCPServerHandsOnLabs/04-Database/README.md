...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T17:03:04+00:00",
...
Bu laboratuvar, Zava Retail sistemi için PostgreSQL veritabanı tasarımını detaylı bir şekilde ele alıyor. Vektör arama yetenekleri, çoklu kiracı veri modelleme ve veri izolasyonu için Satır Düzeyi Güvenlik (RLS) içeren kapsamlı bir perakende şeması uygulamayı öğreneceksiniz.

## Genel Bakış

Veritabanı, MCP sunucumuzun temelini oluşturur ve birden fazla mağazadaki perakende verilerini saklarken sıkı veri izolasyonu sağlar. Doğal dil sorguları kullanarak müşterilerin ürünleri bulmasını sağlayan semantik arama yetenekleri için PostgreSQL ve pgvector uzantısını kullanıyoruz.

Şemamız, modern çoklu kiracı modellerini takip eder ve Satır Düzeyi Güvenlik ile kullanıcıların yalnızca yetkilendirilmiş mağazalarından veri erişimi sağlamasını garanti eder. Bu yaklaşım, kurumsal düzeyde güvenlik sağlarken optimum performansı korur.

## Öğrenme Hedefleri

Bu laboratuvarın sonunda şunları yapabileceksiniz:

- **Tasarım**: Ölçeklenebilir çoklu kiracı perakende veritabanı şemaları tasarlayın  
- **Uygulama**: Vektör arama için PostgreSQL ve pgvector kullanın  
- **Yapılandırma**: Veri izolasyonu için Satır Düzeyi Güvenlik yapılandırın  
- **Oluşturma**: Test için gerçekçi örnek veriler oluşturun  
- **Optimizasyon**: Perakende iş yükleri için veritabanı performansını optimize edin  
- **Uygulama**: Yedekleme ve kurtarma stratejileri oluşturun  

## 🗃️ Veritabanı Mimarisi

### PostgreSQL ve pgvector

Veritabanımız, PostgreSQL'in kurumsal özelliklerini AI destekli arama için pgvector uzantısıyla birleştirir:
...
Veritabanı, Satır Düzeyi Güvenlik ile **paylaşılan veritabanı, paylaşılan şema** çoklu kiracı modelini kullanır:
...
### Mağazalar Tablosu (Kiracı Ana Tablosu)
...
### Sorgu Optimizasyon Görünümleri
...
✅ **Çoklu Kiracı Veritabanı Tasarımı**: Güvenli veri izolasyonu için Satır Düzeyi Güvenlik uygulandı  
✅ **Vektör Arama Yetenekleri**: Semantik ürün araması için pgvector yapılandırıldı  
✅ **Kapsamlı Şema**: Üretime hazır perakende veritabanı şeması oluşturuldu  
✅ **Örnek Veri Oluşturma**: Geliştirme ve test için gerçekçi test verileri oluşturuldu  
✅ **Performans Optimizasyonu**: İndeksler ve sorgu optimizasyonu yapılandırıldı  
✅ **Yedekleme ve Kurtarma**: Güçlü veri koruma stratejileri oluşturuldu  

## 🚀 Sırada Ne Var?

**[Laboratuvar 05: MCP Sunucu Uygulaması](../05-MCP-Server/README.md)** ile devam edin:

- Bu veritabanına bağlanan FastMCP sunucusunu oluşturun  
- MCP protokolü için veritabanı sorgu araçlarını uygulayın  
- Gömüleri kullanarak semantik arama yetenekleri ekleyin  
- Bağlantı havuzu ve hata işleme yapılandırın  

## 📚 Ek Kaynaklar

### PostgreSQL ve pgvector
- [PostgreSQL Belgeleri](https://www.postgresql.org/docs/) - Tam PostgreSQL referansı  
- [pgvector Uzantısı](https://github.com/pgvector/pgvector) - PostgreSQL için vektör benzerlik araması  
- [PostgreSQL Performans Ayarları](https://wiki.postgresql.org/wiki/Performance_Optimization) - Optimizasyon en iyi uygulamaları  

### Çoklu Kiracı Mimarisi
- [Satır Düzeyi Güvenlik](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - PostgreSQL RLS belgeleri  
- [Çoklu Kiracı Veri Mimarisi](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Azure mimari desenleri  
- [Veritabanı Güvenliği En İyi Uygulamaları](https://www.postgresql.org/docs/current/security.html) - PostgreSQL güvenlik rehberi  

### Vektör Veritabanları
- [Vektör Arama Temelleri](https://www.pinecone.io/learn/vector-database/) - Vektör veritabanlarını anlama  
- [Gömü Modelleri](https://platform.openai.com/docs/guides/embeddings) - OpenAI gömü belgeleri  
- [HNSW Algoritması](https://arxiv.org/abs/1603.09320) - Hiyerarşik Gezilebilir Küçük Dünya grafikleri  

---

**Önceki**: [Laboratuvar 03: Ortam Kurulumu](../03-Setup/README.md)  
**Sonraki**: [Laboratuvar 05: MCP Sunucu Uygulaması](../05-MCP-Server/README.md)
