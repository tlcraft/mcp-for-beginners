...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T16:29:23+00:00",
...
## 🎯 Bu Laboratuvar Neleri Kapsar?

Bu uygulamalı laboratuvar, FastMCP framework kullanarak üretime hazır bir MCP sunucusu oluşturmayı öğretir. Sunucunun temel yapısını oluşturacak, veritabanı entegrasyonunu gerçekleştirecek, veri erişim araçları geliştirecek ve yapay zeka destekli perakende analitiği için temel oluşturacaksınız.

## Genel Bakış

MCP sunucusu, perakende analitiği çözümümüzün merkezidir. Yapay zeka asistanları ile PostgreSQL veritabanı arasında bir köprü görevi görerek, standart bir protokol aracılığıyla iş verilerine güvenli ve akıllı erişim sağlar.

Bu laboratuvar, kurumsal desenler ve en iyi uygulamaları takip ederek sağlam ve ölçeklenebilir bir MCP sunucusu oluşturmayı öğretir.

## Öğrenme Hedefleri

Bu laboratuvarın sonunda şunları yapabileceksiniz:

- **Oluşturmak**: Doğru mimari ve organizasyonla bir FastMCP sunucusu oluşturmak  
- **Uygulamak**: Bağlantı havuzu ve hata yönetimi ile veritabanı entegrasyonu  
- **Geliştirmek**: Veritabanı şema incelemesi ve sorgu yürütme için MCP araçları  
- **Yapılandırmak**: Satır Düzeyi Güvenlik (RLS) bağlam yönetimi  
- **Eklemek**: Sağlık izleme ve gözlemlenebilirlik özellikleri  
- **Test Etmek**: MCP sunucusu uygulamanızı yerel olarak ve VS Code ile test etmek  

## 📁 Proje Yapısı

MCP sunucusunun organizasyonuna bir göz atalım:
...
### Ortam Yapılandırması (`config.py`)

Öncelikle sağlam bir yapılandırma sistemi oluşturalım:
...
- **Ortam Değişkeni Yükleme**: Otomatik .env dosyası desteği  
- **Tip Güvenliği**: Dataclass doğrulama ve tip ipuçları  
- **Esnek Varsayılanlar**: Geliştirme için mantıklı varsayılanlar  
- **Doğrulama**: Yapılandırma doğrulama ve yardımcı hata mesajları  
- **Güvenlik**: Hassas değerler yalnızca ortam değişkenlerinden  

## 🗄️ Veritabanı Entegrasyon Katmanı

### PostgreSQL Sağlayıcı (`sales_analysis_postgres.py`)

Veritabanı entegrasyon katmanını uygulayalım:
...
### Temel Veritabanı Katmanı Özellikleri

- **Bağlantı Havuzu**: asyncpg ile verimli kaynak yönetimi  
- **RLS Entegrasyonu**: Otomatik Satır Düzeyi Güvenlik bağlam ayarı  
- **Şema İncelemesi**: Dinamik tablo şema keşfi  
- **Hata Yönetimi**: Kapsamlı hata yönetimi ve günlükleme  
- **Sorgu Formatlama**: Yapay zeka dostu sonuç formatlama  
- **Sağlık İzleme**: Veritabanı bağlantısı ve havuz durumu kontrolleri  

## 🔧 Ana MCP Sunucusu Uygulaması

### FastMCP Sunucusu (`sales_analysis.py`)

Şimdi ana MCP sunucusunu uygulayalım:
...
### Temel MCP Sunucusu Özellikleri

- **Araç Kaydı**: Tip güvenliği ile deklaratif araç tanımları  
- **RLS Bağlam Yönetimi**: Kullanıcı kimliği çıkarımı ve bağlam ayarı  
- **Hata Yönetimi**: Kullanıcı dostu mesajlarla kapsamlı hata yönetimi  
- **Yaşam Döngüsü Yönetimi**: Kaynak temizliği ile doğru başlatma/kapatma  
- **Sağlık İzleme**: Dahili sağlık kontrol uç noktaları  
- **Geliştirme Desteği**: Sıcak yeniden yükleme ve hata ayıklama yetenekleri  

## 🏥 Sağlık İzleme

### Sağlık Kontrol Uygulaması (`health_check.py`)
...
2. **Sağlık Uç Noktalarını Test Edin**:
   ```bash
   # Basic health check
...
### VS Code Entegrasyon Testi

1. **VS Code MCP'yi Yapılandırın**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **AI Sohbetinde Test Edin**:
   - VS Code AI Sohbeti açın  
   - `#zava` yazın ve sunucunuzu seçin  
   - Sorun: "Hangi tablolar mevcut?"  
   - Sorun: "Sipariş sayısına göre en çok 5 mağazayı göster"  

### Birim Testi

Kapsamlı birim testleri oluşturun:
...
✅ **Çalışan MCP Sunucusu**: Veritabanı entegrasyonu ile FastMCP sunucusu  
✅ **Yapılandırma Yönetimi**: Ortama dayalı sağlam yapılandırma  
✅ **Veritabanı Katmanı**: Bağlantı havuzu ile PostgreSQL entegrasyonu  
✅ **MCP Araçları**: Şema incelemesi ve sorgu yürütme araçları  
✅ **RLS Entegrasyonu**: Satır Düzeyi Güvenlik bağlam yönetimi  
✅ **Sağlık İzleme**: Kapsamlı sağlık kontrol uç noktaları  
✅ **Test Stratejisi**: Yerel test ve VS Code entegrasyonu  

## 🚀 Sıradaki Adım

**[Lab 06: Araç Geliştirme](../06-Tools/README.md)** ile devam edin:

- MCP araç koleksiyonunuzu genişletin  
- Gelişmiş sorgu desenleri uygulayın  
- Veri doğrulama ve dönüştürme ekleyin  
- Özel analitik araçlar oluşturun  

## 📚 Ek Kaynaklar

### FastMCP Framework
- [FastMCP Belgeleri](https://github.com/modelcontextprotocol/python-sdk) - Resmi FastMCP rehberi  
- [MCP Spesifikasyonu](https://modelcontextprotocol.io/docs/) - Protokol spesifikasyonu  
- [Araç Geliştirme Rehberi](https://modelcontextprotocol.io/docs/tools/) - MCP araçları oluşturma  

### Veritabanı Entegrasyonu
- [asyncpg Belgeleri](https://magicstack.github.io/asyncpg/current/) - PostgreSQL async sürücüsü  
- [Bağlantı Havuzu En İyi Uygulamalar](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL ayarları  
- [Satır Düzeyi Güvenlik Rehberi](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS uygulaması  

### FastAPI Desenleri
- [FastAPI Belgeleri](https://fastapi.tiangolo.com/) - Web framework referansı  
- [Bağımlılık Enjeksiyonu](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI desenleri  
- [Arka Plan Görevleri](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Async görev yönetimi  

---

**Sonraki**: Araçlarınızı genişletmeye hazır mısınız? [Lab 06: Araç Geliştirme](../06-Tools/README.md) ile devam edin.
