...
यह प्रैक्टिकल लैब आपको FastMCP फ्रेमवर्क का उपयोग करके एक प्रोडक्शन-रेडी MCP सर्वर को लागू करने के लिए मार्गदर्शन करता है। आप सर्वर की मुख्य संरचना बनाएंगे, डेटाबेस इंटीग्रेशन लागू करेंगे, डेटा एक्सेस के लिए टूल्स बनाएंगे, और AI-संचालित रिटेल एनालिटिक्स के लिए आधार तैयार करेंगे।

## अवलोकन

MCP सर्वर हमारे रिटेल एनालिटिक्स समाधान का केंद्र है। यह AI असिस्टेंट्स और PostgreSQL डेटाबेस के बीच पुल का काम करता है, एक मानकीकृत प्रोटोकॉल के माध्यम से सुरक्षित और बुद्धिमान तरीके से बिजनेस डेटा तक पहुंच प्रदान करता है।

यह लैब आपको एंटरप्राइज पैटर्न और सर्वोत्तम प्रथाओं का पालन करते हुए एक मजबूत और स्केलेबल MCP सर्वर बनाने की शिक्षा देती है।

## सीखने के उद्देश्य

इस लैब के अंत तक, आप सक्षम होंगे:

- **बनाना**: उचित आर्किटेक्चर और संगठन के साथ FastMCP सर्वर
- **लागू करना**: कनेक्शन पूलिंग और एरर हैंडलिंग के साथ डेटाबेस इंटीग्रेशन
- **बनाना**: डेटाबेस स्कीमा इंट्रोस्पेक्शन और क्वेरी निष्पादन के लिए MCP टूल्स
- **कॉन्फ़िगर करना**: Row Level Security (RLS) संदर्भ प्रबंधन
- **जोड़ना**: हेल्थ मॉनिटरिंग और ऑब्ज़र्वेबिलिटी फीचर्स
- **परीक्षण करना**: MCP सर्वर कार्यान्वयन को लोकल और VS Code के साथ

## 📁 प्रोजेक्ट संरचना

आइए MCP सर्वर के संगठन को देखें:
...
### एनवायरनमेंट कॉन्फ़िगरेशन (`config.py`)

पहले, एक मजबूत कॉन्फ़िगरेशन सिस्टम बनाएं:
...
### प्रमुख कॉन्फ़िगरेशन विशेषताएं

- **एनवायरनमेंट वेरिएबल लोडिंग**: स्वचालित .env फ़ाइल समर्थन
- **टाइप सेफ्टी**: Dataclass वैलिडेशन और टाइप हिंट्स
- **लचीले डिफॉल्ट्स**: विकास के लिए समझदार डिफॉल्ट्स
- **वैलिडेशन**: सहायक एरर संदेशों के साथ कॉन्फ़िगरेशन वैलिडेशन
- **सुरक्षा**: संवेदनशील मान केवल एनवायरनमेंट वेरिएबल्स से

## 🗄️ डेटाबेस इंटीग्रेशन लेयर

### PostgreSQL प्रोवाइडर (`sales_analysis_postgres.py`)

आइए डेटाबेस इंटीग्रेशन लेयर लागू करें:
...
### प्रमुख डेटाबेस लेयर विशेषताएं

- **कनेक्शन पूलिंग**: asyncpg के साथ कुशल संसाधन प्रबंधन
- **RLS इंटीग्रेशन**: स्वचालित Row Level Security संदर्भ सेटिंग
- **स्कीमा इंट्रोस्पेक्शन**: डायनामिक टेबल स्कीमा डिस्कवरी
- **एरर हैंडलिंग**: व्यापक एरर प्रबंधन और लॉगिंग
- **क्वेरी फॉर्मेटिंग**: AI-अनुकूल परिणाम फॉर्मेटिंग
- **हेल्थ मॉनिटरिंग**: डेटाबेस कनेक्टिविटी और पूल स्टेटस चेक्स

## 🔧 मुख्य MCP सर्वर कार्यान्वयन

### FastMCP सर्वर (`sales_analysis.py`)

अब मुख्य MCP सर्वर लागू करें:
...
### प्रमुख MCP सर्वर विशेषताएं

- **टूल रजिस्ट्रेशन**: टाइप सेफ्टी के साथ डिक्लेरेटिव टूल डिफिनिशन
- **RLS संदर्भ प्रबंधन**: स्वचालित उपयोगकर्ता पहचान निष्कर्षण और संदर्भ सेटिंग
- **एरर हैंडलिंग**: उपयोगकर्ता-अनुकूल संदेशों के साथ व्यापक एरर प्रबंधन
- **लाइफसाइकिल प्रबंधन**: उचित स्टार्टअप/शटडाउन और संसाधन सफाई
- **हेल्थ मॉनिटरिंग**: बिल्ट-इन हेल्थ चेक एंडपॉइंट्स
- **विकास समर्थन**: हॉट रीलोड और डिबगिंग क्षमताएं

## 🏥 हेल्थ मॉनिटरिंग

### हेल्थ चेक कार्यान्वयन (`health_check.py`)
...
2. **हेल्थ एंडपॉइंट्स का परीक्षण करें**:
   ```bash
   # Basic health check
...
### VS Code इंटीग्रेशन परीक्षण

1. **VS Code MCP कॉन्फ़िगर करें**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **AI चैट में परीक्षण करें**:
   - VS Code AI चैट खोलें
   - टाइप करें `#zava` और अपना सर्वर चुनें
   - पूछें: "कौन-कौन से टेबल उपलब्ध हैं?"
   - पूछें: "ऑर्डर्स की संख्या के आधार पर शीर्ष 5 स्टोर्स दिखाएं"

### यूनिट परीक्षण

व्यापक यूनिट टेस्ट बनाएं:
...
✅ **कार्यशील MCP सर्वर**: डेटाबेस इंटीग्रेशन के साथ FastMCP सर्वर  
✅ **कॉन्फ़िगरेशन प्रबंधन**: मजबूत एनवायरनमेंट-आधारित कॉन्फ़िगरेशन  
✅ **डेटाबेस लेयर**: कनेक्शन पूलिंग के साथ PostgreSQL इंटीग्रेशन  
✅ **MCP टूल्स**: स्कीमा इंट्रोस्पेक्शन और क्वेरी निष्पादन टूल्स  
✅ **RLS इंटीग्रेशन**: Row Level Security संदर्भ प्रबंधन  
✅ **हेल्थ मॉनिटरिंग**: व्यापक हेल्थ चेक एंडपॉइंट्स  
✅ **परीक्षण रणनीति**: लोकल परीक्षण और VS Code इंटीग्रेशन  

## 🚀 आगे क्या करें

**[लैब 06: टूल डेवलपमेंट](../06-Tools/README.md)** के साथ जारी रखें:

- अपने MCP टूल संग्रह का विस्तार करें
- उन्नत क्वेरी पैटर्न लागू करें
- डेटा वैलिडेशन और ट्रांसफॉर्मेशन जोड़ें
- विशेष एनालिटिक्स टूल्स बनाएं

## 📚 अतिरिक्त संसाधन

### FastMCP फ्रेमवर्क
- [FastMCP दस्तावेज़](https://github.com/modelcontextprotocol/python-sdk) - आधिकारिक FastMCP गाइड
- [MCP स्पेसिफिकेशन](https://modelcontextprotocol.io/docs/) - प्रोटोकॉल स्पेसिफिकेशन
- [टूल डेवलपमेंट गाइड](https://modelcontextprotocol.io/docs/tools/) - MCP टूल्स बनाना

### डेटाबेस इंटीग्रेशन
- [asyncpg दस्तावेज़](https://magicstack.github.io/asyncpg/current/) - PostgreSQL async ड्राइवर
- [कनेक्शन पूलिंग सर्वोत्तम प्रथाएं](https://www.postgresql.org/docs/current/runtime-config-connection.html) - PostgreSQL ट्यूनिंग
- [Row Level Security गाइड](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - RLS कार्यान्वयन

### FastAPI पैटर्न
- [FastAPI दस्तावेज़](https://fastapi.tiangolo.com/) - वेब फ्रेमवर्क संदर्भ
- [डिपेंडेंसी इंजेक्शन](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI पैटर्न
- [बैकग्राउंड टास्क](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Async टास्क प्रबंधन

---

**अगला**: अपने टूल्स का विस्तार करने के लिए तैयार हैं? [लैब 06: टूल डेवलपमेंट](../06-Tools/README.md) के साथ जारी रखें।
