...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T20:31:32+00:00",
...
यह अध्याय HTTPS का उपयोग करके Model Context Protocol (MCP) के साथ सुरक्षित, स्केलेबल और रियल-टाइम स्ट्रीमिंग को लागू करने के लिए एक व्यापक मार्गदर्शिका प्रदान करता है। इसमें स्ट्रीमिंग के लिए प्रेरणा, उपलब्ध ट्रांसपोर्ट मैकेनिज्म, MCP में स्ट्रीमेबल HTTP को कैसे लागू करें, सुरक्षा सर्वोत्तम प्रथाएँ, SSE से माइग्रेशन, और अपने स्वयं के स्ट्रीमिंग MCP एप्लिकेशन बनाने के लिए व्यावहारिक मार्गदर्शन शामिल है।
...
- लंबी अवधि के कंप्यूटेशन जहां आप उपयोगकर्ता को सूचित रखना चाहते हैं।
...
यह उदाहरण दिखाता है कि सर्वर क्लाइंट को संदेशों की एक श्रृंखला भेजता है जैसे ही वे उपलब्ध होते हैं, बजाय इसके कि सभी संदेश तैयार होने का इंतजार किया जाए।

**यह कैसे काम करता है:**
- सर्वर प्रत्येक संदेश को जैसे ही वह तैयार होता है, भेजता है।
- क्लाइंट प्रत्येक प्राप्त हिस्से को प्राप्त कर प्रिंट करता है।
...
- `WebClient` के `bodyToFlux()` के साथ रिएक्टिव स्ट्रीमिंग उपभोग सक्षम करता है
...
"क्लासिक" तरीके से स्ट्रीमिंग और MCP में स्ट्रीमिंग के बीच के अंतर को इस प्रकार दर्शाया जा सकता है:
...
- **क्लाइंट कार्यान्वयन:**
   ...
   - MCP: अधिक परिष्कृत क्लाइंट जिसमें विभिन्न प्रकार के संदेशों को प्रोसेस करने के लिए मैसेज हैंडलर होता है
...
क्लासिक स्ट्रीमिंग (जैसे हमने ऊपर `/stream` एंडपॉइंट के साथ दिखाया) और MCP के माध्यम से स्ट्रीमिंग के बीच चयन करते समय कुछ सिफारिशें हैं:
...
- **AI एप्लिकेशन के लिए:** MCP का नोटिफिकेशन सिस्टम लंबी अवधि के AI कार्यों के लिए विशेष रूप से उपयोगी है जहां आप उपयोगकर्ताओं को प्रगति की जानकारी देना चाहते हैं।

## MCP में स्ट्रीमिंग

ठीक है, अब तक आपने क्लासिक स्ट्रीमिंग और MCP स्ट्रीमिंग के बीच कुछ सिफारिशें और तुलना देखी हैं। आइए विस्तार से देखें कि आप MCP में स्ट्रीमिंग का लाभ कैसे उठा सकते हैं।
...
क्लाइंट को एक मैसेज हैंडलर लागू करना चाहिए जो नोटिफिकेशन को प्राप्त कर प्रोसेस और प्रदर्शित करे।
...
यह अनुभाग MCP में प्रगति नोटिफिकेशन की अवधारणा, उनके महत्व, और Streamable HTTP का उपयोग करके उन्हें कैसे लागू करें, समझाता है। साथ ही, आपकी समझ को मजबूत करने के लिए एक व्यावहारिक असाइनमेंट भी दिया गया है।

प्रगति नोटिफिकेशन वे रियल-टाइम संदेश हैं जो लंबी अवधि के ऑपरेशनों के दौरान सर्वर से क्लाइंट को भेजे जाते हैं। पूरी प्रक्रिया के समाप्त होने का इंतजार करने के बजाय, सर्वर क्लाइंट को वर्तमान स्थिति के बारे में अपडेट करता रहता है। इससे पारदर्शिता, उपयोगकर्ता अनुभव बेहतर होता है और डिबगिंग आसान होती है।
...
- **सर्वर पर:** प्रत्येक आइटम प्रोसेस होने पर `ctx.info()` या `ctx.log()` का उपयोग करके नोटिफिकेशन भेजें। यह मुख्य परिणाम तैयार होने से पहले क्लाइंट को संदेश भेजता है।
...
### मुख्य बिंदु
- **Origin Header सत्यापन**: DNS रिबाइंडिंग हमलों को रोकने के लिए हमेशा `Origin` हेडर की जांच करें।
...
### चुनौतियाँ
- सुरक्षा और विकास की सरलता के बीच संतुलन बनाना
...
- मौजूदा टूल्स और वर्कफ़्लोज़ के साथ संगतता के लिए परीक्षण करें।
...
माइग्रेशन के दौरान निम्नलिखित चुनौतियों का समाधान करें:
...
HTTP-आधारित ट्रांसपोर्ट जैसे Streamable HTTP का उपयोग करते समय MCP सर्वर लागू करते समय सुरक्षा सर्वोच्च प्राथमिकता होनी चाहिए।
...
2. एक क्लाइंट लागू करें जिसमें नोटिफिकेशन दिखाने के लिए मैसेज हैंडलर हो।
...
MCP स्ट्रीमिंग के साथ अपनी यात्रा जारी रखने और अपने ज्ञान को बढ़ाने के लिए, यह अनुभाग अतिरिक्त संसाधन और अधिक उन्नत एप्लिकेशन बनाने के लिए सुझाव प्रदान करता है।
