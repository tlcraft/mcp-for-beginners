...
- LLM का उपयोग करके MCP सर्वर के साथ सहजता से इंटरैक्ट करना।
- क्लाइंट साइड पर बेहतर अंतिम उपयोगकर्ता अनुभव प्रदान करना।

## दृष्टिकोण

आइए समझते हैं कि हमें कौन सा दृष्टिकोण अपनाना होगा। LLM जोड़ना सरल लगता है, लेकिन क्या हम वास्तव में ऐसा करेंगे?

यहाँ बताया गया है कि क्लाइंट सर्वर के साथ कैसे इंटरैक्ट करेगा:

1. सर्वर के साथ कनेक्शन स्थापित करें।

1. क्षमताओं, प्रॉम्प्ट्स, संसाधनों और टूल्स की सूची बनाएं, और उनके स्कीमा को सेव करें।

1. एक LLM जोड़ें और सेव की गई क्षमताओं और उनके स्कीमा को उस फॉर्मेट में पास करें जिसे LLM समझता है।

1. उपयोगकर्ता के प्रॉम्प्ट को LLM को पास करें, साथ ही क्लाइंट द्वारा सूचीबद्ध टूल्स भी भेजें।

बहुत बढ़िया, अब जब हम उच्च स्तर पर समझ गए हैं कि इसे कैसे करना है, तो नीचे दिए गए अभ्यास में इसे आजमाते हैं।
...
- एक क्लास बनाई जिसमें दो सदस्य हैं, `client` और `openai`, जो हमें क्लाइंट प्रबंधन और LLM के साथ इंटरैक्शन में मदद करेंगे।
...
- एक `McpToolProvider` बनाया जो स्वचालित रूप से MCP सर्वर से सभी टूल्स खोजता और रजिस्टर करता है
- टूल प्रोवाइडर MCP टूल स्कीमा और LangChain4j के टूल फॉर्मेट के बीच रूपांतरण को आंतरिक रूप से संभालता है
- यह दृष्टिकोण मैनुअल टूल लिस्टिंग और रूपांतरण प्रक्रिया को छुपा देता है
...
1. MCP सर्वर से प्रतिक्रिया को LLM के उपयोग के लिए टूल फॉर्मेट में कन्वर्ट करने के लिए निम्नलिखित कोड जोड़ें:
    ...
    ऊपर दिया गया कोड MCP सर्वर से प्रतिक्रिया लेकर उसे LLM के समझने योग्य टूल डिफिनिशन फॉर्मेट में बदलता है।
...
1. सबसे पहले, निम्न कन्वर्टर फ़ंक्शन बनाएं:
    ...
    ऊपर दिए गए `convert_to_llm_tools` फ़ंक्शन में हम MCP टूल प्रतिक्रिया को LLM के समझने योग्य फॉर्मेट में बदलते हैं।
...
1. फिर, अपने क्लाइंट कोड को इस फ़ंक्शन का उपयोग करने के लिए अपडेट करें:
    ...
    यहाँ, हम `convert_to_llm_tool` को कॉल कर रहे हैं ताकि MCP टूल प्रतिक्रिया को बाद में LLM को फीड करने योग्य बनाया जा सके।
...
- एक फ़ंक्शनलिटी परिभाषित की जो `FunctionDefinition` बनाता है जिसे `ChatCompletionsDefinition` को पास किया जाता है। यह वह फॉर्मेट है जिसे LLM समझता है।

1. अब देखें कि हम इस फ़ंक्शन का उपयोग कैसे कर सकते हैं:
...
- LangChain4j के `AiServices` का उपयोग किया ताकि LLM को MCP टूल प्रोवाइडर के साथ ऑटोमैटिकली बाइंड किया जा सके
- फ्रेमवर्क टूल स्कीमा कन्वर्ज़न और फ़ंक्शन कॉलिंग को अपने आप संभालता है
- यह दृष्टिकोण मैनुअल टूल कन्वर्ज़न को खत्म करता है - LangChain4j MCP टूल्स को LLM-समर्थित फॉर्मेट में कन्वर्ट करने की सारी जटिलता संभालता है
...
1. एक मेथड जोड़ें जो हमारे LLM को कॉल करने के लिए उपयोग किया जाएगा:
    ...
    - यदि LLM संकेत देता है कि टूल को कॉल करना चाहिए, तो टूल को कॉल करता है:
...
1. अंतिम चरण, मुख्य कोड को अपडेट करें:
    ...
    - `call_tool` के माध्यम से MCP टूल को कॉल किया, वह फ़ंक्शन जो LLM ने हमारे प्रॉम्प्ट के आधार पर कॉल करने के लिए सुझाया था।
...
1. अंतिम चरण, देखें कि क्या LLM सोचता है कि हमें कोई फ़ंक्शन कॉल करना चाहिए:
    ...
    - प्रत्येक टूल कॉल के लिए नाम और आर्गुमेंट्स पार्स किए और MCP क्लाइंट का उपयोग करके MCP सर्वर पर टूल को कॉल किया। अंत में परिणाम प्रिंट किए।
...
- LangChain4j फ्रेमवर्क अपने आप संभालता है:
  - उपयोगकर्ता प्रॉम्प्ट्स को आवश्यकतानुसार टूल कॉल में कन्वर्ट करना
...
- यह दृष्टिकोण उपयोगकर्ता को एक सहज अनुभव प्रदान करता है जहाँ उन्हें MCP के अंतर्निहित कार्यान्वयन के बारे में जानने की जरूरत नहीं होती
...
- अपने क्लाइंट में LLM जोड़ने से उपयोगकर्ताओं के लिए MCP सर्वर के साथ इंटरैक्शन का बेहतर तरीका मिलता है।
- आपको MCP सर्वर की प्रतिक्रिया को LLM के समझने योग्य फॉर्मेट में कन्वर्ट करना होगा।
