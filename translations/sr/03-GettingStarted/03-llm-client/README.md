...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T11:54:34+00:00",
...
Сјајно, сада када разумемо како то можемо урадити на високом нивоу, хајде да пробамо у следећој вежби.

## Вежба: Креирање клијента са LLM-ом

У овој вежби научићемо како да додамо LLM нашем клијенту.

## Аутентификација коришћењем GitHub Personal Access Token-а
...
Сјајно, за следећи корак хајде да набројимо могућности на серверу.
...
- Овај приступ уклања потребу за ручним листањем и конверзијом алата

### -3- Конверзија могућности сервера у LLM алате

Следећи корак након листања могућности сервера је да их конвертујемо у формат који LLM разуме. Када то урадимо, можемо ове могућности пружити као алате нашем LLM-у.
...
1. Прво, хајде да креирамо следећу конверзиону функцију
    ...
    У функцији `convert_to_llm_tools` узимамо одговор MCP алата и конвертујемо га у формат који LLM разуме.
...
1. Затим, ажурирајмо наш клијентски код да користи ову функцију овако:
    ...
    Овде позивамо `convert_to_llm_tool` да конвертујемо одговор MCP алата у нешто што касније можемо проследити LLM-у.

### .NET

1. Додајмо код за конверзију одговора MCP алата у нешто што LLM може разумети
...
- Дефинисали функционалност која креира `FunctionDefinition` који се прослеђује `ChatCompletionsDefinition`. Ово последње LLM разуме.
...
1. Хајде да видимо како можемо ажурирати постојећи код да искористи ову функцију:
    ...
    - Ажурирали функцију да конвертује одговор MCP алата у LLM алат. Ево кода који смо додали:
        ...
        Улазна шема је део одговора алата, али у атрибуту "properties", па је морамо издвојити. Затим позивамо `ConvertFrom` са детаљима алата. Сада када смо урадили тежак део, хајде да видимо како се позив саставља док обрађујемо кориснички упит.
...
1. Затим, додајмо функцију која ће позивати LLM:
    ...
    - Испитујемо резултат да видимо које функције треба позвати, ако их има.
...
1. Последњи корак, ажурирајмо главни код:
    ...
    Ето, то је био последњи корак, у горњем коду:

    - Позивамо MCP алат преко `call_tool` користећи функцију коју је LLM одлучио да треба позвати на основу нашег упита.
...
Узмите код из вежбе и проширите сервер са још алата. Затим креирајте клијента са LLM-ом, као у вежби, и тестирајте га са различитим упитима да бисте били сигурни да се сви алати на серверу динамички позивају. Овај начин креирања клијента омогућава крајњем кориснику одлично искуство јер може да користи упите уместо тачних команди клијента и не мора да зна да ли се MCP сервер позива.
...
- Потребно је конвертовати одговор MCP сервера у формат који LLM разуме.
