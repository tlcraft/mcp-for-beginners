...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-10T16:26:23+00:00",
...
Обратите пажњу на транспортне механизме stdio и SSE које сте видели у претходним поглављима и како је streamable HTTP транспорт који се обрађује у овом поглављу.
...
- `ServerSentEvent` пружа структуирани стриминг догађаја са типовима догађаја
...
Нотификација је порука послата са сервера ка клијенту да обавести о напретку, статусу или другим догађајима током дуготрајне операције. Нотификације побољшавају транспарентност и корисничко искуство.
...
### Са серверске стране: Слање нотификација
...
У претходном примеру, алат `process_files` шаље три нотификације клијенту док обрађује сваки фајл. Метода `ctx.info()` се користи за слање информативних порука.
...
У овом .NET примеру, алат `ProcessFiles` је означен атрибутом `Tool` и шаље три нотификације клијенту док обрађује сваки фајл. Метода `ctx.Info()` се користи за слање информативних порука.
...
### Са клијентске стране: Примање нотификација
...
У претходном коду, функција `message_handler` проверава да ли је долазна порука нотификација. Ако јесте, исписује нотификацију; у супротном, обрађује је као редовну поруку сервера. Такође, обратите пажњу како је `ClientSession` иницијализован са `message_handler` за обраду долазних нотификација.
...
У овом .NET примеру, функција `MessageHandler` проверава да ли је долазна порука нотификација. Ако јесте, исписује нотификацију; у супротном, обрађује је као редовну поруку сервера. `ClientSession` је иницијализован са обрадом порука преко `ClientSessionOptions`.
...
- **Веза на localhost**: За локални развој, повежите сервере на `localhost` да их не бисте изложили јавном интернету.
- **Аутентификација**: Упроведите аутентификацију (нпр. API кључеве, OAuth) за продукционе инсталације.
...
За апликације које тренутно користе Server-Sent Events (SSE), миграција на Streamable HTTP пружа боље могућности и дугорочнију одрживост за ваше MCP имплементације.
...
Безбедност треба да буде највиши приоритет при имплементацији било ког сервера, посебно када користите HTTP транспорте као што је Streamable HTTP у MCP.
...
- [Microsoft: CORS у ASP.NET Core](https://learn.microsoft.com/en-us/aspnet/core/security/cors?view=aspnetcore-8.0&WT.mc_id=%3Fwt.mc_id%3DMVP_452430)
