...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-13T16:25:09+00:00",
...
Шаблон упутства може изгледати овако:
...
Алат у Model Context Protocol-у (MCP) су функције које AI модел може извршити да би обавио одређене задатке. Ови алати су дизајнирани да побољшају могућности AI модела пружајући структуиране и поуздане операције. Кључни аспекти укључују:

- **Функције које AI модел може извршити**: Алати су извршне функције које AI модел може позвати да обави различите задатке.
- **Јединствено име и опис**: Сваки алат има јединствено име и детаљан опис који објашњава његову сврху и функционалност.
- **Параметри и излази**: Алати прихватају одређене параметре и враћају структуиране резултате, обезбеђујући доследне и предвидиве исходе.
- **Дискретне функције**: Алати обављају дискретне функције као што су претрага веба, калкулације и упити у базу података.

Пример алата може изгледати овако:
...
- **Агентске радње инициране од стране сервера**: Клијенти омогућавају серверима да аутономно иницирају одређене акције или понашања, побољшавајући динамичке могућности система.
...
- **Коришћење ресурса или алата**  
  ...
  - Ако модел процени да је потребан алат (нпр. за преузимање података, израчунавање или позив API-ја), клијент шаље захтев за позив алата серверу, наводећи име алата и параметре.

- **Извршење на серверу**  
  Сервер прими захтев за ресурс или алат, изврши потребне операције (нпр. покретање функције, упит у базу података или преузимање фајла) и врати резултате клијенту у структурираном формату.
...
Испод су неке додатне погодности и проширења протокола које MCP пружа за побољшање искуства програмера и омогућавање напредних сценарија:
...
- **Отказивање захтева**: Клијенти могу отказати захтеве у току, дозвољавајући корисницима да прекину операције које
...
4. **Ограничење учесталости (Rate Limiting)**:  
  Да би се спречила злоупотреба и обезбедила фер употреба ресурса сервера, MCP сервери могу имплементирати ограничења учесталости за позиве алата и приступ ресурсима. Ограничења могу бити по кориснику, по сесији или глобално, и помажу у заштити од DoS напада или прекомерне потрошње ресурса.

Комбиновањем ових механизама, MCP пружа сигурну основу за интеграцију језичких модела са спољним алатима и изворима података, истовремено дајући корисницима и програмерима прецизну контролу над приступом и коришћењем.
...
- **Захтев клијента**  
  Слање са клијента на сервер, ова порука обично садржи:
...
- **Захтев алата**  
  Слање са клијента на сервер када је потребно извршити алат. Ова порука укључује:
  - Назив алата који треба позвати
