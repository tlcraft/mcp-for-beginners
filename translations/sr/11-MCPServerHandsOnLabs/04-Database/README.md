...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:33:10+00:00",
  "source_file": "11-MCPServerHandsOnLabs/04-Database/README.md",
  "language_code": "sr"
}
-->
# Дизајн базе података и шема

## 🎯 Шта овај практични рад покрива

Овај практични рад се детаљно бави дизајном PostgreSQL базе података за систем Zava Retail. Научићете како да имплементирате свеобухватну шему за малопродају са могућностима претраге засноване на векторима, моделовањем података за више корисника и Row Level Security (RLS) за изолацију података.

## Преглед

База података је основа нашег MCP сервера, чувајући малопродајне податке за више продавница уз строго одржавање изолације података. Користимо PostgreSQL са pgvector екстензијом за омогућавање семантичке претраге, што купцима омогућава да пронађу производе користећи природни језик.

Наша шема прати савремене обрасце за више корисника, а Row Level Security осигурава да корисници могу приступити само подацима из својих овлашћених продавница. Овај приступ пружа безбедност на нивоу предузећа уз оптималне перформансе.

## Циљеви учења

На крају овог практичног рада, бићете у могућности да:

- **Дизајнирате** скалабилне шеме базе података за више корисника у малопродаји
- **Имплементирате** PostgreSQL са pgvector за претрагу засновану на векторима
- **Конфигуришете** Row Level Security за изолацију података
- **Генеришете** реалистичне узорке података за тестирање
- **Оптимизујете** перформансе базе података за малопродајне радне оптерећења
- **Имплементирате** стратегије за резервне копије и опоравак података

## 🗃️ Архитектура базе података

### PostgreSQL са pgvector

Наша база података користи предности PostgreSQL-а у комбинацији са pgvector екстензијом за претрагу засновану на вештачкој интелигенцији:
...
### Архитектура за више корисника

База података користи модел **заједничке базе података, заједничке шеме** са Row Level Security:
...
### Табела продавница (мастер за кориснике)
...
### Табела производа са категоријама
...
## 🔍 Имплементација претраге засноване на векторима

### Табела уграђених вектора производа
...
### Функције за претрагу засновану на векторима
...
## 🔐 Подешавање Row Level Security

### Роле базе података и дозволе
...
## 📊 Генерисање узорака података

### Скрипта за реалистичне тест податке
...
✅ **Дизајн базе података за више корисника**: Имплементиран Row Level Security за сигурну изолацију података  
✅ **Могућности претраге засноване на векторима**: Конфигурисан pgvector за семантичку претрагу производа  
✅ **Свеобухватна шема**: Креирана шема базе података спремна за производњу  
✅ **Генерисање узорака података**: Направљени реалистични тест подаци за развој и тестирање  
✅ **Оптимизација перформанси**: Конфигурисани индекси и оптимизација упита  
✅ **Резервне копије и опоравак**: Успостављене робусне стратегије заштите података  

## 🚀 Шта следи

Наставите са **[Практичним радом 05: Имплементација MCP сервера](../05-MCP-Server/README.md)** да:

- Направите FastMCP сервер који се повезује са овом базом података
- Имплементирате алате за упите базе података за MCP протокол
- Додате могућности семантичке претраге користећи уграђене векторе
- Конфигуришете пулинг конекција и руковање грешкама

## 📚 Додатни ресурси

### PostgreSQL & pgvector
- [PostgreSQL документација](https://www.postgresql.org/docs/) - Комплетна референца за PostgreSQL
- [pgvector екстензија](https://github.com/pgvector/pgvector) - Претрага сличности вектора за PostgreSQL
- [Оптимизација перформанси PostgreSQL-а](https://wiki.postgresql.org/wiki/Performance_Optimization) - Најбоље праксе за оптимизацију

### Архитектура за више корисника
- [Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - PostgreSQL документација за RLS
- [Архитектура података за више корисника](https://docs.microsoft.com/azure/architecture/patterns/multitenancy) - Azure обрасци архитектуре
- [Најбоље праксе за безбедност базе података](https://www.postgresql.org/docs/current/security.html) - Водич за безбедност PostgreSQL-а

### Векторске базе података
- [Основе претраге засноване на векторима](https://www.pinecone.io/learn/vector-database/) - Разумевање векторских база података
- [Модели уграђивања](https://platform.openai.com/docs/guides/embeddings) - Документација за OpenAI уграђивања
- [HNSW алгоритам](https://arxiv.org/abs/1603.09320) - Графови за хијерархијски навигабилни мали свет

---

**Претходно**: [Практични рад 03: Подешавање окружења](../03-Setup/README.md)  
**Следеће**: [Практични рад 05: Имплементација MCP сервера](../05-MCP-Server/README.md)
