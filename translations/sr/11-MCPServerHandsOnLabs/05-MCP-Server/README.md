...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T23:01:31+00:00",
  "source_file": "11-MCPServerHandsOnLabs/05-MCP-Server/README.md",
  "language_code": "sr"
}
-->
# MCP Сервер Имплементација

## 🎯 Шта овај лабораторијски рад покрива

Овај практични лабораторијски рад вас води кроз имплементацију MCP сервера спремног за производњу користећи FastMCP оквир. Изградићете основну структуру сервера, имплементирати интеграцију са базом података, креирати алате за приступ подацима и поставити темеље за аналитичке алате засноване на вештачкој интелигенцији у малопродаји.

## Преглед

MCP сервер је срце нашег решења за аналитичке алате у малопродаји. Он представља мост између AI асистената и PostgreSQL базе података, пружајући сигуран и интелигентан приступ пословним подацима кроз стандардизовани протокол.

Овај лабораторијски рад вас учи како да изградите робустан и скалабилан MCP сервер пратећи корпоративне шаблоне и најбоље праксе.

## Циљеви учења

На крају овог лабораторијског рада, бићете у могућности да:

- **Изградите** FastMCP сервер са правилном архитектуром и организацијом
- **Имплементирате** интеграцију са базом података уз коришћење пула конекција и обраду грешака
- **Креирате** MCP алате за интроспекцију шеме базе података и извршавање упита
- **Конфигуришете** управљање контекстом безбедности на нивоу редова (Row Level Security)
- **Додате** функције за праћење здравља система и опсервабилност
- **Тестирате** имплементацију MCP сервера локално и у VS Code-у

## 📁 Структура пројекта

Хајде да погледамо организацију MCP сервера:
...
### Конфигурација окружења (`config.py`)

Прво, креирајмо робустан систем конфигурације:
...
### Кључне карактеристике конфигурације

- **Учитавање променљивих окружења**: Аутоматска подршка за .env датотеке
- **Типска сигурност**: Валидација датакласа и типске назнаке
- **Флексибилни подразумевани параметри**: Смислени подразумевани параметри за развој
- **Валидација**: Валидација конфигурације са корисним порукама о грешкама
- **Безбедност**: Осетљиве вредности само из променљивих окружења

## 🗄️ Слој интеграције са базом података

### PostgreSQL провајдер (`sales_analysis_postgres.py`)

Имплементирајмо слој интеграције са базом података:
...
### Кључне карактеристике слоја базе података

- **Пул конекција**: Ефикасно управљање ресурсима уз asyncpg
- **Интеграција RLS**: Аутоматско постављање контекста безбедности на нивоу редова
- **Интроспекција шеме**: Динамичко откривање шеме табела
- **Обрада грешака**: Свеобухватно управљање грешкама и логовање
- **Форматирање упита**: Резултати прилагођени AI алатима
- **Праћење здравља**: Провера повезаности базе и статуса пула

## 🔧 Главна имплементација MCP сервера

### FastMCP сервер (`sales_analysis.py`)

Сада имплементирајмо главни MCP сервер:
...
### Кључне карактеристике MCP сервера

- **Регистрација алата**: Декларативне дефиниције алата са типском сигурношћу
- **Управљање RLS контекстом**: Аутоматско извлачење идентитета корисника и постављање контекста
- **Обрада грешака**: Свеобухватно управљање грешкама са кориснички пријатељским порукама
- **Управљање животним циклусом**: Правилно покретање/гашење уз чишћење ресурса
- **Праћење здравља**: Уграђене тачке за проверу здравља система
- **Подршка за развој**: Могућност брзог поновног учитавања и дебаговања

## 🏥 Праћење здравља

### Имплементација провере здравља (`health_check.py`)
...
2. **Тестирајте тачке за проверу здравља**:
   ```bash
   # Basic health check
...
### Тестирање интеграције у VS Code-у

1. **Конфигуришите VS Code MCP**:
   ```json
   // .vscode/mcp.json
   {
       "servers": {
...
2. **Тестирајте у AI Chat-у**:
   - Отворите VS Code AI Chat
   - Унесите `#zava` и изаберите ваш сервер
   - Питајте: "Које табеле су доступне?"
   - Питајте: "Прикажи ми топ 5 продавница по броју поруџбина"

### Јединично тестирање

Креирајте свеобухватне јединичне тестове:
...
Након завршетка овог лабораторијског рада, требало би да имате:

✅ **Функционалан MCP сервер**: FastMCP сервер са интеграцијом базе података  
✅ **Управљање конфигурацијом**: Робусна конфигурација заснована на окружењу  
✅ **Слој базе података**: PostgreSQL интеграција са пулом конекција  
✅ **MCP алати**: Алати за интроспекцију шеме и извршавање упита  
✅ **Интеграција RLS**: Управљање контекстом безбедности на нивоу редова  
✅ **Праћење здравља**: Свеобухватне тачке за проверу здравља  
✅ **Стратегија тестирања**: Локално тестирање и интеграција у VS Code  

## 🚀 Шта следи

Наставите са **[Лабораторијом 06: Развој алата](../06-Tools/README.md)** да:

- Проширите вашу MCP колекцију алата
- Имплементирате напредне шаблоне упита
- Додате валидацију и трансформацију података
- Креирате специјализоване аналитичке алате

## 📚 Додатни ресурси

### FastMCP оквир
- [FastMCP документација](https://github.com/modelcontextprotocol/python-sdk) - Званични водич за FastMCP
- [MCP спецификација](https://modelcontextprotocol.io/docs/) - Спецификација протокола
- [Водич за развој алата](https://modelcontextprotocol.io/docs/tools/) - Креирање MCP алата

### Интеграција базе података
- [asyncpg документација](https://magicstack.github.io/asyncpg/current/) - PostgreSQL асинхрони драјвер
- [Најбоље праксе за пул конекција](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Подешавање PostgreSQL-а
- [Водич за безбедност на нивоу редова](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Имплементација RLS-а

### FastAPI шаблони
- [FastAPI документација](https://fastapi.tiangolo.com/) - Референца за веб оквир
- [Инјекција зависности](https://fastapi.tiangolo.com/tutorial/dependencies/) - FastAPI шаблони
- [Позадински задаци](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Управљање асинхроним задацима

---

**Следеће**: Спремни да проширите своје алате? Наставите са [Лабораторијом 06: Развој алата](../06-Tools/README.md)
