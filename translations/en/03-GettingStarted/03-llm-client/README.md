...
So far, you've seen how to create a server and a client. The client has been able to explicitly call the server to list its tools, resources, and prompts. However, this approach isn’t very practical. Your user lives in the agentic era and expects to use prompts and communicate with an LLM to do so. For your user, it doesn’t matter if you use MCP or not to store your capabilities, but they do expect to interact using natural language. So how do we solve this? The solution is to add an LLM to the client.
...
Let's try to understand the approach we need to take. Adding an LLM sounds simple, but how exactly will we do this?

Here's how the client will interact with the server:

1. Establish a connection with the server.

2. List capabilities, prompts, resources, and tools, and save their schema.
...
Great, now that we understand how to do this at a high level, let's try it out in the exercise below.

## Exercise: Creating a client with an LLM

In this exercise, we will learn how to add an LLM to our client.
...
1. Add the following code to convert the response from the MCP Server to a tool format the LLM can use:
    ...
    The above code takes a response from the MCP Server and converts it to a tool definition format the LLM can understand.
...
2. Let's see how we can update some existing code to take advantage of this function above:
    ...
    - Updated the function to convert the MCP tool response to an LLM tool. Let's highlight the code we added:
        ...
        The input schema is part of the tool response but on the "properties" attribute, so we need to extract it. Furthermore, we now call `ConvertFrom` with the tool details. Now that we've done the heavy lifting, let's see how it all comes together as we handle a user prompt next.
