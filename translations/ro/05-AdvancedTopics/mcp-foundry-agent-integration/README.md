...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-14T00:00:29+00:00",
...
Mai întâi, configurează variabilele de mediu și dependențele:
...
with project_client:
    agent = project_client.agents.create_agent(
        model="gpt-4.1-nano", 
        name="mcp_agent", 
        instructions="Ești un asistent de ajutor. Folosește uneltele puse la dispoziție pentru a răspunde întrebărilor. Asigură-te că citezi sursele.",
        tools=[
            {
                "type": "mcp",
...
def create_mcp_agent_example():
    ...
    with project_client:
        # Crează agent cu unelte MCP
        agent = project_client.agents.create_agent(
            model="gpt-4.1-nano", 
            name="documentation_assistant", 
            instructions="Ești un asistent util specializat în documentația Microsoft. Folosește serverul MCP Microsoft Learn pentru a căuta informații precise și actualizate. Citează întotdeauna sursele.",
            tools=[
                {
                    "type": "mcp",
        ...
        while run.status in ["queued", "in_progress", "requires_action"]:
            ...
            print(f"Status rulare: {run.status}")

        # Examinează pașii rulării și apelurile uneltelor
        run_steps = project_client.agents.run_steps.list(thread_id=thread.id, run_id=run.id)
        for step in run_steps:
            print(f"Pas rulare: {step.id}, status: {step.status}, tip: {step.type}")
