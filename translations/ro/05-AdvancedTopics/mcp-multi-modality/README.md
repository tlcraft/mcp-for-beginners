...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T11:16:39+00:00",
...
Aplicațiile multi-modale devin tot mai importante în AI, permițând interacțiuni mai bogate și sarcini mai complexe. Model Context Protocol (MCP) oferă un cadru pentru construirea aplicațiilor multi-modale care pot gestiona diverse tipuri de date, cum ar fi text, imagini și audio.
...
În această lecție, vei învăța cum să construiești o aplicație multi-modală.

## Obiective de învățare

La finalul acestei lecții, vei putea:

- Să înțelegi opțiunile multi-modale
- Să implementezi o aplicație multi-modală.

## Arhitectura pentru Suport Multi-Modal

Implementările MCP multi-modale implică de obicei:

- **Parser-e specifice modalităților**: Componente care convertesc diferite tipuri de media în formate pe care modelul le poate procesa.
- **Unelte specifice modalităților**: Unelte speciale concepute pentru a gestiona modalități specifice (analiza imaginilor, procesarea audio)
- **Management unificat al contextului**: Sistem pentru menținerea contextului între diferite modalități
- **Generarea răspunsurilor**: Capacitatea de a genera răspunsuri care pot include mai multe modalități.

## Exemplu Multi-Modal: Analiza Imaginilor

În exemplul de mai jos, vom analiza o imagine și vom extrage informații.
...
În exemplul precedent, am:

- Creat un `ImageAnalysisTool` care poate analiza imagini folosind un ipotetic `IImageAnalysisService`.
- Configurat serverul MCP pentru a gestiona cereri mai mari și pentru a suporta tipuri de conținut imagine.
- Înregistrat unealta de analiză a imaginilor pe server.
- Implementat o metodă pentru descărcarea imaginilor de la o adresă URL și analizarea lor în funcție de tipul cerut (obiecte, text, fețe etc.).
- Returnat rezultate structurate într-un format conform specificației MCP.

## Exemplu Multi-Modal: Procesarea Audio

Procesarea audio este o altă modalitate comună în aplicațiile multi-modale. Mai jos este un exemplu despre cum să implementezi o unealtă de transcriere audio care poate gestiona fișiere audio și returna transcrieri.
...
- Creat un `AudioTranscriptionTool` care poate transcrie fișiere audio.
- Definit schema uneltei pentru a accepta fie o adresă URL, fie date audio codificate în base64.
- Implementat metoda `execute` pentru a gestiona procesarea audio și transcrierea.
- Configurat serverul MCP pentru a gestiona cereri multi-modale, inclusiv procesarea audio și a imaginilor.
- Înregistrat unealta de transcriere audio pe server.
- Implementat o metodă pentru descărcarea fișierelor audio de la o adresă URL sau decodarea datelor audio în base64.
- Folosit un serviciu `AudioProcessor` pentru a gestiona logica efectivă a transcrierii.
- Pornit serverul MCP pentru a asculta cererile.
