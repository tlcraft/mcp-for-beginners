...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-09-30T19:33:53+00:00",
...
Lab praktis ini membimbing Anda untuk mengimplementasikan server MCP yang siap produksi menggunakan kerangka kerja FastMCP. Anda akan membangun struktur inti server, mengintegrasikan database, membuat alat untuk akses data, dan membangun dasar untuk analitik ritel berbasis AI.

## Ikhtisar

Server MCP adalah inti dari solusi analitik ritel kami. Server ini berfungsi sebagai jembatan antara asisten AI dan database PostgreSQL, menyediakan akses data bisnis yang aman dan cerdas melalui protokol standar.

Lab ini mengajarkan Anda untuk membangun server MCP yang tangguh dan skalabel dengan mengikuti pola dan praktik terbaik tingkat perusahaan.

## Tujuan Pembelajaran

Pada akhir lab ini, Anda akan dapat:

- **Membangun** server FastMCP dengan arsitektur dan organisasi yang tepat
- **Mengimplementasikan** integrasi database dengan pooling koneksi dan penanganan error
- **Membuat** alat MCP untuk introspeksi skema database dan eksekusi query
- **Mengonfigurasi** manajemen konteks Row Level Security
- **Menambahkan** fitur pemantauan kesehatan dan observabilitas
- **Menguji** implementasi server MCP Anda secara lokal dan dengan VS Code

## 📁 Struktur Proyek

Mari kita lihat organisasi server MCP:
...
### Konfigurasi Lingkungan (`config.py`)

Pertama, mari kita buat sistem konfigurasi yang tangguh:
...
- **Pemuat Variabel Lingkungan**: Dukungan otomatis untuk file .env
- **Keamanan Tipe**: Validasi dataclass dan petunjuk tipe
- **Default Fleksibel**: Default yang masuk akal untuk pengembangan
- **Validasi**: Validasi konfigurasi dengan pesan error yang membantu
- **Keamanan**: Nilai sensitif hanya berasal dari variabel lingkungan

## 🗄️ Lapisan Integrasi Database

### Penyedia PostgreSQL (`sales_analysis_postgres.py`)

Mari kita implementasikan lapisan integrasi database:
...
### Fitur Utama Lapisan Database

- **Pooling Koneksi**: Manajemen sumber daya yang efisien dengan asyncpg
- **Integrasi RLS**: Pengaturan konteks Row Level Security secara otomatis
- **Introspeksi Skema**: Penemuan skema tabel secara dinamis
- **Penanganan Error**: Manajemen error yang komprehensif dan logging
- **Pemformatan Query**: Pemformatan hasil yang ramah AI
- **Pemantauan Kesehatan**: Pemeriksaan konektivitas database dan status pool

## 🔧 Implementasi Utama Server MCP

### Server FastMCP (`sales_analysis.py`)

Sekarang mari kita implementasikan server MCP utama:
...
### Fitur Utama Server MCP

- **Registrasi Alat**: Definisi alat deklaratif dengan keamanan tipe
- **Manajemen Konteks RLS**: Ekstraksi identitas pengguna dan pengaturan konteks secara otomatis
- **Penanganan Error**: Manajemen error yang komprehensif dengan pesan yang ramah pengguna
- **Manajemen Siklus Hidup**: Startup/shutdown yang tepat dengan pembersihan sumber daya
- **Pemantauan Kesehatan**: Endpoint pemeriksaan kesehatan bawaan
- **Dukungan Pengembangan**: Hot reload dan kemampuan debugging

## 🏥 Pemantauan Kesehatan

### Implementasi Pemeriksaan Kesehatan (`health_check.py`)
...
2. **Uji di AI Chat**:
   ...
   - Ketik `#zava` dan pilih server Anda
   - Tanyakan: "Tabel apa saja yang tersedia?"
   - Tanyakan: "Tunjukkan 5 toko teratas berdasarkan jumlah pesanan"

### Pengujian Unit

Buat pengujian unit yang komprehensif:
...
✅ **Server MCP yang Berfungsi**: Server FastMCP dengan integrasi database  
✅ **Manajemen Konfigurasi**: Konfigurasi berbasis lingkungan yang tangguh  
✅ **Lapisan Database**: Integrasi PostgreSQL dengan pooling koneksi  
✅ **Alat MCP**: Alat introspeksi skema dan eksekusi query  
✅ **Integrasi RLS**: Manajemen konteks Row Level Security  
✅ **Pemantauan Kesehatan**: Endpoint pemeriksaan kesehatan yang komprehensif  
✅ **Strategi Pengujian**: Pengujian lokal dan integrasi VS Code  

## 🚀 Langkah Selanjutnya

Lanjutkan ke **[Lab 06: Pengembangan Alat](../06-Tools/README.md)** untuk:

- Memperluas koleksi alat MCP Anda
- Mengimplementasikan pola query yang lebih canggih
- Menambahkan validasi dan transformasi data
- Membuat alat analitik khusus

## 📚 Sumber Daya Tambahan

### Kerangka Kerja FastMCP
- [Dokumentasi FastMCP](https://github.com/modelcontextprotocol/python-sdk) - Panduan resmi FastMCP
- [Spesifikasi MCP](https://modelcontextprotocol.io/docs/) - Spesifikasi protokol
- [Panduan Pengembangan Alat](https://modelcontextprotocol.io/docs/tools/) - Membuat alat MCP

### Integrasi Database
- [Dokumentasi asyncpg](https://magicstack.github.io/asyncpg/current/) - Driver PostgreSQL async
- [Praktik Terbaik Pooling Koneksi](https://www.postgresql.org/docs/current/runtime-config-connection.html) - Tuning PostgreSQL
- [Panduan Row Level Security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) - Implementasi RLS

### Pola FastAPI
- [Dokumentasi FastAPI](https://fastapi.tiangolo.com/) - Referensi kerangka kerja web
- [Dependency Injection](https://fastapi.tiangolo.com/tutorial/dependencies/) - Pola FastAPI
- [Tugas Latar Belakang](https://fastapi.tiangolo.com/tutorial/background-tasks/) - Manajemen tugas async

---

**Selanjutnya**: Siap memperluas alat Anda? Lanjutkan ke [Lab 06: Pengembangan Alat](../06-Tools/README.md)
