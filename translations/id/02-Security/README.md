...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-04T18:11:43+00:00",
...
- Mendeskripsikan dan menerapkan kontrol mitigasi yang efektif untuk risiko keamanan MCP, seperti autentikasi yang kuat, prinsip hak akses minimum, manajemen token yang aman, dan verifikasi rantai pasokan.
...
- Mengenali pentingnya memvalidasi metadata alat, memantau perubahan dinamis, dan melindungi dari serangan prompt injection tidak langsung.
...
#### Token Passthrough
Token passthrough secara eksplisit dilarang dalam spesifikasi otorisasi karena memperkenalkan sejumlah risiko keamanan, termasuk:
...
Server MCP tidak akan dapat mengidentifikasi atau membedakan antara Klien MCP ketika klien memanggil dengan token akses yang diterbitkan dari hulu yang mungkin tidak transparan bagi server MCP.  
Log server Resource Server hilir mungkin menunjukkan permintaan yang tampak berasal dari sumber berbeda dengan identitas berbeda, bukan server MCP yang sebenarnya meneruskan token.  
...
Resource Server hilir memberikan kepercayaan kepada entitas tertentu. Kepercayaan ini mungkin mencakup asumsi tentang asal atau pola perilaku klien. Melanggar batas kepercayaan ini dapat menyebabkan masalah yang tidak terduga.  
...
Meskipun server MCP dimulai sebagai “proxy murni” hari ini, mungkin perlu menambahkan kontrol keamanan di kemudian hari. Memulai dengan pemisahan audiens token yang tepat memudahkan evolusi model keamanan.
...
### Pernyataan masalah  
Server MCP mungkin diberikan izin berlebihan terhadap layanan/sumber daya yang diaksesnya. Misalnya, server MCP yang merupakan bagian dari aplikasi penjualan AI yang terhubung ke penyimpanan data perusahaan seharusnya hanya memiliki akses terbatas pada data penjualan dan tidak diizinkan mengakses semua file di penyimpanan tersebut. Mengacu kembali pada prinsip hak akses minimum (salah satu prinsip keamanan tertua), tidak ada sumber daya yang seharusnya memiliki izin melebihi yang diperlukan untuk menjalankan tugas yang dimaksudkan. AI menghadirkan tantangan lebih besar di area ini karena untuk membuatnya fleksibel, sulit untuk menentukan izin yang tepat dibutuhkan.

### Risiko  
- Memberikan izin berlebihan dapat memungkinkan pencurian atau pengubahan data yang sebenarnya tidak seharusnya diakses oleh server MCP. Ini juga bisa menjadi masalah privasi jika data tersebut adalah informasi pribadi yang dapat diidentifikasi (PII).
...
- **Gunakan Role-Based Access Control (RBAC):** Tetapkan peran pada server MCP yang dibatasi secara ketat pada sumber daya dan tindakan tertentu, hindari izin yang luas atau tidak perlu.
...
Server MCP yang berbahaya atau telah dikompromikan dapat menimbulkan risiko besar dengan mengekspos data pelanggan atau memungkinkan tindakan yang tidak diinginkan. Risiko ini sangat relevan dalam beban kerja AI dan berbasis MCP, di mana:
...
5. **Integrasi dengan Azure Content Safety:** Prompt Shields merupakan bagian dari rangkaian Azure AI Content Safety yang lebih luas, yang menyediakan alat tambahan untuk mendeteksi upaya jailbreak, konten berbahaya, dan risiko keamanan lain dalam aplikasi AI.
...
Keamanan rantai pasokan tetap menjadi hal mendasar di era AI, namun cakupan apa yang termasuk dalam rantai pasokan Anda kini telah meluas. Selain paket kode tradisional, Anda sekarang harus secara ketat memverifikasi dan memantau semua komponen terkait AI, termasuk foundation models, layanan embeddings, penyedia konteks, dan API pihak ketiga. Masing-masing dapat menimbulkan kerentanan atau risiko jika tidak dikelola dengan baik.
...
- **Pantau dan audit secara berkelanjutan:** Terapkan pemantauan berkelanjutan untuk semua dependensi, termasuk model dan layanan data, guna mendeteksi kerentanan baru atau serangan rantai pasokan.
- **Terapkan prinsip least privilege dan kontrol akses:** Batasi akses ke model, data, dan layanan hanya pada yang diperlukan agar server MCP Anda dapat berfungsi.
- **Tanggap cepat terhadap ancaman:** Miliki proses untuk memperbaiki atau mengganti komponen yang terkompromi, serta untuk merotasi rahasia atau kredensial jika terjadi pelanggaran.
...
Setiap implementasi MCP mewarisi postur keamanan yang sudah ada dari lingkungan organisasi tempat ia dibangun, jadi saat mempertimbangkan keamanan MCP sebagai bagian dari sistem AI Anda secara keseluruhan, disarankan untuk meningkatkan postur keamanan yang sudah ada. Kontrol keamanan yang sudah mapan berikut ini sangat relevan:

- Praktik pengkodean aman dalam aplikasi AI Anda – lindungi dari [OWASP Top 10](https://owasp.org/www-project-top-ten/), [OWASP Top 10 untuk LLMs](https://genai.owasp.org/download/43299/?tmstv=1731900559), penggunaan vault aman untuk rahasia dan token, penerapan komunikasi aman end-to-end antar semua komponen aplikasi, dan lain-lain.
- Penguatan server – gunakan MFA jika memungkinkan, selalu perbarui patch, integrasikan server dengan penyedia identitas pihak ketiga untuk akses, dan sebagainya.
- Pastikan perangkat, infrastruktur, dan aplikasi selalu diperbarui dengan patch terbaru.
...
- MCP membawa risiko baru—seperti prompt injection, tool poisoning, dan izin berlebihan—yang memerlukan kontrol tradisional dan khusus AI.
- Gunakan praktik autentikasi, otorisasi, dan manajemen token yang kuat, dengan memanfaatkan penyedia identitas eksternal seperti Microsoft Entra ID jika memungkinkan.
...
- Tetap up-to-date dengan spesifikasi MCP yang terus berkembang dan berkontribusi pada komunitas untuk membantu membentuk standar keamanan.
...
Selanjutnya: [Chapter 3: Getting Started](../03-GettingStarted/README.md)
