...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-07-17T07:54:53+00:00",
...
Aplikasi multi-modal semakin penting dalam AI, memungkinkan interaksi yang lebih kaya dan tugas yang lebih kompleks. Model Context Protocol (MCP) menyediakan kerangka kerja untuk membangun aplikasi multi-modal yang dapat menangani berbagai jenis data, seperti teks, gambar, dan audio.
...
Dalam pelajaran ini, Anda akan belajar cara membangun aplikasi multi-modal.
...
- Menerapkan aplikasi multi-modal.

## Arsitektur untuk Dukungan Multi-Modal

Implementasi MCP multi-modal biasanya melibatkan:

- **Parser Khusus Modal**: Komponen yang mengubah berbagai jenis media menjadi format yang dapat diproses model.
- **Alat Khusus Modal**: Alat khusus yang dirancang untuk menangani modalitas tertentu (analisis gambar, pemrosesan audio)
- **Manajemen Konteks Terpadu**: Sistem untuk menjaga konteks di berbagai modalitas
- **Pembuatan Respons**: Kemampuan untuk menghasilkan respons yang mungkin mencakup beberapa modalitas.

## Contoh Multi-Modal: Analisis Gambar

Dalam contoh berikut, kita akan menganalisis sebuah gambar dan mengekstrak informasi.
...
Dalam contoh sebelumnya, kita telah:

- Membuat `ImageAnalysisTool` yang dapat menganalisis gambar menggunakan `IImageAnalysisService` hipotetis.
- Mengonfigurasi server MCP untuk menangani permintaan yang lebih besar dan mendukung tipe konten gambar.
- Mendaftarkan alat analisis gambar ke server.
- Mengimplementasikan metode untuk mengunduh gambar dari URL dan menganalisisnya berdasarkan tipe yang diminta (objek, teks, wajah, dll.).
- Mengembalikan hasil terstruktur dalam format yang sesuai dengan spesifikasi MCP.

## Contoh Multi-Modal: Pemrosesan Audio

Pemrosesan audio adalah modalitas umum lainnya dalam aplikasi multi-modal. Berikut adalah contoh cara mengimplementasikan alat transkripsi audio yang dapat menangani file audio dan mengembalikan transkripsi.
...
- Membuat `AudioTranscriptionTool` yang dapat mentranskripsi file audio.
- Mendefinisikan skema alat untuk menerima URL atau data audio yang dikodekan base64.
- Mengimplementasikan metode `execute` untuk menangani pemrosesan audio dan transkripsi.
- Mengonfigurasi server MCP untuk menangani permintaan multi-modal, termasuk pemrosesan audio dan gambar.
- Mendaftarkan alat transkripsi audio ke server.
- Mengimplementasikan metode untuk mengunduh file audio dari URL atau mendekode data audio base64.
- Menggunakan layanan `AudioProcessor` untuk menangani logika transkripsi sebenarnya.
- Memulai server MCP untuk mendengarkan permintaan.

### Contoh Multi-Modal: Pembuatan Respons Multi-Modal
